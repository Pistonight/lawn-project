    ...
    for (int i = 0; i < mNumEffects; i++) {
        ...
        case EffectType::EFFECT_PARTICLE: {
            TodParticleSystem* aParticleSystem =
                gEffectSystem->mParticleHolder->mParticleSystems.DataArrayTryToGet(
                    aAttachEffect->mEffectID);
        ...
        case EffectType::EFFECT_TRAIL: {
            Trail* aTrail =
                gEffectSystem->mTrailHolder->mTrails.DataArrayTryToGet(aAttachEffect->mEffectID);
        ...
        case EffectType::EFFECT_REANIM: {
            Reanimation* aReanimation =
                gEffectSystem->mReanimationHolder->mReanimations.DataArrayTryToGet(
                    aAttachEffect->mEffectID);
        ...
        case EffectType::EFFECT_ATTACHMENT: {
            Attachment* aAttachment =
                gEffectSystem->mAttachmentHolder->mAttachments.DataArrayTryToGet(
                    aAttachEffect->mEffectID);
            if (aAttachment) {
                aAttachment->Update();
                isEmpty = false;
        ...
        if (isEmpty) {
            ...
            if (aNumEffectsRemaining > 0) {
                memmove(aAttachEffect, aAttachEffect + 1,
                        aNumEffectsRemaining * sizeof(AttachEffect));
                --i;
            }
            mNumEffects--;
        }
...
// 0x404890
void Attachment::PropogateColor(const Color& theColor, bool theEnableAdditiveColor,
                                const Color& theAdditiveColor, bool theEnableOverlayColor,
        ...
        case EffectType::EFFECT_ATTACHMENT: {
            Attachment* aAttachment =
                gEffectSystem->mAttachmentHolder->mAttachments.DataArrayTryToGet(
                    aAttachEffect->mEffectID);
            if (aAttachment) {
                aAttachment->PropogateColor(theColor, theEnableAdditiveColor, theAdditiveColor,
                                            theEnableOverlayColor, theOverlayColor);
            }
            break;
        }
        }
...
void Attachment::Draw(Graphics* g, bool theParentHidden) {
    ...
    DataArray<TodParticleSystem>& aParticleSystems =
        gEffectSystem->mParticleHolder->mParticleSystems;
    ...
    for (int i = 0; i < mNumEffects; i++) {
        ...
        case EffectType::EFFECT_PARTICLE: {
            TodParticleSystem* aParticleSystem =
                aParticleSystems.DataArrayTryToGet(aAttachEffect->mEffectID);
            if (aParticleSystem) {
...
void AttachmentHolder::InitializeHolder() {
    mAttachments.DataArrayInitialize(MAX_ATTACHMENTS, "attachments");
}
...
void AttachmentUpdateAndSetMatrix(AttachmentID& theAttachmentID, SexyTransform2D& theMatrix) {
    ...
    TOD_ASSERT(gEffectSystem);
    Attachment* aAttachment = gEffectSystem->mAttachmentHolder->mAttachments.DataArrayTryToGet(
        (unsigned int)theAttachmentID);
    if (aAttachment) {
        aAttachment->Update();
...
void AttachmentPropogateColor(AttachmentID& theAttachmentID, const Color& theColor,
                              bool theEnableAdditiveColor, const Color& theAdditiveColor,
...
// 0x405540
AttachEffect* CreateEffectAttachment(AttachmentID& theAttachmentID, EffectType theEffectType,
                                     unsigned int theDataID, float theOffsetX, float theOffsetY) {
    TOD_ASSERT(gEffectSystem);
    Attachment* aAttachment = gEffectSystem->mAttachmentHolder->mAttachments.DataArrayTryToGet(
        (unsigned int)theAttachmentID);
    if (aAttachment == nullptr || aAttachment->mDead) {
        ...
        theAttachmentID =
            (AttachmentID)gEffectSystem->mAttachmentHolder->mAttachments.DataArrayGetID(
                aAttachment);
    }
...
AttachEffect* AttachReanim(AttachmentID& theAttachmentID, Reanimation* theReanimation,
    ...
    unsigned int aReanimId =
        gEffectSystem->mReanimationHolder->mReanimations.DataArrayGetID(theReanimation);
    AttachEffect* aAttachEffect = CreateEffectAttachment(theAttachmentID, EffectType::EFFECT_REANIM,
                                                         aReanimId, theOffsetX, theOffsetY);
...
// 0x405600
AttachEffect* AttachParticle(AttachmentID& theAttachmentID, TodParticleSystem* theParticleSystem,
                             float theOffsetX, float theOffsetY) {
    if (theParticleSystem == nullptr)
        return nullptr;

    unsigned int aParticleId =
        gEffectSystem->mParticleHolder->mParticleSystems.DataArrayGetID(theParticleSystem);
    AttachEffect* aAttachEffect = CreateEffectAttachment(
        theAttachmentID, EffectType::EFFECT_PARTICLE, aParticleId, theOffsetX, theOffsetY);
...
AttachEffect* AttachTrail(AttachmentID& theAttachmentID, Trail* theTrail, float theOffsetX,
                          float theOffsetY) {
    unsigned int aTrailId = gEffectSystem->mTrailHolder->mTrails.DataArrayGetID(theTrail);
    AttachEffect* aAttachEffect = CreateEffectAttachment(theAttachmentID, EffectType::EFFECT_TRAIL,
                                                         aTrailId, theOffsetX, theOffsetY);
