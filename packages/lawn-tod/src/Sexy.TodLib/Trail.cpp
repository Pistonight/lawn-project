...
void Trail::Draw(Graphics* g) {
    ...
    Sexy::TriVertex aVertArray[MAX_TRAIL_TRIANGLES][3];
    ...
    for (int i = 0; i < mNumTrailPoints - 1; i++) {
        ...
        float aWidthOverLengthCur =
            FloatTrackEvaluate(mDefinition->mWidthOverLength, aUCur,
                               mTrailInterp[(int)TrailTracks::TRACK_WIDTH_OVER_LENGTH]);
        float aWidthOverLengthNext =
            FloatTrackEvaluate(mDefinition->mWidthOverLength, aUNext,
                               mTrailInterp[(int)TrailTracks::TRACK_WIDTH_OVER_LENGTH]);
        float aWidthOverTimeCur =
            FloatTrackEvaluate(mDefinition->mWidthOverTime, aTimeValue,
                               mTrailInterp[(int)TrailTracks::TRACK_WIDTH_OVER_TIME]);
        float aWidthOverTimeNext =
            FloatTrackEvaluate(mDefinition->mWidthOverTime, aTimeValue,
                               mTrailInterp[(int)TrailTracks::TRACK_WIDTH_OVER_TIME]);
        float aAlphaOverLengthCur =
            FloatTrackEvaluate(mDefinition->mAlphaOverLength, aUCur,
                               mTrailInterp[(int)TrailTracks::TRACK_ALPHA_OVER_LENGTH]);
        float aAlphaOverLengthNext =
            FloatTrackEvaluate(mDefinition->mAlphaOverLength, aUNext,
                               mTrailInterp[(int)TrailTracks::TRACK_ALPHA_OVER_LENGTH]);
        float aAlphaOverTimeCur =
            FloatTrackEvaluate(mDefinition->mAlphaOverTime, aTimeValue,
                               mTrailInterp[(int)TrailTracks::TRACK_ALPHA_OVER_TIME]);
        float aAlphaOverTimeNext =
            FloatTrackEvaluate(mDefinition->mAlphaOverTime, aTimeValue,
                               mTrailInterp[(int)TrailTracks::TRACK_ALPHA_OVER_TIME]);
        int anAlphaCur = ClampInt(
            FloatRoundToInt(aAlphaOverLengthCur * aAlphaOverTimeCur * mColorOverride.mAlpha), 0,
            255);
        int anAlphaNext = ClampInt(
            FloatRoundToInt(aAlphaOverLengthNext * aAlphaOverTimeNext * mColorOverride.mAlpha), 0,
            255);
        ...
        aPosition[0].x = mTrailCenter.x + aPointCur.aPos.x +
                         aNormalCur.x * aWidthOverLengthCur * aWidthOverTimeCur;
        aPosition[0].y = mTrailCenter.y + aPointCur.aPos.y +
                         aNormalCur.y * aWidthOverLengthCur * aWidthOverTimeCur;
        aPosition[1].x = mTrailCenter.x + aPointCur.aPos.x +
                         -aNormalCur.x * aWidthOverLengthCur * aWidthOverTimeCur;
        aPosition[1].y = mTrailCenter.y + aPointCur.aPos.y +
                         -aNormalCur.y * aWidthOverLengthCur * aWidthOverTimeCur;
        aPosition[2].x = mTrailCenter.x + aPointNext.aPos.x +
                         aNormalNext.x * aWidthOverLengthNext * aWidthOverTimeNext;
        aPosition[2].y = mTrailCenter.y + aPointNext.aPos.y +
                         aNormalNext.y * aWidthOverLengthNext * aWidthOverTimeNext;
        aPosition[3].x = mTrailCenter.x + aPointNext.aPos.x +
                         -aNormalNext.x * aWidthOverLengthNext * aWidthOverTimeNext;
        aPosition[3].y = mTrailCenter.y + aPointNext.aPos.y +
                         -aNormalNext.y * aWidthOverLengthNext * aWidthOverTimeNext;
...
Trail* TrailHolder::AllocTrailFromDef(int theRenderOrder, TrailDefinition* theDefinition) {
    ...
    aTrail->mTrailDuration =
        (int)FloatTrackEvaluate(aTrail->mDefinition->mTrailDuration, 0.0f, aDurationInterp);
    return aTrail;
}
