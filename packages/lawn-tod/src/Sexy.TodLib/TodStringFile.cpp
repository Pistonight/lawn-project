...
TodStringListFormat::TodStringListFormat(const char* theFormatName, Font** theFont,
                                         const Color& theColor, int theLineSpacingOffset,
                                         unsigned int theFormatFlags)
    : mFormatName(theFormatName), mNewFont(theFont), mNewColor(theColor),
      mLineSpacingOffset(theLineSpacingOffset), mFormatFlags(theFormatFlags) {}
...
static void TrimUtf8Ends(const char*& theStart, int& theLen) {
    while (theLen > 0 && (unsigned char)*theStart <= 0x20) {
        theStart++;
        theLen--;
    }
    while (theLen > 0 && (unsigned char)theStart[theLen - 1] <= 0x20) {
        theLen--;
    }
}
...
bool TodStringListReadFile(const char* theFileName) {
    ...
    aFileText[aSize] = '\0';
    std::string aFileNameStr(theFileName);
    bool aIsUtf8File = aFileNameStr.size() > 9 &&
                       aFileNameStr.compare(aFileNameStr.size() - 9, 9, "_utf8.txt") == 0;
    if (aSuccess) {
        if (aIsUtf8File)
            aSuccess = TodStringListReadItems(aFileText);
        else
            aSuccess = TodStringListReadItems(ANSIToUTF8(aFileText).c_str());
    }
    p_fclose(pFile);
    delete[] aFileText;
...
void TodStringListLoad(const char* theFileName) {
    if (!TodStringListReadFile(theFileName))
        TodErrorMessageBox(
            Sexy::StrFormat("Failed to load string list file '%s'", theFileName).c_str(), "Error");
}
...
bool CharIsSpaceInFormat(SexyChar theChar, const TodStringListFormat& theCurrentFormat) {
    return theChar == ' ' || (TestBit(theCurrentFormat.mFormatFlags,
                                      TodStringFormatFlag::TOD_FORMAT_IGNORE_NEWLINES) &&
                              theChar == '\n');
}

// 0x519870
int TodWriteString(Graphics* g, const SexyString& theString, int theX, int theY,
                   TodStringListFormat& theCurrentFormat, int theWidth,
                   DrawStringJustification theJustification, bool drawString, int theOffset,
                   int theLength) {
    ...
    while (it != end) {
        ...
        if (aCodePoint == '{') {
            ...
            if (aFormatEnd != nullptr) {
                ...
                if (drawString)
                    aFont->DrawString(g, theX + aXOffset, theY, aString, theCurrentFormat.mNewColor,
                                      g->mClipRect);
...
int TodWriteWordWrappedHelper(Graphics* g, const SexyString& theString, int theX, int theY,
                              TodStringListFormat& theCurrentFormat, int theWidth,
                              DrawStringJustification theJustification, bool drawString,
                              int theOffset, int theLength, int theMaxChars) {
    if (theOffset + theLength > theMaxChars) {
        theLength = theMaxChars - theOffset;
        if (theLength <= 0)
            return -1;
    }
    return TodWriteString(g, theString, theX, theY, theCurrentFormat, theWidth, theJustification,
                          drawString, theOffset, theLength);
}

static bool IsCJKCodepoint(uint32_t c) {
    return (c >= 0x4E00 && c <= 0x9FFF) || (c >= 0x3400 && c <= 0x4DBF) ||
           (c >= 0x3000 && c <= 0x303F) || (c >= 0xF900 && c <= 0xFAFF) ||
           (c >= 0xFF00 && c <= 0xFFEF);
}

// 0x519B50
int TodDrawStringWrappedHelper(Graphics* g, const SexyString& theText, const Rect& theRect,
                               Font* theFont, const Color& theColor,
                               DrawStringJustification theJustification, bool drawString) {
    ...
    while (it != end) {
        ...
        if (aCurChar == '{') {
            ...
            if (aFmtEnd != nullptr) {
                ...
                int aNewAscentOffset = (*aCurrentFormat.mNewFont)->GetAscent() -
                                       (*aCurrentFormat.mNewFont)->GetAscentPadding();
                aLineSpacing = (*aCurrentFormat.mNewFont)->GetLineSpacing() +
                               aCurrentFormat.mLineSpacingOffset;
        ...
        } else if (IsCJKCodepoint(aCurChar) && (int)aCurPos > aLineFeedPos) {
            aSpacePos = aCurPos;
            aSpaceSize = 0;
        }
        ...
        if (aCurWidth > theRect.mWidth) {
            ...
            if (aSpacePos != -1) {
                ...
                bool aAllowDrawText =
                    drawString && aCurY >= g->mClipRect.mY &&
                    aCurY <= g->mClipRect.mY + g->mClipRect.mHeight +
                                 aLineSpacing; // we do all that anyways cause the formatting
                TodWriteWordWrappedHelper(g, theText, theRect.mX, theRect.mY + aYOffset,
                                          aCurrentFormat, theRect.mWidth, theJustification,
                                          aAllowDrawText, aLineFeedPos, aSpacePos - aLineFeedPos,
                                          theMaxChars);
                ...
                it = theText.begin() + aSpacePos + aSpaceSize;
                ...
                aLineWidth = TodWriteWordWrappedHelper(
                    ...
                    theJustification, drawString, aLineFeedPos, aCurPos - aLineFeedPos,
                    theMaxChars);
                if (aLineWidth < 0)
                    break;
            }
    ...
    if (aLineFeedPos < theText.size()) {
        int aLastLineLength = TodWriteWordWrappedHelper(
            g, theText, theRect.mX, theRect.mY + aYOffset, aCurrentFormat, theRect.mWidth,
            theJustification, drawString, aLineFeedPos, theText.size() - aLineFeedPos, theMaxChars);
...
// 0x51A040
void TodDrawStringWrapped(Graphics* g, const SexyString& theText, const Rect& theRect,
                          Font* theFont, const Color& theColor,
    ...
    if (theJustification == DrawStringJustification::DS_ALIGN_LEFT_VERTICAL_MIDDLE ||
        ...
        aRectTodUse.mY +=
            (aRectTodUse.mHeight - TodDrawStringWrappedHelper(g, aTextFinal, aRectTodUse, theFont,
                                                              theColor, theJustification, false)) /
            2;
    }
    TodDrawStringWrappedHelper(g, aTextFinal, aRectTodUse, theFont, theColor, theJustification,
                               true);
}
