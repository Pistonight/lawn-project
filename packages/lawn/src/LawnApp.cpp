...
LawnApp::LawnApp()
{
	...
	mTodCheatKeys = false;
#ifdef PISTON_PATCH // keep game running when lost focus - by default
...
void LawnApp::WriteToRegistry()
{
    ...
    RegistryWriteBoolean("AllowBackgroundPlay", !mPistonSettings.mAllowBackgroundPlay);
...
{
    bool aBool {};
    if (RegistryReadBoolean("AllowBackgroundPlay", &aBool)) {
        mPistonSettings.mAllowBackgroundPlay = aBool;
        mMuteOnLostFocus = !aBool;
    }
