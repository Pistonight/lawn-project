...
#ifdef PISTON_PATCH
	mAllowBackgroundPlayCheckbox = MakeNewCheckbox(SettingsDialog::SETTINGS_PISTON_ALLOW_BACKGROUND_PLAY, this, theApp->mPistonSettings.mAllowBackgroundPlay);
#endif
...
#ifdef PISTON_PATCH
    delete mAllowBackgroundPlayCheckbox;
#endif
...
void SettingsDialog::Draw(Graphics* g)
{
	...
	mAllowBackgroundPlayCheckbox->Resize(40, aY - aScrollOffset + GetTop(), 46, 45);

	mAllowBackgroundPlayCheckbox->mDisabled =
		(mAllowBackgroundPlayCheckbox->mY + mY + mAllowBackgroundPlayCheckbox->mHeight) < mOptionsSlider->mAllowedMouseZone.mY ||
		(mAllowBackgroundPlayCheckbox->mY + mY) >
			(mOptionsSlider->mAllowedMouseZone.mY + mOptionsSlider->mAllowedMouseZone.mHeight);

	TodDrawString(g, "[PISTON_SETTINGS_ALLOW_BACKGROUND_PLAY]", mAllowBackgroundPlayCheckbox->mX + 20, aY + 30, Sexy::FONT_BRIANNETOD12,
				  Color::White, DrawStringJustification::DS_ALIGN_LEFT);
#endif
...
#ifdef PISTON_PATCH
	AddWidget(mAllowBackgroundPlayCheckbox);
#endif
...
#ifdef PISTON_PATCH
	RemoveWidget(mAllowBackgroundPlayCheckbox);
#endif
...
#ifdef PISTON_PATCH
        case SettingsDialog::SETTINGS_PISTON_ALLOW_BACKGROUND_PLAY: {
            bool aChecked = mAllowBackgroundPlayCheckbox->IsChecked();
            mApp->mMuteOnLostFocus = !aChecked;
            mApp->mPistonSettings.mAllowBackgroundPlay = aChecked;
            break;
        }
#endif
	}
