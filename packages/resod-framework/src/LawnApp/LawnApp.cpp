...
Version LawnApp::gResoddedVersion(1, 1, 0, 1032);
...
void LawnApp::DoConfirmBackToMain() {
    LawnDialog* aDialog =
        (LawnDialog*)DoDialog(Dialogs::DIALOG_CONFIRM_BACK_TO_MAIN, true, "[LEAVE_GAME_HEADER]",
...
void LawnApp::DoPauseDialog() {
    ...
    LawnDialog* aDialog = (LawnDialog*)DoDialog(
        Dialogs::DIALOG_PAUSED, true, "GAME PAUSED" /*"[GAME_PAUSED]"*/, "Click to resume game",
...
int LawnApp::LawnMessageBox(int theDialogId, const SexyString& theHeaderName,
                            const SexyString& theLinesName, const SexyString& theButton1Name,
                            const SexyString& theButton2Name, int theButtonMode) {
    Widget* aOldFocus = mWidgetManager->mFocusWidget;

    LawnDialog* aDialog = (LawnDialog*)DoDialog(theDialogId, true, theHeaderName, theLinesName,
                                                theButton1Name, theButtonMode);
...
Dialog* LawnApp::DoDialog(int theDialogId, bool isModal, const SexyString& theDialogHeader,
    ...
    Dialog* aDialog =
        SexyAppBase::DoDialog(theDialogId, isModal, aHeader, aLines, aFooter, theButtonMode);
...
void LawnApp::FinishCreateUserDialog(bool isYes) {
    ...
    if (isYes && aName.empty()) {
        ...
        if (aProfile == nullptr) {
            DoDialog(Dialogs::DIALOG_CREATEUSERERROR, true, "[NAME_CONFLICT]",
                     "[ENTER_UNIQUE_PLAYER_NAME]", "[DIALOG_BUTTON_OK]", Dialog::BUTTONS_FOOTER);
        } else {
            mProfileMgr->Save();
            mPlayerInfo = aProfile;
...
void LawnApp::FinishRenameUserDialog(bool isYes) {
    ...
    if (!mProfileMgr->RenameProfile(anOldName, aNewName)) {
        DoDialog(Dialogs::DIALOG_RENAMEUSERERROR, true, "[NAME_CONFLICT]",
                 "[ENTER_UNIQUE_PLAYER_NAME]", "[DIALOG_BUTTON_OK]", Dialog::BUTTONS_FOOTER);
        return;
    }
...
void LawnApp::FinishNameError(int theId) {
    ...
    NewUserDialog* aNewUserDialog = (NewUserDialog*)GetDialog(
        theId == Dialogs::DIALOG_CREATEUSERERROR ? Dialogs::DIALOG_CREATEUSER
                                                 : Dialogs::DIALOG_RENAMEUSER);
    if (aNewUserDialog) {
...
void LawnApp::DoConfirmSellDialog(const SexyString& theMessage) {
    Dialog* aConfirmDialog = DoDialog(Dialogs::DIALOG_ZEN_SELL, true, "[ZEN_SELL_HEADER]",
...
void LawnApp::DoConfirmPurchaseDialog(const SexyString& theMessage) {
    LawnDialog* aComfirmDialog =
        (LawnDialog*)DoDialog(Dialogs::DIALOG_STORE_PURCHASE, true, "买下这个物品？", theMessage,
                              "", Dialog::BUTTONS_YES_NO);
...
Dialog* LawnApp::NewDialog(int theDialogId, bool isModal, const SexyString& theDialogHeader,
                           const SexyString& theDialogLines, const SexyString& theDialogFooter,
                           int theButtonMode) {
    LawnDialog* aDialog = new LawnDialog(this, theDialogId, isModal, theDialogHeader,
                                         theDialogLines, theDialogFooter, theButtonMode);
...
bool LawnApp::NeedPauseGame() {
    ...
    if (mDialogList.size() == 1 && mDialogList.front()->mId != Dialogs::DIALOG_NEW_GAME) {
        ...
        if (anId == Dialogs::DIALOG_CHOOSER_WARNING ||
            anId == Dialogs::DIALOG_PURCHASE_PACKET_SLOT || anId == Dialogs::DIALOG_IMITATER) {
            return false;
        }
    }
...
void LawnApp::Init() {
    ...
    // [SETUP] - Here you (can) add a link that contains the LATEST version of your mod. I recommend
    // Github as it's free and easy to setup. (And you are probably using it now)

    UpdateChecker::gUpdateHost = "https://raw.githubusercontent.com/LawnProject/ResoddedFramework/"
                                 "refs/heads/main/LawnVersion.txt";

    UpdateChecker::Check();

    if (UpdateChecker::gIsOutdated)
        TodTraceAndLog("[LawnProject] - OUTDATED: Current Version: %s | Latest Version: %s",
                       gVersion.toString().c_str(),
...
bool LawnApp::UpdatePlayerProfileForFinishingLevel() {
    ...
    if (IsAdventureMode()) {
        ...
        if (!HasFinishedAdventure() && (mGameMode == GameMode::GAMEMODE_SCARY_POTTER_3 ||
...
void LawnApp::CheckForGameEnd() {
    ...
    if (IsAdventureMode()) {
        if (mBoard->mPlantsPlaced > 0 && !IsWhackAZombieLevel() && !IsScaryPotterLevel() &&
            !IsBungeeBlitzLevel() && !IsChallengeWithoutSeedBank()) {
            if (mBoard->StageHasPool() && !mBoard->StageIsNight() &&
                !mBoard->mDontPeaUsedPeashooter) {
                mAchievements->GiveAchievement(AchievementID::ACHIEVEMENT_DONT_PEA);
            }
            if (mBoard->StageHasRoof() && !mBoard->HasConveyorBeltSeedBank() &&
                !mBoard->mGroundedUsedCatapult) {
...
void LawnApp::UpdatePlayTimeStats() {
    ...
    if (aSession > 0) {
        ...
        if ((mBoard == nullptr || !mBoard->mPaused) && mHasFocus &&
            mLastTimerTime - mLastUserInputTick <= 10000) {
...
void LawnApp::LoadGroup(const char* theGroupName, int theGroupAveMsToLoad) {
    ...
    if (mShutdown || mCloseRequest)
        return;
    bool aResoddedResourceFound =
        ResoddedFrameworkExtractResourcesByName(mResourceManager, theGroupName);
...
void LawnApp::LoadingThreadProc() {
    ...
    for (int i = 0; i < 3; i++) {
        mNumLoadingThreadTasks +=
            mResourceManager->GetNumResources(groups[i]) * group_ave_ms_to_load[i];
    }
...
void LawnApp::URLOpenFailed(const std::string& theURL) {
    ...
    DoDialog(Dialogs::DIALOG_OPENURL_WAIT, true, "Open Browser", "OK", aString,
...
bool LawnApp::OpenURL(const std::string& theURL, bool shutdownOnOpen) {
    DoDialog(Dialogs::DIALOG_OPENURL_WAIT, true, "Opening Browser", "Opening Browser", "",
             Dialog::BUTTONS_NONE);
...
void LawnApp::ConfirmQuit() {
    ...
    LawnDialog* aDialog = (LawnDialog*)DoDialog(Dialogs::DIALOG_QUIT, true, aHeader, aBody, "",
...
void LawnApp::CenterDialog(Dialog* theDialog, int theWidth, int theHeight) {
    theDialog->Resize((BOARD_WIDTH - theWidth) / 2, (BOARD_HEIGHT - theHeight) / 2, theWidth,
                      theHeight);
}
...
bool LawnApp::IsPuzzleMode() {
    return (mGameMode >= GameMode::GAMEMODE_SCARY_POTTER_1 &&
...
bool LawnApp::IsContinuousChallenge() {
    return IsArtChallenge() || IsSlotMachineLevel() || IsFinalBossLevel() ||
           ...
           mGameMode == GameMode::GAMEMODE_UPSELL || mGameMode == GameMode::GAMEMODE_INTRO ||
...
bool LawnApp::IsIZombieLevel() {
    ...
    return mGameMode == GameMode::GAMEMODE_PUZZLE_I_ZOMBIE_1 ||
...
bool LawnApp::IsScaryPotterLevel() {
    if (mGameMode >= GameMode::GAMEMODE_SCARY_POTTER_1 &&
        mGameMode <= GameMode::GAMEMODE_SCARY_POTTER_ENDLESS)
        return true;
...
bool LawnApp::IsMiniBossLevel() {
    ...
    return (IsAdventureMode() && mPlayerInfo->mLevel == 10) ||
...
bool LawnApp::IsChallengeWithoutSeedBank() {
    return mGameMode == GameMode::GAMEMODE_CHALLENGE_RAINING_SEEDS ||
           mGameMode == GameMode::GAMEMODE_UPSELL || mGameMode == GameMode::GAMEMODE_INTRO ||
           IsWhackAZombieLevel() || IsSquirrelLevel() || IsScaryPotterLevel() ||
...
Reanimation* LawnApp::AddReanimation(float theX, float theY, int theRenderOrder,
                                     ReanimationType theReanimationType) {
    return mEffectSystem->mReanimationHolder->AllocReanimation(theX, theY, theRenderOrder,
                                                               theReanimationType);
}

TodParticleSystem* LawnApp::AddTodParticle(float theX, float theY, int theRenderOrder,
                                           ParticleEffect theEffect) {
    return mEffectSystem->mParticleHolder->AllocParticleSystem(theX, theY, theRenderOrder,
                                                               theEffect);
}

ParticleSystemID LawnApp::ParticleGetID(TodParticleSystem* theParticle) {
    return (ParticleSystemID)mEffectSystem->mParticleHolder->mParticleSystems.DataArrayGetID(
        theParticle);
}

ReanimationID LawnApp::ReanimationGetID(Reanimation* theReanimation) {
    return (ReanimationID)mEffectSystem->mReanimationHolder->mReanimations.DataArrayGetID(
        theReanimation);
}

TodParticleSystem* LawnApp::ParticleGet(ParticleSystemID theParticleID) {
    return mEffectSystem->mParticleHolder->mParticleSystems.DataArrayGet(
        (unsigned int)theParticleID);
}

TodParticleSystem* LawnApp::ParticleTryToGet(ParticleSystemID theParticleID) {
    return mEffectSystem->mParticleHolder->mParticleSystems.DataArrayTryToGet(
        (unsigned int)theParticleID);
}

Reanimation* LawnApp::ReanimationGet(ReanimationID theReanimationID) {
    return mEffectSystem->mReanimationHolder->mReanimations.DataArrayGet(
        (unsigned int)theReanimationID);
}

Reanimation* LawnApp::ReanimationTryToGet(ReanimationID theReanimationID) {
    return mEffectSystem->mReanimationHolder->mReanimations.DataArrayTryToGet(
        (unsigned int)theReanimationID);
}
...
bool LawnApp::CanSpawnYetis() {
    ...
    return HasFinishedAdventure() && (mPlayerInfo->mFinishedAdventure >= 2 ||
                                      mPlayerInfo->mLevel >= aZombieDef.mStartingLevel);
}
...
bool LawnApp::HasBeatenChallenge(GameMode theGameMode) {
    ...
    if (IsSurvivalEndless(theGameMode) || IsEndlessScaryPotter(theGameMode) ||
        IsEndlessIZombie(theGameMode)) {
        return false;
    }
...
void LawnApp::CrazyDaveEnter() {
    ...
    Reanimation* aCrazyDaveReanim =
        AddReanimation(0.0f, 0.0f, 0, ReanimationType::REANIM_CRAZY_DAVE);
...
void LawnApp::CrazyDaveLeave() {
    ...
    if (aCrazyDaveReanim) {
        ...
        aCrazyDaveReanim->PlayReanim("anim_leave", ReanimLoopType::REANIM_PLAY_ONCE_AND_HOLD, 20,
                                     24.0f);
...
void LawnApp::CrazyDaveDoneHanding() {
    ...
    ReanimatorTrackInstance* aHandTrackInstance =
        aCrazyDaveReanim->GetTrackInstanceByName("Dave_handinghand");
...
void LawnApp::CrazyDaveTalkMessage(const SexyString& theMessage) {
    ...
    if (mCrazyDaveState != CrazyDaveState::CRAZY_DAVE_TALKING || doSound) {
        if (doHanding) {
            aCrazyDaveReanim->PlayReanim("anim_talk_handing", ReanimLoopType::REANIM_LOOP, 50,
                                         12.0f);
            ...
            aCrazyDaveReanim->PlayReanim("anim_crazy", ReanimLoopType::REANIM_PLAY_ONCE_AND_HOLD,
                                         50, 12.0f);
            ...
            aCrazyDaveReanim->PlayReanim("anim_smalltalk",
                                         ReanimLoopType::REANIM_PLAY_ONCE_AND_HOLD, 50, 12.0f);
            ...
            aCrazyDaveReanim->PlayReanim("anim_mediumtalk",
                                         ReanimLoopType::REANIM_PLAY_ONCE_AND_HOLD, 50, 12.0f);
            ...
            Reanimation* aWallnutReanim =
                AddReanimation(0.0f, 0.0f, 0, ReanimationType::REANIM_WALLNUT);
            ...
            Reanimation* aHammerReanim =
                AddReanimation(0.0f, 0.0f, 0, ReanimationType::REANIM_HAMMER);
            aHammerReanim->PlayReanim("anim_whack_zombie",
                                      ReanimLoopType::REANIM_PLAY_ONCE_AND_HOLD, 0, 24.0f);
            aHammerReanim->mAnimTime = 1.0f;

            ReanimatorTrackInstance* aHandTrackInstance =
                aCrazyDaveReanim->GetTrackInstanceByName("Dave_handinghand");
            AttachEffect* aAttachEffect =
                AttachReanim(aHandTrackInstance->mAttachmentID, aHammerReanim, 62.0f, 445.0f);
            ...
            aFertilizerReanim->PlayReanim("bag", ReanimLoopType::REANIM_PLAY_ONCE_AND_HOLD, 0,
                                          24.0f);
            ...
            Reanimation* aTreeFoodReanim =
                AddReanimation(0.0f, 0.0f, 0, ReanimationType::REANIM_TREEOFWISDOM_TREEFOOD);
            ...
            if (aWordsCount < 23) {
                ...
                aCrazyDaveReanim->PlayReanim("anim_blahblah",
                                             ReanimLoopType::REANIM_PLAY_ONCE_AND_HOLD, 50, 12.0f);
...
void LawnApp::UpdateCrazyDave() {
    ...
    if (mCrazyDaveState == CrazyDaveState::CRAZY_DAVE_ENTERING ||
        mCrazyDaveState == CrazyDaveState::CRAZY_DAVE_TALKING) {
        ...
        if (aCrazyDaveReanim->mLoopCount > 0) {
            aCrazyDaveReanim->PlayReanim("anim_idle_handing", ReanimLoopType::REANIM_LOOP, 20,
                                         12.0f);
            mCrazyDaveState = CrazyDaveState::CRAZY_DAVE_HANDING_IDLING;
        }
    } else if (mCrazyDaveState == CrazyDaveState::CRAZY_DAVE_LEAVING &&
               aCrazyDaveReanim->mLoopCount > 0) {
        CrazyDaveDie();
    }
...
void LawnApp::DrawCrazyDave(Graphics* g) {
    ...
    if (mCrazyDaveMessageText.size()) {
        ...
        if (clickToContinue) {
            TodDrawString(g, "click to continue", aPosX + 139, aPosY + 140, FONT_PICO129,
...
int LawnApp::GetNumPreloadingTasks() {
    int aTaskCount = 10;
    if (mPlayerInfo) {
        for (SeedType i = SeedType::SEED_PEASHOOTER; i < SeedType::NUM_SEED_TYPES;
             i = (SeedType)((int)i + 1)) {
        ...
        for (ZombieType i = ZombieType::ZOMBIE_NORMAL; i < ZombieType::NUM_ZOMBIE_TYPES;
             i = (ZombieType)((int)i + 1)) {
            if (HasFinishedAdventure() ||
                mPlayerInfo->mLevel >= GetZombieDefinition(i).mStartingLevel) {
...
SexyString LawnApp::Pluralize(int theCount, const SexyString& theSingular,
                              const SexyString& thePlural) {
    if (theCount == 1) {
...
SexyString LawnApp::GetMoneyString(int theAmount) {
    ...
    if (aValue > 999999) {
        return StrFormat("$%d,%03d,%03d", aValue / 1000000,
...
void LawnApp::BetaAddFile(std::list<std::string>& theUploadFileList, std::string theFileName,
                          std::string theShortName) {}

void LawnApp::TraceLoadGroup(const char* theGroupName, int theGroupTime, int theTotalGroupWeigth,
                             int theTaskWeight) {}
