...
void BassMusicInterface::PlayMusic(int theSongId, int theOffset, bool noLoop) {
    ...
    if (anItr != mMusicMap.end()) {
        ...
        BASS_ChannelSetAttribute(aMusicInfo->GetHandle(), BASS_ATTRIB_VOL,
                                 aMusicInfo->mVolume * 100);
        ...
        if (aMusicInfo->mHMusic) {
            ...
            BASS_ChannelFlags(
                aMusicInfo->mHMusic,
                BASS_MUSIC_POSRESET | BASS_MUSIC_RAMP | (noLoop ? 0 : BASS_MUSIC_LOOP), -1);
        ...
        if (aMusicInfo->mHMusic) {
            if (theOffset == -1)
                ...
                BASS_ChannelFlags(aMusicInfo->mHMusic,
                                  BASS_MUSIC_RAMP | (noLoop ? 0 : BASS_MUSIC_LOOP), -1);
...
void BassMusicInterface::SetMusicAmplify(int theSongId, double theAmp) {
    ...
    if (anItr != mMusicMap.end()) {
        BassMusicInfo* aMusicInfo = &anItr->second;
        BASS_ChannelSetAttribute(aMusicInfo->GetHandle(), BASS_ATTRIB_MUSIC_AMPLIFY,
                                 (int)(theAmp * 100));
    }
}
