...
void XMLParser::SetEncodingType(XMLEncodingType theEncoding) {
    ...
    case UTF_16:
        ...
        break;
    case UTF_16_LE:
        mGetCharFunc = &XMLParser::GetUTF16LEChar;
        mForcedEncodingType = true;
        break;
    case UTF_16_BE:
        mGetCharFunc = &XMLParser::GetUTF16BEChar;
        mForcedEncodingType = true;
        break;
    }
}
...
bool XMLParser::AddAttribute(XMLElement* theElement, const SexyString& theAttributeKey,
                             const SexyString& theAttributeValue) {
    ...
    if (p_fread(&aTempChar, 1, 1, mFile) == 1) {
        if ((aTempChar & 0x80) != 0) {
            if ((aTempChar & 0xC0) != 0xC0)
                return false; // sanity check: high bit should not be set without the next highest
                              // bit being set too.
            ...
            *aBytesReadPtr++ = aTempChar;
            ...
            for (aLen = 0; aLen < (int)(sizeof(aMaskData) / sizeof(*aMaskData)); ++aLen) {
                if ((aTempChar & aMaskData[aLen]) == ((aMaskData[aLen] << 1) & aMaskData[aLen]))
                    break;
            }
            if (aLen >= (int)(sizeof(aMaskData) / sizeof(*aMaskData)))
                return false;
        ...
        if ((aTempChar >= 0xD800 && aTempChar <= 0xDFFF) ||
            (aTempChar >= 0xFFFE && aTempChar <= 0xFFFF))
            return false;

        if (aTempChar == 0xFEFF &&
            mFirstChar) // zero-width non breaking space as the first char is a byte order marker.
        {
            mFirstChar = false;
            ...
            if (aVal == 1) {
                ...
                if (theElement->mType == XMLElement::TYPE_COMMENT) {
                    ...
                    *aStrPtr += (SexyChar)c;
                    ...
                    if ((c == L'>') && (aLen >= 3) && ((*aStrPtr)[aLen - 2] == L'-') &&
                        ((*aStrPtr)[aLen - 3] == L'-')) {
                    ...
                    if (c == L'"') {
                        ...
                        if (c == L'<') {
                            ...
                            if (theElement->mType == XMLElement::TYPE_START) {
                                ...
                                if (aAttributeKey == "/") {
                                    // We will get this if we have a space before the />, so we can
                                    // ignore it
                                    ...
                                    if (aLastAttributeKey.length() > 0) {
                                        ...
                                        if ((aLen > 0) && (theElement->mValue[aLen - 1] == '/')) {
                                            // Its an empty element, fake start and end segments
                                            theElement->mValue =
                                                theElement->mValue.substr(0, aLen - 1);
                                            insertEnd = true;
                                        }
                                    }
                                }
                                ...
                                if (insertEnd) {
                                    SexyString anAddString = "</" + theElement->mValue + ">";
                                    ...
                                    for (int i = 0; i < anAddLength; i++)
                                        mBufferedText[anOldSize + i] =
                                            (wchar_t)(anAddString[anAddLength - i - 1]);

                                    // clear out aAttributeKey, since it contains "/" as its value
                                    // and will insert it into the element's attribute map.
                                    aAttributeKey = "";

                                    // OLD: mBufferedText = "</" + theElement->mValue + ">" +
                                    // mBufferedText;
                                }
                        ...
                        if (theElement->mType == XMLElement::TYPE_START) {
                            if (hasSpace) {
                                if ((!doingAttribute) || ((!AttributeVal) && (c != '=')) ||
                                    ((AttributeVal) &&
                                     ((aAttributeValue.length() > 0) || gotEndQuote))) {
                                    if (doingAttribute) {
                                        aAttributeKey = XMLDecodeString(aAttributeKey);
                                        aAttributeValue = XMLDecodeString(aAttributeValue);
