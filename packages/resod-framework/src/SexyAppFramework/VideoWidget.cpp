...
#include <SexyAppFramework/VideoWidget.h>
#include <filesystem>
...
void VideoWidget::LoadVideo(const SexyString& aPath) {
    ...
    mVidStreamIndex =
        av_find_best_stream(mFormatContext, AVMEDIA_TYPE_VIDEO, -1, -1, &mVideoCodec, 0);
    mVideoDecoder = avcodec_alloc_context3(mVideoCodec);
    mVideoDecoder->thread_count = 0;
    avcodec_parameters_to_context(mVideoDecoder,
                                  mFormatContext->streams[mVidStreamIndex]->codecpar);
    ...
    mSwSContext =
        sws_getContext(mVidWidth, mVidHeight, (AVPixelFormat)mVideoFrame->format, mVidWidth,
                       mVidHeight, AV_PIX_FMT_RGB0, SWS_BILINEAR, nullptr, nullptr, nullptr);
    ...
    case RenderingBackend::BACKEND_OPENGL: {
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, mVidWidth, mVidHeight, 0, GL_RGBA, GL_UNSIGNED_BYTE,
                     nullptr);
    ...
    case RenderingBackend::BACKEND_SDL3: {
        mVideoTexture = SDL_CreateTexture(((SDL3Renderer*)mApp->mRenderer)->mBackendRenderer,
                                          SDL_PIXELFORMAT_IYUV, SDL_TEXTUREACCESS_STREAMING,
                                          mVidWidth, mVidHeight);
    ...
    mAudioStreamIndex =
        av_find_best_stream(mFormatContext, AVMEDIA_TYPE_AUDIO, -1, -1, &mAudioCodec, 0);
    mAudioDecoder = avcodec_alloc_context3(mAudioCodec);
    avcodec_parameters_to_context(mAudioDecoder,
    ...
    swr_alloc_set_opts2(&mSwrContext, &anOutLayout, AV_SAMPLE_FMT_S16, 44100, &anInLayout,
                        mAudioDecoder->sample_fmt, mAudioDecoder->sample_rate, 0, nullptr);
        ...
        while (av_read_frame(mFormatContext, &mPacket) >= 0) {
            if (mPacket.stream_index == mAudioStreamIndex) {
                ...
                if (avcodec_receive_frame(mAudioDecoder, mAudioFrame) == 0) {
                    ...
                    int aLineSize = 0;
                    av_samples_alloc(&aOutput, &aLineSize, mAudioChannels, mAudioFrame->nb_samples,
                                     AV_SAMPLE_FMT_S16, 0);
                    int aNumSamples =
                        swr_convert(mSwrContext, &aOutput, mAudioFrame->nb_samples,
...
void VideoWidget::Draw(Graphics* g) {
    ...
    if (mVideoStartedRendering)
        g->mDestImage->BltRawTexture(mVideoTexture, mVidWidth, mVidHeight,
                                     ...
                                     Rect(0, 0, mVidWidth, mVidHeight), g->mClipRect, g->mColor,
                                     g->mDrawMode);
    else {
        g->PushState();
            ...
            if (avcodec_receive_frame(mVideoDecoder, mVideoFrame) == 0) {
                ...
                case RenderingBackend::BACKEND_SDL3: {
                    if (mVideoFrame->data[0] && mVideoFrame->data[1] && mVideoFrame->data[2]) {
                        SDL_UpdateYUVTexture((SDL_Texture*)mVideoTexture, nullptr,
                                             mVideoFrame->data[0], mVideoFrame->linesize[0],
                                             mVideoFrame->data[1], mVideoFrame->linesize[1],
                ...
                case RenderingBackend::BACKEND_OPENGL: {
                    ...
                    sws_scale(mSwSContext, mVideoFrame->data, mVideoFrame->linesize, 0,
                              mVideoFrame->height, aDest, aDestLineSize);

                    glBindTexture(GL_TEXTURE_2D, *(GLuint*)mVideoTexture);
                    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, mVideoFrame->width, mVideoFrame->height,
            ...
            while (avcodec_receive_frame(mAudioDecoder, mAudioFrame) == 0) {
                if (aProcessedBuffers > 0) {
                    ...
                    int aNumSamples = swr_convert(mSwrContext, &aOutput, mAudioFrame->nb_samples,
                                                  mAudioFrame->data, mAudioFrame->nb_samples);
                    int aSize = aNumSamples * mAudioChannels * sizeof(int16_t);
                    alBufferData(aCurrentBuffer, mAudioFormat, aOutput, aSize, 44100);
                    alSourceQueueBuffers(mVidAudioSource, 1, &aCurrentBuffer);
                    av_freep(&aOutput);
