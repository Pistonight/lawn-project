...
enum SexyMessageBoxFlags {
    MESSAGEBOX_ERROR = 0x00000001u,
    MESSAGEBOX_WARNING = 0x00000002u,
    MESSAGEBOX_INFORMATION = 0x00000004u,

    MESSAGEBOX_BUTTONS_LEFT_TO_RIGHT = 0x00000010u,
    MESSAGEBOX_BUTTONS_RIGHT_TO_LEFT = 0x00000020u,

    MESSAGEBOX_BTN_OK = 0x00000100u,
    MESSAGEBOX_BTN_CANCEL = 0x00000200u,
    MESSAGEBOX_BTN_YES = 0x00000400u,
    MESSAGEBOX_BTN_NO = 0x00000800u,

    MESSAGEBOX_OK_CANCEL = MESSAGEBOX_BTN_OK | MESSAGEBOX_BTN_CANCEL,
...
protected:
    ...
    // Registry helpers
    bool RegistryRead(const std::string& theValueName, JSONRegistryType* theType, uint8_t* theValue,
                      uint32_t* theLength);
    bool RegistryReadKey(const std::string& theValueName, JSONRegistryType* theType,
                         uint8_t* theValue, uint32_t* theLength, int theMainKey = 0);
    bool RegistryWrite(const std::string& theValueName, JSONRegistryType theType,
...
public:
    ...
    virtual Dialog* NewDialog(int theDialogId, bool isModal, const SexyString& theDialogHeader,
    ...
    virtual int MsgBox(const std::string& theText, const std::string& theTitle = "Message",
                       int theFlags = 0);
    ...
    virtual void
    HandleNotifyGameMessage(int theType,
                            int theParam); // for HWND_BROADCAST of mNotifyGameMessage (0-1000 are
                                           // reserved for SexyAppBase for theType)
    ...
    virtual SharedImageRef GetSharedImage(const std::string& theFileName,
                                          const std::string& theVariant = "", bool* isNew = NULL);
    ...
    GPUImage* CreateCrossfadeImage(Image* theImage1, const Rect& theRect1, Image* theImage2,
    ...
    virtual Dialog* DoDialog(int theDialogId, bool isModal, const SexyString& theDialogHeader,
                             const SexyString& theDialogLines, const SexyString& theDialogFooter,
                             int theButtonMode);
    ...
    bool RegistryWriteData(const std::string& theValueName, const uint8_t* theValue,
                           uint32_t theLength);
    ...
    bool ReadBufferFromFile(const std::string& theFileName, Buffer* theBuffer,
                            bool dontWriteToDemo = false); // UNICODE
    bool WriteBytesToFile(const std::string& theFileName, const void* theData,
