...
Version::Version(const Version& theVersion)
    : Version(theVersion.mMajor, theVersion.mMinor, theVersion.mPatch, theVersion.mBuildNumber) {}
...
bool Version::isOlderThan(const Version& theVersion) {
    ...
    if (mBuildNumber != -1 &&
        theVersion.mBuildNumber != -1) // making the build number -1 makes it ignore version checks
    {
...
bool Version::isEqual(const Version& theVersion) {
    return mMajor == theVersion.mMajor && mMajor == theVersion.mMinor &&
           mPatch == theVersion.mPatch && mBuildNumber == theVersion.mBuildNumber;
}
