...
std::string GetCallStack() {
    ...
    for (size_t i = 0; i < trace.frames.size(); i++) {
        // KiUserExceptionDispatcher is the border between the main code and the exception so we
        // start the stack from there -Electr0Gunner
...
void SEHCatcher::ShowErrorDialog(const std::string& theErrorTitle,
    ...
    while (!mExiting) {
        ...
        ImGui::BeginChild("ErrorDetails", ImVec2(0, 200), true,
        ...
        if (ImGui::Button("Send Issue", ImVec2(buttonWidth, 0))) {
            if (!SDL_OpenURL(mSubmitHost.c_str())) {
                SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "Error", mSubmitErrorMessage.c_str(),
                                         aWindow);
            }
