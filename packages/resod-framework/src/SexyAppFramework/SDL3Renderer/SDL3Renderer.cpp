...
bool SDL3Renderer::InitRendererObject() {
    ...
    if (mBackendRenderer == nullptr) {
        SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "Renderer Creation Failed", SDL_GetError(),
                                 nullptr);
        return false;
    }
    ...
    mTargetTexture =
        SDL_CreateTexture(mBackendRenderer, SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_TARGET,
                          mApp->mHighResolution ? mPresentationRect.mWidth : mWidth,
                          mApp->mHighResolution ? mPresentationRect.mHeight : mHeight);
    if (mTargetTexture == nullptr) {
        SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "Screen Texture-Buffer Creation Failed",
...
void SDL3Renderer::SetLinearBlend_SDL(SDL_Texture* theTexture, bool linearBlend) {
    SDL_SetTextureScaleMode(theTexture, linearBlend ? SDL_ScaleMode::SDL_SCALEMODE_LINEAR
...
bool SDL3Renderer::Redraw(Rect* theClipRect) {
    ...
    SDL_SetTextureScaleMode(mTargetTexture, mApp->mScreenFiltering == MODE_LINEAR
    ...
    if (theClipRect) {
        if (mApp->mHighResolution) {
            ...
            SDL_Rect clipRect = SDL_Rect{theClipRect->mX, theClipRect->mY, theClipRect->mWidth,
                                         theClipRect->mHeight};
            SDL_SetRenderClipRect(mBackendRenderer, &clipRect);
        }
    }
    if (mApp->mHighResolution) {
        SDL_FRect aDest = {(float)mPresentationRect.mX, (float)mPresentationRect.mY,
                           (float)mPresentationRect.mWidth, (float)mPresentationRect.mHeight};
        gRenderingPreDrawError =
            !SDL_RenderTexture(mBackendRenderer, mTargetTexture, nullptr, &aDest);
    } else
        gRenderingPreDrawError =
            !SDL_RenderTexture(mBackendRenderer, mTargetTexture, nullptr, nullptr);
...
void* SDL3Renderer::CreateTexture(void* thePixels, int theWidth, int theHeight,
                                  RawPixelFormat thePixelFormat, int theAlignment) {
    SDL_Texture* aTex = SDL_CreateTexture(mBackendRenderer, SDL_PIXELFORMAT_ARGB8888,
                                          SDL_TEXTUREACCESS_TARGET, theWidth, theHeight);

    SDL_SetTextureScaleMode(aTex, SDL_SCALEMODE_LINEAR);
    SDL_UpdateTexture(aTex, nullptr, thePixels,
                      theWidth * SDL_BYTESPERPIXEL(SDL_PIXELFORMAT_ARGB8888));
    gSDLTextureCount++;
...
void SDL3Renderer::UpdateViewport() {
    ...
    SDL_SetRenderLogicalPresentation(mBackendRenderer, mApp->mHighResolution ? 0 : mWidth,
                                     mApp->mHighResolution ? 0 : mHeight,
                                     mApp->mHighResolution ? SDL_LOGICAL_PRESENTATION_DISABLED
...
bool SDL3Renderer::Init() {
    ...
    const SDL_DisplayMode* aMode =
        SDL_GetCurrentDisplayMode(SDL_GetDisplayForWindow(mApp->mWindow->mInternalWindow));
...
bool SDL3Renderer::RecoverBits(MemoryImage* theImage) {
    ...
    if (aData->mBitsChangedCount !=
        theImage->mBitsChangedCount) // bits have changed since texture was created
        return false;
...
void SDL3TextureData::CreateTextures(MemoryImage* theImage, void* theRendererData) {
    ...
    if (createTexture) {
        mPixelFormat = theImage->mGPUFlags & ImageFlag_UseA4R4G4B4 ? PixelFormat_A4R4G4B4
                                                                   : PixelFormat_A8R8G8B8;
        SDL_PixelFormat aFmt = mPixelFormat == PixelFormat_A8R8G8B8 ? SDL_PIXELFORMAT_ARGB8888
                                                                    : SDL_PIXELFORMAT_RGBA4444;
        mTexturePtr = SDL_CreateTexture((SDL_Renderer*)theRendererData, aFmt,
                                        SDL_TEXTUREACCESS_TARGET, aWidth, aHeight);

        SDL_SetTextureScaleMode(mTexturePtr, SDL_SCALEMODE_LINEAR);
        SDL_UpdateTexture(mTexturePtr, nullptr, theImage->GetBits(),
                          aWidth * SDL_BYTESPERPIXEL(aFmt));

        SDL3Renderer::gSDLTextureCount++;
    } else if (mBitsChangedCount != theImage->mBitsChangedCount) {
        SDL_UpdateTexture(mTexturePtr, nullptr, theImage->GetBits(),
                          aWidth * SDL_BYTESPERPIXEL(mPixelFormat == PixelFormat_A8R8G8B8
...
void SDL3Renderer::Blt(Image* theImage, float theX, float theY, const Rect& theSrcRect,
                       const Color& theColor, int theDrawMode, bool linearFilter) {
    ...
    SDL_FRect aDestRect = {(float)theX * aScaleX, (float)theY * aScaleX,
                           (float)theSrcRect.mWidth * aScaleY, (float)theSrcRect.mHeight * aScaleY};
    SDL_SetRenderTarget(mBackendRenderer, mTargetTexture);
...
void SDL3Renderer::BltClipF(Image* theImage, float theX, float theY, const Rect& theSrcRect,
    ...
    SDL_SetTextureAlphaMod(aTexture, theColor.mAlpha);
    SDL_SetTextureBlendMode(aTexture, Get_SDL_NativeBlendMode(ChooseBlendMode(theDrawMode)));
    SDL_Rect aClipRect = GetTransformedClipRect(theClipRect, mWidth, mHeight, mPresentationRect,
                                                mApp->mHighResolution);
...
void SDL3Renderer::BltMirror(Image* theImage, float theX, float theY, const Rect& theSrcRect,
                             const Color& theColor, int theDrawMode, bool linearFilter) {
    MemoryImage* aImg = (MemoryImage*)theImage;
    ...
    SDL_RenderTextureRotated(mBackendRenderer, aTexture, &aSrcRect, &aDestRect, 0, nullptr,
                             SDL_FLIP_HORIZONTAL);
    SDL_SetRenderTarget(mBackendRenderer, nullptr);
}
...
void SDL3Renderer::StretchBlt(Image* theImage, const Rect& theDestRect, const Rect& theSrcRect,
    ...
    SDL_FRect aSrcRect = {(float)theSrcRect.mX, (float)theSrcRect.mY, (float)theSrcRect.mWidth,
                          (float)theSrcRect.mHeight};
    SDL_FRect aDestRect = {(float)theDestRect.mX * aScaleX, (float)theDestRect.mY * aScaleY,
                           (float)theDestRect.mWidth * aScaleX,
                           (float)theDestRect.mHeight * aScaleY};
    SDL_SetRenderTarget(mBackendRenderer, mTargetTexture);
...
void SDL3Renderer::BltRotated(Image* theImage, float theX, float theY, const Rect& theClipRect,
                              const Color& theColor, int theDrawMode, double theRot,
    ...
    SDL_SetRenderDrawBlendMode(mBackendRenderer,
                               Get_SDL_NativeBlendMode(ChooseBlendMode(theDrawMode)));
    SDL_SetRenderDrawColor(mBackendRenderer, theColor.mRed, theColor.mGreen, theColor.mBlue,
                           theColor.mAlpha);
    float aScaleX = 1.0f;
    float aScaleY = 1.0f;
    if (mApp->mHighResolution) {
    ...
    SDL_RenderTextureRotated(mBackendRenderer, aTexture, &aSrcRect, &aDestRect, theRot, &aRotCenter,
                             SDL_FLIP_NONE);
    SDL_SetRenderClipRect(mBackendRenderer, nullptr);
    SDL_SetRenderTarget(mBackendRenderer, nullptr);
}
...
void SDL3Renderer::BltTransformed(Image* theImage, const Rect& theClipRect, const Color& theColor,
                                  int theDrawMode, const Rect& theSrcRect,
                                  const SexyMatrix3& theTransform, bool linearFilter, float theX,
    ...
    SDL_FColor aColor = {theColor.GetRed() / 255.0f, theColor.GetGreen() / 255.0f,
                         theColor.GetBlue() / 255.0f, theColor.GetAlpha() / 255.0f};

    SDL_Vertex vertices[4] = {
        {ScaleNativeSDLPoint(TransformToPoint(x1, y1, theTransform, theX, theY), aScaleX, aScaleY),
         aColor,
         {u1, v1}}, // TL
        {ScaleNativeSDLPoint(TransformToPoint(x2, y2, theTransform, theX, theY), aScaleX, aScaleY),
         aColor,
         {u2, v1}}, // TR
        {ScaleNativeSDLPoint(TransformToPoint(x3, y3, theTransform, theX, theY), aScaleX, aScaleY),
         aColor,
         {u1, v2}}, // BL
        {ScaleNativeSDLPoint(TransformToPoint(x4, y4, theTransform, theX, theY), aScaleX, aScaleY),
         aColor,
         {u2, v2}} // BR
    };
...
void SDL3Renderer::DrawLine(double theStartX, double theStartY, double theEndX, double theEndY,
    ...
    SDL_RenderLine(mBackendRenderer, theStartX * aScaleX, theStartY * aScaleY, theEndX * aScaleX,
                   theEndY * aScaleY);
...
void SDL3Renderer::FillRect(const Rect& theRect, const Color& theColor, int theDrawMode) {
    ...
    SDL_FRect aRect = {(float)theRect.mX * aScaleX, (float)theRect.mY * aScaleY,
                       (float)theRect.mWidth * aScaleX, (float)theRect.mHeight * aScaleY};
...
void SDL3Renderer::DrawTriangle(const TriVertex& p1, const TriVertex& p2, const TriVertex& p3,
                                const Color& theColor, int theDrawMode) {
    SDL_SetRenderTarget(mBackendRenderer, mTargetTexture);
    SDL_SetRenderDrawBlendMode(mBackendRenderer,
                               Get_SDL_NativeBlendMode(ChooseBlendMode(theDrawMode)));
    ...
    SDL_Vertex vertices[3] = {
        {ScaleNativeSDLPoint(SDL_FPoint{p1.x, p1.y}, aScaleX, aScaleY), aColor, {p1.u, p1.v}},
...
void SDL3Renderer::DrawTriangleTex(const TriVertex& p1, const TriVertex& p2, const TriVertex& p3,
                                   const Color& theColor, int theDrawMode, Image* theTexture,
                                   bool blend) {
    ...
    int indices[] = {0, 1, 2};
    SDL_TextureAddressMode aMode =
        mCurrentUVWrapMode == UV_WRAP ? SDL_TEXTURE_ADDRESS_WRAP : SDL_TEXTURE_ADDRESS_CLAMP;
    ...
    SDL_SetRenderClipRect(mBackendRenderer, nullptr);
    SDL_SetRenderTextureAddressMode(mBackendRenderer, SDL_TEXTURE_ADDRESS_AUTO,
                                    SDL_TEXTURE_ADDRESS_AUTO);
    SDL_SetRenderTarget(mBackendRenderer, nullptr);
}

void SDL3Renderer::DrawTrianglesTex(const TriVertex theVertices[][3], int theNumTriangles,
                                    const Color& theColor, int theDrawMode, Image* theTexture,
    ...
    for (int aTriangleIdx = 0; aTriangleIdx < theNumTriangles; aTriangleIdx++) {
        ...
        for (int aVert = 0; aVert < 3; aVert++) {
            ...
            anExtractedColor[aVert].r = ((theCurrentVertex[aVert].color >> 16) & 0xFF) * aColor.r *
                                        aConvertBackToCorectRange;
            anExtractedColor[aVert].g = ((theCurrentVertex[aVert].color >> 8) & 0xFF) * aColor.g *
                                        aConvertBackToCorectRange;
            anExtractedColor[aVert].b =
                (theCurrentVertex[aVert].color & 0xFF) * aColor.b * aConvertBackToCorectRange;
            anExtractedColor[aVert].a = ((theCurrentVertex[aVert].color >> 24) & 0xFF) * aColor.a *
                                        aConvertBackToCorectRange;
...
void SDL3Renderer::DrawTrianglesTexStrip(const TriVertex theVertices[], int theNumTriangles,
                                         const Color& theColor, int theDrawMode, Image* theTexture,
                                         float tx, float ty, bool blend) {
...
void SDL3Renderer::BltRawTexture(void* theTexture, int theTexWidth, int theTexHeight,
