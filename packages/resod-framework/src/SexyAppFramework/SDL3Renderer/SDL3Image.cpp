...
bool SDL3Image::GenerateSurface() {
    ...
    mSurface = SDL_CreateTexture(mSDL3Renderer->mBackendRenderer, SDL_PIXELFORMAT_ARGB8888,
...
void SDL3Image::CreateImageBuffers() {
    if (Check3D(this) || mSurface != nullptr)
        return;
    mSurface = SDL_CreateTexture(mSDL3Renderer->mBackendRenderer, SDL_PIXELFORMAT_RGBA8888,
                                 SDL_TEXTUREACCESS_TARGET, mWidth, mHeight);
}
...
void SDL3Image::ImplFillRect(const Rect& theRect, const Color& theColor, int theDrawMode) {
    ...
    SDL_FRect aRect = {(float)theRect.mX, (float)theRect.mY, (float)theRect.mWidth,
                       (float)theRect.mHeight};

    SDL_SetRenderDrawColor(mSDL3Renderer->mBackendRenderer, theColor.mRed, theColor.mGreen,
                           theColor.mBlue, theColor.mAlpha);
    SDL_SetRenderDrawBlendMode(
        mSDL3Renderer->mBackendRenderer,
...
void SDL3Image::ImplBlt(Image* theImage, int theX, int theY, const Rect& theSrcRect,
                        const Color& theColor, int theDrawMode) {
    ...
    SDL_FRect aDestRect = {(float)theX, (float)theY, (float)theSrcRect.mWidth,
                           (float)theSrcRect.mHeight};
    SDL_SetRenderTarget(mSDL3Renderer->mBackendRenderer, (SDL_Texture*)mSurface);
    ...
    SDL_SetTextureBlendMode(aTexture, mSDL3Renderer->Get_SDL_NativeBlendMode(
                                          mSDL3Renderer->ChooseBlendMode(theDrawMode)));
    SDL_RenderTexture(mSDL3Renderer->mBackendRenderer, aTexture, &aSrcRect, &aDestRect);
    SDL_SetRenderTarget(mSDL3Renderer->mBackendRenderer, nullptr);
}

void SDL3Image::ImplBltF(Image* theImage, float theX, float theY, const Rect& theSrcRect,
...
void SDL3Image::ImplBltRotated(Image* theImage, float theX, float theY, const Rect& theSrcRect,
                               const Rect& theClipRect, const Color& theColor, int theDrawMode,
    ...
    SDL_RenderTextureRotated(mSDL3Renderer->mBackendRenderer, aTexture, &aSrcRect, &aDestRect,
                             theRot, &aRotCenter, SDL_FLIP_NONE);
    SDL_SetRenderTarget(mSDL3Renderer->mBackendRenderer, nullptr);
}
...
void SDL3Image::ImplStretchBlt(Image* theImage, const Rect& theDestRect, const Rect& theSrcRect,
    ...
    SDL_Rect aClipRect = {theClipRect.mX, theClipRect.mY, theClipRect.mWidth, theClipRect.mHeight};
    SDL_SetRenderClipRect(mSDL3Renderer->mBackendRenderer, &aClipRect);
    SDL_RenderTextureRotated(mSDL3Renderer->mBackendRenderer, aTexture, &aSrcRect, &aDestRect, 0,
...
void SDL3Image::ImplBltTrianglesTex(Image* theTexture, const TriVertex theVertices[][3],
...
void SDL3Image::ImplBltMirror(Image* theImage, int theX, int theY, const Rect& theSrcRect,
                              const Color& theColor, int theDrawMode) {
    MemoryImage* aImg = (MemoryImage*)theImage;
...
void SDL3Image::ImplStretchBltMirror(Image* theImage, const Rect& theDestRectOrig,
    ...
    SDL_FRect aSrcRect = {(float)theSrcRect.mX, (float)theSrcRect.mY, (float)theSrcRect.mWidth,
                          (float)theSrcRect.mHeight};
    SDL_FRect aDestRect = {(float)theDestRectOrig.mX, (float)theDestRectOrig.mY,
                           (float)theDestRectOrig.mWidth, (float)theDestRectOrig.mHeight};
    SDL_SetRenderTarget(mSDL3Renderer->mBackendRenderer, (SDL_Texture*)mSurface);
...
void SDL3Image::ImplBltRawTexture(void* theTexture, int theTexWidth, int theTexHeight,
