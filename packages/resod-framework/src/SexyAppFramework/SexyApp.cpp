...
SexyApp::SexyApp() {
    ...
    SetString("UPDATE_CHECK_BODY", "Contacting PopCap.com to determine if there are any updates "
...
void SexyApp::ReadFromRegistry() {
    ...
    mRegisterLink =
        "http://www.popcap.com/register.php?theGame=" + mProdName + "&referid=" + mReferId;
...
bool SexyApp::OpenHTMLTemplate(const std::string& theTemplateFile,
...
bool SexyApp::ShouldCheckForUpdate() {
    ...
    return (
        (mLastVerCheckQueryTime == 0) || (!mLastShutdownWasGraceful) ||
...
std::string SexyApp::GetGameSEHInfo() {
    ...
    std::string anInfoString =
        SexyAppBase::GetGameSEHInfo() + "Times Played: " + std::string(aGamesPlayedStr) + "\r\n" +
        "Build Num: " + StrFormat("%d", mBuildNum) + "\r\n" + "Build Date: " + mBuildDate + "\r\n";
