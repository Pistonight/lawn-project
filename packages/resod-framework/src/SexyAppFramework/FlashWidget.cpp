...
#include <SexyAppFramework/FlashWidget.h>
...
struct __declspec(uuid("d27cdb6c-ae6d-11cf-96b8-444553540000")) IShockwaveFlash : IDispatch {
    ...
    __declspec(property(get = GetDeviceFont, put = PutDeviceFont)) VARIANT_BOOL DeviceFont;
    __declspec(property(get = GetEmbedMovie, put = PutEmbedMovie)) VARIANT_BOOL EmbedMovie;
    ...
    __declspec(property(get = GetBackgroundColor, put = PutBackgroundColor)) long BackgroundColor;
...
struct __declspec(uuid("d27cdb70-ae6d-11cf-96b8-444553540000")) IFlashFactory : IUnknown {};

struct __declspec(uuid("a6ef9860-c720-11d0-9337-00a0c90dcaa9")) IDispatchEx : IDispatch {
    //
    // Wrapper methods for error-handling
    //
    ...
    HRESULT RemoteInvokeEx(long id, unsigned long lcid, unsigned long dwFlags,
                           struct DISPPARAMS* pdp, VARIANT* pvarRes, struct EXCEPINFO* pei,
                           struct IServiceProvider* pspCaller, unsigned int cvarRefArg,
                           unsigned int* rgiRefArg, VARIANT* rgvarRefArg);
    HRESULT DeleteMemberByName(_bstr_t bstrName, unsigned long grfdex);
    HRESULT DeleteMemberByDispID(long id);
    HRESULT GetMemberProperties(long id, unsigned long grfdexFetch, unsigned long* pgrfdex);
...
struct __declspec(uuid("d27cdb72-ae6d-11cf-96b8-444553540000")) IFlashObjectInterface
    : IDispatchEx {};

struct __declspec(uuid("d27cdb71-ae6d-11cf-96b8-444553540000")) FlashObjectInterface;
...
public:
    ...
    virtual ~FlashSink() { mFlashWidget->mCOMCount--; }
    ...
    HRESULT Init(FlashWidget* theFlashWidget) {
        ...
        if ((mFlashWidget->mFlashInterface->QueryInterface(IID_IConnectionPointContainer,
                                                           (void**)&aConnectionPoint) == S_OK) &&
            (aConnectionPoint->FindConnectionPoint(
                 __uuidof(ShockwaveFlashObjects::_IShockwaveFlashEvents), &mConnectionPoint) ==
             S_OK)) {
    ...
    HRESULT FSCommand(_bstr_t command, _bstr_t args) {
        if (mFlashWidget->mFlashListener != NULL)
            mFlashWidget->mFlashListener->FlashCommand(mFlashWidget->mId, (char*)command,
                                                       (char*)args);
        return S_OK;
    }
};
...
public:
    ...
    HRESULT STDMETHODCALLTYPE ContextSensitiveHelp(/* [in] */ BOOL fEnterMode) { return S_OK; }
    ...
    HRESULT STDMETHODCALLTYPE GetWindowContext(/* [out] */ IOleInPlaceFrame __RPC_FAR * __RPC_FAR *
                                                   ppFrame,
                                               /* [out] */ IOleInPlaceUIWindow __RPC_FAR *
                                                   __RPC_FAR * ppDoc,
    ...
    HRESULT STDMETHODCALLTYPE Scroll(/* [in] */ SIZE scrollExtant) { return S_OK; }

    HRESULT STDMETHODCALLTYPE OnUIDeactivate(/* [in] */ BOOL fUndoable) { return S_OK; }
    ...
    HRESULT STDMETHODCALLTYPE OnPosRectChange(/* [in] */ LPCRECT lprcPosRect) { return S_OK; }

    HRESULT STDMETHODCALLTYPE OnInPlaceActivateEx(/* [out] */ BOOL __RPC_FAR* pfNoRedraw,
                                                  /* [in] */ DWORD dwFlags) {
        return S_OK;
    }

    HRESULT STDMETHODCALLTYPE OnInPlaceDeactivateEx(/* [in] */ BOOL fNoRedraw) { return S_OK; }
    ...
    HRESULT STDMETHODCALLTYPE ReleaseDC(/* [in] */ HDC hDC) { return E_INVALIDARG; }
    ...
    HRESULT STDMETHODCALLTYPE InvalidateRect(/* [in] */ LPCRECT pRect, /* [in] */ BOOL fErase) {
        if (pRect == NULL) {
            ...
            mFlashWidget->mDirtyRect = Rect(pRect->left, pRect->top, pRect->right - pRect->left,
                                            pRect->bottom - pRect->top);
            mFlashWidget->mFlashDirty = true;
        } else {
            mFlashWidget->mDirtyRect = mFlashWidget->mDirtyRect.Union(Rect(
    ...
    HRESULT STDMETHODCALLTYPE ScrollRect(/* [in] */ INT dx,
                                         /* [in] */ INT dy,
    ...
    HRESULT STDMETHODCALLTYPE OnDefWindowMessage(/* [in] */ UINT msg,
                                                 /* [in] */ WPARAM wParam,
                                                 /* [in] */ LPARAM lParam,
...
FlashWidget::FlashWidget(int theId, FlashListener* theFlashListener) {
    ...
    if (mFlashLibHandle != NULL) {
        ...
        DllGetClassObjectFunc aDllGetClassObjectFunc =
            (DllGetClassObjectFunc)GetProcAddress(mFlashLibHandle, "DllGetClassObject");
        aResult =
            aDllGetClassObjectFunc(CLSID_ShockwaveFlash, IID_IClassFactory, (void**)&aClassFactory);
        aClassFactory->CreateInstance(NULL, IID_IOleObject, (void**)&mOleObject);
        aClassFactory->Release();
    } else {
        CoCreateInstance(CLSID_ShockwaveFlash, NULL, CLSCTX_INPROC_SERVER, IID_IOleObject,
                         (void**)&mOleObject);
    }
...
double FlashWidget::GetFlashVersion() {
    ...
    if (FAILED(CoCreateInstance(CLSID_ShockwaveFlash, NULL, CLSCTX_INPROC_SERVER, IID_IOleObject,
                                (void**)&anOleObject)))
        return 0.0;
...
void FlashWidget::MouseMove(int x, int y) {
    ...
    LRESULT aResult;
    Point anAbsPos = GetAbsPos();
    mWindowlessObject->OnWindowMessage(WM_MOUSEMOVE, 0,
                                       MAKELPARAM(x + anAbsPos.mX, y + anAbsPos.mY), &aResult);
...
void FlashWidget::MouseDown(int x, int y, int theBtnNum, int theClickCount) {
    if (theBtnNum == 0) {
        LRESULT aResult;
        Point anAbsPos = GetAbsPos();
        mWindowlessObject->OnWindowMessage(WM_LBUTTONDOWN, MK_LBUTTON,
                                           MAKELPARAM(x + anAbsPos.mX, y + anAbsPos.mY), &aResult);
        CheckCursor();
    }
}
...
void FlashWidget::MouseUp(int x, int y, int theBtnNum, int theClickCount) {
    if (theBtnNum == 0) {
        LRESULT aResult;
        Point anAbsPos = GetAbsPos();
        mWindowlessObject->OnWindowMessage(WM_LBUTTONUP, 0,
                                           MAKELPARAM(x + anAbsPos.mX, y + anAbsPos.mY), &aResult);
        CheckCursor();
    }
}
...
void FlashWidget::MouseDrag(int x, int y) {
    if (mWidgetManager->IsLeftButtonDown()) {
        LRESULT aResult;
        Point anAbsPos = GetAbsPos();
        mWindowlessObject->OnWindowMessage(WM_MOUSEMOVE, MK_LBUTTON,
                                           MAKELPARAM(x + anAbsPos.mX, y + anAbsPos.mY), &aResult);
        CheckCursor();
    }
}
