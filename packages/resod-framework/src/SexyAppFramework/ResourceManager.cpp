...
bool ResourceManager::ParseCommonResource(XMLElement& theElement, BaseRes* theRes, ResMap& theMap) {
    ...
    const SexyString& aPath = theElement.mAttributes["path"];
    if (aPath.empty())
...
bool ResourceManager::ParseSoundResource(XMLElement& theElement) {
    ...
    if (anItr != theElement.mAttributes.end())
        sexysscanf(anItr->second.c_str(), "%lf", &aRes->mVolume);

    anItr = theElement.mAttributes.find("pan");
    if (anItr != theElement.mAttributes.end())
        sexysscanf(anItr->second.c_str(), "%d", &aRes->mPanning);
...
bool ResourceManager::ParseImageResource(XMLElement& theElement) {
    ...
    aRes->mPurgeBits = (theElement.mAttributes.find("nobits") != theElement.mAttributes.end()) ||
                       ((mApp->Is3DAccelerated()) && (theElement.mAttributes.find("nobits3d") !=
                                                      theElement.mAttributes.end())) ||
                       ((!mApp->Is3DAccelerated()) &&
                        (theElement.mAttributes.find("nobits2d") != theElement.mAttributes.end()));
    aRes->mA8R8G8B8 = theElement.mAttributes.find("a8r8g8b8") != theElement.mAttributes.end();
    aRes->mMinimizeSubdivisions =
        theElement.mAttributes.find("minsubdivide") != theElement.mAttributes.end();
    ...
    if (anItr != theElement.mAttributes.end())
        sexysscanf(anItr->second.c_str(), "%x", &aRes->mAlphaColor);
    ...
    if (anItr != theElement.mAttributes.end()) {
        ...
        else if (sexystricmp(aType, "once") == 0)
            anAnimType = AnimType_Once;
        else if (sexystricmp(aType, "loop") == 0)
            anAnimType = AnimType_Loop;
        else if (sexystricmp(aType, "pingpong") == 0)
...
bool ResourceManager::LoadAlphaGridImage(ImageRes* theRes, GPUImage* theImage) {
    ...
    if (anAlphaImage->mWidth != aCelWidth || anAlphaImage->mHeight != aCelHeight)
        return Fail(StrFormat("GridAlphaImage size mismatch between %s and %s",
                              theRes->mPath.c_str(), theRes->mAlphaGridImage.c_str()));
...
bool ResourceManager::DoLoadImage(ImageRes* theRes) {
    bool lookForAlpha =
        theRes->mAlphaImage.empty() && theRes->mAlphaGridImage.empty() && theRes->mAutoFindAlpha;
    ...
    SharedImageRef aSharedImageRef =
        gSexyAppBase->GetSharedImage(theRes->mPath, theRes->mVariant, &isNew);
...
bool ResourceManager::DoLoadFont(FontRes* theRes) {
    ...
    if (theRes->mSysFont) {
        aFont = new SysFont(theRes->mPath, theRes->mSize, theRes->mBold, theRes->mItalic,
                            theRes->mUnderline);
