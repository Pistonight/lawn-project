...
SysFont::SysFont(const std::string& theFace, int thePointSize, bool bold, bool italics,
                 bool underline) {
    Init(gSexyAppBase, theFace, thePointSize, bold, italics, underline, false);
}

SysFont::SysFont(SexyAppBase* theApp, const std::string& theFace, int thePointSize, bool bold,
...
#ifdef WIN32
        anError = FT_New_Face(mApp->mFreeTypeLib, ("C:/Windows/Fonts/" + theFace + ".ttf").c_str(),
                              0, &aFontFace);
#else
        mFontData = nullptr;
        mAscent = 0;
...
void SysFont::DrawString(Graphics* g, int theX, int theY, const SexyString& theString,
        ...
        if (g->mDestImage != &Graphics::mStaticImage) {
            if (mDrawShadow) {
                ...
                g->mDestImage->BltRawTexture(
                    mFontData->mAtlas.mAtlas, aGlyph.mWidth, aGlyph.mHeight,
                    Rect(aDrawX + g->mTransX + 1, aDrawY - mAscent + 1 + g->mTransY, aGlyph.mWidth,
                         aGlyph.mHeight),
                    Rect(aGlyph.mX, aGlyph.mY, aGlyph.mWidth, aGlyph.mHeight), theClipRect,
                    aShadowColor, 0);
            }
            ...
            mApp->mRenderer->BltRawTexture(
                ...
                Rect(aDrawX + g->mTransX + 1, aDrawY - mAscent + 1 + g->mTransY, aGlyph.mWidth,
                     aGlyph.mHeight),
                Rect(aGlyph.mX, aGlyph.mY, aGlyph.mWidth, aGlyph.mHeight), theClipRect, theColor,
                0);
        }
    ...
    if (g->mDestImage != &Graphics::mStaticImage) {
        if (mUnderlined)
            g->mDestImage->DrawLine(theX, underlineY, StringWidth(theString), underlineY, theColor,
                                    0);
    } else {
        if (mUnderlined)
            mApp->mRenderer->DrawLine(theX + g->mTransX + 1, underlineY, StringWidth(theString),
                                      underlineY, theColor, 0);
    }
}
...
void TrueTypeData::Init() {
    ...
    int aRowHeight = 0;
    uint32_t* anAtlasPixels =
        new uint32_t[mAtlas.mWidth * mAtlas.mHeight]; // TODO: RESIZE TO FIT ALL CHARACTERS PROPERLY
    ...
    mAtlas.mAtlas = mApp->mRenderer->CreateTexture(anAtlasPixels, mAtlas.mWidth, mAtlas.mHeight,
                                                   RawPixelFormat::RAW_FORMAT_RGBA, 1);
