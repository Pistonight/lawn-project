...
public:
    ...
    SexyAutoPerf(const char* theName) : mName(theName), mIsStarted(true) {
...
#define SEXY_AUTO_PERFL(theName)                                                                   \
    SEXY_AUTO_PERF_2(                                                                              \
        theName,                                                                                   \
        __LINE__) // __LINE__ doesn't work correctly if Edit-and-Continue (/ZI) is enabled
...
#define SEXY_PERF_END_COND(theName, theCond)                                                       \
    if (theCond)                                                                                   \
    SexyPerf::StopTiming(theName)
#define SEXY_AUTO_PERF_MULTI_COND(theName, theSuffix, theCond)                                     \
    SexyAutoPerf anAutoPerf##theSuffix(theName, theCond);
#define SEXY_AUTO_PERF_COND_2(theName, theSuffix, theCond)                                         \
    SEXY_AUTO_PERF_MULTI_COND(theName, theSuffix, theCond);
