...
void Gamepad::AddRumbleEffect(float theLowFrequency, float theHighFrequency,
                              uint32_t theMsDuration) {
    if (mInternalGamepad == nullptr)
        return;
    SDL_RumbleGamepad(mInternalGamepad, (uint16_t)(theLowFrequency * UINT16_MAX),
                      (uint16_t)(theHighFrequency * UINT16_MAX), theMsDuration);
}
