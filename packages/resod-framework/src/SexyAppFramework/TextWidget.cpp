...
#include <SexyAppFramework/TextWidget.h>
...
void TextWidget::DrawColorString(Graphics* g, const SexyString& theString, int x, int y,
                                 bool useColors) {
    int aWidth = 0;
...
void TextWidget::DrawColorStringHilited(Graphics* g, const SexyString& theString, int x, int y,
                                        int theStartPos, int theEndPos) {
    ...
    if (theEndPos > theStartPos) {
        ...
        Graphics aClipG(*g);
        aClipG.ClipRect(x + aXOfs, y - g->GetFont()->GetAscent(), aWidth,
                        g->GetFont()->GetHeight());
    ...
    if (GetColorStringWidth(theLine) <= mWidth - 8) {
        ...
        while (aCurPos < (int)theLine.length()) {
            ...
            if (GetColorStringWidth(aNewString) > mWidth - 8) {
                ...
                aCurString = "  " + SexyChar(0xFF) + (SexyChar)aColor.mRed +
                             (SexyChar)aColor.mGreen + (SexyChar)aColor.mBlue +
...
bool TextWidget::SelectionReversed() {
    return ((mHiliteArea[1][1] < mHiliteArea[0][1]) ||
            ((mHiliteArea[1][1] == mHiliteArea[0][1]) && (mHiliteArea[1][0] < mHiliteArea[0][0])));
}
...
SexyString TextWidget::GetSelection() {
    SexyString aSelString = "";
    int aSelIndices[2];
    bool first = true;

    bool reverse = SelectionReversed();
    for (int aLineNum = mHiliteArea[reverse ? 1 : 0][1];
         aLineNum <= mHiliteArea[reverse ? 0 : 1][1]; aLineNum++) {
