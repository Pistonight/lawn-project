...
void HTTPTransfer::Post(const std::string& theBaseURL, const std::string& theRelURL,
    ...
    if (gSexyAppBase != NULL) {
        if (gSexyAppBase->mPlayingDemoBuffer) {
            ...
            if (gSexyAppBase->PrepareDemoCommand(false)) {
                if ((!gSexyAppBase->mDemoIsShortCmd) &&
                    (gSexyAppBase->mDemoCmdNum == DEMO_HTTP_RESULT)) {
                    ...
                    if (aTransferId == mTransferId) {
                        ...
                        // TODO:
                        // OutputDebugString(StrFormat("Found State Change on %s (Id %d) to %d at
                        // %d\r\n", mURL.c_str(), mTransferId, mResult,
                        // gSexyAppBase->mUpdateCount).c_str());
                    } else {
                        // Not us, go back
