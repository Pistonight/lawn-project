...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
static inline void lClip(SWHelper::XYZStruct& dst, const SWHelper::XYZStruct& on,
                         const SWHelper::XYZStruct& off, const float edge) {
    ...
    dst.mDiffuse = ((int)((((off.mDiffuse >> 24) & 0xff) +
                           (((on.mDiffuse >> 24) & 0xff) - ((off.mDiffuse >> 24) & 0xff)) * delta))
                    << 24) |
                   ((int)((((off.mDiffuse >> 16) & 0xff) +
                           (((on.mDiffuse >> 16) & 0xff) - ((off.mDiffuse >> 16) & 0xff)) * delta))
                    << 16) |
                   ((int)((((off.mDiffuse >> 8) & 0xff) +
                           (((on.mDiffuse >> 8) & 0xff) - ((off.mDiffuse >> 8) & 0xff)) * delta))
                    << 8) |
                   ((int)((((off.mDiffuse >> 0) & 0xff) +
                           (((on.mDiffuse >> 0) & 0xff) - ((off.mDiffuse >> 0) & 0xff)) * delta)));
}

// --------------------------------------------------------------------------------------------------------------------------------

static inline void rClip(SWHelper::XYZStruct& dst, const SWHelper::XYZStruct& on,
                         const SWHelper::XYZStruct& off, const float edge) {
    float delta = (edge - off.mX) / (on.mX - off.mX);
    dst.mX = off.mX + (on.mX - off.mX) * delta;
    dst.mY = off.mY + (on.mY - off.mY) * delta;
...
static inline void tClip(SWHelper::XYZStruct& dst, const SWHelper::XYZStruct& on,
                         const SWHelper::XYZStruct& off, const float edge) {
...
static inline void bClip(SWHelper::XYZStruct& dst, const SWHelper::XYZStruct& on,
                         const SWHelper::XYZStruct& off, const float edge) {
    float delta = (edge - off.mY) / (on.mY - off.mY);
    dst.mX = off.mX + (on.mX - off.mX) * delta;
    dst.mY = off.mY + (on.mY - off.mY) * delta;
...
static inline unsigned int leClip(SWHelper::XYZStruct** src, SWHelper::XYZStruct** dst,
                                  const float edge) {
...
static inline unsigned int reClip(SWHelper::XYZStruct** src, SWHelper::XYZStruct** dst,
                                  const float edge) {
    SWHelper::XYZStruct** _dst = dst;
...
static inline unsigned int teClip(SWHelper::XYZStruct** src, SWHelper::XYZStruct** dst,
                                  const float edge) {
    SWHelper::XYZStruct** _dst = dst;
...
static inline unsigned int beClip(SWHelper::XYZStruct** src, SWHelper::XYZStruct** dst,
                                  const float edge) {
    SWHelper::XYZStruct** _dst = dst;
...
static inline int clipShape(SWHelper::XYZStruct** dst, SWHelper::XYZStruct** src, const float left,
...
void SWHelper::SWDrawShape(XYZStruct* theVerts, int theNumVerts, MemoryImage* theImage,
    ...
    // format.  Our framework allows us to specify a 'real' resolution to use, and conforms
    // everything down to whatever the screen is displaying.  So, we usually use 800x600 for
    // absolute resolution in game.
    //
    // So, to turn the clip rectangle into the "screen resolution" from "page resolution" (page is
    // what we work in, screen is what displays), multiply the rectangle member variables by
    // mPageWidthAdjust and mPageHeightAdjust.
    //
    ...
    // reasons).  So, a final clip to
    // 0,0-mSWTexture->mTextureInfo.dwWidth,mSWTexture->mTextureInfo.dwHeight is necessary when
    ...
    for (;;) {
        ...
        if (vCount) {
            ...
            if (textured) {
                for (unsigned int i = 0; i < vCount; ++i) {
                    pVerts[i].u =
                        static_cast<int>(clipped[i]->mU * (float)theImage->mWidth * 65536.0f);
                    pVerts[i].v =
                        static_cast<int>(clipped[i]->mV * (float)theImage->mHeight * 65536.0f);
                }
                ...
                //				unsigned int	temp = static_cast<unsigned
                // int>(mSWTexture->mTextureInfo.lPitch) /
            ...
            SWDrawTriangle(textured, talpha, vertexColor, globalargb, pVerts, pFrameBuffer,
                           thePitch, &textureInfo, globalDiffuse, thePixelFormat, blend);
...
void Sexy::SWTri_AddDrawTriFunc(bool textured, bool talpha, bool mod_argb, bool global_argb,
...
#include <SexyAppFramework/SWTri_DrawTriangleInc1.cpp>

void SWHelper::SWDrawTriangle(bool textured, bool talpha, bool mod_argb, bool global_argb,
                              SWVertex* pVerts, unsigned int* pFrameBuffer,
                              const unsigned int bytepitch, const SWTextureInfo* textureInfo,
