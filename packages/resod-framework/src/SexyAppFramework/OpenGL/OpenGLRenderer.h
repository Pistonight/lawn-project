...
const std::unordered_map<BlendMode, GLBlendFunc> gGLBlendDefines = {
    {BlendMode::BLENDMODE_NORMAL,
     {GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA, GL_ONE, GL_ONE_MINUS_SRC_ALPHA, GL_FUNC_ADD,
      GL_FUNC_ADD}},
    {BlendMode::BLENDMODE_ADD, {GL_SRC_ALPHA, GL_ONE, GL_ZERO, GL_ONE, GL_FUNC_ADD, GL_FUNC_ADD}},
...
public:
    ...
    virtual GPUImage* NewGPUImage() { return new OpenGLImage(this); }
    ...
    virtual void* CreateTexture(void* thePixels, int theWidth, int theHeight,
                                RawPixelFormat thePixelFormat, int theAlignment = 4);
    ...
    virtual void Blt(Image* theImage, float theX, float theY, const Rect& theSrcRect,
                     const Color& theColor, int theDrawMode, bool linearFilter = false);
    ...
    virtual void StretchBlt(Image* theImage, const Rect& theDestRect, const Rect& theSrcRect,
                            const Rect& theClipRect, const Color& theColor, int theDrawMode,
                            bool fastStretch, bool mirror = false);
    ...
    virtual void DrawTrianglesTex(const TriVertex theVertices[][3], int theNumTriangles,
                                  const Color& theColor, int theDrawMode, Image* theTexture,
                                  float tx = 0, float ty = 0, bool blend = true);
    ...
    glm::vec2 TransformToPoint(float x, float y, const SexyMatrix3& m, float aTransX = 0,
                               float aTransY = 0) {
        glm::vec2 result;
    ...
    glm::vec2 RotatePointAroundPivot(const glm::vec2 point, const glm::vec2 center,
                                     float angleRad) {
