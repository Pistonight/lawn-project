...
static const int gGLVersions[][2] = {{4, 6}, {4, 5}, {4, 4}, {4, 3}, {4, 2}, {4, 1},
                                     {4, 0}, {3, 3}, {3, 2}, {3, 1}, {3, 0}};
...
uint32_t* OpenGLRenderer::CaptureFrameBuffer() {
    if (mApp->mHighResolution) {
        ...
        glReadPixels(0, 0, mPresentationRect.mWidth, mPresentationRect.mHeight, GL_BGRA,
                     GL_UNSIGNED_BYTE, thePixels);
        for (int y = 0; y < mPresentationRect.mHeight / 2; ++y) {
            uint32_t* row = thePixels + y * mPresentationRect.mWidth;
            uint32_t* opp =
                thePixels + (mPresentationRect.mHeight - 1 - y) * mPresentationRect.mWidth;
...
void OpenGLRenderer::Cleanup() {
    ...
    for (anItr = mImageSet.begin(); anItr != mImageSet.end(); ++anItr) {
        ...
        if (anNativeImage != nullptr &&
            anNativeImage->mFBO != 0) // Delete the FBO incase the renderer resets
        {
...
bool OpenGLRenderer::InitBuffers() {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8,
                 ...
                 mApp->mHighResolution ? mPresentationRect.mHeight : mHeight, 0, GL_BGRA,
...
bool OpenGLRenderer::Redraw(Rect* theClipRect) {
    ...
    glViewport(0, 0, mApp->mHighResolution ? mPresentationRect.mWidth : mWidth,
               mApp->mHighResolution ? mPresentationRect.mHeight : mHeight);
    glScissor(0, 0, mApp->mHighResolution ? mPresentationRect.mWidth : mWidth,
              mApp->mHighResolution ? mPresentationRect.mHeight : mHeight);
    ...
    for (const auto cmd : mCommandBuffer) {
        ...
        glBindSampler(0, cmd.mUVWrapMode == UV_WRAP ? mSamplers[cmd.mFilterMode].mWrap
                                                    : mSamplers[cmd.mFilterMode].mClamp);
        if (cmd.mHasClipRect) {
            ...
            if (mApp->mHighResolution) {
                ...
                doScissorFromTL(cmd.mClipRect.mX, cmd.mClipRect.mY, cmd.mClipRect.mWidth,
                                cmd.mClipRect.mHeight, mHeight);
        } else
        ...
        glBufferSubData(GL_ARRAY_BUFFER, 0, cmd.mVertices.size() * sizeof(Vertex),
                        cmd.mVertices.data());
...
bool OpenGLRenderer::CreateImageTexture(MemoryImage* theImage) {
    ...
    aData->CheckCreateTextures(
        theImage, nullptr); // We don't need extra variables when creating OpenGL Textures
...
void* OpenGLRenderer::CreateTexture(void* thePixels, int theWidth, int theHeight,
                                    RawPixelFormat thePixelFormat, int theAlignment) {
    GLuint aTexID;
    glGenTextures(1, &aTexID);
    glBindTexture(GL_TEXTURE_2D, aTexID);
    glPixelStorei(GL_UNPACK_ALIGNMENT, theAlignment);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, theWidth, theHeight, 0,
                 thePixelFormat == RAW_FORMAT_R ? GL_RED : GL_BGRA, GL_UNSIGNED_BYTE, thePixels);
    gGLTextureCount++;
...
void OpenGLTextureData::CreateTextures(MemoryImage* theImage, void* theRendererData) {
    ...
    if (createTexture) {
        ...
        if (theImage->mGPUFlags & ImageFlag_UseA4R4G4B4)
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA4, aWidth, aHeight, 0, GL_BGRA,
                         GL_UNSIGNED_SHORT_4_4_4_4_REV, theImage->GetBits());
        else
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, aWidth, aHeight, 0, GL_BGRA,
                         GL_UNSIGNED_INT_8_8_8_8_REV, theImage->GetBits());
        ...
        if (bits) {
            glBindTexture(GL_TEXTURE_2D, mTexID);
            glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
            glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, aWidth, aHeight, GL_BGRA,
                            GL_UNSIGNED_INT_8_8_8_8_REV, bits);
        } else {
            // todo: log error
        }
    }
...
void OpenGLRenderer::Blt(Image* theImage, float theX, float theY, const Rect& theSrcRect,
    ...
    glm::vec4 aColor = {(float)theColor.mRed / 255.0f, (float)theColor.mGreen / 255.0f,
                        (float)theColor.mBlue / 255.0f, (float)theColor.mAlpha / 255.0f};
...
void OpenGLRenderer::BltClipF(Image* theImage, float theX, float theY, const Rect& theSrcRect,
...
void OpenGLRenderer::BltMirror(Image* theImage, float theX, float theY, const Rect& theSrcRect,
                               const Color& theColor, int theDrawMode, bool linearFilter) {
    MemoryImage* aImg = (MemoryImage*)theImage;
...
void OpenGLRenderer::StretchBlt(Image* theImage, const Rect& theDestRect, const Rect& theSrcRect,
                                const Rect& theClipRect, const Color& theColor, int theDrawMode,
...
void OpenGLRenderer::BltRotated(Image* theImage, float theX, float theY, const Rect& theClipRect,
...
void OpenGLRenderer::BltTransformed(Image* theImage, const Rect& theClipRect, const Color& theColor,
...
void OpenGLRenderer::DrawLine(double theStartX, double theStartY, double theEndX, double theEndY,
                              const Color& theColor, int theDrawMode) {
    GLDrawCommand aCmd;
    aCmd.mPrimitiveType = GL_LINES;
    aCmd.mTextureID = 0;
    aCmd.mBlendMode = ChooseBlendMode(theDrawMode);
    glm::vec4 color = glm::vec4(theColor.mRed / 255.0f, theColor.mGreen / 255.0f,
                                theColor.mBlue / 255.0f, theColor.mAlpha / 255.0f);
...
void OpenGLRenderer::DrawTriangle(const TriVertex& p1, const TriVertex& p2, const TriVertex& p3,
                                  const Color& theColor, int theDrawMode) {
    GLDrawCommand aCmd;
    aCmd.mTextureID = 0;
    aCmd.mPrimitiveType = GL_TRIANGLES;
...
void OpenGLRenderer::DrawTriangleTex(const TriVertex& p1, const TriVertex& p2, const TriVertex& p3,
...
void OpenGLRenderer::DrawTrianglesTex(const TriVertex theVertices[][3], int theNumTriangles,
                                      const Color& theColor, int theDrawMode, Image* theTexture,
...
void OpenGLRenderer::DrawTrianglesTexStrip(const TriVertex theVertices[], int theNumTriangles,
                                           const Color& theColor, int theDrawMode,
                                           Image* theTexture, float tx, float ty, bool blend) {
...
void OpenGLRenderer::FillPoly(const Point theVertices[], int theNumVertices,
                              const Rect& theClipRect, const Color& theColor, int theDrawMode,
                              int tx, int ty) {
    if (theNumVertices < 3)
        return;
...
void OpenGLRenderer::BltRawTexture(void* theTexture, int theTexWidth, int theTexHeight,
                                   const Rect& theDestRect, const Rect& theSrcRect,
                                   const Rect& theClipRect, const Color& theColor,
                                   int theDrawMode) {
