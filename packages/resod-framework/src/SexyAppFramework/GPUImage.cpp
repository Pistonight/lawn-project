...
void GPUImage::FillScanLinesWithCoverage(Span* theSpans, int theSpanCount, const Color& theColor,
                                         int theDrawMode, const uint8_t* theCoverage, int theCoverX,
    ...
    aTempImage.FillScanLinesWithCoverage(theSpans, theSpanCount, theColor, theDrawMode, theCoverage,
                                         theCoverX - l, theCoverY - t, theCoverWidth,
                                         theCoverHeight);
...
bool GPUImage::PolyFill3D(const Point theVertices[], int theNumVertices, const Rect& theClipRect,
                          const Color& theColor, int theDrawMode, int tx, int ty, bool comvex) {
    if (Check3D(this)) {
        mRenderer->FillPoly(theVertices, theNumVertices, theClipRect, theColor, theDrawMode, tx,
                            ty);
        return true;
    } else
        return false;
...
void GPUImage::ImplDrawLine(double theStartX, double theStartY, double theEndX, double theEndY,
...
void GPUImage::ImplDrawLineAA(double theStartX, double theStartY, double theEndX, double theEndY,
                              const Color& theColor, int theDrawMode) {}
...
void GPUImage::ImplBlt(Image* theImage, int theX, int theY, const Rect& theSrcRect,
                       const Color& theColor, int theDrawMode) {}

void GPUImage::Blt(Image* theImage, int theX, int theY, const Rect& theSrcRect,
                   const Color& theColor, int theDrawMode) {
...
void GPUImage::ImplBltF(Image* theImage, float theX, float theY, const Rect& theSrcRect,
                        const Rect& theClipRect, const Color& theColor, int theDrawMode) {}

void GPUImage::BltF(Image* theImage, float theX, float theY, const Rect& theSrcRect,
                    const Rect& theClipRect, const Color& theColor, int theDrawMode) {
    theImage->mDrawn = true;
    ...
    if (Check3D(this)) {
        ...
        if (anIntersect.mWidth != aDestRect.mWidth || anIntersect.mHeight != aDestRect.mHeight) {
            if (anIntersect.mWidth != 0 && anIntersect.mHeight != 0)
                mRenderer->BltClipF(theImage, theX, theY, theSrcRect, theClipRect, theColor,
                                    theDrawMode);
        } else
...
void GPUImage::ImplBltRotated(Image* theImage, float theX, float theY, const Rect& theSrcRect,
                              const Rect& theClipRect, const Color& theColor, int theDrawMode,
...
void GPUImage::BltRotated(Image* theImage, float theX, float theY, const Rect& theSrcRect,
    ...
    if (Check3D(this)) {
        mRenderer->BltRotated(theImage, theX, theY, theClipRect, theColor, theDrawMode, theRot,
                              theRotCenterX, theRotCenterY, theSrcRect);
        return;
    }
    ...
    ImplBltRotated(theImage, theX, theY, theSrcRect, theClipRect, theColor, theDrawMode, theRot,
                   theRotCenterX, theRotCenterY);

    DeleteAllNonSurfaceData();
}
void GPUImage::ImplStretchBlt(Image* theImage, const Rect& theDestRect, const Rect& theSrcRect,
                              const Rect& theClipRect, const Color& theColor, int theDrawMode,
                              bool fastStretch) {}
...
void GPUImage::StretchBlt(Image* theImage, const Rect& theDestRect, const Rect& theSrcRect,
    ...
    if (Check3D(this)) {
        mRenderer->StretchBlt(theImage, theDestRect, theSrcRect, theClipRect, theColor, theDrawMode,
                              fastStretch);
        return;
    }

    if ((mDrawToBits) || (mHasAlpha) || (mHasTrans)) {
        MemoryImage::StretchBlt(theImage, theDestRect, theSrcRect, theClipRect, theColor,
                                theDrawMode, fastStretch);
        return;
    }
    if (!mNoLock)
        ImplStretchBlt(theImage, theDestRect, theSrcRect, theClipRect, theColor, theDrawMode,
                       fastStretch);

    DeleteAllNonSurfaceData();
}
void GPUImage::ImplBltMatrix(Image* theImage, float x, float y, const SexyMatrix3& theMatrix,
                             const Rect& theClipRect, const Color& theColor, int theDrawMode,
                             const Rect& theSrcRect, bool blend) {}

void GPUImage::BltMatrix(Image* theImage, float x, float y, const SexyMatrix3& theMatrix,
                         const Rect& theClipRect, const Color& theColor, int theDrawMode,
                         const Rect& theSrcRect, bool blend) {
    theImage->mDrawn = true;

    if (Check3D(this)) {
        mRenderer->BltTransformed(theImage, theClipRect, theColor, theDrawMode, theSrcRect,
                                  theMatrix, blend, x, y, true);
        return;
    }
...
void GPUImage::ImplBltTrianglesTex(Image* theTexture, const TriVertex theVertices[][3],
                                   int theNumTriangles, const Rect& theClipRect,
                                   const Color& theColor, int theDrawMode, float tx, float ty,
                                   bool blend) {}

void GPUImage::BltTrianglesTex(Image* theTexture, const TriVertex theVertices[][3],
                               int theNumTriangles, const Rect& theClipRect, const Color& theColor,
                               int theDrawMode, float tx, float ty, bool blend) {
    theTexture->mDrawn = true;

    if (Check3D(this)) {
        mRenderer->DrawTrianglesTex(theVertices, theNumTriangles, theColor, theDrawMode, theTexture,
                                    tx, ty, blend);
        return;
    }

    ImplBltTrianglesTex(theTexture, theVertices, theNumTriangles, theClipRect, theColor,
                        theDrawMode, tx, ty, blend);

    DeleteAllNonSurfaceData();
}

void GPUImage::ImplBltMirror(Image* theImage, int theX, int theY, const Rect& theSrcRect,
                             const Color& theColor, int theDrawMode) {}

void GPUImage::BltMirror(Image* theImage, int theX, int theY, const Rect& theSrcRect,
                         const Color& theColor, int theDrawMode) {
    DBG_ASSERTE((theColor.mRed >= 0) && (theColor.mRed <= 255));
    DBG_ASSERTE((theColor.mGreen >= 0) && (theColor.mGreen <= 255));
    DBG_ASSERTE((theColor.mBlue >= 0) && (theColor.mBlue <= 255));
...
void GPUImage::ImplStretchBltMirror(Image* theImage, const Rect& theDestRectOrig,
                                    const Rect& theSrcRect, const Rect& theClipRect,
                                    const Color& theColor, int theDrawMode, bool fastStretch) {}

void GPUImage::StretchBltMirror(Image* theImage, const Rect& theDestRectOrig,
                                const Rect& theSrcRect, const Rect& theClipRect,
                                const Color& theColor, int theDrawMode, bool fastStretch) {
    theImage->mDrawn = true;
    ...
    if (Check3D(this)) {
        mRenderer->StretchBlt(theImage, theDestRectOrig, theSrcRect, theClipRect, theColor,
                              theDrawMode, fastStretch, true);
        return;
    }

    ImplStretchBltMirror(theImage, theDestRectOrig, theSrcRect, theClipRect, theColor, theDrawMode,
                         fastStretch);

    DeleteAllNonSurfaceData();
}

void GPUImage::ImplBltRawTexture(void* theTexture, int theTexWidth, int theTexHeight,
                                 const Rect& theDestRect, const Rect& theSrcRect,
                                 const Rect& theClipRect, const Color& theColor, int theDrawMode,
                                 bool fastStretch) {}

void GPUImage::BltRawTexture(void* theTexture, int theTexWidth, int theTexHeight,
                             const Rect& theDestRect, const Rect& theSrcRect,
                             const Rect& theClipRect, const Color& theColor, int theDrawMode) {
    CommitBits();

    if (Check3D(this)) {
        mRenderer->BltRawTexture(theTexture, theTexWidth, theTexHeight, theDestRect, theSrcRect,
                                 theClipRect, theColor, theDrawMode);
        return;
    }

    ImplBltRawTexture(theTexture, theTexWidth, theTexHeight, theDestRect, theSrcRect, theClipRect,
                      theColor, theDrawMode, true);
