...
template <typename T> using GlyphMap = std::unordered_map<uint32_t, T>;
...
public:
    ...
    virtual void DrawStringEx(Graphics* g, int theX, int theY, const SexyString& theString,
...
public:
    ...
    ImageFont(
        Image* theFontImage); // for constructing your own image font without a file descriptor
