...
void WidgetManager::DoMouseUps(Widget* theWidget, uint32_t theDownCode) {
    ...
    for (int i = 0; i < 3; i++) {
        if ((theDownCode & (1 << i)) != 0) {
            ...
            theWidget->MouseUp(mLastMouseX - theWidget->mX, mLastMouseY - theWidget->mY,
                               aClickCountTable[i]);
        }
    }
}
...
void WidgetManager::RemapMouse(int& theX, int& theY) {
    theX = (theX - mMouseSourceRect.mX) * mMouseDestRect.mWidth / mMouseSourceRect.mWidth +
           mMouseDestRect.mX;
    theY = (theY - mMouseSourceRect.mY) * mMouseDestRect.mHeight / mMouseSourceRect.mHeight +
           mMouseDestRect.mY;
}
...
void WidgetManager::RemoveBaseModal(Widget* theWidget) {
    ...
    while (mPreModalInfoList.size() > 0) {
        ...
        bool done =
            (aPreModalInfo->mPrevBaseModalWidget != NULL) || (mPreModalInfoList.size() == 1);
