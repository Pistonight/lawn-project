...
MemoryImage::MemoryImage(const MemoryImage& theMemoryImage)
    : Image(theMemoryImage), mApp(theMemoryImage.mApp), mHasAlpha(theMemoryImage.mHasAlpha),
      ...
      mIsVolatile(theMemoryImage.mIsVolatile), mPurgeBits(theMemoryImage.mPurgeBits),
      mWantPal(theMemoryImage.mWantPal), mGPUFlags(theMemoryImage.mGPUFlags),
      mBitsChangedCount(theMemoryImage.mBitsChangedCount), mGPUData(NULL) {
    ...
    if (theMemoryImage.mColorIndices != NULL) {
        mColorIndices = new uint8_t[mWidth * mHeight];
        memcpy(mColorIndices, theMemoryImage.mColorIndices, mWidth * mHeight * sizeof(uint8_t));
    } else
        mColorIndices = NULL;
    ...
    if (theMemoryImage.mNativeAlphaData != NULL) {
        if (theMemoryImage.mColorTable == NULL) {
            ...
            memcpy(mNativeAlphaData, theMemoryImage.mNativeAlphaData,
    ...
    if (true) //(mLockedSurfaceDesc.ddpfPixelFormat.dwRGBBitCount == 32)
    {
        if (theColor.mAlpha == 255) {
            ...
            if (abs(dv) < abs(dh)) {
                ...
                uint32_t* aDestPixels =
                    ((uint32_t*)aSurface) + ((int)theStartY * aRowWidth) + (int)theStartX;
                *aDestPixels = aColor;
                aDestPixels++;
            ...
            uint32_t src =
                0xFF000000 |
                ((((((theColor.mRed * theColor.mAlpha + 0x80) >> 8) * aRMask) + aRRoundAdd) >> 8) &
                 aRMask) |
                ((((((theColor.mGreen * theColor.mAlpha + 0x80) >> 8) * aGMask) + aGRoundAdd) >>
                  8) &
                 aGMask) |
                ((((((theColor.mBlue * theColor.mAlpha + 0x80) >> 8) * aBMask) + aBRoundAdd) >> 8) &
                 aBMask);
...
void MemoryImage::AdditiveDrawLine(double theStartX, double theStartY, double theEndX,
                                   double theEndY, const Color& theColor) {
    double aMinX = std::min(theStartX, theEndX);
    double aMinY = std::min(theStartY, theEndY);
    double aMaxX = std::max(theStartX, theEndX);
    double aMaxY = std::max(theStartY, theEndY);
    ...
    uint8_t* aMaxTable = mApp->mAdd8BitMaxTable;
    uint32_t* aSurface = GetBits();
    ...
    if (true) //(mLockedSurfaceDesc.ddpfPixelFormat.dwRGBBitCount == 32)
    {
        ...
        if (abs(dv) < abs(dh)) {
            ...
            *(aDestPixels++) =
                0xFF000000 | (r << aRedShift) | (g << aGreenShift) | (b << aBlueShift);
            ...
            uint32_t* aDestPixels =
                ((uint32_t*)aSurface) + ((int)theStartY * mWidth) + (int)theStartX;
...
void MemoryImage::DrawLine(double theStartX, double theStartY, double theEndX, double theEndY,
    ...
    if (theStartY == theEndY) {
        ...
        FillRect(Rect(aStartX, theStartY, aEndX - aStartX + 1, theEndY - theStartY + 1), theColor,
                 theDrawMode);
        return;
        ...
        FillRect(Rect(theStartX, aStartY, theEndX - theStartX + 1, aEndY - aStartY + 1), theColor,
                 theDrawMode);
        return;
    }
...
void MemoryImage::NormalDrawLineAA(double theStartX, double theStartY, double theEndX,
                                   double theEndY, const Color& theColor) {
...
#include <SexyAppFramework/GENERIC_DrawLineAA.inc>
...
void MemoryImage::AdditiveDrawLineAA(double theStartX, double theStartY, double theEndX,
                                     double theEndY, const Color& theColor) {}

void MemoryImage::DrawLineAA(double theStartX, double theStartY, double theEndX, double theEndY,
                             const Color& theColor, int theDrawMode) {
    if (theStartY == theEndY) {
        int aStartX = std::min(theStartX, theEndX);
        int aEndX = std::max(theStartX, theEndX);
    ...
    if ((mBitsChanged) && (!mForcedMode)) {
        ...
        if (mBits != NULL) {
            ...
            for (int i = 0; i < aSize; i++) {
                uint8_t anAlpha = (uint8_t)(*ptr++ >> 24);
...
uint8_t* MemoryImage::GetRLAlphaData() {
    CommitBits();
...
#include <SexyAppFramework/MI_GetRLAlphaData.inc>

#undef NEXT_SRC_COLOR
        } else {
            uint8_t* aSrcPtr = mColorIndices;
...
uint8_t* MemoryImage::GetRLAdditiveData(Renderer* theNative) {
    ...
    if (oldAlpha == 0xFF) {
        ...
        for (int aRow = theRect.mY; aRow < theRect.mY + theRect.mHeight; aRow++) {
            ...
            for (int i = 0; i < theRect.mWidth; i++) {
                ...
                *(aDestPixels++) =
                    (aNewDestAlpha << 24) |
                    ((((dest & 0x0000FF) * oma) >> 8) + (((src & 0x0000FF) * newAlpha) >> 8) &
                     0x0000FF) |
                    ((((dest & 0x00FF00) * oma) >> 8) + (((src & 0x00FF00) * newAlpha) >> 8) &
                     0x00FF00) |
                    ((((dest & 0xFF0000) * oma) >> 8) + (((src & 0xFF0000) * newAlpha) >> 8) &
                     0xFF0000);
#endif
            }
        }
...
void MemoryImage::AdditiveBlt(Image* theImage, int theX, int theY, const Rect& theSrcRect,
                              const Color& theColor) {
...
#include <SexyAppFramework/MI_AdditiveBlt.inc>

#undef NEXT_SRC_COLOR
#undef SRC_TYPE
        } else {
            uint32_t* aColorTable = aSrcMemoryImage->mColorTable;
            uint8_t* aSrcBits = aSrcMemoryImage->mColorIndices;
...
void MemoryImage::NormalBlt(Image* theImage, int theX, int theY, const Rect& theSrcRect,
                            const Color& theColor) {
    theImage->mDrawn = true;
...
#include <SexyAppFramework/MI_NormalBlt.inc>
...
#undef EACH_ROW
            ...
            uint32_t* aColorTable = aSrcMemoryImage->mColorTable;
            uint8_t* aSrcPixelsRow =
                aSrcMemoryImage->mColorIndices + (theSrcRect.mY * theImage->mWidth) + theSrcRect.mX;

#define NEXT_SRC_COLOR (aColorTable[*(aSrcPtr++)])
#define READ_SRC_COLOR (aColorTable[*(aSrcPtr)])
#define EACH_ROW uint8_t* aSrcPtr = aSrcPixelsRow
...
void MemoryImage::Blt(Image* theImage, int theX, int theY, const Rect& theSrcRect,
                      const Color& theColor, int theDrawMode) {
    theImage->mDrawn = true;
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
void MemoryImage::BltF(Image* theImage, float theX, float theY, const Rect& theSrcRect,
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
bool MemoryImage::BltRotatedClipHelper(float& theX, float& theY, const Rect& theSrcRect,
                                       const Rect& theClipRect, double theRot, FRect& theDestRect,
    ...
    float px[4] = {0, static_cast<float>(theSrcRect.mWidth), static_cast<float>(theSrcRect.mWidth),
                   0};
    float py[4] = {0, 0, static_cast<float>(theSrcRect.mWidth),
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
bool MemoryImage::StretchBltClipHelper(const Rect& theSrcRect, const Rect& theClipRect,
                                       const Rect& theDestRect, FRect& theSrcRectOut,
                                       Rect& theDestRectOut) {
    theDestRectOut = Rect(theDestRect.mX, theDestRect.mY, theDestRect.mWidth, theDestRect.mHeight)
                         .Intersection(theClipRect);

    double aXFactor = theSrcRect.mWidth / (double)theDestRect.mWidth;
    double aYFactor = theSrcRect.mHeight / (double)theDestRect.mHeight;

    theSrcRectOut =
        FRect(theSrcRect.mX + (theDestRectOut.mX - theDestRect.mX) * aXFactor,
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
bool MemoryImage::StretchBltMirrorClipHelper(const Rect& theSrcRect, const Rect& theClipRect,
                                             const Rect& theDestRect, FRect& theSrcRectOut,
                                             Rect& theDestRectOut) {
    theDestRectOut = Rect(theDestRect.mX, theDestRect.mY, theDestRect.mWidth, theDestRect.mHeight)
                         .Intersection(theClipRect);
    ...
    theSrcRectOut =
        FRect(theSrcRect.mX + (aRightClip)*aXFactor,
              theSrcRect.mY + (theDestRectOut.mY - theDestRect.mY) * aYFactor,
              theSrcRect.mWidth + (theDestRectOut.mWidth - theDestRect.mWidth) * aXFactor,
              theSrcRect.mHeight + (theDestRectOut.mHeight - theDestRect.mHeight) * aYFactor);
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
void MemoryImage::BltRotated(Image* theImage, float theX, float theY, const Rect& theSrcRect,
                             const Rect& theClipRect, const Color& theColor, int theDrawMode,
    ...
    FRect aDestRect;
    if (!BltRotatedClipHelper(theX, theY, theSrcRect, theClipRect, theRot, aDestRect, theRotCenterX,
                              theRotCenterY))
        return;
    ...
    if (aMemoryImage != NULL) {
        if (aMemoryImage->mColorTable == NULL) {
            uint32_t* aSrcBits =
                aMemoryImage->GetBits() + theSrcRect.mX + theSrcRect.mY * theSrcRect.mWidth;

#define SRC_TYPE uint32_t
#define READ_COLOR(ptr) (*(ptr))

            if (theDrawMode == Graphics::DRAWMODE_NORMAL) {
#include <SexyAppFramework/MI_BltRotated.inc>
            } else {
#include <SexyAppFramework/MI_BltRotated_Additive.inc>
            }
...
#undef READ_COLOR
            ...
            uint8_t* aSrcBits =
                aMemoryImage->mColorIndices + theSrcRect.mX + theSrcRect.mY * theSrcRect.mWidth;
...
#include <SexyAppFramework/MI_SlowStretchBlt.inc>
...
void MemoryImage::FastStretchBlt(Image* theImage, const Rect& theDestRect, const FRect& theSrcRect,
    ...
    if (aSrcMemoryImage != NULL) {
        uint32_t* aDestPixelsRow =
            ((uint32_t*)GetBits()) + (theDestRect.mY * mWidth) + theDestRect.mX;
        uint32_t* aSrcPixelsRow = (uint32_t*)aSrcMemoryImage->GetBits();
        ;
        ...
        if (theColor == Color::White) {
            for (int y = 0; y < theDestRect.mHeight; y++) {
                ...
                for (int x = 0; x < theDestRect.mWidth; x++) {
                    ...
                    uint32_t* aSrcPixels =
                        aSrcPixelsRow + ((int)aSrcX) + (aSrcMemoryImage->mWidth * ((int)aSrcY));
                    ...
                    if (a != 0) {
                        ...
                        *(aDestPixels++) =
                            (aNewDestAlpha << 24) |
                            ((((dest & 0x0000FF) * oma) >> 8) + (((src & 0x0000FF) * a) >> 8) &
                             0x0000FF) |
                            ((((dest & 0x00FF00) * oma) >> 8) + (((src & 0x00FF00) * a) >> 8) &
                             0x00FF00) |
                            ((((dest & 0xFF0000) * oma) >> 8) + (((src & 0xFF0000) * a) >> 8) &
                             0xFF0000);
                    } else
                        aDestPixels++;
                }
...
void MemoryImage::StretchBlt(Image* theImage, const Rect& theDestRect, const Rect& theSrcRect,
                             const Rect& theClipRect, const Color& theColor, int theDrawMode,
                             bool fastStretch) {
    theImage->mDrawn = true;
...
void MemoryImage::BltMatrixHelper(Image* theImage, float x, float y, const SexyMatrix3& theMatrix,
                                  const Rect& theClipRect, const Color& theColor, int theDrawMode,
                                  const Rect& theSrcRect, void* theSurface, int theBytePitch,
                                  int thePixelFormat, bool blend) {
    ...
    SWHelper::XYZStruct aVerts[4] = {{-w2, -h2, u0, v0, static_cast<long>(0xFFFFFFFF)},
                                     {w2, -h2, u1, v0, static_cast<long>(0xFFFFFFFF)},
                                     {-w2, h2, u0, v1, static_cast<long>(0xFFFFFFFF)},
                                     {w2, h2, u1, v1, static_cast<long>(0xFFFFFFFF)}};
    ...
    SWHelper::SWDrawShape(aVerts, 4, anImage, theColor, theDrawMode, theClipRect, theSurface,
                          theBytePitch, thePixelFormat, blend, false);
}

void MemoryImage::BltMatrix(Image* theImage, float x, float y, const SexyMatrix3& theMatrix,
                            const Rect& theClipRect, const Color& theColor, int theDrawMode,
    ...
    BltMatrixHelper(theImage, x, y, theMatrix, theClipRect, theColor, theDrawMode, theSrcRect,
                    aSurface, aPitch, aFormat, blend);
    BitsChanged();
}

void MemoryImage::BltTrianglesTexHelper(Image* theTexture, const TriVertex theVertices[][3],
                                        int theNumTriangles, const Rect& theClipRect,
                                        const Color& theColor, int theDrawMode, void* theSurface,
                                        int theBytePitch, int thePixelFormat, float tx, float ty,
                                        bool blend) {
    ...
    for (int i = 0; i < theNumTriangles; i++) {
        ...
        SWHelper::SWDrawShape(aVerts, 3, anImage, theColor, theDrawMode, theClipRect, theSurface,
                              theBytePitch, thePixelFormat, blend, vertexColor);
    }
}

void MemoryImage::FillScanLinesWithCoverage(Span* theSpans, int theSpanCount, const Color& theColor,
                                            int theDrawMode, const uint8_t* theCoverage,
                                            int theCoverX, int theCoverY, int theCoverWidth,
                                            int theCoverHeight) {
...
void MemoryImage::BltTrianglesTex(Image* theTexture, const TriVertex theVertices[][3],
                                  int theNumTriangles, const Rect& theClipRect,
                                  const Color& theColor, int theDrawMode, float tx, float ty,
                                  bool blend) {
    ...
    BltTrianglesTexHelper(theTexture, theVertices, theNumTriangles, theClipRect, theColor,
                          theDrawMode, aSurface, aPitch, aFormat, tx, ty, blend);
    BitsChanged();
}

void MemoryImage::BltRawTexture(void* theTexture, int theTexWidth, int theTexHeight,
                                const Rect& theDestRect, const Rect& theSrcRect,
                                const Rect& theClipRect, const Color& theColor, int theDrawMode) {

    uint32_t* aNewPixels =
        mApp->mRenderer->GetBitsFromTexture(theTexture, theTexWidth, theTexHeight);
