...
void EditWidget::ClearWidthCheckFonts() {
    for (WidthCheckList::iterator anItr = mWidthCheckList.begin(); anItr != mWidthCheckList.end();
         ++anItr)
        delete anItr->mFont;
...
void EditWidget::Draw(Graphics* g) // Already translated
{
    ...
    for (int i = 0; i < 2; i++) {
        ...
        if (i == 1) {
            int aCursorX = mFont->StringWidth(aString.substr(0, mCursorPos)) -
                           mFont->StringWidth(aString.substr(0, mLeftPos));
            int aHiliteX = aCursorX + 2;
            if ((mHilitePos != -1) && (mCursorPos != mHilitePos))
                aHiliteX = mFont->StringWidth(aString.substr(0, mHilitePos)) -
                           mFont->StringWidth(aString.substr(0, mLeftPos));
            ...
            aClipG->ClipRect(4 + std::min(aCursorX, aHiliteX), (mHeight - mFont->GetHeight()) / 2,
                             abs(aHiliteX - aCursorX), mFont->GetHeight());
        } else
        ...
        aClipG->DrawString(aString.substr(mLeftPos), 4,
                           (mHeight - mFont->GetHeight()) / 2 + mFont->GetAscent());
...
void EditWidget::UpdateCaretPos() {
    ...
    if (aPoint.mX < 10)
        aPoint.mX = 10;
    else if (aPoint.mX > anApp->mWidth - 10)
        aPoint.mX = anApp->mWidth - 10;
    if (aPoint.mY < 10)
        aPoint.mY = 10;
    else if (aPoint.mY > anApp->mHeight - 10)
        aPoint.mY = anApp->mHeight - 10;
    ...
    if ((theChar == 3) || (theChar == 24)) {
        ...
        if ((mHilitePos != -1) && (mHilitePos != mCursorPos)) {
            if (mCursorPos < mHilitePos)
                mWidgetManager->mApp->CopyToClipboard(
                    SexyStringToString(GetDisplayString().substr(mCursorPos, mHilitePos)));
            else
                mWidgetManager->mApp->CopyToClipboard(
                    SexyStringToString(GetDisplayString().substr(mHilitePos, mCursorPos)));

            if (theChar == 3) {
                removeHilite = false;
            } else {
                mString = mString.substr(0, std::min(mCursorPos, mHilitePos)) +
        ...
        if (aBaseString.length() > 0) {
            ...
            while (it != end) {
                ...
                if (mFont->CharWidth(aCodepoint) != 0 &&
                    mEditListener->AllowChar(mId, aCodepoint)) {
            ...
            if (mHilitePos == -1) {
                ...
                mString = mString.substr(0, std::min(mCursorPos, mHilitePos)) + aString +
                          mString.substr(std::max(mCursorPos, mHilitePos));
                mCursorPos = std::min(mCursorPos, mHilitePos);
                mHilitePos = -1;
            }
        ...
        if (controlDown) {
            // Get to whitespace
            while ((mCursorPos < (int)mString.length() - 1) &&
                   (IsPartOfWord(mString[mCursorPos + 1])))
                mCursorPos++;
        ...
        if (utf8::is_valid(aCharString) && (mFont->StringWidth(aCharString) > 0) &&
            mEditListener->AllowChar(mId, theChar)) {
            if ((mHilitePos != -1) && (mHilitePos != mCursorPos)) {
                // Replace selection with new character
                mString = mString.substr(0, std::min(mCursorPos, mHilitePos)) +
                          SexyString(1, theChar) + mString.substr(std::max(mCursorPos, mHilitePos));
                mCursorPos = std::min(mCursorPos, mHilitePos);
                mHilitePos = -1;
...
void EditWidget::FocusCursor(bool bigJump) {
    ...
    if (mFont != NULL) {
        SexyString& aString = GetDisplayString();
        while ((mWidth - 8 > 0) && (mFont->StringWidth(aString.substr(0, mCursorPos)) -
                                        mFont->StringWidth(aString.substr(0, mLeftPos)) >=
                                    mWidth - 8)) {
            if (bigJump)
