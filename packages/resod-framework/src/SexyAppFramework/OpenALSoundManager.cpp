...
bool OpenALSoundManager::LoadSound(unsigned int theSfxID, const std::string& theFilename) {
    ...
    return LoadWAVSound(theSfxID, theFilename + ".wav") ||
...
int ov_pak_open(PFILE* f, OggVorbis_File* vf, char* initial, long ibytes) {
    ov_callbacks callbacks = {(size_t (*)(void*, size_t, size_t, void*))p_fread,
                              ...
                              (int (*)(void*))p_fclose, (long (*)(void*))p_ftell};
...
bool OpenALSoundManager::LoadOGGSound(unsigned int theSfxID, const std::string& theFilename) {
    ...
    while (aNumBytes > 0) {
        long ret =
            ov_read(&vf, aPtr, std::min(aNumBytes, (int64_t)4096), 0, 2, 1, &current_section);
...
bool OpenALSoundManager::LoadMP3Sound(unsigned int theSfxID, const std::string& theFilename) {
    ...
    uint64_t numberOfSamplesActuallyDecoded =
        drmp3_read_pcm_frames_s16(&aMP3, aMP3.totalPCMFrameCount, pDecodedInterleavedPCMFrames);

    ALuint buffer;
    alGenBuffers(1, &buffer);
    alBufferData(buffer, aMP3.channels == 2 ? AL_FORMAT_STEREO16 : AL_FORMAT_MONO16,
                 pDecodedInterleavedPCMFrames, aSize, numberOfSamplesActuallyDecoded);
...
bool OpenALSoundManager::LoadWAVSound(unsigned int theSfxID, const std::string& theFilename) {
    ...
    size_t numberOfSamplesActuallyDecoded =
        drwav_read_pcm_frames_s32(&aWAV, aWAV.totalPCMFrameCount, pDecodedInterleavedPCMFrames);

    ALuint buffer;
    alGenBuffers(1, &buffer);
    alBufferData(buffer, aWAV.channels == 2 ? AL_FORMAT_STEREO16 : AL_FORMAT_MONO16,
                 pDecodedInterleavedPCMFrames, aSize, numberOfSamplesActuallyDecoded);
...
bool OpenALSoundManager::LoadFLACSound(unsigned int theSfxID, const std::string& theFilename) {
    ...
    ALuint buffer;
    alGenBuffers(1, &buffer);
    alBufferData(buffer, aFLAC->channels == 2 ? AL_FORMAT_STEREO16 : AL_FORMAT_MONO16,
                 pDecodedInterleavedPCMFrames, aSize, numberOfSamplesActuallyDecoded);
...
bool OpenALSoundManager::LoadAUSound(unsigned int theSfxID, const std::string& theFilename) {
    ...
    ALuint buffer;
    alGenBuffers(1, &buffer);
    alBufferData(buffer, anAUFile.mChannels == 1 ? AL_FORMAT_MONO16 : AL_FORMAT_STEREO16,
                 anAUFile.mSamples.data(), anAUFile.mSamples.size() * sizeof(int16_t),
...
bool Sexy::OpenALSoundManager::WriteWAV(unsigned int theSfxID, const std::string& theFilename,
    ...
    if (mSourceSounds[theSfxID]) {
        for (int i = 0; i < MAX_CHANNELS; i++) {
            if (mPlayingSounds[i] != NULL &&
                mPlayingSounds[i]->mSourceSoundBuffer == mSourceSounds[theSfxID]) {
                delete mPlayingSounds[i];
...
bool Sexy::OpenALSoundManager::GetTheFileTime(const std::string& theDepFile,
                                              uint64_t* theFileTime) {
    return false;
}
