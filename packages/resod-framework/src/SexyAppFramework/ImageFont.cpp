...
SingleDataElement::SingleDataElement(const std::string theString) : mString(theString) {
    mIsList = false;
}
...
FontLayer::FontLayer(const FontLayer& theFontLayer)
    : mFontData(theFontLayer.mFontData), mRequiredTags(theFontLayer.mRequiredTags),
      mExcludedTags(theFontLayer.mExcludedTags), mImage(theFontLayer.mImage),
      mDrawMode(theFontLayer.mDrawMode), mOffset(theFontLayer.mOffset),
      mSpacing(theFontLayer.mSpacing), mMinPointSize(theFontLayer.mMinPointSize),
      mMaxPointSize(theFontLayer.mMaxPointSize), mPointSize(theFontLayer.mPointSize),
      mAscent(theFontLayer.mAscent), mAscentPadding(theFontLayer.mAscentPadding),
      mHeight(theFontLayer.mHeight), mDefaultHeight(theFontLayer.mDefaultHeight),
      mColorMult(theFontLayer.mColorMult), mColorAdd(theFontLayer.mColorAdd),
...
bool FontData::GetColorFromDataElement(DataElement* theElement, Color& theColor) {
    if (theElement->mIsList) {
        ...
        theColor = Color((int)(aFactorVector[0] * 255), (int)(aFactorVector[1] * 255),
                         (int)(aFactorVector[2] * 255), (int)(aFactorVector[3] * 255));
...
bool FontData::HandleCommand(const ListDataElement& theParams) {
    ...
    if (stricmp(aCmd.c_str(), "Define") == 0) {
        if (theParams.mElementVector.size() == 3) {
            if (!theParams.mElementVector[1]->mIsList) {
                std::string aDefineName =
                    StringToUpper(((SingleDataElement*)theParams.mElementVector[1])->mString);
                ...
                if (!IsImmediate(aDefineName)) {
                    ...
                    if (theParams.mElementVector[2]->mIsList) {
                        ...
                        SingleDataElement* aDefParam =
                            (SingleDataElement*)theParams.mElementVector[2];

                        DataElement* aDerefVal = Dereference(aDefParam->mString);

                        if (aDerefVal)
                            mDefineMap.insert(
                                DataElementMap::value_type(aDefineName, aDerefVal->Duplicate()));
                        else
                            mDefineMap.insert(
                                DataElementMap::value_type(aDefineName, aDefParam->Duplicate()));
                    }
                } else
        ...
        if (theParams.mElementVector.size() == 2) {
            ...
            if ((!theParams.mElementVector[1]->mIsList) &&
                (StringToInt(((SingleDataElement*)theParams.mElementVector[1])->mString,
                             &aPointSize))) {
            ...
            if ((DataToStringVector(theParams.mElementVector[1], &aFromVector)) &&
                ...
                if (aFromVector.size() == aToVector.size()) {
                    for (uint32_t aMapIdx = 0; aMapIdx < aFromVector.size(); aMapIdx++) {
                        if ((aFromVector[aMapIdx].length() == 1) &&
                            (aToVector[aMapIdx].length() == 1)) {
                            mCharMap[(uint32_t)aFromVector[aMapIdx][0]] =
                                (uint32_t)aToVector[aMapIdx][0];
                        } else
                            invalidParamFormat = true;
                    }
                } else
        ...
        if (theParams.mElementVector.size() == 2) {
            if (!theParams.mElementVector[1]->mIsList) {
                ...
                if (!mFontLayerMap.insert(FontLayerMap::value_type(aLayerName, aFontLayer))
                         .second) {
        ...
        if (theParams.mElementVector.size() == 3) {
            ...
            if ((!theParams.mElementVector[1]->mIsList) &&
                (DataToLayer(theParams.mElementVector[2], &aSourceLayer))) {
                std::string aLayerName =
                    StringToUpper(((SingleDataElement*)theParams.mElementVector[1])->mString);
        ...
        if (theParams.mElementVector.size() == 4) {
            ...
            if ((DataToLayer(theParams.mElementVector[1], &aLayer)) &&
                ...
                if ((StringToInt(((SingleDataElement*)theParams.mElementVector[2])->mString,
                                 &aMinPointSize)) &&
                    (StringToInt(((SingleDataElement*)theParams.mElementVector[3])->mString,
                                 &aMaxPointSize))) {
        ...
        if (theParams.mElementVector.size() == 3) {
            ...
            if ((DataToLayer(theParams.mElementVector[1], &aLayer)) &&
                (DataToIntVector(theParams.mElementVector[2], &anOffset)) &&
                (anOffset.size() == 2)) {
            ...
            if ((DataToLayer(theParams.mElementVector[1], &aLayer)) &&
                ...
                if (aCharsVector.size() == aCharWidthsVector.size()) {
                    for (uint32_t i = 0; i < aCharsVector.size(); i++) {
                        if (utf8::is_valid(aCharsVector[i])) {
                            std::string aCharStr = aCharsVector[i];
                            auto it = aCharStr.begin();
                            auto end = aCharStr.end();
                            uint32_t aCodepoint = 0;
                            while (it != end)
                                aCodepoint = utf8::next(it, end);
                            aLayer->mCharData[aCodepoint].mWidth = aCharWidthsVector[i];
                        } else
                            invalidParamFormat = true;
                    }
                } else
                ...
                if (aCharsVector.size() == aRectList.mElementVector.size()) {
                    if ((Image*)aLayer->mImage != NULL) {
                        ...
                        for (uint32_t i = 0; i < aCharsVector.size(); i++) {
                            ...
                            {
                                Rect aRect = Rect(aRectElement[0], aRectElement[1], aRectElement[2],
                                                  aRectElement[3]);

                                if ((aRect.mX < 0) || (aRect.mY < 0) ||
                                    (aRect.mX + aRect.mWidth > anImageWidth) ||
                                ...
                                while (it != end)
                                    aCodepoint = utf8::next(it, end);
                                aLayer->mCharData[aCodepoint].mImageRect = aRect;
                                ;
                            } else
                                invalidParamFormat = true;
                        }
                        ...
                        for (auto& it : aLayer->mCharData)
                            if (aLayer->mCharData[it.first].mImageRect.mHeight +
                                    aLayer->mCharData[it.first].mOffset.mY >
                                aLayer->mDefaultHeight)
                                aLayer->mDefaultHeight =
                                    aLayer->mCharData[it.first].mImageRect.mHeight +
                                    aLayer->mCharData[it.first].mOffset.mY;
                    } else {
                ...
                if (aCharsVector.size() == aRectList.mElementVector.size()) {
                    for (uint32_t i = 0; i < aCharsVector.size(); i++) {
                        ...
                        if (utf8::is_valid(aCharsVector[i]) &&
                            ...
                            while (it != end)
                                aCodepoint = utf8::next(it, end);
                            aLayer->mCharData[aCodepoint].mOffset =
                ...
                if (aPairsVector.size() == anOffsetsVector.size()) {
                    for (uint32_t i = 0; i < aPairsVector.size(); i++) {
                        if (aPairsVector[i].length() == 2) {
                            aLayer->mCharData[aPairsVector[i][0]]
                                .mKerningOffsets[aPairsVector[i][1]] = anOffsetsVector[i];
                        } else
                            invalidParamFormat = true;
                    }
                } else
                ...
                if (aCharsVector.size() == aCharOrdersVector.size()) {
                    for (uint32_t i = 0; i < aCharsVector.size(); i++) {
                        if (utf8::is_valid(aCharsVector[i])) {
                            ...
                            while (it != end)
                                aCodepoint = utf8::next(it, end);
                            aLayer->mCharData[aCodepoint].mOrder = aCharOrdersVector[i];
                        } else
                            invalidParamFormat = true;
                    }
                } else
...
bool FontData::LoadLegacy(Image* theFontImage, const std::string& theFontDescFileName) {
    ...
    FontLayerMap::iterator anItr =
        mFontLayerMap.insert(FontLayerMap::value_type("", aFontLayer)).first;
    if (anItr == mFontLayerMap.end())
        return false;
    ...
    while (!feof(aStream)) {
        ...
        aFontLayer->mCharData[aChar].mImageRect =
    ...
    for (c = 'A'; c <= 'Z'; c++)
        if ((aFontLayer->mCharData[c].mWidth == 0) &&
            (aFontLayer->mCharData[c - 'A' + 'a'].mWidth != 0))
            mCharMap[c] = c - 'A' + 'a';
...
ActiveFontLayer::ActiveFontLayer(const ActiveFontLayer& theActiveFontLayer)
    : mBaseFontLayer(theActiveFontLayer.mBaseFontLayer),
      mScaledImage(theActiveFontLayer.mScaledImage), mOwnsImage(theActiveFontLayer.mOwnsImage),
      mScaledCharImageRects(theActiveFontLayer.mScaledCharImageRects) {
    if (mOwnsImage)
        mScaledImage = (Sexy::Image*)mBaseFontLayer->mFontData->mApp->CopyImage(mScaledImage);
}
...
ImageFont::ImageFont(const ImageFont& theImageFont)
    : Font(theImageFont), mScale(theImageFont.mScale), mFontData(theImageFont.mFontData),
      mPointSize(theImageFont.mPointSize), mTagVector(theImageFont.mTagVector),
    ...
    while (anItr != mFontData->mFontLayerList.end()) {
        ...
        if ((mPointSize >= aFontLayer->mMinPointSize) &&
            ...
            for (i = 0; i < aFontLayer->mRequiredTags.size(); i++)
                if (std::find(mTagVector.begin(), mTagVector.end(), aFontLayer->mRequiredTags[i]) ==
                    mTagVector.end())
                    active = false;
            ...
            if (active) {
                ...
                if ((mScale == 1.0) &&
                    ...
                    for (auto& it : aFontLayer->mCharData)
                        anActiveFontLayer->mScaledCharImageRects[it.first] =
                            it.second.mImageRect; // glyph code and data
                } else {
                ...
                int aLineSpacingOffset =
                    (aFontLayer->mLineSpacingOffset * aPointSize) / aLayerPointSize;
...
int ImageFont::CharWidthKern(uint32_t theChar, uint32_t thePrevChar) {
    ...
    if (mFontData->mCharMap.find(theChar) != mFontData->mCharMap.end())
        theChar = mFontData->mCharMap[theChar];
    if (thePrevChar != 0 && mFontData->mCharMap.find(thePrevChar) != mFontData->mCharMap.end())
        thePrevChar = mFontData->mCharMap[thePrevChar];
        ...
        if (aLayerPointSize == 0) {
            ...
            if (thePrevChar != 0) {
                aSpacing = (anActiveFontLayer->mBaseFontLayer->mSpacing +
                            anActiveFontLayer->mBaseFontLayer->mCharData[thePrevChar]
                                .mKerningOffsets[theChar]) *
                           mScale;
            } else
                aSpacing = 0;
        } else {
            aCharWidth = (anActiveFontLayer->mBaseFontLayer->mCharData[theChar].mWidth *
                          aPointSize / aLayerPointSize);
...
void ImageFont::DrawStringEx(Graphics* g, int theX, int theY, const SexyString& theString,
    ...
    uint32_t aPrevCode = 0;

    while (it != end) {
        uint32_t aCodePoint = utf8::next(it, end);

        uint32_t aChar = GetMappedChar(aCodePoint);

        uint32_t aNextChar = 0;
        auto itCopy = it;

        if (itCopy != end)
            aNextChar = utf8::next(itCopy, end);

        aPrevCode = aCodePoint;
        ...
        while (anItr != mActiveLayerList.end()) {
            ...
            if (aScale == 1.0) {
                ...
                anImageY = theY - (anActiveFontLayer->mBaseFontLayer->mAscent -
                                   ...
                                   anActiveFontLayer->mBaseFontLayer->mCharData[aChar].mOffset.mY);
                aCharWidth = anActiveFontLayer->mBaseFontLayer->mCharData[aChar].mWidth;

                if (aNextChar != 0) {
                    aSpacing = anActiveFontLayer->mBaseFontLayer->mSpacing +
                               anActiveFontLayer->mBaseFontLayer->mCharData[aChar]
                                   .mKerningOffsets[aNextChar];
                } else
                    aSpacing = 0;
            } else {
                anImageX = aLayerXPos +
                           (int)((anActiveFontLayer->mBaseFontLayer->mOffset.mX +
                                 ...
                                 aScale);
                anImageY =
                    theY - (int)((anActiveFontLayer->mBaseFontLayer->mAscent -
                                  anActiveFontLayer->mBaseFontLayer->mOffset.mY -
                                  anActiveFontLayer->mBaseFontLayer->mCharData[aChar].mOffset.mY) *
                                 aScale);
            ...
            Color aColor;
            aColor.mRed = std::min(
                (theColor.mRed * anActiveFontLayer->mBaseFontLayer->mColorMult.mRed / 255) +
                    anActiveFontLayer->mBaseFontLayer->mColorAdd.mRed,
                255);
            aColor.mGreen = std::min(
                (theColor.mGreen * anActiveFontLayer->mBaseFontLayer->mColorMult.mGreen / 255) +
                    anActiveFontLayer->mBaseFontLayer->mColorAdd.mGreen,
                255);
            aColor.mBlue = std::min(
                (theColor.mBlue * anActiveFontLayer->mBaseFontLayer->mColorMult.mBlue / 255) +
                    anActiveFontLayer->mBaseFontLayer->mColorAdd.mBlue,
                255);
            aColor.mAlpha = std::min(
                (theColor.mAlpha * anActiveFontLayer->mBaseFontLayer->mColorMult.mAlpha / 255) +
            ...
            anActiveFontLayer->mScaledCharImageRects[aChar]); g->SetColor(anOrigColor);
            g->SetDrawMode(anOldDrawMode);*/

            if (theDrawnAreas != NULL) {
                Rect aDestRect =
                    Rect(anImageX, anImageY, anActiveFontLayer->mScaledCharImageRects[aChar].mWidth,
                         anActiveFontLayer->mScaledCharImageRects[aChar].mHeight);
                ...
                else
                {
                        ...
                        if (theDestRect.mX + theDestRect.mWidth > theDrawnArea->mX +
                theDrawnArea->mWidth) theDrawnArea->mWidth = theDestRect.mX + theDestRect.mWidth -
                theDrawnArea->mX;
                        ...
                        if (theDestRect.mY + theDestRect.mHeight > theDrawnArea->mY +
                theDrawnArea->mHeight) theDrawnArea->mHeight = theDestRect.mY + theDestRect.mHeight
                - theDrawnArea->mY;
                }*/
            }
...
SexyChar ImageFont::GetMappedChar(uint32_t value) {
    if (mFontData->mCharMap.find(value) != mFontData->mCharMap.end())
        return mFontData->mCharMap[value];
    else
        return value;
}

void ImageFont::DrawString(Graphics* g, int theX, int theY, const SexyString& theString,
                           const Color& theColor, const Rect& theClipRect) {
