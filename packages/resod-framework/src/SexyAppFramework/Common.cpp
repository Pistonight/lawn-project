...
std::string gAppDataFolder =
    std::filesystem::path(std::getenv("HOME")).string() + "/Library/Application Support/";
#else
...
std::string Sexy::GetPathFrom(const std::string& theRelPath, const std::string& theDir) {
    ...
    for (;;) {
        ...
        if ((aFirstSlash == -1) ||
            ((aFirstForwardSlash != -1) && (aFirstForwardSlash < aFirstSlash)))
        ...
        if (aChDir.compare("..") == 0) {
            ...
            while ((aLastDirStart > 0) && (aNewPath[aLastDirStart - 1] != '\\') &&
                   (aNewPath[aLastDirStart - 1] != '/'))
                aLastDirStart--;

            std::string aLastDir =
                aNewPath.substr(aLastDirStart, aNewPath.length() - aLastDirStart - 1);
...
bool Sexy::AllowAllAccess(const std::string& theFileName) {
    ...
    BOOL(WINAPI * fnSetSecurityDescriptorDacl)(PSECURITY_DESCRIPTOR pSecurityDescriptor,
                                               BOOL bDaclPresent, PACL pDacl, BOOL bDaclDefaulted);
    BOOL(WINAPI * fnInitializeSecurityDescriptor)(PSECURITY_DESCRIPTOR pSecurityDescriptor,
                                                  DWORD dwRevision);
    BOOL(WINAPI * fnAllocateAndInitializeSid)(
        ...
        PSID * pSid);
    DWORD(WINAPI * fnSetEntriesInAcl)(ULONG cCountOfExplicitEntries,
    ...
    *(void**)&fnSetSecurityDescriptorDacl =
        (void*)GetProcAddress(aLib, "SetSecurityDescriptorDacl");
    *(void**)&fnInitializeSecurityDescriptor =
        (void*)GetProcAddress(aLib, "InitializeSecurityDescriptor");
    ...
    *(void**)&fnFreeSid = (void*)GetProcAddress(aLib, "FreeSid");
    ...
    if (fnAllocateAndInitializeSid(&SIDAuthWorld, 1, SECURITY_WORLD_RID, 0, 0, 0, 0, 0, 0, 0,
                                   &pEveryoneSID)) {
        EXPLICIT_ACCESS ea;
        ...
        if (fnSetEntriesInAcl(1, &ea, NULL, &pACL) == ERROR_SUCCESS) {
            ...
            PSECURITY_DESCRIPTOR pSD =
                (PSECURITY_DESCRIPTOR) new char[SECURITY_DESCRIPTOR_MIN_LENGTH];
...
std::string Sexy::Evaluate(const std::string& theString, const DefinesMap& theDefinesMap) {
    ...
    for (;;) {
        ...
        std::string aName =
            anEvaluatedString.substr(aPercentPos + 1, aSecondPercentPos - aPercentPos - 1);
