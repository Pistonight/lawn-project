...
void ButtonWidget::DrawButtonImage(Graphics* g, Image* theImage, const Rect& theRect, int x,
                                   int y) {
    if (theRect.mWidth != 0)
...
void ButtonWidget::MouseEnter() {
    ...
    if (mIsDown || (HaveButtonImage(mOverImage, mOverRect)) ||
        (mColors[COLOR_LABEL_HILITE] != mColors[COLOR_LABEL]))
        MarkDirty();
