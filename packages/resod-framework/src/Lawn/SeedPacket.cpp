...
void SeedPacket::PickNextSlotMachineSeed() {
    ...
    SeedType SLOT_SEED_TYPES[] = {
        SeedType::SEED_SUNFLOWER,        SeedType::SEED_PEASHOOTER,
    ...
    for (int i = 0; i < LENGTH(SLOT_SEED_TYPES); i++) {
        ...
        if (mIndex == 2 && aSeedType != SeedType::SEED_SLOT_MACHINE_DIAMOND) {
            if (aSeedType == mBoard->mSeedBank->mSeedPackets[0].mSlotMachiningNextSeed ||
                aSeedType == mBoard->mSeedBank->mSeedPackets[1].mSlotMachiningNextSeed) {
                aWeight += aWeight / 2;
            }
        }
...
void SeedPacket::FlashIfReady() {
    ...
    if (!mBoard->HasConveyorBeltSeedBank()) {
        ...
        mApp->AddTodParticle(mX + mBoard->mSeedBank->mX, mY + mBoard->mSeedBank->mY,
                             aRenderPosition, ParticleEffect::PARTICLE_SEED_PACKET_FLASH);
    }
    ...
    } else if (mBoard->mTutorialState == TutorialState::TUTORIAL_LEVEL_2_REFRESH_SUNFLOWER &&
               mPacketType == SeedType::SEED_SUNFLOWER) {
        mBoard->SetTutorialState(TutorialState::TUTORIAL_LEVEL_2_PICK_UP_SUNFLOWER);
    } else if (mBoard->mTutorialState == TutorialState::TUTORIAL_MORESUN_REFRESH_SUNFLOWER &&
               mPacketType == SeedType::SEED_SUNFLOWER) {
...
void SeedPacket::Update() {
    ...
    if (mSlotMachineCountDown > 0) {
        ...
        float aFlipsPerSecont = TodAnimateCurveFloat(SLOT_MACHINE_TIME, 0, mSlotMachineCountDown,
                                                     6.0f, 2.0f, TodCurves::CURVE_LINEAR);
...
void SeedPacketDrawSeed(Graphics* g, float x, float y, SeedType theSeedType,
                        SeedType theImitaterType, float theOffsetX, float theOffsetY,
                        float theScale) {
    ...
    if (aSeedType == SeedType::SEED_POTATOMINE && g->mScaleX <= 1.0f) {
        ...
        Plant::DrawSeedType(&aSeedG, theSeedType, theImitaterType, DrawVariation::VARIATION_NORMAL,
                            x + theOffsetX, y + theOffsetY);
    }
}

void DrawSeedPacket(Graphics* g, float x, float y, SeedType theSeedType, SeedType theImitaterType,
                    float thePercentDark, int theGrayness, bool theDrawCost,
    ...
    int aPacketBackground = theSeedType == SeedType::SEED_IMITATER                   ? 0
    ...
    if (g->mScaleX > 1 && !mIgnorePacketSpriteScale) {
        TodDrawImageCelScaledF(g, Sexy::IMAGE_SEEDPACKET_LARGER, x, y, 0, 0, g->mScaleX * 0.5f,
                               g->mScaleY * 0.5f);
    } else {
        if (!mIgnorePacketSpriteScale)
            g->SetLinearBlend(false);
        TodDrawImageCelScaledF(g, Sexy::IMAGE_SEEDS, x, y, aPacketBackground, 0, g->mScaleX,
                               g->mScaleY);
    }
    ...
    if (thePercentDark > 0.0f) {
        ...
        aPlantG.ClipRect(x, y, SEED_PACKET_WIDTH * aPlantG.mScaleX,
                         aDarknessHeight * aPlantG.mScaleY);
        TodDrawImageCelScaledF(&aPlantG, Sexy::IMAGE_SEEDS, x, y, aPacketBackground, 0,
                               aPlantG.mScaleX, aPlantG.mScaleY);
        if (aDrawSeedInMiddle) {
            SeedPacketDrawSeed(&aPlantG, x, y, theSeedType, theImitaterType, aOffsetX, aOffsetY,
                               aScale);
        }
    }
    ...
    if (theDrawCost) {
        ...
        if (gLawnApp->mBoard && gLawnApp->mBoard->PlantUsesAcceleratedPricing(aSeedType)) {
            if (theUseCurrentCost) {
                aCostStr = StrFormat(
                    "%d", gLawnApp->mBoard->GetCurrentPlantCost(theSeedType, theImitaterType));
            } else {
        ...
        if (g->mScaleX == 1.0f && g->mScaleY == 1.0f) {
            TodDrawString(g, aCostStr, x + aTextOffsetX, y + aTextOffsetY, aTextFont, Color::Black,
                          DS_ALIGN_LEFT);
        } else {
            SexyMatrix3 aMatrix;
            TodScaleTransformMatrix(aMatrix, aTextOffsetX * g->mScaleX + x + g->mTransX,
                                    aTextOffsetY * g->mScaleY + y + g->mTransY, g->mScaleX,
                                    g->mScaleY);
            if (g->mScaleX > 1.8f) {
                g->SetLinearBlend(false);
            }
...
void SeedPacket::Draw(Graphics* g) {
    ...
    if (mSlotMachineCountDown > 0) {
        ...
        DrawSeedPacket(&aClipG, 0.0f, aOffsetY, mPacketType, SeedType::SEED_NONE, 0.0f, 128, false,
                       false);
        DrawSeedPacket(&aClipG, 0.0f, mHeight + aOffsetY, mSlotMachiningNextSeed,
                       SeedType::SEED_NONE, 0.0f, 128, false, false);
    } else {
        ...
        if ((mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_BEGHOULED && !mActive) ||
            ...
            aPercentDark = 0.0f;
        } else if (mBoard->mTutorialState == TutorialState::TUTORIAL_LEVEL_1_PICK_UP_PEASHOOTER &&
                   mBoard->mTutorialTimer == -1 && mPacketType == SeedType::SEED_PEASHOOTER) {
            ...
            aPercentDark = 0.0f;
                   ...
                   !mBoard->PlantingRequirementsMet(aUseSeedType)) {
            aGrayness = 128;
        }

        DrawSeedPacket(g, mOffsetX, 0.0f, mPacketType, mImitaterType, aPercentDark, aGrayness,
                       aDrawCost, true);
    }
}
...
void SeedPacket::MouseDown(int x, int y, int theClickCount) {
    ...
    if (mApp->IsSlotMachineLevel()) {
        if (!mBoard->mAdvice->IsBeingDisplayed()) {
            mBoard->DisplayAdvice("[ADVICE_SLOT_MACHINE_PULL]",
                                  MessageStyle::MESSAGE_STYLE_HINT_TALL_FAST,
                                  AdviceType::ADVICE_NONE);
        }
        mBoard->mChallenge->mSlotMachineRollCount =
            std::min(mBoard->mChallenge->mSlotMachineRollCount, 2);
        return;
    }
        ...
        if (mBoard->mTutorialState == TutorialState::TUTORIAL_LEVEL_1_PICK_UP_PEASHOOTER) {
            ...
            if (mPacketType == SeedType::SEED_SUNFLOWER) {
                ...
                mBoard->SetTutorialState(TutorialState::TUTORIAL_MORESUN_REFRESH_SUNFLOWER);
                   ...
                   mBoard->mTutorialState ==
                       TutorialState::TUTORIAL_WHACK_A_ZOMBIE_BEFORE_PICK_SEED) {
...
bool SeedPacket::MouseHitTest(int theX, int theY, HitResult* theHitResult) {
    ...
    if (theX >= mX + mOffsetX && theX < mX + mOffsetX + mWidth && theY >= mY &&
        theY < mY + mHeight) {
...
void SeedBank::Draw(Graphics* g) {
    ...
    if (mApp->IsSlotMachineLevel()) {
        ...
        Rect theSrcRect(IMAGE_SEEDBANK->mWidth - aExtraWidth - 12, 0, aExtraWidth + 12,
                        IMAGE_SEEDBANK->mHeight);
    ...
    if (mApp->mGameScene == GameScenes::SCENE_PLAYING && mApp->mGamepads[0] != nullptr) {
        ...
        if (mApp->UsingGamepad()) {
            ...
            if (aSeedPacket->mPacketType != SeedType::SEED_NONE) {
                g->DrawImage(Sexy::IMAGE_SEED_SELECTOR, aSeedPacket->mX + aSeedPacket->mOffsetX - 5,
                             3);
            }
        }
    }
    ...
    if (!mBoard->HasConveyorBeltSeedBank()) {
        SexyString aMoneyLabel = StrFormat("%d", std::max(mBoard->mSunMoney, 0));
...
void SeedPacket::SetPacketType(SeedType theSeedType, SeedType theImitaterType) {
    ...
    if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_BEGHOULED ||
        ...
        (mApp->IsSurvivalMode() && mBoard->mChallenge->mSurvivalStage > 0))
        return;

    if ((Plant::IsUpgrade(aUseSeedType) && !gLawnApp->IsSurvivalMode()) ||
        Plant::GetRefreshTime(mPacketType, mImitaterType) == 5000) {
        mRefreshTime = 3500;
        mRefreshing = true;
        mActive = false;
