        ...
        if (theContext.mReading) {
            ...
            for (int i = 0; i < aCount; i++) {
                ...
                theContext.SyncBytes(&aDataID, sizeof(aDataID),
                                     StrFormat("%s_List_Data[%d]", aListOwner.c_str(), i));
            ...
            theContext.SyncVar(aCount, StrFormat("%s_List_Size", aListOwner.c_str()));
            int i = 0;
            for (TodListNode<unsigned int>* aNode = theDataIDList->mHead; aNode != nullptr;
                 aNode = aNode->mNext) {
...
void SyncParticleEmitter(TodParticleSystem* theParticleSystem,
                         TodParticleEmitter* theParticleEmitter, SaveContext& theContext) {
    std::string aIDStr = StrFormat(
        "%u", theParticleSystem->mParticleHolder->mEmitters.DataArrayGetID(theParticleEmitter));
    int aEmitterDefIndex = 0;
    if (theContext.mReading) {
        theContext.SyncVar(aEmitterDefIndex,
                           StrFormat("ParticleEmitterDefinition_%s", aIDStr.c_str()));
        theParticleEmitter->mParticleSystem = theParticleSystem;
        theParticleEmitter->mEmitterDef =
            &theParticleSystem->mParticleDef->mEmitterDefs[aEmitterDefIndex];
    } else {
        aEmitterDefIndex =
            (int)(theParticleEmitter->mEmitterDef - theParticleSystem->mParticleDef->mEmitterDefs);
        theContext.SyncVar(aEmitterDefIndex,
                           StrFormat("ParticleEmitterDefinition_%s", aIDStr.c_str()));
    }
    ...
    for (TodListNode<ParticleID>* aNode = theParticleEmitter->mParticleList.mHead; aNode != nullptr;
         aNode = aNode->mNext) {
        TodParticle* aParticle = theParticleSystem->mParticleHolder->mParticles.DataArrayGet(
...
void SyncParticleSystem(Board* theBoard, TodParticleSystem* theParticleSystem,
                        SaveContext& theContext) {
    std::string aIDStr = StrFormat(
        "%u", theBoard->mApp->mEffectSystem->mParticleHolder->mParticleSystems.DataArrayGetID(
                  theParticleSystem));
    ...
    SyncDataIDList((TodList<unsigned int>*)&theParticleSystem->mEmitterList, theContext,
                   ...
                   StrFormat("ParticleSystem_%s", aIDStr.c_str()));
    for (TodListNode<ParticleEmitterID>* aNode = theParticleSystem->mEmitterList.mHead;
...
void SyncReanimation(Board* theBoard, Reanimation* theReanimation, SaveContext& theContext) {
    std::string aIDStr = StrFormat(
        "%u", theBoard->mApp->mEffectSystem->mReanimationHolder->mReanimations.DataArrayGetID(
                  theReanimation));
    theContext.SyncReanimationDef(theReanimation->mDefinition,
                                  StrFormat("Reanimation_%s", aIDStr.c_str()));
    if (theContext.mReading) {
    ...
    if (theReanimation->mDefinition->mTrackCount != 0) {
        ...
        if (theContext.mReading) {
            theReanimation->mTrackInstances =
                (ReanimatorTrackInstance*)FindGlobalAllocator(aSize)->Calloc(aSize);
        }
        theContext.SyncBytes(theReanimation->mTrackInstances, aSize,
                             StrFormat("Reanimation_%s_Tracks", aIDStr.c_str()));

        for (int aTrackIndex = 0; aTrackIndex < theReanimation->mDefinition->mTrackCount;
             aTrackIndex++) {
            ReanimatorTrackInstance& aTrackInstance = theReanimation->mTrackInstances[aTrackIndex];
            theContext.SyncImage(
                aTrackInstance.mImageOverride,
...
void SyncTrail(Board* theBoard, Trail* theTrail, SaveContext& theContext) {
    theContext.SyncTrailDef(
        ...
        StrFormat("Trail_%u",
                  theBoard->mApp->mEffectSystem->mTrailHolder->mTrails.DataArrayGetID(theTrail)));
    if (theContext.mReading) {
...
void LawnSyncGame(Board* theBoard, SaveContext& theContext) {
    ...
    SYNC_DATA_ARRAY(TodParticleSystem,
                    theBoard->mApp->mEffectSystem->mParticleHolder->mParticleSystems)
    ...
    while (
        theBoard->mApp->mEffectSystem->mParticleHolder->mParticleSystems.IterateNext(aParticle)) {
        SyncParticleSystem(theBoard, aParticle, theContext);
    }

    Reanimation* aReanimation = nullptr;
    while (theBoard->mApp->mEffectSystem->mReanimationHolder->mReanimations.IterateNext(
        aReanimation)) {
