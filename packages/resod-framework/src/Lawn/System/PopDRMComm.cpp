...
PopDRMComm::PopDRMComm() {
    ...
    mWindowMessage[PopDRMComm::PopCapDRM_EnableLocking] =
        RegisterClipboardFormat("PopCapDRM_EnableLocking");
    mWindowMessage[PopDRMComm::PopCapDRM_EnableTiming] =
        RegisterClipboardFormat("PopCapDRM_EnableTiming");
    mWindowMessage[PopDRMComm::PopCapDRM_QueryData] =
        RegisterClipboardFormat("PopCapDRM_QueryData");
    mWindowMessage[PopDRMComm::PopCapDRM_IPC] = RegisterClipboardFormat("PopCapDRM_IPC");
    mWindowMessage[PopDRMComm::PopCapDRM_IPC_Response] =
        RegisterClipboardFormat("PopCapDRM_IPC_Response");
    mWindowMessage[PopDRMComm::PopCapDRM_ForceLock] =
        RegisterClipboardFormat("PopCapDRM_ForceLock");
    mWindowMessage[PopDRMComm::PopCapDRM_BuyGame] = RegisterClipboardFormat("PopCapDRM_BuyGame");
    mWindowMessage[PopDRMComm::PopCapDRM_ShowScreen] =
        RegisterClipboardFormat("PopCapDRM_ShowScreen");
...
bool PopDRMComm::DoIPC() {
    ...
    if (!mIsWindow) {
        mEventHwnd = CreateEvent(NULL, NULL, NULL, NULL);
        PostMessage((HWND)0xFFFF, mWindowMessage[PopDRMComm::PopCapDRM_IPC], GetCurrentProcessId(),
                    (LPARAM)mDrmGameWnd);
