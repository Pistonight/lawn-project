...
void PottedPlant::InitializePottedPlant(SeedType theSeedType) {
    ...
    mFacing = (FacingDirection)RandRangeInt((int)FacingDirection::FACING_RIGHT,
                                            (int)FacingDirection::FACING_LEFT);
...
template <typename T, size_t N>
void ProfileSyncer::SyncArrayFromSize(const SexyString& theName, T (&theArray)[N],
                                      int theRealSize) {
    if (mReading) {
        if (json_has_key(mJSON, theName) && mJSON[theName].is_array()) {
            const auto& aJsonArray = mJSON[theName];
