...
void Music::SetupMusicFileForTune(MusicFile theMusicFile, MusicTune theMusicTune) {
    ...
    default:
        if (theMusicFile == MusicFile::MUSIC_FILE_MAIN_MUSIC ||
            theMusicFile == MusicFile::MUSIC_FILE_DRUMS ||
            theMusicFile == MusicFile::MUSIC_FILE_HIHATS) {
            aTrackCount = 29;
            aTrackStart1 = 0;
            aTrackEnd1 = 29;
        }
...
void Music::MusicLoadCreditsSong() {
    ...
    if (aBass->mMusicMap.find((int)MusicFile::MUSIC_FILE_CREDITS_ZOMBIES_ON_YOUR_LAWN) ==
        aBass->mMusicMap.end())
        LoadSong(MusicFile::MUSIC_FILE_CREDITS_ZOMBIES_ON_YOUR_LAWN,
                 "sounds/ZombiesOnYourLawn.ogg");
#endif
}
...
void Music::PlayFromOffset(MusicFile theMusicFile, int theOffset, double theVolume) {
    ...
    if (aMusicInfo->mHStream) {
        ...
        BASS_ChannelFlags(aMusicInfo->mHMusic,
                          BASS_MUSIC_POSRESET | BASS_MUSIC_RAMP | BASS_MUSIC_LOOP, -1);
...
void Music::UpdateBurstTracker() {
    ...
    case MusicBurstState::MUSIC_BURST_OFF:
        if (mApp->mBoard->CountZombiesOnScreen() >= 10 ||
            mBurstOverride == MusicBurstType::MUSIC_BURST_ADDON)
            StartBurst();
        break;
    case MusicBurstState::MUSIC_BURST_STARTING:
        if (aBurstScheme == MusicBurstType::MUSIC_BURST_ADDON) {
            aFadeTrackVolume = TodAnimateCurveFloat(400, 0, mBurstStateCounter, 0.0f, 1.0f,
            ...
            if (mMusicDrumsState == MusicDrumsState::MUSIC_DRUMS_OFF) {
                ...
                aMainTrackVolume = TodAnimateCurveFloat(400, 0, mBurstStateCounter, 1.0f, 0.0f,
                                                        TodCurves::CURVE_LINEAR);
                if (mBurstStateCounter == 0) {
                    mMusicBurstState = MusicBurstState::MUSIC_BURST_ON;
                    mBurstStateCounter = 800;
    ...
    case MusicBurstState::MUSIC_BURST_ON:
        ...
        if (mBurstStateCounter == 0 && ((mApp->mBoard->CountZombiesOnScreen() < 4 &&
                                         mBurstOverride == MusicBurstType::MUSIC_BURST_INVALID) ||
                                        mBurstOverride == MusicBurstType::MUSIC_BURST_REPLACE)) {
            if (aBurstScheme == MusicBurstType::MUSIC_BURST_ADDON) {
    ...
    case MusicBurstState::MUSIC_BURST_FINISHING:
        if (aBurstScheme == MusicBurstType::MUSIC_BURST_ADDON)
            aFadeTrackVolume = TodAnimateCurveFloat(800, 0, mBurstStateCounter, 1.0f, 0.0f,
                                                    TodCurves::CURVE_LINEAR);
        else
            aMainTrackVolume = TodAnimateCurveFloat(400, 0, mBurstStateCounter, 0.0f, 1.0f,
                                                    TodCurves::CURVE_LINEAR);
    ...
    case MusicDrumsState::MUSIC_DRUMS_FADING:
        if (aBurstScheme == MusicBurstType::MUSIC_BURST_REPLACE)
            aDrumsVolume = TodAnimateCurveFloat(800, 0, mDrumsStateCounter, 1.0f, 0.0f,
                                                TodCurves::CURVE_LINEAR);
        else
            aDrumsVolume = TodAnimateCurveFloat(50, 0, mDrumsStateCounter, 1.0f, 0.0f,
                                                TodCurves::CURVE_LINEAR);
        if (mDrumsStateCounter == 0)
            mMusicDrumsState = MusicDrumsState::MUSIC_DRUMS_OFF;
        break;
...
void Music::UpdateBurstAudio() {
    ...
    if (aBurstScheme == MusicBurstType::MUSIC_BURST_ADDON) {
        ...
        mMusicInterface->SetSongVolume(mCurMusicFileMain, aMainTrackVolume);
        mMusicInterface->SetSongVolume(mCurMusicFileDrums, aDrumsVolume);
        BASS_ChannelSetPosition(GetBassMusicHandle(mCurMusicFileDrums), mQueuedDrumTrackPackedOrder,
                                BASS_POS_BYTE);
    }
}
...
void Music::MusicUpdate() {
    if (mFadeOutCounter > 0) {
        ...
        if (mFadeOutCounter == 0)
            ...
            float aFadeLevel = TodAnimateCurveFloat(mFadeOutDuration, 0, mFadeOutCounter, 1.0f,
                                                    0.0f, TodCurves::CURVE_LINEAR);
...
void Music::StartGameMusic() {
    ...
    if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_ZEN_GARDEN ||
        ...
        MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_MINIGAME_LOONBOON);
              ...
              (mApp->mPlayerInfo->GetLevel() == 10 || mApp->mPlayerInfo->GetLevel() == 20 ||
