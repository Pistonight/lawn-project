...
void ZombiePickerInit(ZombiePicker* theZombiePicker) {
    ...
    memset(theZombiePicker->mAllWavesZombieTypeCount, 0,
           sizeof(theZombiePicker->mAllWavesZombieTypeCount));
}

void Board::PutZombieInWave(ZombieType theZombieType, int theWaveNumber,
                            ZombiePicker* theZombiePicker) {
    TOD_ASSERT(theWaveNumber < MAX_ZOMBIE_WAVES &&
               theZombiePicker->mZombieCount < MAX_ZOMBIES_IN_WAVE);
...
void Board::PutInMissingZombies(int theWaveNumber, ZombiePicker* theZombiePicker) {
    for (ZombieType aZombieType = ZombieType::ZOMBIE_NORMAL;
         aZombieType < ZombieType::NUM_ZOMBIE_TYPES;
         aZombieType = (ZombieType)((int)aZombieType + 1)) {
        if (theZombiePicker->mZombieTypeCount[(int)aZombieType] <= 0 &&
            aZombieType != ZombieType::ZOMBIE_YETI && CanZombieSpawnOnLevel(aZombieType, mLevel)) {
...
void Board::PickZombieWaves() {
    if (mApp->IsAdventureMode()) {
        ...
        if (mApp->IsSurvivalMode() || aGameMode == GameMode::GAMEMODE_CHALLENGE_LAST_STAND)
        ...
        else if (aGameMode == GameMode::GAMEMODE_CHALLENGE_ZEN_GARDEN ||
                 aGameMode == GameMode::GAMEMODE_TREE_OF_WISDOM || mApp->IsSquirrelLevel())
            mNumWaves = 0;
            ...
            mNumWaves = 12;
                 ...
                 aGameMode == GameMode::GAMEMODE_CHALLENGE_GRAVE_DANGER ||
                 aGameMode == GameMode::GAMEMODE_CHALLENGE_HIGH_GRAVITY ||
            ...
            mNumWaves = 20;
        else if (mApp->IsStormyNightLevel() || mApp->IsLittleTroubleLevel() ||
                 mApp->IsBungeeBlitzLevel() || aGameMode == GameMode::GAMEMODE_CHALLENGE_COLUMN ||
                 mApp->IsShovelLevel() ||
                 aGameMode == GameMode::GAMEMODE_CHALLENGE_WAR_AND_PEAS_2 ||
                 aGameMode == GameMode::GAMEMODE_CHALLENGE_WALLNUT_BOWLING_2 ||
    ...
    for (int aWave = 0; aWave < mNumWaves; aWave++) {
        ...
        if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_LAST_STAND) {
            aZombiePoints =
                (mChallenge->mSurvivalStage * GetNumWavesPerSurvivalStage() + aWave + 10) * 2 / 5 +
                1;
        } else if (mApp->IsSurvivalMode() && mChallenge->mSurvivalStage > 0) {
            aZombiePoints =
                (mChallenge->mSurvivalStage * GetNumWavesPerSurvivalStage() + aWave) * 2 / 5 + 1;
        ...
        if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_COLUMN) {
            ...
            aZombiePoints *= 3;
        } else if (mApp->IsShovelLevel() || mApp->IsBungeeBlitzLevel() ||
                   mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_PORTAL_COMBAT ||
                   mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_INVISIGHOUL) {
            aZombiePoints *= 2;
        }

        if (aIntroZombieType != ZombieType::ZOMBIE_INVALID &&
            aIntroZombieType != ZombieType::ZOMBIE_DUCKY_TUBE) {
            bool aSpawnIntro = false;
            if ((aIntroZombieType == ZombieType::ZOMBIE_DIGGER ||
                 aIntroZombieType == ZombieType::ZOMBIE_BALLOON)) {
...
void Board::LoadBackgroundImages() {
    ...
    case BackgroundType::BACKGROUND_1_DAY:
        ...
        if ((mApp->IsAdventureMode() && mLevel <= 4) ||
...
void Board::InitZombieWavesForLevel(int theForLevel) {
    if (mApp->IsWhackAZombieLevel() ||
        (mApp->IsWallnutBowlingLevel() && !mApp->IsFirstTimeAdventureMode())) {
        mChallenge->InitZombieWaves();
        return;
    }

    for (int aZombieType = ZombieType::ZOMBIE_NORMAL; aZombieType < ZombieType::NUM_ZOMBIE_TYPES;
         aZombieType++) {
...
void Board::FreezeEffectsForCutscene(bool theFreeze) {
    ...
    while (IterateParticles(aParticle)) {
        if (aParticle->mEffectType == ParticleEffect::PARTICLE_GRAVE_BUSTER) {
            if (aParticle->mEffectType == ParticleEffect::PARTICLE_POOL_SPARKLY &&
                mIceTrapCounter == 0) {
...
Rect Board::GetShovelButtonRect() {
    Rect aRect(GetSeedBankExtraWidth() + 456, 0, Sexy::IMAGE_SHOVELBANK->GetWidth(),
               Sexy::IMAGE_SEEDBANK->GetHeight());
...
void Board::GetZenButtonRect(GameObjectType theObjectType, Rect& theRect) {
    ...
    theRect.mX = 30;
    bool usable = true;
    for (int anObject = GameObjectType::OBJECT_TYPE_WATERING_CAN;
         anObject <= GameObjectType::OBJECT_TYPE_NEXT_GARDEN; anObject++) {
    ...
    for (int anObject = GameObjectType::OBJECT_TYPE_WATERING_CAN; anObject < theObjectType;
         anObject++) {
        if (CanUseGameObject((GameObjectType)anObject)) {
            theRect.mX += 70;
        }
...
void Board::InitLevel() {
    ...
    GameMode aGameMode = mApp->mGameMode;
    if (aGameMode != GameMode::GAMEMODE_TREE_OF_WISDOM &&
        aGameMode != GameMode::GAMEMODE_CHALLENGE_ZEN_GARDEN) {
...
Reanimation* Board::CreateRakeReanim(float theRakeX, float theRakeY, int theRenderOrder) {
    Reanimation* aReanim =
        mApp->AddReanimation(GridToPixelX(theRakeX, theRakeY) + 20,
                             GridToPixelY(theRakeX, theRakeY), theRenderOrder, REANIM_RAKE);
    aReanim->mAnimRate = 0;
...
void Board::PlaceRake() {
    ...
    if (mApp->IsScaryPotterLevel()) {
        ...
        if (!StageHasZombieWalkInFromRight() ||
...
void Board::InitLawnMowers() {
    ...
    if (aGameMode == GameMode::GAMEMODE_CHALLENGE_BEGHOULED ||
        ...
        aGameMode == GameMode::GAMEMODE_CHALLENGE_ZEN_GARDEN ||
        aGameMode == GameMode::GAMEMODE_TREE_OF_WISDOM ||
        aGameMode == GameMode::GAMEMODE_CHALLENGE_LAST_STAND ||
        aGameMode == GameMode::GAMEMODE_CHALLENGE_ZOMBIQUARIUM || mApp->IsSquirrelLevel() ||
        mApp->IsIZombieLevel() ||
...
void Board::StartLevel() {
    ...
    if (mApp->IsSurvivalMode() && mChallenge->mSurvivalStage > 0) {
        mApp->EraseFile(GetSavedGameName(mApp->mGameMode, mApp->mPlayerInfo->mId) + ".data");
        mApp->EraseFile(GetSavedGameName(mApp->mGameMode, mApp->mPlayerInfo->mId) + ".schema");
...
void Board::UpdateLevelEndSequence() {
    if (mNextSurvivalStageCounter > 0) {
        if (!IsScaryPotterDaveTalking()) {
            ...
            if (mApp->IsAdventureMode() && mApp->IsScaryPotterLevel() &&
                mNextSurvivalStageCounter == 300) {
                mApp->CrazyDaveEnter();
    ...
    if (mBoardFadeOutCounter == 300) {
        if (!IsSurvivalStageWithRepick() &&
            !(mLevel == 9 || mLevel == 19 || mLevel == 29 || mLevel == 39 || mLevel == 49)) {
    ...
    if (CanDropLoot() && !IsSurvivalStageWithRepick()) {
        ...
        if (aLawnMower) {
            ...
            SoundInstance* aSoundInstance =
                mApp->mSoundManager->GetSoundInstance(Sexy::SOUND_POINTS);
            if (aSoundInstance) {
...
void Board::FadeOutLevel() {
    ...
    } else if (IsSurvivalStageWithRepick() || IsLastStandStageWithRepick() ||
               mApp->IsEndlessIZombie(mApp->mGameMode)) {
        aNeedSoundEffect = false;
    }
    if (aNeedSoundEffect) {
        mApp->mMusic->StopAllMusic();
    ...
    if (mApp->IsScaryPotterLevel() && !IsFinalScaryPotterStage()) {
        ...
        if (mApp->IsAdventureMode()) {
            ...
            SexyString aStreakStr = mApp->IsEndlessScaryPotter(mApp->mGameMode)
                                        ? "[ADVICE_MORE_SCARY_POTS]"
                                        : "[ADVICE_3_IN_A_ROW]";
            SexyString aMessage =
                TodReplaceNumberString(aStreakStr, "{STREAK}", mChallenge->mSurvivalStage + 1);
            PuzzleSaveStreak();
            ClearAdvice(AdviceType::ADVICE_NONE);
            DisplayAdvice(aMessage, MessageStyle::MESSAGE_STYLE_BIG_MIDDLE,
    ...
    if (mApp->IsEndlessIZombie(mApp->mGameMode)) {
        mNextSurvivalStageCounter = 500;
        SexyString aMessage = TodReplaceNumberString("[ADVICE_MORE_IZOMBIE]", "{STREAK}",
                                                     mChallenge->mSurvivalStage + 1);
        PuzzleSaveStreak();
        ClearAdvice(AdviceType::ADVICE_NONE);
    ...
    if (!IsSurvivalStageWithRepick()) {
        ...
        mNextSurvivalStageCounter = 500;
        DisplayAdvice("[ADVICE_MORE_ZOMBIES]", MessageStyle::MESSAGE_STYLE_BIG_MIDDLE,
                      AdviceType::ADVICE_NONE);
...
void Board::DisplayAdvice(const SexyString& theAdvice, MessageStyle theMessageStyle,
...
void Board::DisplayAdviceAgain(const SexyString& theAdvice, MessageStyle theMessageStyle,
                               AdviceType theHelpIndex) {
    if (theHelpIndex != AdviceType::ADVICE_NONE) {
...
void Board::RefreshSeedPacketFromCursor() {
    if (mCursorObject->mCursorType == CursorType::CURSOR_TYPE_PLANT_FROM_USABLE_COIN) {
        ...
        TOD_ASSERT(mCursorObject->mSeedBankIndex >= 0 &&
                   mCursorObject->mSeedBankIndex < mSeedBank->mNumPackets);
...
void Board::DoPlantingEffects(int theGridX, int theGridY, Plant* thePlant) {
    ...
    if (IsPoolSquare(theGridX, theGridY)) {
        mApp->PlayFoley(FoleyType::FOLEY_PLANT_WATER);
        mApp->AddTodParticle(aXPos, aYPos, RenderLayer::RENDER_LAYER_TOP,
                             ParticleEffect::PARTICLE_PLANTING_POOL);
    } else {
        mApp->PlayFoley(FoleyType::FOLEY_PLANT);
        mApp->AddTodParticle(aXPos, aYPos, RenderLayer::RENDER_LAYER_TOP,
                             ParticleEffect::PARTICLE_PLANTING);
    }
}
...
Plant* Board::AddPlant(int theGridX, int theGridY, SeedType theSeedType, SeedType theImitaterType) {
    ...
    bool aIsMushroom =
        (aActualType == SeedType::SEED_PUFFSHROOM || aActualType == SeedType::SEED_SUNSHROOM ||
    ...
    int aSunPlantsCount =
        CountPlantByType(SeedType::SEED_SUNSHROOM) + CountPlantByType(SeedType::SEED_SUNFLOWER);
    ...
    while (IteratePlants(aPlant)) {
        ...
        if (Plant::IsFlying(aPlant->mSeedType)) {
            ...
            thePlantOnLawn->mFlyingPlant = aPlant;
                   ...
                   (aSeedType == SeedType::SEED_LILYPAD &&
                    mApp->mGameMode != GameMode::GAMEMODE_CHALLENGE_ZEN_GARDEN)) {
...
Plant* Board::GetTopPlantAt(int theGridX, int theGridY, PlantPriority thePriority) {
    ...
    case PlantPriority::TOPPLANT_EATING_ORDER:
        if (aPlantOnLawn.mPumpkinPlant)
            return aPlantOnLawn.mPumpkinPlant;
    ...
    case PlantPriority::TOPPLANT_ANY:
        if (aPlantOnLawn.mFlyingPlant)
            return aPlantOnLawn.mFlyingPlant;
        else if (aPlantOnLawn.mNormalPlant)
            return aPlantOnLawn.mNormalPlant;
        else if (aPlantOnLawn.mPumpkinPlant)
            return aPlantOnLawn.mPumpkinPlant;
        else
            return aPlantOnLawn.mUnderPlant;
...
int Board::CountEmptyPotsOrLilies(SeedType theSeedType) {
    ...
    while (IteratePlants(aPlant)) {
        if (aPlant->mSeedType == theSeedType &&
            !GetTopPlantAt(aPlant->mPlantCol, aPlant->mRow,
...
bool Board::IsValidCobCannonSpotHelper(int theGridX, int theGridY) {
    ...
    if (aPlantOnLawn.mNormalPlant &&
        aPlantOnLawn.mNormalPlant->mSeedType == SeedType::SEED_KERNELPULT)
        return true;
...
bool Board::IsValidCobCannonSpot(int theGridX, int theGridY) {
    if (!IsValidCobCannonSpotHelper(theGridX, theGridY) ||
        !IsValidCobCannonSpotHelper(theGridX + 1, theGridY))
        return false;
...
bool Board::HasValidCobCannonSpot() {
    Plant* aPlant = nullptr;
    while (IteratePlants(aPlant)) {
        if (aPlant->mSeedType == SeedType::SEED_KERNELPULT &&
            IsValidCobCannonSpot(aPlant->mPlantCol, aPlant->mRow)) {
            return true;
        }
    }
    return false;
...
Projectile* Board::AddProjectile(int theX, int theY, int theRenderOrder, int theRow,
...
ZombieType Board::GetIntroducedZombieType() {
    ...
    for (ZombieType aZombieType = ZombieType::ZOMBIE_NORMAL;
        ...
        if ((aZombieType != ZombieType::ZOMBIE_YETI || mApp->CanSpawnYetis()) &&
            aZombieDef.mStartingLevel == mLevel) {
            return aZombieType;
        }
    }
    return ZombieType::ZOMBIE_INVALID;
...
ZombieType Board::PickGraveRisingZombieType(int theZombiePoints) {
    ...
    aZombieWeightArray[1].mWeight =
        GetZombieDefinition(ZombieType::ZOMBIE_TRAFFIC_CONE).mPickWeight;
    if (!StageHasGraveStones()) {
    ...
    for (int i = 0; i < aCount; i++) {
        ...
        if ((mApp->IsFirstTimeAdventureMode() && mLevel < aZombieDef.mStartingLevel) ||
            (!mZombieAllowed[aZombieType] && aZombieType != ZombieType::ZOMBIE_NORMAL)) {
...
ZombieType Board::PickZombieType(int theZombiePoints, int theWaveIndex,
                                 ZombiePicker* theZombiePicker) {
    int aPickCount = 0;
    ...
    for (int aZombieType = ZombieType::ZOMBIE_NORMAL; aZombieType < ZombieType::NUM_ZOMBIE_TYPES;
        ...
        if (aZombieType == ZombieType::ZOMBIE_BUNGEE && mApp->IsSurvivalEndless(aGameMode)) {
            ...
            if (mApp->IsSurvivalEndless(aGameMode)) {
                ...
                int aAllowedWave = aFirstAllowedWave -
                                   TodAnimateCurve(18, 50, aFlags, 0, 15, TodCurves::CURVE_LINEAR);
        ...
        if (mApp->IsSurvivalMode()) {
            ...
            if (aZombieType == ZombieType::ZOMBIE_GARGANTUAR ||
                ...
                aPickWeight = TodAnimateCurve(10, 50, aFlags, aPickWeight, aPickWeight / 10,
                                              TodCurves::CURVE_LINEAR);
            } else if (aZombieType == ZombieType::ZOMBIE_TRAFFIC_CONE) {
                aPickWeight = TodAnimateCurve(10, 50, aFlags, aPickWeight, aPickWeight / 4,
                                              TodCurves::CURVE_LINEAR);
            }
        }
...
bool Board::IsZombieTypePoolOnly(ZombieType theZombieType) {
    return (theZombieType == ZombieType::ZOMBIE_SNORKEL ||
...
bool Board::RowCanHaveZombieType(int theRow, ZombieType theZombieType) {
    ...
    if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_RESODDED &&
        mPlantRow[theRow] == PlantRowType::PLANTROW_DIRT && mCurrentWave < 5) {
        return false;
    }
    if (mPlantRow[theRow] == PlantRowType::PLANTROW_POOL &&
        !Zombie::ZombieTypeCanGoInPool(theZombieType)) {
        return false;
    }
    if (mPlantRow[theRow] == PlantRowType::PLANTROW_HIGH_GROUND &&
    ...
    if (theRow == 0 && !mApp->IsSurvivalMode()) {
        if (theZombieType == ZombieType::ZOMBIE_GARGANTUAR ||
...
Zombie* Board::AddZombieInRow(ZombieType theZombieType, int theRow, int theFromWave) {
    ...
    if (theZombieType == ZombieType::ZOMBIE_BOBSLED && aZombie->IsOnBoard()) {
        for (int _i = 0; _i < 3; _i++) {
            mZombies.DataArrayAlloc()->ZombieInitialize(theRow, ZombieType::ZOMBIE_BOBSLED, false,
                                                        aZombie, theFromWave);
        }
    }
    return aZombie;
...
PlantingReason Board::CanPlantAt(int theGridX, int theGridY, SeedType theSeedType) {
    if (theGridX < 0 || theGridX >= MAX_GRID_SIZE_X || theGridY < 0 ||
        theGridY >= MAX_GRID_SIZE_Y) {
    ...
    if (theSeedType == SeedType::SEED_INSTANT_COFFEE) {
        ...
        if (!aPlantOnLawn.mNormalPlant || !aPlantOnLawn.mNormalPlant->mIsAsleep ||
            ...
            aPlantOnLawn.mNormalPlant->mOnBungeeState ==
                PlantOnBungeeState::GETTING_GRABBED_BY_BUNGEE) {
    ...
    if (aHasGrave) {
        return Plant::IsFlying(theSeedType) ? PlantingReason::PLANTING_OK
                                            : PlantingReason::PLANTING_NOT_ON_GRAVE;
    }
    ...
    if (aGridSquare == GridSquareType::GRIDSQUARE_DIRT ||
        aGridSquare == GridSquareType::GRIDSQUARE_NONE) {
        return PlantingReason::PLANTING_NOT_HERE;
    }
    ...
    if (theSeedType == SeedType::SEED_LILYPAD || theSeedType == SeedType::SEED_TANGLEKELP ||
        ...
        return (aNormalPlant || aUnderPlant) ? PlantingReason::PLANTING_NOT_HERE
                                             : PlantingReason::PLANTING_OK;
    }
    if (Plant::IsFlying(theSeedType)) {
        return aPlantOnLawn.mFlyingPlant ? PlantingReason::PLANTING_NOT_HERE
                                         : PlantingReason::PLANTING_OK;
    }
    ...
    if (aNormalPlant) {
        ...
        if ((theSeedType == SeedType::SEED_WALLNUT || theSeedType == SeedType::SEED_TALLNUT) &&
            aAidPurchased) {
    ...
    if (theSeedType == SeedType::SEED_COBCANNON && !IsValidCobCannonSpot(theGridX, theGridY)) {
        return PlantingReason::PLANTING_NEEDS_UPGRADE;
               ...
               aGridSquare != GridSquareType::GRIDSQUARE_POOL) {
        return PlantingReason::PLANTING_NOT_HERE;
    }
...
Zombie* Board::ZombieHitTest(int theMouseX, int theMouseY) {
    ...
    while (IterateZombies(aZombie)) {
        ...
        if (mApp->mGameScene == GameScenes::SCENE_LEVEL_INTRO &&
            Zombie::IsZombotany(aZombie->mZombieType))
            continue;
...
bool Board::IsPlantInGoldWateringCanRange(int theMouseX, int theMouseY, Plant* thePlant) {
    ...
    if (GetTopPlantAt(thePlant->mPlantCol, thePlant->mRow, PlantPriority::TOPPLANT_ZEN_TOOL_ORDER)
    == thePlant)
    {
    ...
    */
    ...
    if (GetTopPlantAt(thePlant->mPlantCol, thePlant->mRow,
                      PlantPriority::TOPPLANT_ZEN_TOOL_ORDER) == thePlant) {
        return thePlant->mX + 40 >= aMinX && thePlant->mX + 40 < aMaxX &&
               thePlant->mY + 40 >= aMinY && thePlant->mY + 40 < aMaxY;
    }
    return false;
}
...
void Board::HighlightPlantsForMouse(int theMouseX, int theMouseY) {
    if (mCursorObject->mCursorType == CursorType::CURSOR_TYPE_WATERING_CAN &&
        mApp->mPlayerInfo->mPurchases[(int)StoreItem::STORE_ITEM_GOLD_WATERINGCAN]) {
        Plant* aPlant = nullptr;
        while (IteratePlants(aPlant)) {
            if (IsPlantInGoldWateringCanRange(theMouseX, theMouseY, aPlant)) {
                aPlant->mHighlighted = true;
                Plant* aFlowerPot = GetTopPlantAt(aPlant->mPlantCol, aPlant->mRow,
                                                  PlantPriority::TOPPLANT_ONLY_UNDER_PLANT);
                if (aFlowerPot) {
...
void Board::UpdateMousePosition() {
    ...
    if (aCursorSeedType == SeedType::SEED_INSTANT_COFFEE) {
        int aGridX = PlantingPixelToGridX(mApp->mWidgetManager->mLastMouseX,
                                          mApp->mWidgetManager->mLastMouseY, aCursorSeedType);
        int aGridY = PlantingPixelToGridY(mApp->mWidgetManager->mLastMouseX,
                                          mApp->mWidgetManager->mLastMouseY, aCursorSeedType);

        Plant* aPlant = GetTopPlantAt(aGridX, aGridY, PlantPriority::TOPPLANT_ONLY_NORMAL_POSITION);
        if (aPlant && aPlant->mIsAsleep &&
            CanPlantAt(aGridX, aGridY, SeedType::SEED_INSTANT_COFFEE) ==
                PlantingReason::PLANTING_OK) {
            aPlant->mHighlighted = true;
        }
    } else if (aCursorSeedType == SeedType::SEED_WALLNUT ||
               aCursorSeedType == SeedType::SEED_TALLNUT) {
        int aGridX = PlantingPixelToGridX(mApp->mWidgetManager->mLastMouseX,
                                          mApp->mWidgetManager->mLastMouseY, aCursorSeedType);
        int aGridY = PlantingPixelToGridY(mApp->mWidgetManager->mLastMouseX,
                                          mApp->mWidgetManager->mLastMouseY, aCursorSeedType);
        ...
        if (aPlant && aPlant->mSeedType == SeedType::SEED_PUMPKINSHELL &&
            CanPlantAt(aGridX, aGridY, SeedType::SEED_PUMPKINSHELL) ==
                PlantingReason::PLANTING_OK) {
            aPlant->mHighlighted = true;
        }
    }
...
void Board::UpdateToolTip() {
    if (!mApp->mWidgetManager->mMouseIn || !mApp->mActive || mTimeStopCounter > 0 ||
    ...
    if (mApp->mGameScene == GameScenes::SCENE_LEVEL_INTRO) {
        ...
        if (mSeedBank->ContainsPoint(mApp->mWidgetManager->mLastMouseX,
        ...
        SexyString aZombieName =
            StrFormat("[%s]", GetZombieDefinition(aZombie->mZombieType).mZombieName);
    ...
    if (aSeedPacket->mPacketType == SeedType::SEED_IMITATER &&
    ...
    if (gLawnApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_BEGHOULED ||
        ...
        mToolTip->SetLabel(
            StrFormat("%s", Plant::GetNameString(aSeedPacket->mPacketType, aUseSeedType).c_str()));
    }
    ...
    mToolTip->mX = (SEED_PACKET_WIDTH - mToolTip->mWidth) / 2 + mSeedBank->mX +
                   aSeedPacket->mOffsetX + aSeedPacket->mX;
...
void Board::MouseDownCobcannonFire(int x, int y, int theClickCount) {
    if (theClickCount >= 0 && y >= 80) {
        if (mCobCannonCursorDelayCounter > 0 &&
            Distance2D(x, y, mCobCannonMouseX, mCobCannonMouseY) < 100.0f) {
            return;
        }
...
void Board::MouseDownWithPlant(int x, int y, int theClickCount) {
    ...
    if (aReason != PlantingReason::PLANTING_OK) {
        ...
        if (aReason == PlantingReason::PLANTING_ONLY_ON_GRAVES) {
            ...
            SexyString aSeedName = Plant::GetNameString(
                mChallenge->GetArtChallengeSeed(aGridX, aGridY), SeedType::SEED_NONE);
            SexyString aMessage = TodReplaceString("[ADVICE_WRONG_ART_TYPE]", "{SEED}", aSeedName);
            DisplayAdvice(aMessage, MessageStyle::MESSAGE_STYLE_HINT_FAST,
                          AdviceType::ADVICE_PLANT_WRONG_ART_TYPE);
    ...
    if ((aPlantingSeedType == SeedType::SEED_WALLNUT ||
         aPlantingSeedType == SeedType::SEED_TALLNUT) &&
        aNormalPlant) {
    ...
    if (aPlantingSeedType == SeedType::SEED_COBCANNON) {
        Plant* aRightPlant =
            GetTopPlantAt(aGridX + 1, aGridY, PlantPriority::TOPPLANT_ONLY_NORMAL_POSITION);
        if (aRightPlant) {
            aRightPlant->Die();
        }
    }
    ...
    if (mCursorObject->mCursorType == CursorType::CURSOR_TYPE_PLANT_FROM_GLOVE) {
        mApp->mZenGarden->MovePlant(mPlants.DataArrayTryToGet(mCursorObject->mGlovePlantID), aGridX,
                                    aGridY);
    ...
    if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_COLUMN) {
        for (int aRow = 0; aRow < MAX_GRID_SIZE_Y; aRow++) {
            if (aRow == aGridY ||
                CanPlantAt(aGridX, aRow, aPlantingSeedType) != PlantingReason::PLANTING_OK)
                continue;

            if (aPlantingSeedType == SeedType::SEED_WALLNUT ||
                aPlantingSeedType == SeedType::SEED_TALLNUT) {
                aNormalPlant =
                    GetTopPlantAt(aGridX, aRow, PlantPriority::TOPPLANT_ONLY_NORMAL_POSITION);
    ...
    if (mTutorialState == TutorialState::TUTORIAL_LEVEL_1_PLANT_PEASHOOTER) {
        SetTutorialState(mPlants.mSize >= 2 ? TutorialState::TUTORIAL_LEVEL_1_COMPLETED
...
void Board::TutorialArrowShow(int theX, int theY) {
    ...
    TodParticleSystem* aParticle =
        mApp->AddTodParticle(theX, theY, MakeRenderOrder(RenderLayer::RENDER_LAYER_TOP, 0, 0),
                             ParticleEffect::PARTICLE_SEED_PACKET_PICK);
    ...
    if (aPlant == nullptr) {
        ...
        if (mTutorialState == TutorialState::TUTORIAL_SHOVEL_DIG ||
            ...
            SetTutorialState(CountPlantByType(SeedType::SEED_PEASHOOTER) == 0
                                 ? TutorialState::TUTORIAL_SHOVEL_COMPLETED
...
Plant* Board::SpecialPlantHitTest(int x, int y) {
    ...
    while (IteratePlants(aPlant)) {
        if (aPlant->mSeedType == SeedType::SEED_PUMPKINSHELL) {
            float aMinDist = GetTopPlantAt(aPlant->mPlantCol, aPlant->mRow,
                                           PlantPriority::TOPPLANT_ONLY_NORMAL_POSITION)
                                 ? 25
                                 : 0;
...
bool Board::MouseHitTestPlant(int x, int y, HitResult* theHitResult) {
    ...
    if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_ZEN_GARDEN) {
        ...
        if (mCursorObject->mCursorType == CursorType::CURSOR_TYPE_WATERING_CAN &&
            ...
            Plant* aTopPlant =
                GetTopPlantAt(PixelToGridX(x - 30, y - 20), PixelToGridY(x - 30, y - 20),
...
bool Board::MouseHitTest(int x, int y, HitResult* theHitResult) {
    ...
    if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_ZEN_GARDEN) {
        ...
        if (mCursorObject->mCursorType == CursorType::CURSOR_TYPE_CHOCOLATE &&
            ...
            canClick = true;
                   ...
                   mApp->mZenGarden->IsStinkySleeping()) {
            canClick = true;
        }
    ...
    if ((mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_ZEN_GARDEN ||
        ...
        for (int i = (int)GameObjectType::OBJECT_TYPE_WATERING_CAN;
             i <= (int)GameObjectType::OBJECT_TYPE_NEXT_GARDEN; i++) {
            GameObjectType aTool = (GameObjectType)i;
            if (CanUseGameObject(aTool) && (aTool != GameObjectType::OBJECT_TYPE_TREE_FOOD ||
                                            mChallenge->TreeOfWisdomCanFeed())) {
                Rect aButtonRect = GetShovelButtonRect();
    ...
    if (mApp->IsScaryPotterLevel() &&
        ...
        GridItem* aScaryPot =
            GetGridItemAt(GridItemType::GRIDITEM_SCARY_POT, PixelToGridX(x, y), PixelToGridY(x, y));
        if (aScaryPot) {
...
void Board::PickUpTool(GameObjectType theObjectType) {
    if (mPaused ||
        (mApp->mGameScene != GameScenes::SCENE_PLAYING && !mCutScene->IsInShovelTutorial()))
        return;
    ...
    case GameObjectType::OBJECT_TYPE_WATERING_CAN:
        if (mTutorialState == TutorialState::TUTORIAL_ZEN_GARDEN_PICKUP_WATER) {
            ...
            DisplayAdvice("[ADVICE_ZEN_GARDEN_WATER_PLANT]",
                          MessageStyle::MESSAGE_STYLE_ZEN_GARDEN_LONG, AdviceType::ADVICE_NONE);
            TutorialArrowRemove();
        }
        mCursorObject->mCursorType = CursorType::CURSOR_TYPE_WATERING_CAN;
        mApp->PlayFoley(FoleyType::FOLEY_DROP);
    ...
    case GameObjectType::OBJECT_TYPE_CHOCOLATE:
        if (mApp->mPlayerInfo->mPurchases[(int)StoreItem::STORE_ITEM_CHOCOLATE] >
            PURCHASE_COUNT_OFFSET) {
            mCursorObject->mCursorType = CursorType::CURSOR_TYPE_CHOCOLATE;
        } else {
            mApp->PlaySample(Sexy::SOUND_BUZZER);
    ...
    case GameObjectType::OBJECT_TYPE_TREE_FOOD:
        if (mChallenge->TreeOfWisdomCanFeed()) {
            if (mApp->mPlayerInfo->mPurchases[(int)StoreItem::STORE_ITEM_TREE_FOOD] >
                PURCHASE_COUNT_OFFSET) {
...
void Board::MouseDown(int x, int y, int theClickCount) {
    ...
    if (mApp->UsingGamepad() && mApp->mGamepads[0] != nullptr &&
        ...
        // These are derived from mCursorObject which is updated in CursorObject::Update to snap to
        // grid.
    ...
    if (aCursor == CursorType::CURSOR_TYPE_SHOVEL ||
        ...
        aCursor == CursorType::CURSOR_TYPE_CHOCOLATE || aCursor == CursorType::CURSOR_TYPE_GLOVE ||
...
bool Board::CanInteractWithBoardButtons() {
    ...
    return mApp->mGameMode == GameMode::GAMEMODE_UPSELL ||
...
void Board::MouseUp(int x, int y, int theClickCount) {
    ...
    if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_BEGHOULED && mChallenge->MouseUp(x, y) &&
        theClickCount > 0)
        return;
    ...
    if (CanInteractWithBoardButtons() && theClickCount > 0) {
        if (mMenuButton->IsMouseOver() && !mApp->GetDialog(Dialogs::DIALOG_GAME_OVER) &&
            ...
            } else if (mApp->mGameMode != GAMEMODE_CHALLENGE_ZEN_GARDEN &&
...
void Board::SpawnZombiesFromPool() {
    ...
    if (mLevel == 21 || mLevel == 22 || mLevel == 31 || mLevel == 32) {
        aCount = 2;
        aZombiePoints = 3;
    } else if (mLevel == 23 || mLevel == 24 || mLevel == 25 || mLevel == 33 || mLevel == 34 ||
               mLevel == 35) {
        aCount = 3;
        aZombiePoints = 5;
    } else {
...
void Board::BungeeDropZombie(BungeeDropGrid* theBungeeDropGrid, ZombieType theZombieType) {
    TodWeightedGridArray* aGrid = TodPickFromWeightedGridArray(theBungeeDropGrid->mGridArray,
                                                               theBungeeDropGrid->mGridArrayCount);
    aGrid->mWeight = 1;
...
int Board::TotalZombiesHealthInWave(int theWaveIndex) {
    ...
    Zombie* aZombie = nullptr;
    while (IterateZombies(aZombie)) {
        if (aZombie->mFromWave == theWaveIndex && !aZombie->mMindControlled &&
            !aZombie->IsDeadOrDying() && aZombie->mZombieType != ZombieType::ZOMBIE_BUNGEE &&
            aZombie->mRelatedZombieID == ZombieID::ZOMBIEID_NULL) {
            aTotalHealth += aZombie->mBodyHealth + aZombie->mHelmHealth +
                            aZombie->mShieldHealth * 0.2f + aZombie->mFlyingHealth;
        }
    }
    return aTotalHealth;
}
...
void Board::SpawnZombieWave() {
    ...
    if (mApp->IsBungeeBlitzLevel()) {
        ...
        TOD_ASSERT(mCurrentWave >= 0 && mCurrentWave < MAX_ZOMBIE_WAVES &&
...
int Board::GetSurvivalFlagsCompleted() {
    int aWavesPerFlag = GetNumWavesPerFlag();
    int aFlagsCompleted =
        mChallenge->mSurvivalStage * GetNumWavesPerSurvivalStage() / aWavesPerFlag;
    int aCurrentWave = mCurrentWave;
...
void Board::ZombiesWon(Zombie* theZombie) {
    ...
    if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_ZOMBIQUARIUM) {
        ...
        SexyString aFlagStr =
            mApp->Pluralize(GetSurvivalFlagsCompleted(), "[ONE_FLAG]", "[COUNT_FLAGS]");
        ...
        aGameOverMsg = TodReplaceNumberString("[ENDLESS_PUZZLE_DEATH_MESSAGE]", "{STREAK}",
...
bool Board::IsFinalSurvivalStage() {
    ...
    int aFlags =
        GetNumWavesPerSurvivalStage() * (mChallenge->mSurvivalStage + 1) / GetNumWavesPerFlag();
    if (mApp->IsSurvivalNormal(mApp->mGameMode)) {
        return aFlags >= 5;
    }
...
void Board::UpdateSunSpawning() {
    if (StageIsNight() || HasLevelAwardDropped() ||
    ...
    mNumSunsFallen++;
    mSunCountDown = std::min(SUN_COUNTDOWN_MAX, SUN_COUNTDOWN + mNumSunsFallen * 10) +
                    Rand(SUN_COUNTDOWN_RANGE);
    CoinType aSunType = mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_SUNNY_DAY
...
void Board::NextWaveComing() {
    if (mCurrentWave + 1 == mNumWaves) {
        bool anIsLastStandFinal =
            mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_LAST_STAND && IsLastStandFinalStage();

        if ((!IsSurvivalStageWithRepick() || anIsLastStandFinal) &&
    ...
    } else if ((mApp->IsWhackAZombieLevel() && mCurrentWave == mNumWaves - 1) ||
               IsFlagWave(mCurrentWave)) {
...
void Board::UpdateZombieSpawning() {
    ...
    if (mZombieCountDown > 200 && mZombieCountDownStart - mZombieCountDown > 400 &&
        ...
        if (mIceTimer[aRow]) {
            ...
            if (mIceTimer[aRow] == 0) {
                ...
                if (aParticleIce) {
                    ...
                    int aRenderPosition =
                        MakeRenderOrder(RenderLayer::RENDER_LAYER_GROUND, aRow, 3);
                    aParticleIce = mApp->AddTodParticle(aPosX, aPosY, aRenderPosition,
                                                        ParticleEffect::PARTICLE_ICE_SPARKLE);
...
void Board::UpdateProgressMeter() {
    if (mApp->IsFinalBossLevel()) {
        ...
        if (aBoss && !aBoss->IsDeadOrDying()) {
            mProgressMeterWidth =
                150 * (aBoss->mBodyMaxHealth - aBoss->mBodyHealth) / aBoss->mBodyMaxHealth;
        } else {
        ...
        if (mZombieHealthToNextWave != -1) {
            ...
            float aHealthFraction =
                (aDamageTarget - aHealthCurrent + mZombieHealthToNextWave) / (float)aDamageTarget;
            aFraction = std::max(aHealthFraction, aFraction);
        }

        int aLength =
            ClampInt(aCurrentWaveLength +
                         FloatRoundToInt((aNextWaveLength - aCurrentWaveLength) * aFraction),
                     1, 150);
        int aDelta = aLength - mProgressMeterWidth;
        if ((aDelta > aWaveLength && (mMainCounter % 5 == 0)) ||
            (aDelta > 0 && (mMainCounter % 20 == 0))) {
            mProgressMeterWidth++;
        }
    }
}
...
void Board::UpdateTutorial() {
    ...
    if (mTutorialState == TutorialState::TUTORIAL_LEVEL_1_PICK_UP_PEASHOOTER &&
        ...
        DisplayAdvice("[ADVICE_CLICK_PEASHOOTER]", MessageStyle::MESSAGE_STYLE_TUTORIAL_LEVEL1_STAY,
                      AdviceType::ADVICE_NONE);
        TutorialArrowShow(mSeedBank->mX + mSeedBank->mSeedPackets[0].mX,
                          mSeedBank->mY + mSeedBank->mSeedPackets[0].mY);
        mTutorialTimer = -1;
    ...
    if (mApp->IsFirstTimeAdventureMode() && mLevel >= 3 && mLevel != 5 && mLevel <= 7 &&
        mTutorialState == TutorialState::TUTORIAL_OFF && mCurrentWave >= 5 &&
        !gShownMoreSunTutorial && mSeedBank->mSeedPackets[1].CanPickUp() &&
...
void Board::SetTutorialState(TutorialState theTutorialState) {
    ...
    case TutorialState::TUTORIAL_LEVEL_1_REFRESH_PEASHOOTER:
        DisplayAdvice("[ADVICE_PLANTED_PEASHOOTER]",
                      MessageStyle::MESSAGE_STYLE_TUTORIAL_LEVEL1_STAY, AdviceType::ADVICE_NONE);
        mSunCountDown = 400;
        break;
    ...
    case TutorialState::TUTORIAL_MORESUN_PICK_UP_SUNFLOWER:
        TutorialArrowShow(mSeedBank->mX + mSeedBank->mSeedPackets[1].mX,
                          mSeedBank->mY + mSeedBank->mSeedPackets[1].mY);
        break;
    ...
    case TutorialState::TUTORIAL_SHOVEL_PICKUP: {
        DisplayAdvice("[ADVICE_CLICK_SHOVEL]", MessageStyle::MESSAGE_STYLE_HINT_STAY,
                      AdviceType::ADVICE_NONE);
        Rect aShovelButtonRect = GetShovelButtonRect();
    ...
    case TutorialState::TUTORIAL_SHOVEL_DIG:
        DisplayAdvice("[ADVICE_CLICK_PLANT]", MessageStyle::MESSAGE_STYLE_HINT_STAY,
                      AdviceType::ADVICE_NONE);
        TutorialArrowRemove();
        break;

    case TutorialState::TUTORIAL_SHOVEL_KEEP_DIGGING:
        DisplayAdvice("[ADVICE_KEEP_DIGGING]", MessageStyle::MESSAGE_STYLE_HINT_STAY,
                      AdviceType::ADVICE_NONE);
        break;
...
void Board::UpdateGame() {
    ...
    if (StageHasFog() && mFogBlownCountDown > 0) {
        ...
        if (mApp->mGameScene == GameScenes::SCENE_LEVEL_INTRO) {
            mFogOffset = TodAnimateCurveFloat(200, 0, mFogBlownCountDown, aMaxFogOffset, 0,
                                              TodCurves::CURVE_EASE_OUT);
        } else if (mFogBlownCountDown < 2000) {
            mFogOffset = TodAnimateCurveFloat(2000, 0, mFogBlownCountDown, aMaxFogOffset, 0,
                                              TodCurves::CURVE_EASE_OUT);
        } else if (mFogOffset < aMaxFogOffset) {
            mFogOffset = TodAnimateCurveFloat(-5, aMaxFogOffset, mFogOffset * 1.1f, 0,
                                              aMaxFogOffset, TodCurves::CURVE_LINEAR);
        }
    }
    ...
    if (mIceTrapCounter > 0) {
        mIceTrapCounter--;
        if (mIceTrapCounter == 0) {
            TodParticleSystem* aPoolSparklyParticle =
                mApp->ParticleTryToGet(mPoolSparklyParticleID);
...
void Board::Update() {
    ...
    if (mCoinBankFadeCount > 0 &&
        mApp->GetDialog(Dialogs::DIALOG_PURCHASE_PACKET_SLOT) == nullptr) {
        mCoinBankFadeCount--;
    }
    UpdateLayers();
    ...
    if (mBackground == BackgroundType::BACKGROUND_3_POOL &&
        mPoolSparklyParticleID == ParticleSystemID::PARTICLESYSTEMID_NULL && mDrawCount > 0) {
...

    {
        ...
        if (std::abs(aRawStickX) > 0.2f || std::abs(aRawStickY) > 0.2f ||
            aPad->IsButtonDown(GamepadButtons::BUTTON_SOUTH) ||
            aPad->IsButtonDown(GamepadButtons::BUTTON_EAST) ||
            aPad->IsButtonDown(GamepadButtons::BUTTON_WEST) ||
        ...
        if (mApp->mGameScene == GameScenes::SCENE_PLAYING) {
            ...
            // -------------------------------------------------------
            {
                ...
                auto GetHitboxY = [&](int aGridX, int aGridY) -> float {
                    if (StageHasRoof())
                        return (float)(aGridY * 85 + LAWN_YMIN +
                                       (aGridX < 5 ? (5 - aGridX) * 20 : 0) + 42);
                    if (StageHasPool())
        ...
        if (abs(aPad->GetLeftAxisXPosition()) > 0.1f || abs(aPad->GetLeftAxisYPosition()) > 0.1f ||
            ...
            aPad->IsButtonDown(GamepadButtons::BUTTON_DPAD_RIGHT) ||
            aPad->IsButtonDown(GamepadButtons::BUTTON_SOUTH) ||
            aPad->IsButtonDown(GamepadButtons::BUTTON_EAST) ||
            aPad->IsButtonDown(GamepadButtons::BUTTON_WEST) ||
            aPad->IsButtonDown(GamepadButtons::BUTTON_NORTH) ||
        ...
        if ((mApp->mGameScene == GameScenes::SCENE_PLAYING ||
            ...
            int aActGridX = std::clamp(PixelToGridXKeepOnBoard((int)mGamepadX, (int)mGamepadY), 0,
                                       MAX_GRID_SIZE_X - 1);
            int aActGridY = std::clamp(PixelToGridYKeepOnBoard((int)mGamepadX, (int)mGamepadY), 0,
                                       MAX_GRID_SIZE_Y - 1);
            ...
            bool aJustPressedLShoulder =
                aPad->IsButtonJustPressed(GamepadButtons::BUTTON_LEFT_SHOULDER);
            bool aJustPressedRShoulder =
                aPad->IsButtonJustPressed(GamepadButtons::BUTTON_RIGHT_SHOULDER);
            ...
            if (aJustPressedSouth) {
                ...
                if (!aIsGardenMode &&
                    ...
                    if (aSP->mPacketType != SeedType::SEED_NONE) {
                        if (aSP->CanPickUp()) {
                            ...
                            Plant* aPlantBelow = GetTopPlantAt(
                                aActGridX, aActGridY, PlantPriority::TOPPLANT_ONLY_NORMAL_POSITION);
                            if (aPlantBelow != nullptr) {
                                PlantingReason aSeedReason =
                                    CanPlantAt(aActGridX, aActGridY, aSP->mPacketType);
                            ...
                            int aSeedX =
                                aSP->mX + mSeedBank->mX + aSP->mOffsetX + SEED_PACKET_WIDTH / 2;
                ...
                if (aHoldingSeed || !aIsWhackLevel) {
                    // Extra guard: If A is pressed while holding a shovel in a normal level, do
                    // nothing
                    if (mCursorObject->mCursorType != CursorType::CURSOR_TYPE_SHOVEL) {
            ...
            if (aJustPressedWest) {
                ...
                if (mCursorObject->mCursorType == CursorType::CURSOR_TYPE_NORMAL) {
                    ...
                    if (aVY >= 80) // must be below the seed bank area
                    {
                        ...
                        if (!aTooClose) {
                            Plant* aCobcannon =
                                mPlants.DataArrayTryToGet(mCursorObject->mCobCannonPlantID);
                            if (aCobcannon) {
            ...
            if (aJustPressedLShoulder && mSeedBank->mNumPackets > 0) {
                mSeedBank->mIndexGamepad =
                    std::clamp(mSeedBank->mIndexGamepad - 1, 0, mSeedBank->mNumPackets - 1);
                mApp->PlaySample(Sexy::SOUND_TAP);
            }
...
void Board::UpdateLayers() {
    if (mWidgetManager) {
        ...
        for (DialogList::iterator anIter = mApp->mDialogList.begin();
             anIter != mApp->mDialogList.end(); ++anIter) {
...
bool Board::RowCanHaveZombies(int theRow) {
    ...
    return (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_RESODDED && theRow <= 4) ||
           mPlantRow[theRow] != PlantRowType::PLANTROW_DIRT;
}
...
void Board::DrawBackdrop(Graphics* g) {
    ...
    switch (mBackground) {
    case BackgroundType::BACKGROUND_1_DAY:
        aBgImage = Sexy::IMAGE_BACKGROUND1;
        break;
    case BackgroundType::BACKGROUND_2_NIGHT:
        aBgImage = Sexy::IMAGE_BACKGROUND2;
        break;
    case BackgroundType::BACKGROUND_3_POOL:
        aBgImage = Sexy::IMAGE_BACKGROUND3;
        break;
    case BackgroundType::BACKGROUND_4_FOG:
        aBgImage = Sexy::IMAGE_BACKGROUND4;
        break;
    case BackgroundType::BACKGROUND_5_ROOF:
    ...
    case BackgroundType::BACKGROUND_6_BOSS:
        ...
        break;
    case BackgroundType::BACKGROUND_MUSHROOM_GARDEN:
        aBgImage = Sexy::IMAGE_BACKGROUND_MUSHROOMGARDEN;
        break;
    case BackgroundType::BACKGROUND_GREENHOUSE:
        aBgImage = Sexy::IMAGE_BACKGROUND_GREENHOUSE;
        break;
    case BackgroundType::BACKGROUND_ZOMBIQUARIUM:
    ...
    if (mLevel == 1 && mApp->IsFirstTimeAdventureMode()) {
        ...
        int aWidth = TodAnimateCurve(0, 1000, mSodPosition, 0, Sexy::IMAGE_SOD1ROW->GetWidth(),
                                     TodCurves::CURVE_LINEAR);
        Rect aSrcRect(0, 0, aWidth, Sexy::IMAGE_SOD1ROW->GetHeight());
        g->DrawImage(Sexy::IMAGE_SOD1ROW, 239 - BOARD_OFFSET, 265, aSrcRect);
    } else if (((mLevel == 2 || mLevel == 3) && mApp->IsFirstTimeAdventureMode()) ||
               mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_RESODDED) {
        g->DrawImage(Sexy::IMAGE_BACKGROUND1UNSODDED, -BOARD_OFFSET, 0);
        g->DrawImage(Sexy::IMAGE_SOD1ROW, 239 - BOARD_OFFSET, 265);
        int aWidth = TodAnimateCurve(0, 1000, mSodPosition, 0, Sexy::IMAGE_SOD3ROW->GetWidth(),
                                     TodCurves::CURVE_LINEAR);
        ...
        if (aBgImage == Sexy::IMAGE_BACKGROUND_MUSHROOMGARDEN ||
            aBgImage == Sexy::IMAGE_BACKGROUND_GREENHOUSE || aBgImage == Sexy::IMAGE_AQUARIUM1) {
...
void Board::AddBossRenderItem(RenderItem* theRenderList, int& theCurRenderItem,
                              Zombie* theBossZombie) {
...
void Board::DrawGameObjects(Graphics* g) {
...

    {
        ...
        while (IteratePlants(aPlant)) {
            if (aPlant->mOnBungeeState == PlantOnBungeeState::NOT_ON_BUNGEE) {
                AddGameObjectRenderItemPlant(aRenderList, aRenderItemCount,
                                             RenderObjectType::RENDER_ITEM_PLANT, aPlant);

                if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_ZEN_GARDEN &&
                    aPlant->mPottedPlantIndex != -1) {
                ...
                if ((aPlant->mSeedType == SeedType::SEED_MAGNETSHROOM ||
                     aPlant->mSeedType == SeedType::SEED_GOLD_MAGNET) &&
                    aPlant->DrawMagnetItemsOnTop()) {
                    RenderItem& aRenderItem = aRenderList[aRenderItemCount];
                    aRenderItem.mRenderObjectType =
                        RenderObjectType::RENDER_ITEM_PLANT_MAGNET_ITEMS;
    ...
    }
    {
        Coin* aCoin = nullptr;
        while (IterateCoins(aCoin)) {
            AddGameObjectRenderItemCoin(aRenderList, aRenderItemCount,
                                        RenderObjectType::RENDER_ITEM_COIN, aCoin);
        }
    }
    {
        Zombie* aZombie = nullptr;
        while (IterateZombies(aZombie)) {
            if (aZombie->mZombieType == ZombieType::ZOMBIE_BOSS) {
                ...
                AddGameObjectRenderItemZombie(aRenderList, aRenderItemCount,
                                              RenderObjectType::RENDER_ITEM_ZOMBIE, aZombie);
                ...
                if (aZombie->mZombieType == ZombieType::ZOMBIE_BUNGEE) {
                    RenderItem& aRenderItem = aRenderList[aRenderItemCount];
                    aRenderItem.mRenderObjectType =
                        RenderObjectType::RENDER_ITEM_ZOMBIE_BUNGEE_TARGET;
                    aRenderItem.mZPos =
    ...
    }
    {
        Projectile* aProjectile = nullptr;
        while (IterateProjectiles(aProjectile)) {
            AddGameObjectRenderItemProjectile(aRenderList, aRenderItemCount,
                                              RenderObjectType::RENDER_ITEM_PROJECTILE,
                                              aProjectile);
    ...
    }
    {
        ...
        while (IterateGridItems(aGridItem)) {
            ...
            if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_ZEN_GARDEN &&
                ...
                aRenderItem.mZPos = MakeRenderOrder(RenderLayer::RENDER_LAYER_PARTICLE, 0,
                                                    aGridItem->mPosY - 30.0f);
                aRenderItem.mGridItem = aGridItem;
                aRenderItemCount++;
            }
        }
    ...
    }
    {
        ...
        } else if (mCutScene->IsAfterSeedChooser() || mCutScene->IsInShovelTutorial() ||
                   mHelpIndex == AdviceType::ADVICE_CLICK_TO_CONTINUE) {
            aZPos = MakeRenderOrder(RenderLayer::RENDER_LAYER_UI_BOTTOM, 0, 1);
        } else {
            aZPos = MakeRenderOrder(RenderLayer::RENDER_LAYER_ABOVE_UI, 0, 0);
        }
        ...
        AddUIRenderItem(aRenderList, aRenderItemCount, RenderObjectType::RENDER_ITEM_BOTTOM_UI,
                        aZPos);
    ...
    if (mApp->mGameScene == GameScenes::SCENE_ZOMBIES_WON) {
        ...
        AddUIRenderItem(aRenderList, aRenderItemCount, RenderObjectType::RENDER_ITEM_DOOR_MASK,
                        aZPos);
    }
    if (StageHasFog()) {
    ...
    AddGameObjectRenderItemCursorPreview(aRenderList, aRenderItemCount,
                                         RenderObjectType::RENDER_ITEM_CURSOR_PREVIEW,
                                         mCursorPreview);
    ...
    if (mApp->UsingGamepad() && mVisualGamepadX >= 0.0f &&
        ...
        int aGridX = PixelToGridXKeepOnBoard((int)mGamepadX, (int)mGamepadY);
        int aGridY = PixelToGridYKeepOnBoard((int)mGamepadX, (int)mGamepadY);
        // Offset 802 places the cursor above Lily Pads/Flower Pots (800) but below normal plants
        // (805)
        int aZPos = MakeRenderOrder(RenderLayer::RENDER_LAYER_PLANT, aGridY,
                                    802 - GridToPixelX(aGridX, aGridY));
    ...
    for (int i = 0; i < aRenderItemCount; i++) {
        ...
        case RenderObjectType::RENDER_ITEM_GAMEPAD_CURSOR: {
            ...
            if (!mApp->UsingGamepad() || mVisualGamepadX < 0.0f ||
                ...
                break;
            {
                // mVisualGamepadX/Y = grid-locked lerp position (matches console's grid-snapped
                // cursor). mGamepadX/Y = raw analog position (used only for roof-skew grid check).
                ...
                g->SetColorizeImages(true);
                {
                    if (StageHasRoof()) {
                        ...
                        TodScaleTransformMatrix(
                            aMatrix, aX + IMAGE_GAMEPAD_CURSOR_FRAME->mWidth / 2 - 12 + g->mTransX,
                        ...
                        // Use raw mGamepadX/Y for grid check — immediate skew response (no lerp
                        // delay).
                        int aGridX = PixelToGridXKeepOnBoard((int)mGamepadX, (int)mGamepadY);
                        if (aGridX <= 4) {
                        ...
                        TodBltMatrix(g, IMAGE_GAMEPAD_CURSOR_FRAME, aMatrix, g->mClipRect,
                                     ...
                                     Rect(0, 0, IMAGE_GAMEPAD_CURSOR_FRAME->mWidth,
                                          IMAGE_GAMEPAD_CURSOR_FRAME->mHeight));
                        ...
                        TodBltMatrix(
                            g, IMAGE_GAMEPAD_CURSOR_FRAME_SHADOW, aShadowMatrix, g->mClipRect,
                        ...
                        g->SetColor(
                            Color(255, 255, 200, 80 + (int)(40 * cosf(mMainCounter * 0.05f))));
                        g->DrawImage(IMAGE_GAMEPAD_CURSOR_FRAME_SHADOW, aX + 4, aY + 4, aW - 8,
                                     aH - 8);
...
bool Board::HasProgressMeter() {
    if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_BEGHOULED ||
        mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_BEGHOULED_TWIST ||
        mApp->IsFinalBossLevel() || mApp->IsSlotMachineLevel() || mApp->IsSquirrelLevel() ||
        mApp->IsIZombieLevel())
        return true;
...
void Board::DrawProgressMeter(Graphics* g) {
    ...
    int aClipWidth = TodAnimateCurve(0, PROGRESS_METER_COUNTER, mProgressMeterWidth, 0, 143,
                                     TodCurves::CURVE_LINEAR);
    ...
    if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_BEGHOULED ||
        ...
        for (int aFlagWave = 1; aFlagWave <= aNumFlagWaves; aFlagWave++) {
            ...
            if (aTotalWavesAtFlag < mCurrentWave) {
                ...
                aHeight =
                    TodAnimateCurve(100, 0, mFlagRaiseCounter, 0, 14, TodCurves::CURVE_LINEAR);
            }

            int aPosX = TodAnimateCurve(0, mNumWaves, aTotalWavesAtFlag, aFlagsPosEnd, 606,
                                        TodCurves::CURVE_LINEAR);
...
void Board::DrawHouseDoorTop(Graphics* g) {
    switch (mBackground) {
    case BackgroundType::BACKGROUND_1_DAY:
        g->DrawImage(Sexy::IMAGE_BACKGROUND1_GAMEOVER_MASK, -130, 202);
        break;
    case BackgroundType::BACKGROUND_2_NIGHT:
        g->DrawImage(Sexy::IMAGE_BACKGROUND2_GAMEOVER_MASK, -128, 207);
        break;
    case BackgroundType::BACKGROUND_3_POOL:
        g->DrawImage(Sexy::IMAGE_BACKGROUND3_GAMEOVER_MASK, -172, 234);
        break;
    case BackgroundType::BACKGROUND_4_FOG:
        g->DrawImage(Sexy::IMAGE_BACKGROUND4_GAMEOVER_MASK, -173, 133);
        break;
    case BackgroundType::BACKGROUND_5_ROOF:
        g->DrawImage(Sexy::IMAGE_BACKGROUND5_GAMEOVER_MASK, -220, 81);
        break;
    case BackgroundType::BACKGROUND_6_BOSS:
        g->DrawImage(Sexy::IMAGE_BACKGROUND6_GAMEOVER_MASK, -220, 81);
        break;
    default:
        break;
    }
    ...
    if (mApp->IsAdventureMode()) {
        ...
        if (mApp->IsSurvivalMode() || mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_LAST_STAND) {
            ...
            if (aFlags > 0) {
                ...
                SexyString aCompletedStr =
                    TodReplaceString("[FLAGS_COMPLETED]", "{FLAGS}", aFlagStr);
                aLevelStr = StrFormat("%s - %s", TodStringTranslate(aLevelStr).c_str(),
                                      aCompletedStr.c_str());
            }
        } else if (mApp->IsEndlessIZombie(mApp->mGameMode) ||
                   mApp->IsEndlessScaryPotter(mApp->mGameMode)) {
            ...
            if (aStreak > 0) {
                SexyString aStreakStr =
                    TodReplaceNumberString("[ENDLESS_STREAK]", "{STREAK}", aStreak);
                aLevelStr =
                    StrFormat("%s - %s", TodStringTranslate(aLevelStr).c_str(), aStreakStr.c_str());
            }
        }
    }
...
void Board::DrawZenWheelBarrowButton(Graphics* g, int theOffsetY) {
    ...
    if (aPlant && mCursorObject->mCursorType != CursorType::CURSOR_TYPE_PLANT_FROM_WHEEL_BARROW) {
        if (mChallenge->mChallengeState == ChallengeState::STATECHALLENGE_ZEN_FADING) {
            g->DrawImage(Sexy::IMAGE_ZEN_WHEELBARROW, aButtonRect.mX - 7,
                         aButtonRect.mY + theOffsetY - 3);
        } else {
            g->DrawImage(Sexy::IMAGE_ZEN_WHEELBARROW, aButtonRect.mX - 7,
                         aButtonRect.mY + theOffsetY + 4);
        }

        if (aPlant->mPlantAge == PottedPlantAge::PLANTAGE_SMALL) {
            mApp->mZenGarden->DrawPottedPlant(g, aButtonRect.mX + 23,
                                              aButtonRect.mY + theOffsetY - 8, aPlant, 0.6f, true);
        } else if (aPlant->mPlantAge == PottedPlantAge::PLANTAGE_MEDIUM) {
            mApp->mZenGarden->DrawPottedPlant(g, aButtonRect.mX + 28,
                                              aButtonRect.mY + theOffsetY + 2, aPlant, 0.5f, true);
        } else {
            mApp->mZenGarden->DrawPottedPlant(g, aButtonRect.mX + 34,
                                              aButtonRect.mY + theOffsetY + 12, aPlant, 0.4f, true);
        }
    } else {
        g->DrawImage(Sexy::IMAGE_ZEN_WHEELBARROW, aButtonRect.mX - 7,
                     aButtonRect.mY + theOffsetY - 3);
...
void Board::DrawZenButtons(Graphics* g) {
    int aOffsetY = 0;
    if (mChallenge->mChallengeState == ChallengeState::STATECHALLENGE_ZEN_FADING) {
        aOffsetY = TodAnimateCurve(50, 0, mChallenge->mChallengeStateCounter, 0, -72,
                                   TodCurves::CURVE_EASE_IN_OUT);
    }
    ...
    for (GameObjectType aTool = GameObjectType::OBJECT_TYPE_WATERING_CAN;
        ...
        if (aTool == GameObjectType::OBJECT_TYPE_NEXT_GARDEN) {
            aButtonRect.mX = 564;
            if (!mMenuButton->mBtnNoDraw) {
                g->DrawImage(Sexy::IMAGE_ZEN_NEXTGARDEN, aButtonRect.mX + 2,
                             aButtonRect.mY + aOffsetY);
            }
        } else {
            GetZenButtonRect(aTool, aButtonRect);
            g->DrawImage(Sexy::IMAGE_SHOVELBANK, aButtonRect.mX, aButtonRect.mY + aOffsetY);
            if ((int)mCursorObject->mCursorType ==
                (int)CursorType::CURSOR_TYPE_WATERING_CAN + (int)aTool - 6) {
                continue;
            }

            if (aTool == GameObjectType::OBJECT_TYPE_WATERING_CAN) {
                if (mApp->mPlayerInfo->mPurchases[(int)StoreItem::STORE_ITEM_GOLD_WATERINGCAN]) {
                    g->DrawImage(Sexy::IMAGE_WATERINGCANGOLD, aButtonRect.mX - 2,
                                 aButtonRect.mY + aOffsetY - 6);
                } else {
                    g->DrawImage(Sexy::IMAGE_WATERINGCAN, aButtonRect.mX - 2,
                                 aButtonRect.mY + aOffsetY - 6);
                }
                ...
                g->DrawImage(Sexy::IMAGE_FERTILIZER, aButtonRect.mX - 6,
                             aButtonRect.mY + aOffsetY - 7);
                g->SetColorizeImages(false);
                ...
                int aCharges = mApp->mPlayerInfo->mPurchases[(int)StoreItem::STORE_ITEM_BUG_SPRAY] -
                               PURCHASE_COUNT_OFFSET;
                if (aCharges == 0) {
                    g->SetColorizeImages(true);
                ...
                g->DrawImage(Sexy::IMAGE_PHONOGRAPH, aButtonRect.mX + 2,
                             aButtonRect.mY + aOffsetY + 2);
            } else if (aTool == GameObjectType::OBJECT_TYPE_CHOCOLATE) {
                int aCharges = mApp->mPlayerInfo->mPurchases[(int)StoreItem::STORE_ITEM_CHOCOLATE] -
                               PURCHASE_COUNT_OFFSET;
                if (aCharges == 0) {
                    g->SetColorizeImages(true);
                    g->SetColor(Color(128, 128, 128));
                }
                g->DrawImage(Sexy::IMAGE_CHOCOLATE, aButtonRect.mX + 6,
                             aButtonRect.mY + aOffsetY + 4);
                g->SetColorizeImages(false);
                ...
                if (mCursorObject->mCursorType != CursorType::CURSOR_TYPE_PLANT_FROM_GLOVE &&
                    mCursorObject->mCursorType != CursorType::CURSOR_TYPE_PLANT_FROM_WHEEL_BARROW) {
                    g->DrawImage(Sexy::IMAGE_ZEN_GARDENGLOVE, aButtonRect.mX - 6,
                                 aButtonRect.mY + aOffsetY - 4);
                }
            } else if (aTool == GameObjectType::OBJECT_TYPE_MONEY_SIGN) {
                g->DrawImage(Sexy::IMAGE_ZEN_MONEYSIGN, aButtonRect.mX - 5,
                             aButtonRect.mY + aOffsetY - 4);
                ...
                int aCharges = mApp->mPlayerInfo->mPurchases[(int)StoreItem::STORE_ITEM_TREE_FOOD] -
                               PURCHASE_COUNT_OFFSET;
                if (aCharges <= 0) {
                    g->SetColorizeImages(true);
                    g->SetColor(Color(128, 128, 128));
                ...
                g->DrawImage(Sexy::IMAGE_TREEFOOD, aButtonRect.mX - 6,
                             aButtonRect.mY + aOffsetY - 7);
                g->SetColorizeImages(false);
...
void Board::DrawDebugText(Graphics* g) {
    ...
    case DebugTextMode::DEBUG_TEXT_ZOMBIE_SPAWN: {
        ...
        aText += StrFormat("ZombieCountDown: %d/%d (%.0f%%)\n", mZombieCountDown,
                           mZombieCountDownStart, aCountDownFraction);
        ...
        if (mZombieHealthToNextWave != -1) {
            ...
            float aHealthFraction = (float)(mZombieHealthToNextWave - aTotalHealth + aHealthRange) /
                                    (float)aHealthRange;
        ...
        if (aBossZombie) {
            ...
            aText += StrFormat("Health: %d of %d\n", aBossZombie->mBodyHealth,
                               aBossZombie->mBodyMaxHealth);
        }
    ...
    case DebugTextMode::DEBUG_TEXT_MUSIC: {
        ...
        if (mApp->mMusic->mCurMusicFileMain == MusicFile::MUSIC_FILE_NONE) {
            ...
            if (mApp->mMusic->mMusicBurstState == MusicBurstState::MUSIC_BURST_OFF) {
                ...
                aText +=
                    StrFormat("On at least until %d/%d", mApp->mMusic->mBurstStateCounter, 800);
            ...
            if (mApp->mMusic->mMusicDrumsState == MusicDrumsState::MUSIC_DRUMS_OFF) {
                ...
                aText +=
                    StrFormat(", Drums fading off %d/%d", mApp->mMusic->mDrumsStateCounter, 50);
            }
            aText += StrFormat("\n");
            ...
            if (mApp->mMusic->IsTrackerMusic(mApp->mMusic->mCurMusicTune)) {
                ...
                aText += StrFormat("Music order %02d row %02d\n", LOWORD(aPackedOrderMain),
                                   HIWORD(aPackedOrderMain) / 4);
                if (mApp->mMusic->mCurMusicTune == MusicTune::MUSIC_TUNE_DAY_GRASSWALK ||
                    mApp->mMusic->mCurMusicTune == MusicTune::MUSIC_TUNE_POOL_WATERYGRAVES ||
                    mApp->mMusic->mCurMusicTune == MusicTune::MUSIC_TUNE_FOG_RIGORMORMIST ||
                    mApp->mMusic->mCurMusicTune == MusicTune::MUSIC_TUNE_ROOF_GRAZETHEROOF) {
                    int aPackedOrderHihats =
                        mApp->mMusic->GetMusicOrder(mApp->mMusic->mCurMusicFileHihats);
                    int aPackedOrderDrums =
                        mApp->mMusic->GetMusicOrder(mApp->mMusic->mCurMusicFileDrums);
                    if (aCurrentOrder == LOWORD(aPackedOrderHihats) &&
                        aCurrentOrder == LOWORD(aPackedOrderDrums)) {
                    ...
                    HMUSIC aMusicHandle1 =
                        mApp->mMusic->GetBassMusicHandle(mApp->mMusic->mCurMusicFileMain);
                    HMUSIC aMusicHandle2 =
                        mApp->mMusic->GetBassMusicHandle(mApp->mMusic->mCurMusicFileHihats);
                    HMUSIC aMusicHandle3 =
                        mApp->mMusic->GetBassMusicHandle(mApp->mMusic->mCurMusicFileDrums);
                ...
                if (mApp->mMusic->mCurMusicFileDrums != MusicFile::MUSIC_FILE_NONE) {
                    int aDrumsPosition =
                        mApp->mMusic->GetMusicPosition(mApp->mMusic->mCurMusicFileDrums);
                    aText += StrFormat("Music drums position %d\n", aDrumsPosition);
                }

                if (mApp->mMusic->mCurMusicFileHihats != MusicFile::MUSIC_FILE_NONE) {
                    int aHihatsPosition =
                        mApp->mMusic->GetMusicPosition(mApp->mMusic->mCurMusicFileHihats);
    ...
    case DebugTextMode::DEBUG_TEXT_CONTROLLER:
        ...
        for (int i = 0; i < MAX_GAMEPADS; i++) {
            ...
            aText += StrFormat(
                "rumble: %s\n",
                mApp->mGamepads[0]->IsButtonDown(GamepadButtons::BUTTON_LEFT_SHOULDER) ? "on"
                                                                                       : "off");
...
void Board::DrawTopRightUI(Graphics* g) {
    if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_ZEN_GARDEN) {
        if (mChallenge->mChallengeState == STATECHALLENGE_ZEN_FADING) {
            mMenuButton->mY = TodAnimateCurve(50, 0, mChallenge->mChallengeStateCounter, -10, -50,
                                              TodCurves::CURVE_EASE_IN_OUT);
            mStoreButton->mX = TodAnimateCurve(50, 0, mChallenge->mChallengeStateCounter, 678, 800,
                                               TodCurves::CURVE_EASE_IN_OUT);
        } else {
            mMenuButton->mY = -10;
            mStoreButton->mX = 678;
...
void Board::DrawUIBottom(Graphics* g) {
    ...
    if (mBackground == BackgroundType::BACKGROUND_GREENHOUSE ||
        mBackground == BackgroundType::BACKGROUND_ZOMBIQUARIUM) {
        g->SetDrawMode(Graphics::DRAWMODE_ADDITIVE);
        g->DrawImage(IMAGE_BACKGROUND_GREENHOUSE_OVERLAY, Rect(0, 0, BOARD_WIDTH, BOARD_HEIGHT),
                     Rect(0, 0, IMAGE_BACKGROUND_GREENHOUSE_OVERLAY->mWidth,
                          IMAGE_BACKGROUND_GREENHOUSE_OVERLAY->mHeight));
        g->SetDrawMode(Graphics::DRAWMODE_NORMAL);
    }
...
void Board::DrawUICoinBank(Graphics* g) {
    ...
    g->DrawString(aCoinLabel, aPosX + 116 - Sexy::FONT_CONTINUUMBOLD14->StringWidth(aCoinLabel),
                  aPosY + 24);
    g->SetColorizeImages(false);
}
...
bool Board::IsScaryPotterDaveTalking() {
    return mApp->IsScaryPotterLevel() && mNextSurvivalStageCounter > 0 &&
           mApp->mCrazyDaveState != CrazyDaveState::CRAZY_DAVE_OFF;
}
...
void Board::DoTypingCheck(KeyCode theKey) {
    ...
    if (mApp->mPinataCheck->Check(theKey)) {
        if (mApp->CanDoPinataMode()) {
            ...
            if (mApp->mGameScene == GameScenes::SCENE_PLAYING) {
                DisplayAdvice("[CANT_USE_CODE]", MessageStyle::MESSAGE_STYLE_BIG_MIDDLE_FAST,
                              AdviceType::ADVICE_NONE);
            }
            mApp->PlaySample(Sexy::SOUND_BUZZER);
            return;
...
void Board::KeyDown(KeyCode theKey) {
    ...
    if (mApp->mGameScene == GameScenes::SCENE_LEVEL_INTRO &&
        ...
        if (mCursorObject->mCursorType != CursorType::CURSOR_TYPE_NORMAL) {
            RefreshSeedPacketFromCursor();
        } else if (CanInteractWithBoardButtons() &&
                   mApp->mGameScene != GameScenes::SCENE_ZOMBIES_WON) {
            mApp->DoNewOptions(false);
        }
    }
...
void Board::KeyChar(SexyChar theChar) {
    ...
    if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_ZEN_GARDEN) {
        if (theChar == 'm') {
            if (!mApp->mZenGarden->IsZenGardenFull(true)) {
                ...
                aPottedPlant.mDrawVariation =
                    (DrawVariation)RandRangeInt((int)DrawVariation::VARIATION_MARIGOLD_WHITE,
                                                (int)DrawVariation::VARIATION_MARIGOLD_LIGHT_GREEN);
        ...
        if (theChar == 'f') {
            Plant* aPlant = nullptr;
            while (IteratePlants(aPlant)) {
                if (GetZenToolAt(aPlant->mPlantCol, aPlant->mRow) == nullptr &&
                    aPlant->mPottedPlantIndex >= 0) {
                    PottedPlant* aPottedPlant =
                        mApp->mZenGarden->PottedPlantFromIndex(aPlant->mPottedPlantIndex);
        ...
        if (theChar == 'r') {
            ...
            while (IteratePlants(aPlant)) {
                if (aPlant->mPottedPlantIndex >= 0) {
                    ...
                    PottedPlant* aPottedPlant =
                        &mApp->mPlayerInfo->mPottedPlant[aPlant->mPottedPlantIndex];
    ...
    if (mApp->mGameMode == GameMode::GAMEMODE_TREE_OF_WISDOM) {
        if (theChar == 'f') {
            if (mApp->mPlayerInfo->mPurchases[(int)StoreItem::STORE_ITEM_TREE_FOOD] <=
                PURCHASE_COUNT_OFFSET) {
                mApp->mPlayerInfo->mPurchases[(int)StoreItem::STORE_ITEM_TREE_FOOD] =
                    PURCHASE_COUNT_OFFSET + 1;
            }
    ...
    if (theChar == '<') {
        ...
        } else if ((mApp->IsScaryPotterLevel() && !IsFinalScaryPotterStage()) ||
                   mApp->IsEndlessIZombie(mApp->mGameMode)) {
            if (mNextSurvivalStageCounter == 0) {
                RemoveAllZombies();
                FadeOutLevel();
        ...
        if (mApp->IsSurvivalEndless(mApp->mGameMode)) {
            ...
            for (int y = 0; y < MAX_GRID_SIZE_X; y++) {
                for (int x = 0; x < MAX_GRID_SIZE_Y; x++) {
                    ...
                    if (CanPlantAt(x, y, SeedType::SEED_PUMPKINSHELL) ==
                        PlantingReason::PLANTING_OK) {
                        if (x <= 6 || IsPoolSquare(x, y)) {
                            AddPlant(x, y, SeedType::SEED_PUMPKINSHELL, SeedType::SEED_NONE);
                        }
                    }
                    if (CanPlantAt(x, y, SeedType::SEED_GATLINGPEA) ==
                        PlantingReason::PLANTING_OK) {
                        if (x < 5) {
                            ...
                            if (CanPlantAt(x, y, SeedType::SEED_INSTANT_COFFEE) ==
                                PlantingReason::PLANTING_OK) {
            ...
            for (int y = 0; y < MAX_GRID_SIZE_Y; ++y) {
                for (int x = 0; x < MAX_GRID_SIZE_X; ++x) {
                    if (StageHasRoof() &&
                        CanPlantAt(x, y, SeedType::SEED_FLOWERPOT) == PlantingReason::PLANTING_OK) {
                        AddPlant(x, y, SeedType::SEED_FLOWERPOT, SeedType::SEED_NONE);
                    }
                    if (CanPlantAt(x, y, SeedType::SEED_LILYPAD) == PlantingReason::PLANTING_OK) {
                        AddPlant(x, y, SeedType::SEED_LILYPAD, SeedType::SEED_NONE);
                    }
                    if (CanPlantAt(x, y, SeedType::SEED_THREEPEATER) ==
                        PlantingReason::PLANTING_OK) {
...
bool Board::HasConveyorBeltSeedBank() {
    return mApp->IsFinalBossLevel() || mApp->IsMiniBossLevel() || mApp->IsShovelLevel() ||
           mApp->IsWallnutBowlingLevel() || mApp->IsLittleTroubleLevel() ||
           mApp->IsStormyNightLevel() || mApp->IsBungeeBlitzLevel() ||
           mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_PORTAL_COMBAT ||
           mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_COLUMN ||
           mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_INVISIGHOUL;
}
...
bool Board::StageHasFog() {
    return !mApp->IsStormyNightLevel() &&
           mApp->mGameMode != GameMode::GAMEMODE_CHALLENGE_INVISIGHOUL &&
           mBackground == BackgroundType::BACKGROUND_4_FOG;
}
...
int Board::GetSeedPacketPositionX(int theIndex) {
    ...
    if (HasConveyorBeltSeedBank())
        return theIndex * 50 + 91;

    if (mSeedBank->mNumPackets <= 7)
        return theIndex * 59 + 85;
    else if (mSeedBank->mNumPackets == 8)
        return theIndex * 54 + 81;
    else if (mSeedBank->mNumPackets == 9)
...
int Board::PlantingPixelToGridY(int theX, int theY, SeedType theSeedType) {
    ...
    if (theSeedType == SeedType::SEED_INSTANT_COFFEE) {
        ...
        if (aGridYDown != aGridY) {
            Plant* aPlantDown =
                GetTopPlantAt(aGridX, aGridYDown, PlantPriority::TOPPLANT_ONLY_NORMAL_POSITION);
        ...
        if (aGridYUp != aGridY) {
            Plant* aPlantUp =
                GetTopPlantAt(aGridX, aGridYUp, PlantPriority::TOPPLANT_ONLY_NORMAL_POSITION);
...
unsigned int Board::SeedNotRecommendedForLevel(SeedType theSeedType) {
    ...
    if (StageHasRoof() &&
        (theSeedType == SeedType::SEED_SPIKEWEED || theSeedType == SeedType::SEED_SPIKEROCK)) {
...
void Board::DropLootPiece(int thePosX, int thePosY, int theDropFactor) {
    if (mApp->IsFirstTimeAdventureMode()) {
        if (mLevel == 22 && mCurrentWave > 5 && !mApp->mPlayerInfo->mHasUnlockedMinigames &&
            CountCoinByType(CoinType::COIN_PRESENT_MINIGAMES) == 0) {
            mApp->PlayFoley(FoleyType::FOLEY_ART_CHALLENGE);
            AddCoin(thePosX, thePosY, CoinType::COIN_PRESENT_MINIGAMES,
                    CoinMotion::COIN_MOTION_COIN);
            return;
        }
        if (mLevel == 36 && mCurrentWave > 5 && !mApp->mPlayerInfo->mHasUnlockedPuzzleMode &&
            CountCoinByType(CoinType::COIN_PRESENT_PUZZLE_MODE) == 0) {
            mApp->PlayFoley(FoleyType::FOLEY_ART_CHALLENGE);
            AddCoin(thePosX, thePosY, CoinType::COIN_PRESENT_PUZZLE_MODE,
                    CoinMotion::COIN_MOTION_COIN);
            return;
        }
    }
    ...
    if (mApp->IsFirstTimeAdventureMode() && mLevel == 11 && !mDroppedFirstCoin &&
        mCurrentWave > 5) {
        aDropHit = 1000;
    }
    if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_COLUMN) {
    ...
    if (mApp->IsWhackAZombieLevel()) {
        ...
        int aSunChanceMax = mSunMoney > 500   ? 2800
                            : mSunMoney > 350 ? 3100
                            : mSunMoney > 200 ? 3700
                                              : 5000;
        if (aDropHit >= aSunChanceMin * theDropFactor &&
            aDropHit <= aSunChanceMax * theDropFactor) {
    ...
    if (mApp->mZenGarden->CanDropChocolate()) {
        if (mApp->IsAdventureMode() && !mApp->IsFirstTimeAdventureMode()) {
            ...
            aChocolateChance =
                aPottedPlantChance + (mApp->IsSurvivalEndless(mApp->mGameMode) ? 9 : 36);
        }
    }
    ...
    if (aDropHit < aPottedPlantChance * theDropFactor) {
        ...
        aCoinType = mApp->mPlayerInfo->mPurchases[StoreItem::STORE_ITEM_PACKET_UPGRADE] < 1
                        ? CoinType::COIN_GOLD
...
bool Board::CanDropLoot() {
    return !mCutScene->ShouldRunUpsellBoard() &&
           (!mApp->IsFirstTimeAdventureMode() || mLevel >= 11);
}
...
void Board::DoFwoosh(int theRow) {
    ...
    for (int i = 0; i < 12; i++) {
        ...
        Reanimation* aFwoosh =
            mApp->AddReanimation(aPosX, aPosY, aRenderOrder, ReanimationType::REANIM_JALAPENO_FIRE);
...
void Board::UpdateFwoosh() {
    ...
    int aFwooshRemaining =
        TodAnimateCurve(50, 0, --mFwooshCountDown, 12, 0, TodCurves::CURVE_LINEAR);
    for (int aRow = 0; aRow < MAX_GRID_SIZE_Y; aRow++) {
...
void Board::UpdateGridItems() {
    GridItem* aGridItem = nullptr;
    while (IterateGridItems(aGridItem)) {
        if (mEnableGraveStones && aGridItem->mGridItemType == GridItemType::GRIDITEM_GRAVESTONE &&
            aGridItem->mGridItemCounter < 100) {
...
bool Board::PlantingRequirementsMet(SeedType theSeedType) {
    switch (theSeedType) {
    case SeedType::SEED_GATLINGPEA:
        return CountPlantByType(SeedType::SEED_REPEATER);
    case SeedType::SEED_TWINSUNFLOWER:
        return CountPlantByType(SeedType::SEED_SUNFLOWER);
    case SeedType::SEED_GLOOMSHROOM:
        return CountPlantByType(SeedType::SEED_FUMESHROOM);
    case SeedType::SEED_CATTAIL:
        return CountEmptyPotsOrLilies(SeedType::SEED_LILYPAD);
    case SeedType::SEED_WINTERMELON:
        return CountPlantByType(SeedType::SEED_MELONPULT);
    case SeedType::SEED_GOLD_MAGNET:
        return CountPlantByType(SeedType::SEED_MAGNETSHROOM);
    case SeedType::SEED_SPIKEROCK:
        return CountPlantByType(SeedType::SEED_SPIKEWEED);
    case SeedType::SEED_COBCANNON:
        return HasValidCobCannonSpot();
    default:
        return true;
    }
}

std::vector<Zombie*> Board::GetZombiesInRadius(int theRow, int theX, int theY, int theRadius,
                                               int theRowRange, int theDamageRangeFlags) {
...
void Board::KillAllZombiesInRadius(int theRow, int theX, int theY, int theRadius, int theRowRange,
                                   bool theBurn, int theDamageRangeFlags) {
    Zombie* aZombie = nullptr;
    while (IterateZombies(aZombie)) {
        if (aZombie->EffectedByDamage(theDamageRangeFlags)) {
            Rect aZombieRect = aZombie->GetZombieRect();
    ...
    while (IterateGridItems(aGridItem)) {
        if (aGridItem->mGridItemType == GridItemType::GRIDITEM_LADDER) {
            if (GridInRange(aGridItem->mGridX, aGridItem->mGridY, aGridX, aGridY, theRowRange,
                            theRowRange)) {
                aGridItem->GridItemDie();
            }
        }
...
int Board::GetNumWavesPerSurvivalStage() {
    if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_LAST_STAND ||
        mApp->IsSurvivalNormal(mApp->mGameMode)) {
        return 10;
...
bool Board::CanUseGameObject(GameObjectType theGameObject) {
    ...
    if (theGameObject == GameObjectType::OBJECT_TYPE_NEXT_GARDEN) {
        return mApp->mPlayerInfo->mPurchases[(int)StoreItem::STORE_ITEM_MUSHROOM_GARDEN] ||
...
bool Board::IsZombieTypeSpawnedOnly(ZombieType theZombieType) {
    return (theZombieType == ZombieType::ZOMBIE_BACKUP_DANCER ||
            theZombieType == ZombieType::ZOMBIE_BOBSLED || theZombieType == ZombieType::ZOMBIE_IMP);
}
