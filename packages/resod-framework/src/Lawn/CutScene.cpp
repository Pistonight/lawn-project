...
void CutScene::PlaceAZombie(ZombieType theZombieType, int theGridX, int theGridY) {
    ...
    if (aPutOnDuckyTube) {
        ...
        mApp->ReanimationGet(aZombie->mBodyReanimID)
            ->AssignRenderGroupToTrack("Zombie_duckytube", RENDER_GROUP_NORMAL);
    }
    if (mBoard->StageHasRoof()) {
        aZombie->mPosY -=
            theGridY * 2 - theGridX * 7 + 30; // 7 * (5 - theGridX) - 2 * (5 - theGridY) + 5;
        aZombie->mPosX -= 5.0f;
    }
    ...
    aZombie->mRenderOrder = Board::MakeRenderOrder(RenderLayer::RENDER_LAYER_LAWN, 0,
                                                   (theGridX % 2) * 2 + theGridY * 4);
...
bool CutScene::CanZombieGoInGridSpot(ZombieType theZombieType, int theGridX, int theGridY,
                                     bool theZombieGrid[5][5]) {
    ...
    if (Is2x2Zombie(theZombieType)) {
        ...
        if (theZombieGrid[theGridX - 1][theGridY] || theZombieGrid[theGridX][theGridY - 1] ||
            theZombieGrid[theGridX - 1][theGridY - 1]) {
            return false;
        }
    }
    ...
    if (Is2x2Zombie(theZombieType) || theZombieType == ZombieType::ZOMBIE_ZAMBONI ||
...
void CutScene::FindPlaceForStreetZombies(ZombieType theZombieType, bool theZombieGrid[5][5],
                                         int& thePosX, int& thePosY) {
...
bool CutScene::Is2x2Zombie(ZombieType theZombieType) {
    return theZombieType == ZombieType::ZOMBIE_GARGANTUAR ||
...
bool CutScene::IsSurvivalRepick() {
    return (mApp->IsSurvivalMode() && mBoard->mChallenge->mSurvivalStage > 0 &&
...
bool CutScene::IsNonScrollingCutscene() {
    return mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_ICE ||
           ...
           mApp->IsScaryPotterLevel() || mApp->IsIZombieLevel() || mApp->IsWhackAZombieLevel() ||
           mApp->IsShovelLevel() || mApp->IsSquirrelLevel() || mApp->IsWallnutBowlingLevel();
}
...
bool CutScene::CanGetPacketUpgrade() {
    ...
    return mApp->mPlayerInfo->mPurchases[StoreItem::STORE_ITEM_PACKET_UPGRADE] == 0 &&
...
bool CutScene::CanGetSecondPacketUpgrade() {
    ...
    return mApp->mPlayerInfo->mPurchases[StoreItem::STORE_ITEM_PACKET_UPGRADE] == 1 &&
           mApp->mPlayerInfo->mCoins >= aCost && mApp->mPlayerInfo->mDidntPurchasePacketUpgrade < 2;
}
...
void CutScene::StartLevelIntro() {
    ...
    if (mApp->IsFirstTimeAdventureMode() && aLevel <= 2) {
        mReadySetPlantTime = 0;
    } else if (mApp->IsShovelLevel() || mApp->IsSquirrelLevel() || mApp->IsWallnutBowlingLevel() ||
               mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_ZOMBIQUARIUM ||
               mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_LAST_STAND ||
               mApp->mGameMode == GameMode::GAMEMODE_TREE_OF_WISDOM || mApp->IsIZombieLevel() ||
               mApp->IsWhackAZombieLevel() || mApp->IsScaryPotterLevel()) {
        mReadySetPlantTime = 0;
    } else {
    ...
    if (mApp->IsFirstTimeAdventureMode() && aLevel == 11) {
    ...
    } else if (mApp->IsFirstTimeAdventureMode() && aLevel >= 13 && aLevel <= 24 && aLevel != 15 &&
               aLevel != 20 && aLevel != 21 && CanGetPacketUpgrade()) {
        mCrazyDaveDialogStart = 1501;
    } else if (mApp->IsFirstTimeAdventureMode() && aLevel >= 16 && aLevel <= 24 && aLevel != 20 &&
               aLevel != 21 && CanGetSecondPacketUpgrade()) {
        ...
        mBoard->mMenuButton->mBtnNoDraw = false;
               ...
               !mApp->HasBeatenChallenge(GameMode::GAMEMODE_SCARY_POTTER_1)) {
    ...
    if (!aHouseMessage.empty()) {
        mBoard->DisplayAdvice(aHouseMessage, MessageStyle::MESSAGE_STYLE_HOUSE_NAME,
...
void CutScene::CancelIntro() {
    ...
    if (mCutsceneTime > mCrazyDaveTime + TimePanLeftStart || !mBoard->ChooseSeedsOnCurrentLevel()) {
        mCutsceneTime = TimeIntroEnd + mLawnMowerTime + mSodTime + mGraveStoneTime +
                        mCrazyDaveTime + mFogTime + mBossTime + mReadySetPlantTime - 20;
...
int CutScene::CalcPosition(int theTimeStart, int theTimeEnd, int thePositionStart,
                           int thePositionEnd) {
    return TodAnimateCurve(theTimeStart, theTimeEnd, mCutsceneTime, thePositionStart,
                           thePositionEnd, TodCurves::CURVE_EASE_IN_OUT);
}
...
void CutScene::AnimateBoard() {
    ...
    if (mCutsceneTime > aTimePanRightStart && mCutsceneTime <= aTimePanRightEnd) {
        int aPanOffset = CalcPosition(aTimePanRightStart, aTimePanRightEnd, -aBoardOffset,
                                      BOARD_IMAGE_WIDTH_OFFSET - mApp->mWidth);
    ...
    if (mBoard->ChooseSeedsOnCurrentLevel()) {
        ...
        if (mCutsceneTime > aTimeSeedChoserSlideOnStart &&
            mCutsceneTime <= aTimeSeedChoserSlideOnEnd) {
            aSeedChoser->Move(0, CalcPosition(aTimeSeedChoserSlideOnStart,
                                              aTimeSeedChoserSlideOnEnd, SEED_CHOOSER_OFFSET_Y, 0));
        ...
        if (mCutsceneTime > aTimeSeedChoserSlideOffStart &&
            mCutsceneTime <= aTimeSeedChoserSlideOffEnd) {
            aSeedChoser->Move(0,
                              CalcPosition(aTimeSeedChoserSlideOffStart, aTimeSeedChoserSlideOffEnd,
                                           0, SEED_CHOOSER_OFFSET_Y));
            aSeedChoser->mMenuButton->mDisabled = true;
        }
    }

    if (mCutsceneTime > aTimePanLeftStart) {
        int aPanOffset = CalcPosition(aTimePanLeftStart, aTimePanLeftEnd,
                                      BOARD_IMAGE_WIDTH_OFFSET - mApp->mWidth, 0);
        mBoard->Move(-aPanOffset, 0);
    }

    int aTimePrepareEnd = 0;
    if (!mBoard->ChooseSeedsOnCurrentLevel()) {
        aTimePrepareEnd = mBossTime + mFogTime + mGraveStoneTime + mSodTime -
                          TimeSeedChoserSlideOnStart + TimePanLeftEnd;
    }
    ...
    if (!mApp->IsChallengeWithoutSeedBank() && mCutsceneTime > aTimeSeedBankOnStart &&
        ...
        int aSeedBankY =
            CalcPosition(aTimeSeedBankOnStart, aTimeSeedBankOnEnd, -IMAGE_SEEDBANK->GetHeight(), 0);
    ...
    if (mCutsceneTime > aTimeSeedBankRightStart) {
        int aSeedBankX = CalcPosition(aTimeSeedBankRightStart, aTimeSeedBankRightEnd,
                                      SEED_BANK_OFFSET_X, SEED_BANK_OFFSET_X_END);
        int aDarken = TodAnimateCurve(aTimeSeedBankRightStart, aTimeSeedBankRightEnd, mCutsceneTime,
                                      255, 128, TodCurves::CURVE_EASE_OUT);
    ...
    if (mSodTime > 0) {
        ...
        mBoard->mSodPosition = TodAnimateCurve(aTimeRollSodStart, aTimeRollSodEnd, mCutsceneTime, 0,
                                               1000, TodCurves::CURVE_LINEAR);

        if (mCutsceneTime == aTimeRollSodStart) {
            mApp->PlayFoley(FoleyType::FOLEY_DIGGER);
            if (mBoard->mLevel == 1) {
                mApp->AddReanimation(0, 0,
                                     Board::MakeRenderOrder(RenderLayer::RENDER_LAYER_TOP, 0, 0),
                                     ReanimationType::REANIM_SODROLL);
                mApp->AddTodParticle(35, 348,
                                     Board::MakeRenderOrder(RenderLayer::RENDER_LAYER_TOP, 0, 1),
                                     ParticleEffect::PARTICLE_SOD_ROLL);
            } else if (mBoard->mLevel == 2) {
                mApp->AddReanimation(0, -102,
                                     Board::MakeRenderOrder(RenderLayer::RENDER_LAYER_TOP, 0, 0),
                                     ReanimationType::REANIM_SODROLL);
                mApp->AddReanimation(0, 111,
                                     Board::MakeRenderOrder(RenderLayer::RENDER_LAYER_TOP, 0, 0),
                                     ReanimationType::REANIM_SODROLL);
                mApp->AddTodParticle(35, 246,
                                     Board::MakeRenderOrder(RenderLayer::RENDER_LAYER_TOP, 0, 1),
                                     ParticleEffect::PARTICLE_SOD_ROLL);
                mApp->AddTodParticle(35, 459,
                                     Board::MakeRenderOrder(RenderLayer::RENDER_LAYER_TOP, 0, 1),
                                     ParticleEffect::PARTICLE_SOD_ROLL);
            } else if (mBoard->mLevel == 4) {
                mApp->AddReanimation(-3, -198,
                                     Board::MakeRenderOrder(RenderLayer::RENDER_LAYER_TOP, 0, 0),
                                     ReanimationType::REANIM_SODROLL);
                mApp->AddReanimation(-3, 203,
                                     Board::MakeRenderOrder(RenderLayer::RENDER_LAYER_TOP, 0, 0),
                                     ReanimationType::REANIM_SODROLL);
                mApp->AddTodParticle(32, 150,
                                     Board::MakeRenderOrder(RenderLayer::RENDER_LAYER_TOP, 0, 1),
                                     ParticleEffect::PARTICLE_SOD_ROLL);
                mApp->AddTodParticle(32, 511,
                                     Board::MakeRenderOrder(RenderLayer::RENDER_LAYER_TOP, 0, 1),
                                     ParticleEffect::PARTICLE_SOD_ROLL);
            }
        }
    ...
    if (!IsSurvivalRepick()) {
        for (int aGridY = 0; aGridY < MAX_GRID_SIZE_Y; aGridY++) {
            int aTimeLawnMowerStart =
                TimeLawnMowerStart[aGridY] + mSodTime + mGraveStoneTime + mCrazyDaveTime;
            if (mCutsceneTime > aTimeLawnMowerStart) {
                LawnMower* aLawnMower = mBoard->FindLawnMowerInRow(aGridY);
                if (aLawnMower) {
                    aLawnMower->mVisible = true;
                    aLawnMower->mPosX = CalcPosition(
                        aTimeLawnMowerStart, aTimeLawnMowerStart + TimeLawnMowerDuration, -80, -21);
                }
            }
        }
    }
    ...
    if (mApp->IsStormyNightLevel() &&
        (mCutsceneTime == aTimePanRightEnd - 1000 || mCutsceneTime == aTimePanLeftEnd)) {
    ...
    int aTimeReadySetPlant = TimeReadySetPlantStart + mLawnMowerTime + mSodTime + mGraveStoneTime +
                             mCrazyDaveTime + mFogTime + mBossTime;
    if (mReadySetPlantTime > 0 && mCutsceneTime == aTimeReadySetPlant) {
        mApp->AddReanimation(400, 324,
...
void CutScene::ShowShovel() {
    if (mApp->IsWhackAZombieLevel() || mApp->IsWallnutBowlingLevel() ||
...
bool CutScene::IsShowingCrazyDave() {
    return mApp->mGameScene == GameScenes::SCENE_LEVEL_INTRO &&
           (mCrazyDaveTime > 0 && mCutsceneTime < TimePanRightEnd + mCrazyDaveTime);
}
...
void CutScene::Update() {
    ...
    if (IsShowingCrazyDave() &&
        (!mBoard->mPaused || mApp->mGameMode != GameMode::GAMEMODE_UPSELL)) {
    ...
    if (!aCutsceneTimeStop) {
        ...
        if (mCutsceneTime == TimeSeedChoserSlideOnEnd + mCrazyDaveTime &&
            mBoard->ChooseSeedsOnCurrentLevel()) {
            StartSeedChooser();
        }
    }

    int aTimeStart = TimeIntroEnd + mLawnMowerTime + mSodTime + mGraveStoneTime + mCrazyDaveTime +
                     mFogTime + mBossTime + mReadySetPlantTime;
...
void CutScene::UpdateZombiesWon() {
    ...
    if (mCutsceneTime == LostTimeBrainGraphicStart - 400 ||
        mCutsceneTime == LostTimeBrainGraphicStart - 900) {
    ...
    if (mCutsceneTime == LostTimeBrainGraphicStart) {
        ...
        Reanimation* aReanimation = mApp->AddReanimation(-BOARD_OFFSET, 0, aRenderPosition,
    ...
    if (mCutsceneTime == LostTimeEnd) {
        if (mApp->IsSurvivalMode()) {
            ...
            SexyString aFlagsStr = mApp->Pluralize(aFlagsCompleted, "[ONE_FLAG]", "[COUNT_FLAGS]");
            SexyString aStr = TodReplaceString("[SURVIVAL_DEATH_MESSAGE]", "{FLAGS}", aFlagsStr);
...
void CutScene::AdvanceCrazyDaveDialog(bool theJustSkipping) {
    ...
    if (!mApp->AdvanceCrazyDaveText()) {
        ...
        if (mApp->IsFinalBossLevel() && mApp->IsAdventureMode()) {
            ...
            aCrazyDaveReanim->PlayReanim("anim_grab", ReanimLoopType::REANIM_PLAY_ONCE_AND_HOLD, 0,
                                         18.0f);
    ...
    if ((aMessageIndex == 1503 || aMessageIndex == 1553) && !theJustSkipping) {
        ...
        SexyString aBodyString =
            TodReplaceNumberString("[UPGRADE_DIALOG_BODY]", "{SLOTS}", aNumPackets + 1);
        SexyString aAmountString = mApp->GetMoneyString(aCost);
        Dialog* aDialog = mApp->DoDialog(Dialogs::DIALOG_PURCHASE_PACKET_SLOT, true, aAmountString,
                                         aBodyString, "", Dialog::BUTTONS_YES_NO);
        aDialog->mX += 120;
        aDialog->mY += 130;
...
void CutScene::KeyDown(KeyCode theKey) {
    if (mApp->mGameMode == GameMode::GAMEMODE_UPSELL) {
        if (mApp->mTodCheatKeys && theKey == KeyCode::KEYCODE_ESCAPE) {
            ...
            mCrazyDaveCountDown = 1;
        } else if (theKey == KeyCode::KEYCODE_SPACE || theKey == KeyCode::KEYCODE_RETURN ||
                   theKey == KeyCode::KEYCODE_ESCAPE) {
            mApp->CrazyDaveStopSound();
            mApp->PlaySample(SOUND_PAUSE);
            mApp->mMusic->GameMusicPause(true);

            int aResult = mApp->LawnMessageBox(Dialogs::DIALOG_MESSAGE, "[UPSELL_PAUSE_HEADER]",
                                               "[UPSELL_PAUSE_BODY]", "[UPSELL_RESUME_BUTTON]",
                                               "[MAIN_MENU_BUTTON]", Dialog::BUTTONS_YES_NO);
        ...
        if ((theKey == KeyCode::KEYCODE_SPACE || theKey == KeyCode::KEYCODE_RETURN) &&
            ...
            AdvanceCrazyDaveDialog(false);
                   ...
                   (theKey == KeyCode::KEYCODE_SPACE || theKey == KeyCode::KEYCODE_RETURN ||
                    theKey == KeyCode::KEYCODE_ESCAPE)) {
            CancelIntro();
        }
    }
...
int CutScene::ParseDelayTimeFromMessage() {
    ...
    if (anIndex != SexyString::npos) {
        SexyString aDelayTimeStr =
            aCrazyDaveText.substr(anIndex + 7, aCrazyDaveText.find("}") - anIndex - 7);
...
int CutScene::ParseTalkTimeFromMessage() {
    ...
    if (anIndex != -1) {
        SexyString aTalkTimeStr =
            aCrazyDaveText.substr(anIndex + 6, aCrazyDaveText.find("}") - anIndex - 6);
...
void CutScene::UpdateUpsell() {
    ...
    case 3305: {
        ...
        AttachEffect* anAttachEffect = AttachReanim(
            aCrazyDaveReanim->GetTrackInstanceByName("Dave_handinghand")->mAttachmentID,
            aReanimSquash, 92.0f, 387.0f);
    ...
    case 3306: {
        Reanimation* aReanimThreepeater =
            mApp->AddReanimation(0, 0, 0, ReanimationType::REANIM_THREEPEATER);
        ...
        for (int i = 1; i < 4; i++) {
            ...
            aReanimHead->AttachToAnotherReanimation(aReanimThreepeater,
                                                    StrFormat("anim_head%d", i).c_str());
        }
        AttachEffect* anAttachEffect =
            AttachReanim(aCrazyDaveReanim->GetTrackInstanceByName("Dave_body1")->mAttachmentID,
                         aReanimThreepeater, 0.0f, 0.0f);
    ...
    case 3307: {
        Reanimation* aReanimMagnet =
            mApp->AddReanimation(0, 0, 0, ReanimationType::REANIM_MAGNETSHROOM);
        ...
        AttachEffect* anAttachEffect =
            AttachReanim(aCrazyDaveReanim->GetTrackInstanceByName("Dave_pot")->mAttachmentID,
...
void CutScene::UpdateIntro() {
    mBoard->Move(TodAnimateCurve(TimeIntro_PanRightStart, TimeIntro_PanRightEnd, mCutsceneTime, 100,
                                 -100, TodCurves::CURVE_LINEAR),
                 0);
...
void CutScene::DrawIntro(Graphics* g) {
    ...
    if (mCutsceneTime > TimeIntro_PresentsFadeIn && mCutsceneTime <= aTimePanRightStart) {
        int anAlpha =
            mCutsceneTime < aTimePanRightStart - 600
                ? TodAnimateCurve(TimeIntro_PresentsFadeIn, TimeIntro_PresentsFadeIn + 300,
                                  mCutsceneTime, 0, 255, TodCurves::CURVE_LINEAR)
                : TodAnimateCurve(aTimePanRightStart - 600, aTimePanRightStart - 300, mCutsceneTime,
                                  255, 0, TodCurves::CURVE_LINEAR);

        TodDrawString(g, "[INTRO_PRESENTS]", BOARD_WIDTH / 2 - mBoard->mX, 310 - mBoard->mY,
                      FONT_BRIANNETOD32, Color(255, 255, 255, anAlpha),
                      DrawStringJustification::DS_ALIGN_CENTER);
    }

    if (mCutsceneTime > TimeIntro_LogoStart && mCutsceneTime <= TimeIntro_PanRightEnd) {
        float aScale = TodAnimateCurveFloat(TimeIntro_LogoStart, TimeIntro_LogoEnd, mCutsceneTime,
                                            5, 1, TodCurves::CURVE_EASE_OUT);
        float aCenter = aScale * 0.5;
        int aOffsetX = BOARD_WIDTH / 2 - mBoard->mX, aOffsetY = BOARD_HEIGHT / 2 - mBoard->mY;
        Rect aRect(aOffsetX - BOARD_WIDTH * aCenter, aOffsetY - 75 * aScale, BOARD_WIDTH * aScale,
                   150 * aScale);
    ...
    if (mCutsceneTime > TimeIntro_FadeOut && mCutsceneTime <= TimeIntro_FadeOutEnd) {
        g->SetColor(Color(0, 0, 0,
                          TodAnimateCurve(TimeIntro_FadeOut, TimeIntro_FadeOutEnd, mCutsceneTime, 0,
