...
NewOptionsDialog::NewOptionsDialog(LawnApp* theApp, bool theFromGameSelector)
    : Dialog(nullptr, nullptr, Dialogs::DIALOG_NEWOPTIONS, true, "Options", "", "",
             Dialog::BUTTONS_NONE) {
    ...
    mAlmanacButton =
        MakeButton(NewOptionsDialog::NewOptionsDialog_Almanac, this, "[VIEW_ALMANAC_BUTTON]");
    mRestartButton =
    ...
    mBackToGameButton = MakeNewButton(
        Dialog::ID_OK, this, "[BACK_TO_GAME]", nullptr, IMAGE_OPTIONS_BACKTOGAMEBUTTON0,
        IMAGE_OPTIONS_BACKTOGAMEBUTTON0, IMAGE_OPTIONS_BACKTOGAMEBUTTON2);
...
void NewOptionsDialog::ButtonDepress(int theId) {
    ...
    case NewOptionsDialog::NewOptionsDialog_Almanac: {
        AlmanacDialog* aDialog =
            mApp->DoAlmanacDialog(SeedType::SEED_NONE, ZombieType::ZOMBIE_INVALID);
    ...
    case NewOptionsDialog::NewOptionsDialog_MainMenu: {
        ...
        } else if (mApp->mBoard && mApp->mBoard->mCutScene &&
                   mApp->mBoard->mCutScene->IsSurvivalRepick()) {
            mApp->DoConfirmBackToMain();
        } else {
    ...
    case NewOptionsDialog::NewOptionsDialog_Restart: {
        if (mApp->mBoard) {
            ...
            LawnDialog* aDialog =
                (LawnDialog*)mApp->DoDialog(Dialogs::DIALOG_CONFIRM_RESTART, true, aDialogTitle,
                                            aDialogMessage, "", Dialog::BUTTONS_YES_NO);
    ...
    case NewOptionsDialog::NewOptionsDialog_Settings: {
        {
            SettingsDialog* aDialog = new SettingsDialog(mApp);
            mApp->CenterDialog(aDialog, 600, 420);
            mApp->AddDialog(Dialogs::DIALOG_SETTINGS, aDialog);
