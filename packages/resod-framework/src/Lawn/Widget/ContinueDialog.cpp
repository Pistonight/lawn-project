...
ContinueDialog::ContinueDialog(LawnApp* theApp)
    : LawnDialog(theApp, Dialogs::DIALOG_CONTINUE, true, "[CONTINUE_GAME_HEADER]", "",
    ...
    if (theApp->IsAdventureMode()) {
        ...
        mContinueButton =
            MakeButton(ContinueDialog::ContinueDialog_Continue, this, "[CONTINUE_BUTTON]");
        mNewGameButton =
            MakeButton(ContinueDialog::ContinueDialog_NewGame, this, "[NEW_GAME_BUTTON]");
    }
...
void ContinueDialog::Resize(int theX, int theY, int theWidth, int theHeight) {
    ...
    int aBtnWidth =
        IMAGE_BUTTON_LEFT->mWidth + IMAGE_BUTTON_MIDDLE->mWidth * 3 + IMAGE_BUTTON_RIGHT->mWidth;
...
*/
    ...
    mContinueButton->Resize(mLawnYesButton->mX - 20, mLawnYesButton->mY - aBtnHeight, aBtnWidth,
                            aBtnHeight);
    ...
    if (theId == ContinueDialog::ContinueDialog_Continue) {
        ...
        if (mApp->IsAdventureMode()) {
            LawnDialog* aDialog = (LawnDialog*)mApp->DoDialog(
                Dialogs::DIALOG_RESTARTCONFIRM, true, TodStringTranslate("[RESTART_LEVEL_HEADER]"),
                TodStringTranslate("[RESTART_LEVEL]"), "", Dialog::BUTTONS_OK_CANCEL);
            aDialog->mLawnYesButton->mLabel = TodStringTranslate("[RESTART_BUTTON]");
            // aDialog->CalcSize(0, 0);
        } else {
            LawnDialog* aDialog = (LawnDialog*)mApp->DoDialog(Dialogs::DIALOG_RESTARTCONFIRM, true,
                                                              "[NEW_GAME_HEADER]", "[NEW_GAME]", "",
