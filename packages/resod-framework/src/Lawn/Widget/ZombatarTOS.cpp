...
ZombatarTOS::ZombatarTOS(LawnApp* theApp)
    ...
    mTOSSlider = new Slider(IMAGE_ZOMBATAR_TOS_SLIDER, IMAGE_ZOMBATAR_TOS_SLIDER_THUMB,
                            ZombatarTOS::ZombatarTOS_Slider, this);
    mTOSSlider->mHorizontal = false;
    mTOSSlider->SetValue(0);

    mBackButton = MakeNewButton(
        ZombatarTOS::ZombatarTOS_Back, this, "", nullptr, Sexy::IMAGE_ZOMBATAR_BACK_BUTTON,
        Sexy::IMAGE_ZOMBATAR_BACK_BUTTON_HIGHLIGHT, Sexy::IMAGE_ZOMBATAR_BACK_BUTTON_HIGHLIGHT);

    mAcceptButton = MakeNewButton(
        ZombatarTOS::ZombatarTOS_Accept, this, "", nullptr, Sexy::IMAGE_ZOMBATAR_ACCEPT_BUTTON,
        Sexy::IMAGE_ZOMBATAR_ACCEPT_BUTTON_HIGHLIGHT, Sexy::IMAGE_ZOMBATAR_ACCEPT_BUTTON_HIGHLIGHT);
    mAcceptButton->mDisabled = true;

    mTOSCheckbox =
        MakeNewCheckbox(ZombatarTOS::ZombatarTOS_TOSCheckbox, this, !theApp->mIsWindowed);
...
void ZombatarTOS::Resize(int theX, int theY, int theWidth, int theHeight) {
    ...
    mBackButton->Resize(GetLeft(), 340, Sexy::IMAGE_ZOMBATAR_BACK_BUTTON->mWidth,
                        Sexy::IMAGE_ZOMBATAR_BACK_BUTTON->mHeight);
    mAcceptButton->Resize(GetWidth() - 60, 340, Sexy::IMAGE_ZOMBATAR_BACK_BUTTON->mWidth,
                          Sexy::IMAGE_ZOMBATAR_BACK_BUTTON->mHeight);
...
void ZombatarTOS::Draw(Graphics* g) {
    ...
    TodDrawStringWrapped(
        g, "[ZOMBATAR_TOS]",
        Rect(GetLeft() + 10, GetTop() - (mTOSSlider->mVal * 510), GetWidth() - 90, mHeight),
        Sexy::FONT_PICO129, Color(255, 255, 255), DrawStringJustification::DS_ALIGN_LEFT);
}
