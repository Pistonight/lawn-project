...
AchievementsWidget::AchievementsWidget(LawnApp* theApp) {
    ...
    mWidth = BOARD_WIDTH;
    mHeight =
        IMAGE_ACHEESEMENTS_CHINA->mHeight + IMAGE_SELECTORSCREEN_ACHIEVEMENTS_BG->mHeight + 15700;
    mScrollValue = 0;
    mScrollDirection = -1;
    mScrollDecay = 1;
    mPressedMoreButton = false;

    mBackButton = MakeNewButton(AchievementsWidget::ACHIEVEMENTS_BACK, this, "", nullptr,
                                Sexy::IMAGE_BLANK, Sexy::IMAGE_ACHEESEMENTS_BACK_HIGHLIGHT,
                                Sexy::IMAGE_ACHEESEMENTS_BACK_HIGHLIGHT);
    mBackButton->Resize(128, 55, Sexy::IMAGE_ACHEESEMENTS_MORE_ROCK->mWidth,
                        Sexy::IMAGE_ACHEESEMENTS_MORE_ROCK->mHeight);
    mBackButton->mClip = false;

    mMoreButton = MakeNewButton(AchievementsWidget::ACHIEVEMENTS_MORE, this, "", nullptr,
                                Sexy::IMAGE_ACHEESEMENTS_MORE_BUTTON,
                                Sexy::IMAGE_ACHEESEMENTS_MORE_BUTTON_HIGHLIGHT,
                                Sexy::IMAGE_ACHEESEMENTS_MORE_BUTTON_HIGHLIGHT);
    mMoreButton->Resize(700, 450, Sexy::IMAGE_ACHEESEMENTS_MORE_BUTTON->mWidth - 25,
                        Sexy::IMAGE_ACHEESEMENTS_MORE_BUTTON->mHeight - 50);
...
void AchievementsWidget::Draw(Graphics* g) {
    ...
    for (int i = 0; i < NUM_ACHIEVEMENT_TYPES; i++) {
        bool aHasAchievement =
            (mApp->mPlayerInfo != nullptr && mApp->mPlayerInfo->mEarnedAchievements[i]);
...
void AchievementsWidget::Update() {
    ...
    if (mHasFocus) {
        if ((mMoreButton->mIsOver && !mMoreButton->mDisabled) ||
            (mBackButton->mIsOver && !mBackButton->mDisabled)) {
