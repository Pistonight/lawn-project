...
static StoreItem gStoreItemSpots[NUM_STORE_PAGES][MAX_PAGE_SPOTS] = {
    ...
    {STORE_ITEM_PLANT_SPIKEROCK, STORE_ITEM_PLANT_GOLD_MAGNET, STORE_ITEM_PLANT_WINTERMELON,
     STORE_ITEM_PLANT_COBCANNON, STORE_ITEM_PLANT_IMITATER, STORE_ITEM_FIRSTAID, STORE_ITEM_INVALID,
     STORE_ITEM_INVALID},
    {STORE_ITEM_POTTED_MARIGOLD_1, STORE_ITEM_POTTED_MARIGOLD_2, STORE_ITEM_POTTED_MARIGOLD_3,
     STORE_ITEM_GOLD_WATERINGCAN, STORE_ITEM_FERTILIZER, STORE_ITEM_BUG_SPRAY,
     STORE_ITEM_PHONOGRAPH, STORE_ITEM_GARDENING_GLOVE},
    {STORE_ITEM_MUSHROOM_GARDEN, STORE_ITEM_AQUARIUM_GARDEN, STORE_ITEM_WHEEL_BARROW,
     STORE_ITEM_STINKY_THE_SNAIL, STORE_ITEM_TREE_OF_WISDOM, STORE_ITEM_TREE_FOOD,
...
bool StoreScreen::IsFullVersionOnly(StoreItem theStoreItem) {
    ...
    if (theStoreItem == STORE_ITEM_PACKET_UPGRADE &&
        mApp->mPlayerInfo->mPurchases[STORE_ITEM_PACKET_UPGRADE] >= 2)
        return true;
...
bool StoreScreen::IsPottedPlant(StoreItem theStoreItem) {
    return theStoreItem == STORE_ITEM_POTTED_MARIGOLD_1 ||
           theStoreItem == STORE_ITEM_POTTED_MARIGOLD_2 ||
...
*/
    ...
    if (theStoreItem == STORE_ITEM_ROOF_CLEANER) {
        return mApp->IsTrialStageLocked() ||
               (!mApp->HasFinishedAdventure() && mApp->mPlayerInfo->GetLevel() < 42);
    }
    if (theStoreItem == STORE_ITEM_PLANT_GLOOMSHROOM) {
        return mApp->IsTrialStageLocked() ||
               (!mApp->HasFinishedAdventure() && mApp->mPlayerInfo->GetLevel() < 35);
    }
    ...
    if (theStoreItem == STORE_ITEM_PLANT_WINTERMELON ||
        theStoreItem == STORE_ITEM_PLANT_COBCANNON || theStoreItem == STORE_ITEM_PLANT_IMITATER ||
        theStoreItem == STORE_ITEM_FIRSTAID) {
        return !mApp->HasFinishedAdventure();
    }
    return false;
...
void StoreScreen::DrawItemIcon(Graphics* g, int theItemPosition, StoreItem theItemType,
                               bool theIsForHighlight) {
    ...
    if (theItemType == STORE_ITEM_PACKET_UPGRADE) {
        ...
        SexyString aSlotText =
            TodReplaceNumberString("[STORE_UPGRADE_SLOTS]", "{SLOTS}", aNumSlots);
        ...
        TodDrawString(g, "x5", aPosX + 56, aPosY + 62, Sexy::FONT_HOUSEOFTERROR16, Color::White,
                      DS_ALIGN_RIGHT);
        ...
        DrawSeedPacket(g, aPosX, aPosY, (SeedType)(theItemType + 40), SEED_NONE, 0, 255, false,
                       false);
    }
...
void StoreScreen::DrawItem(Graphics* g, int theItemPosition, StoreItem theItemType) {
    ...
    if (theItemType != STORE_ITEM_PVZ) {
        ...
        TodDrawString(g, aCostString, aPosX + 23, aPosY + 85, Sexy::FONT_BRIANNETOD12, Color::Black,
                      DS_ALIGN_CENTER);
    }
    if (IsComingSoon(theItemType)) {
        Rect aRect(aPosX, aPosY, 60, 70);
        if (theItemType == STORE_ITEM_PLANT_TWINSUNFLOWER ||
            theItemType == STORE_ITEM_PACKET_UPGRADE) {
            aRect.mX -= 4;
        }
        TodDrawStringWrapped(g, "[COMING_SOON]", aRect, Sexy::FONT_HOUSEOFTERROR16,
                             Color(255, 0, 0), DS_ALIGN_CENTER_VERTICAL_MIDDLE);
...
void StoreScreen::Draw(Graphics* g) {
    ...
    if (!mPrevButton->mDisabled) {
        ...
        TodDrawString(g, aPageString, STORESCREEN_PAGESTRING_X, STORESCREEN_COINBANK_Y,
                      Sexy::FONT_BRIANNETOD12, Color(80, 80, 80), DS_ALIGN_CENTER);
    }
}
...
void StoreScreen::UpdateMouse() {
            ...
            if (Rect(aItemX, aItemY, 50, 87).Contains(aMouseX, aMouseY)) {
                ...
                case STORE_ITEM_PACKET_UPGRADE:
                    if (mApp->mPlayerInfo->mPurchases[STORE_ITEM_PACKET_UPGRADE] < 4)
                        aMessageIndex =
                            mApp->mPlayerInfo->mPurchases[STORE_ITEM_PACKET_UPGRADE] + 2011;
                    else
                        aMessageIndex = 2014;
                    break;
                ...
                if (IsFullVersionOnly(aItemType) ||
                    (!IsItemSoldOut(aItemType) && !IsItemUnavailable(aItemType) &&
    ...
    mApp->SetCursor(mBackButton->mIsOver || mPrevButton->mIsOver || mNextButton->mIsOver ||
                            aShowFinger
                        ? CURSOR_HAND
                        : CURSOR_POINTER);
}
    ...
    if (mApp->mCrazyDaveState != CRAZY_DAVE_OFF && mApp->mCrazyDaveState != CRAZY_DAVE_ENTERING) {
        ...
        if (mApp->mCrazyDaveMessageIndex >= 4000 && mApp->mCrazyDaveMessageIndex < 4004) {
            ...
            if (mApp->mCrazyDaveMessageIndex == 4004) {
                mApp->mAchievements->GiveAchievement(AchievementID::ACHIEVEMENT_MORTICULTURALIST,
                                                     true);
                mInCutscene = false;
                EnableButtons(true);
            }
...
void StoreScreen::PurchaseItem(StoreItem theStoreItem) {
    ...
    if (!CanAffordItem(theStoreItem)) {
        Dialog* aDialog =
            mApp->DoDialog(DIALOG_NOT_ENOUGH_MONEY, true, "[NOT_ENOUGH_MONEY]",
                           "[CANNOT_AFFORD_ITEM]", "[DIALOG_BUTTON_OK]", BUTTONS_FOOTER);
        mWaitForDialog = true;
        aDialog->WaitForResult(true);
        mWaitForDialog = false;
    } else {
        LawnDialog* aComfirmDialog = (LawnDialog*)mApp->DoDialog(
            DIALOG_STORE_PURCHASE, true, "[BUY_ITEM_HEADER]", "[BUY_ITEM]", "", BUTTONS_YES_NO);
        ...
        if (aComfirmResult == ID_OK) {
            ...
            if (theStoreItem == STORE_ITEM_PACKET_UPGRADE) {
                ...
                SexyString aDialogLines =
                    StrFormat(TodStringTranslate("[NOW_YOU_CAN_CHOOSE_X_SEEDS]").c_str(),
                              6 + mApp->mPlayerInfo->mPurchases[theStoreItem]);
                Dialog* aDialog =
                    mApp->DoDialog(DIALOG_UPGRADED, true, "[MORE_SLOTS]", aDialogLines,
                ...
                mApp->mPlayerInfo->mPurchases[theStoreItem] = time(nullptr);
                       ...
                       theStoreItem == STORE_ITEM_BUG_SPRAY) {
                ...
                LawnDialog* aDialog =
                    (LawnDialog*)mApp->DoDialog(DIALOG_STORE_PURCHASE, true, "[VISIT_TREE_HEADER]",
                ...
                mPottedPlantSpecs.InitializePottedPlant(SEED_MARIGOLD);
                mPottedPlantSpecs.mDrawVariation = (DrawVariation)RandRangeInt(
                    VARIATION_MARIGOLD_WHITE, VARIATION_MARIGOLD_LIGHT_GREEN);
                mApp->mPlayerInfo->mPurchases[theStoreItem] = GetCurrentDaysSince2000();
            } else {
                TOD_ASSERT(theStoreItem >= STORE_ITEM_PLANT_GATLINGPEA &&
                           theStoreItem < (StoreItem)MAX_PURCHASES);
                mApp->mPlayerInfo->mPurchases[theStoreItem] = 1;
            }
            ...
            if (theStoreItem >= STORE_ITEM_PLANT_GATLINGPEA &&
                theStoreItem <= STORE_ITEM_PLANT_IMITATER) {
...
void StoreScreen::MouseDown(int x, int y, int theClickCount) {
    ...
    for (int aItemPos = 0; aItemPos < MAX_PAGE_SPOTS; aItemPos++) {
        ...
        if (Rect(aItemX, aItemY, 50, 87).Contains(x, y)) {
            if (IsFullVersionOnly(aItemType)) {
                mWaitForDialog = true;
                mApp->LawnMessageBox(DIALOG_MESSAGE, "[GET_FULL_VERSION_TITLE]",
                                     "[FULL_VERSION_TO_BUY]", "[DIALOG_BUTTON_OK]", "",
                                     BUTTONS_FOOTER);
                mWaitForDialog = false;
            } else if (aItemType == STORE_ITEM_PVZ) {
                mWaitForDialog = true;
                int aResult =
                    mApp->LawnMessageBox(DIALOG_MESSAGE, "[BUY_PVZ_TITLE]", "[BUY_PVZ_BODY]",
            ...
            } else if (!IsItemSoldOut(aItemType) && !IsItemUnavailable(aItemType) &&
                       !IsComingSoon(aItemType))
