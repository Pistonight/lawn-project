...
#include <Lawn/Widget/AchievementsWidget.h>
...
GameSelector::GameSelector(LawnApp* theApp) {
    ...
    mAdventureButton = MakeNewButton(GameSelector::GameSelector_Adventure, this, "", nullptr,
    ...
    mMinigameButton = MakeNewButton(GameSelector::GameSelector_Minigame, this, "", nullptr,
    ...
    mPuzzleButton = MakeNewButton(GameSelector::GameSelector_Puzzle, this, "", nullptr,
    ...
    mSurvivalButton = MakeNewButton(GameSelector::GameSelector_Survival, this, "", nullptr,
                                    Sexy::IMAGE_REANIM_SELECTORSCREEN_VASEBREAKER_BUTTON,
                                    Sexy::IMAGE_REANIM_SELECTORSCREEN_VASEBREAKER_HIGHLIGHT,
                                    Sexy::IMAGE_REANIM_SELECTORSCREEN_VASEBREAKER_HIGHLIGHT);
    mSurvivalButton->Resize(0, 0, Sexy::IMAGE_REANIM_SELECTORSCREEN_VASEBREAKER_BUTTON->mWidth,
                            124);
    ...
    mAchievementsButton = MakeNewButton(GameSelector::GameSelector_Achievements, this, "", nullptr,
    ...
    mAchievementsButton->Resize(
        0, mApp->mHeight - Sexy::IMAGE_SELECTORSCREEN_ACHIEVEMENTS_PEDESTAL->mHeight - 35,
    ...
    mZombatarButton = MakeNewButton(GameSelector::GameSelector_Zombatar, this, "", nullptr,
                                    Sexy::IMAGE_BLANK, Sexy::IMAGE_BLANK, Sexy::IMAGE_BLANK);
    mZombatarButton->Resize(327, 428, Sexy::IMAGE_REANIM_SELECTORSCREEN_WOODSIGN3_PRESS->mWidth,
    ...
    mOptionsButton = MakeNewButton(
        GameSelector::GameSelector_Options, this, "", nullptr, Sexy::IMAGE_SELECTORSCREEN_OPTIONS1,
        Sexy::IMAGE_SELECTORSCREEN_OPTIONS2, Sexy::IMAGE_SELECTORSCREEN_OPTIONS2);
    mOptionsButton->Resize(0, 0, Sexy::IMAGE_SELECTORSCREEN_OPTIONS1->mWidth,
                           Sexy::IMAGE_SELECTORSCREEN_OPTIONS1->mHeight + 23);
    ...
    mHelpButton = MakeNewButton(GameSelector::GameSelector_Help, this, "", nullptr,
                                Sexy::IMAGE_SELECTORSCREEN_HELP1, Sexy::IMAGE_SELECTORSCREEN_HELP2,
                                Sexy::IMAGE_SELECTORSCREEN_HELP2);
    mHelpButton->Resize(0, 0, Sexy::IMAGE_SELECTORSCREEN_HELP1->mWidth,
                        Sexy::IMAGE_SELECTORSCREEN_HELP1->mHeight + 33);
    mHelpButton->mBtnNoDraw = true;
    mHelpButton->mMouseVisible = false;
    mHelpButton->mButtonOffsetY = 30;
    mHelpButton->mClip = false;

    mQuitButton = MakeNewButton(GameSelector::GameSelector_Quit, this, "", nullptr,
                                Sexy::IMAGE_SELECTORSCREEN_QUIT1, Sexy::IMAGE_SELECTORSCREEN_QUIT2,
                                Sexy::IMAGE_SELECTORSCREEN_QUIT2);
    mQuitButton->Resize(0, 0, Sexy::IMAGE_SELECTORSCREEN_QUIT1->mWidth + 10,
                        Sexy::IMAGE_SELECTORSCREEN_QUIT1->mHeight + 10);
    ...
    mChangeUserButton = MakeNewButton(GameSelector::GameSelector_ChangeUser, this, "", nullptr,
                                      Sexy::IMAGE_BLANK, Sexy::IMAGE_BLANK, Sexy::IMAGE_BLANK);
    mChangeUserButton->Resize(0, 0, 250, 30);
    mChangeUserButton->mBtnNoDraw = true;
    mChangeUserButton->mMouseVisible = false;
    mChangeUserButton->mClip = false;

    mOverlayWidget = new GameSelectorOverlay(this);
    mOverlayWidget->Resize(0, 0, BOARD_WIDTH, BOARD_HEIGHT);

    mStoreButton = MakeNewButton(
        GameSelector::GameSelector_Store, this, "", nullptr, Sexy::IMAGE_SELECTORSCREEN_STORE,
        Sexy::IMAGE_SELECTORSCREEN_STOREHIGHLIGHT, Sexy::IMAGE_SELECTORSCREEN_STOREHIGHLIGHT);
    mStoreButton->Resize(405, 484, Sexy::IMAGE_SELECTORSCREEN_STORE->mWidth,
                         Sexy::IMAGE_SELECTORSCREEN_STORE->mHeight);
    mStoreButton->mMouseVisible = false;
    mStoreButton->mClip = false;

    mAlmanacButton = MakeNewButton(
        GameSelector::GameSelector_Almanac, this, "", nullptr, Sexy::IMAGE_SELECTORSCREEN_ALMANAC,
        Sexy::IMAGE_SELECTORSCREEN_ALMANACHIGHLIGHT, Sexy::IMAGE_SELECTORSCREEN_ALMANACHIGHLIGHT);
    mAlmanacButton->Resize(327, 428, Sexy::IMAGE_SELECTORSCREEN_ALMANAC->mWidth,
                           Sexy::IMAGE_SELECTORSCREEN_ALMANAC->mHeight);
    ...
    Reanimation* aSelectorReanim =
        mApp->AddReanimation(0.5f, 0.5f, 0, ReanimationType::REANIM_SELECTOR_SCREEN);
    ...
    ReanimatorTrackInstance* anAdventureTrack =
        aSelectorReanim->GetTrackInstanceByName("SelectorScreen_Adventure_button");
    ...
    for (int i = 0; i < 6; i++) {
        Reanimation* aCloudReanim =
            mApp->AddReanimation(0.5f, 0.5f, 0, ReanimationType::REANIM_SELECTOR_SCREEN);
        std::string aAnimName = Sexy::StrFormat("anim_cloud%d", (i > 1 ? i + 2 : i + 1));
        aCloudReanim->PlayReanim(aAnimName.c_str(), ReanimLoopType::REANIM_PLAY_ONCE_AND_HOLD, 0,
                                 0.0f);
    ...
    for (int i = 0; i < 3; i++) {
        Reanimation* aFlowerReanim =
            mApp->AddReanimation(0.5f, 0.5f, 0, ReanimationType::REANIM_SELECTOR_SCREEN);
        std::string aAnimName = Sexy::StrFormat("anim_flower%d", i + 1);
        aFlowerReanim->PlayReanim(aAnimName.c_str(), ReanimLoopType::REANIM_PLAY_ONCE_AND_HOLD, 0,
                                  0.0f);
    ...
    if (UpdateChecker::gIsOutdated) {
        SexyString aDialogText = StrFormat(TodStringTranslate("[UPDATE_OUTDATED_MESSAGE]").c_str(),
                                           LawnApp::gVersion.toString().c_str(),
                                           UpdateChecker::gLatestVersion.toString().c_str());
        mApp->DoDialog(DIALOG_VERSION_NOTICE, true, "[UPDATE_OUTDATED_HEADER]", aDialogText,
...
void GameSelector::SyncButtons() {
    ...
    if (aAlmanacAvailable) {
        ...
        if (aStoreOpen)
            ...
            aSelectorReanim->SetImageOverride("almanac_key_shadow",
    ...
    ReanimatorTrackInstance* aMinigameTrack =
        aSelectorReanim->GetTrackInstanceByName("SelectorScreen_Survival_button");
    ReanimatorTrackInstance* aPuzzleTrack =
        aSelectorReanim->GetTrackInstanceByName("SelectorScreen_Challenges_button");
    ReanimatorTrackInstance* aSurvivalTrack =
        aSelectorReanim->GetTrackInstanceByName("SelectorScreen_ZenGarden_button");
    ...
    if (mShowStartButton) {
        aSelectorReanim->AssignRenderGroupToPrefix("SelectorScreen_Adventure_",
                                                   RENDER_GROUP_HIDDEN);
        aSelectorReanim->AssignRenderGroupToTrack("SelectorScreen_StartAdventure_shadow",
                                                  RENDER_GROUP_NORMAL);
        ...
        aSelectorReanim->AssignRenderGroupToPrefix("SelectorScreen_StartAdventure_",
                                                   RENDER_GROUP_HIDDEN);
        aSelectorReanim->AssignRenderGroupToTrack("SelectorScreen_Adventure_shadow",
                                                  RENDER_GROUP_NORMAL);
...
void GameSelector::AddTrophySparkle() {
    ...
    TodParticleSystem* aTrophyParticle = mApp->AddTodParticle(
        85.0f, 290.0f, RenderLayer::RENDER_LAYER_TOP, ParticleEffect::PARTICLE_TROPHY_SPARKLE);
...
void GameSelector::Draw(Graphics* g) {
    ...
    if (mSelectorState == SelectorAnimState::SELECTOR_OPEN) {
        ...
        g->DrawImageF(mOptionsButton->mButtonImage,
    ...
    if (mApp->mPlayerInfo && mApp->mPlayerInfo->mName.size() &&
        ...
        SexyString aWelcomeStr = mApp->mPlayerInfo->mName + '!';
...
void GameSelector::DrawOverlay(Graphics* g) {
    ...
    if (!mApp->IsIceDemo() && !mShowStartButton) {
        ...
        TodDrawImageCelF(g, Sexy::IMAGE_SELECTORSCREEN_LEVELNUMBERS, aTransAreaX + 486.0f,
                         aTransAreaY + 47.0f, aStage, 0);
        if (aSub < 10) {
            TodDrawImageCelF(g, Sexy::IMAGE_SELECTORSCREEN_LEVELNUMBERS, aTransSubX + 509.0f,
                             aTransSubY + 50.0f, aSub, 0);
        } else if (aSub == 10) {
            TodDrawImageCelF(g, Sexy::IMAGE_SELECTORSCREEN_LEVELNUMBERS, aTransSubX + 509.0f,
                             aTransSubY + 50.0f, 1, 0);
            TodDrawImageCelF(g, Sexy::IMAGE_SELECTORSCREEN_LEVELNUMBERS, aTransSubX + 518.0f,
                             aTransSubY + 51.0f, 0, 0);
        }
        g->SetColorizeImages(false);
    }

    if (mZenGardenButton->mVisible && mApp->mZenGarden->PlantsNeedWater()) {
        g->DrawImage(Sexy::IMAGE_PLANTSPEECHBUBBLE, mZenGardenButton->mX + 106,
                     mZenGardenButton->mY + 36);
    ...
    if (mHasTrophy) {
        ...
        if (aMouseX > 49 && aMouseX < 135 && aMouseY > 249 && aMouseY < 475) {
            if (mApp->EarnedGoldTrophy()) {
                mToolTip->SetLabel(LawnApp::Pluralize(mApp->mPlayerInfo->mFinishedAdventure,
...
void GameSelector::Update() {
    ...
    if (mSlideCounter > 0) {
        int aNewX = TodAnimateCurve(gSlideStartTime, 0, mSlideCounter, mStartX, mDestX,
                                    TodCurves::CURVE_EASE_IN_OUT);
        int aNewY = TodAnimateCurve(gSlideStartTime, 0, mSlideCounter, mStartY, mDestY,
                                    TodCurves::CURVE_EASE_IN_OUT);
        Move(aNewX, aNewY);
        ...
        mMinigameButton->mButtonOffsetX = aNewX;
        mMinigameButton->mButtonOffsetY = aNewY;
        mPuzzleButton->mButtonOffsetX = aNewX;
        mPuzzleButton->mButtonOffsetY = aNewY;
        mSurvivalButton->mButtonOffsetX = aNewX;
        mSurvivalButton->mButtonOffsetY = aNewY;

        mOptionsButton->mButtonOffsetX = aNewX;
        mOptionsButton->mButtonOffsetY = aNewY + 15;
        mHelpButton->mButtonOffsetX = aNewX;
        mHelpButton->mButtonOffsetY = aNewY + 30;
        mQuitButton->mButtonOffsetX = aNewX + 5;
        mQuitButton->mButtonOffsetY = aNewY + 5;

        mChangeUserButton->mButtonOffsetX = aNewX;
        mChangeUserButton->mButtonOffsetY = aNewY;

        mStoreButton->mButtonOffsetX = aNewX;
        mStoreButton->mButtonOffsetY = aNewY;
        mAlmanacButton->mButtonOffsetX = aNewX;
    ...
    if (mStartingGame) {
        ...
        mAdventureButton->SetColor(ButtonWidget::COLOR_BKG, mStartingGameCounter % 20 < 10
                                                                ? Color(80, 80, 80)
                                                                : Color::White);
    ...
    case SelectorAnimState::SELECTOR_OPEN:
        ...
        if (aSelectorReanim->mLoopCount > 0) {
            aSelectorReanim->AssignRenderGroupToTrack("SelectorScreen_Adventure_button",
                                                      RENDER_GROUP_HIDDEN);
            aSelectorReanim->AssignRenderGroupToTrack("SelectorScreen_StartAdventure_button",
                                                      RENDER_GROUP_HIDDEN);
            aSelectorReanim->AssignRenderGroupToTrack("SelectorScreen_Survival_button",
                                                      RENDER_GROUP_HIDDEN);
            aSelectorReanim->AssignRenderGroupToTrack("SelectorScreen_Challenges_button",
                                                      RENDER_GROUP_HIDDEN);
            aSelectorReanim->AssignRenderGroupToTrack("SelectorScreen_ZenGarden_button",
                                                      RENDER_GROUP_HIDDEN);
            ...
            if (mApp->mPlayerInfo == nullptr) {
                ...
                aSelectorReanim->PlayReanim("anim_sign", ReanimLoopType::REANIM_PLAY_ONCE_AND_HOLD,
                                            0, 30.0f);
            ...
            if (mApp->mPlayerInfo && mApp->mPlayerInfo->mNeedsMessageOnGameSelector) {
                ...
                mApp->LawnMessageBox(Dialogs::DIALOG_MESSAGE, "[ADVENTURE_COMPLETE_HEADER]",
                                     "[ADVENTURE_COMPLETE_BODY]", "[DIALOG_BUTTON_OK]", "",
    ...
    case SelectorAnimState::SELECTOR_NEW_USER:
        if (mApp->GetDialog(Dialogs::DIALOG_CREATEUSER) == nullptr) {
            aSelectorReanim->PlayReanim("anim_sign", ReanimLoopType::REANIM_PLAY_ONCE_AND_HOLD, 0,
                                        30.0f);
    ...
    TrackButton(mAdventureButton,
                mShowStartButton ? "SelectorScreen_StartAdventure_button"
    ...
    if (mHasFocus) {
        if ((mMinigameButton->mIsOver && !mMinigameButton->mDisabled) ||
            ...
            (mQuitButton->mIsOver && !mQuitButton->mDisabled) ||
            (mHelpButton->mIsOver && !mHelpButton->mDisabled) ||
...
void GameSelector::TrackButton(DialogButton* theButton, const char* theTrackName, float theOffsetX,
                               float theOffsetY) {
    Reanimation* aSelectorReanim = mApp->ReanimationGet(mSelectorReanimID);
    ...
    if (mApp->mDebugKeysEnabled) {
        ...
        if (theChar == 'c' || theChar == 'C') {
            ...
            for (int i = 1; i < NUM_GAME_MODES; i++)
                if (i != (int)GameMode::GAMEMODE_TREE_OF_WISDOM &&
                    i != (int)GameMode::GAMEMODE_SCARY_POTTER_ENDLESS &&
...
void GameSelector::MouseDown(int x, int y, int theClickCount) {
    for (int i = 0; i < 3; i++) {
        ...
        if (aFlowerReanim->mAnimRate <= 0.0f &&
            Distance2D(x, y, gFlowerCenter[i][0], gFlowerCenter[i][1]) < 20.0f) {
...
void GameSelector::ButtonPress(int theId, int theClickCount) {
    if (theId == GameSelector::GameSelector_Adventure ||
        theId == GameSelector::GameSelector_Minigame ||
        theId == GameSelector::GameSelector_Puzzle ||
        theId == GameSelector::GameSelector_Survival ||
...
void GameSelector::ClickedAdventure() {
    if (mApp->IsTrialStageLocked() && (mLevel >= 25 || mApp->HasFinishedAdventure())) {
        if (mApp->LawnMessageBox(Dialogs::DIALOG_MESSAGE, "[REPLAY_LEVEL_HEADER]",
                                 "[REPLAY_LEVEL_BODY]", "[DIALOG_BUTTON_YES]", "[DIALOG_BUTTON_NO]",
    ...
    Reanimation* aHandReanim =
        mApp->AddReanimation(-70.0f, 10.0f, 0, ReanimationType::REANIM_ZOMBIE_HAND);
...
bool GameSelector::ShouldDoZenTuturialBeforeAdventure() {
    return !mApp->HasFinishedAdventure() && mApp->mPlayerInfo->GetLevel() == 45 &&
           mApp->mPlayerInfo->mNumPottedPlants == 0;
}

void GameSelector::ButtonDepress(int theId) {
    if (theId == GameSelector::GameSelector_Minigame && mMinigamesLocked) {
        mApp->LawnMessageBox(Dialogs::DIALOG_MESSAGE, "[MODE_LOCKED]", "[MINIGAME_LOCKED_MESSAGE]",
                             "[DIALOG_BUTTON_OK]", "", Dialog::BUTTONS_FOOTER);
        return;
    }
    if (theId == GameSelector::GameSelector_Puzzle && mPuzzleLocked) {
        mApp->LawnMessageBox(Dialogs::DIALOG_MESSAGE, "[MODE_LOCKED]", "[PUZZLE_LOCKED_MESSAGE]",
                             "[DIALOG_BUTTON_OK]", "", Dialog::BUTTONS_FOOTER);
        return;
    }
    if (theId == GameSelector::GameSelector_Survival && mSurvivalLocked) {
        mApp->LawnMessageBox(Dialogs::DIALOG_MESSAGE, "[MODE_LOCKED]", "[SURVIVAL_LOCKED_MESSAGE]",
                             "[DIALOG_BUTTON_OK]", "", Dialog::BUTTONS_FOOTER);
        return;
    }
