...
void TitleScreen::Draw(Graphics* g) {
    ...
    if (mTitleState == TitleState::TITLESTATE_POPCAP_LOGO) {
        ...
        if (mTitleStateCounter < mTitleStateDuration - 50) {
            if (!mDisplayPartnerLogo) {
                anAlpha =
                    TodAnimateCurve(50, 0, mTitleStateCounter, 255, 0, TodCurves::CURVE_LINEAR);
            }
        } else {
            anAlpha = TodAnimateCurve(mTitleStateDuration, mTitleStateDuration - 50,
                                      mTitleStateCounter, 0, 255, TodCurves::CURVE_LINEAR);
        }
        g->SetColorizeImages(true);
        g->SetColor(Color(255, 255, 255, anAlpha));
        g->DrawImage(IMAGE_POPCAP_LOGO, (mWidth - IMAGE_POPCAP_LOGO->mWidth) / 2,
    ...
    if (mTitleState == TitleState::TITLESTATE_PARTNER_LOGO) {
        ...
        if (mTitleStateCounter >= mTitleStateDuration - 35) {
            anAlpha = TodAnimateCurve(mTitleStateDuration, mTitleStateDuration - 35,
                                      mTitleStateCounter, 0, 255, TodCurves::CURVE_LINEAR);
        ...
        g->SetColor(Color(255, 255, 255, anAlpha));
        g->DrawImage(IMAGE_PARTNER_LOGO, (mWidth - IMAGE_PARTNER_LOGO->mWidth) / 2,
                     (mHeight - IMAGE_PARTNER_LOGO->mHeight) / 2);
        g->SetColorizeImages(false);
    ...
    if (mCurBarWidth >= mTotalBarWidth) {
        ...
        float aScale = TodAnimateCurveFloatTime(0, mTotalBarWidth, mCurBarWidth, 1, 0.5f,
                                                TodCurves::CURVE_LINEAR);
        SexyTransform2D aTransform;
        TodScaleRotateTransformMatrix(aTransform, aGrassX + 11.0f + aRollLen,
                                      aGrassY - 3.0f - 35.0f * aScale + 35.0f, aRotation, aScale,
                                      aScale);
        Rect aSrcRect(0, 0, IMAGE_REANIM_SODROLLCAP->mWidth, IMAGE_REANIM_SODROLLCAP->mHeight);
        TodBltMatrix(g, IMAGE_REANIM_SODROLLCAP, aTransform, g->mClipRect, Color::White,
...
void TitleScreen::Update() {
    ...
    if (aLoadingPercent > mPrevLoadingPercent + 0.01f || mLoadingThreadComplete) {
        float aBarWidth = TodAnimateCurveFloatTime(0, 1, aLoadingPercent, 0, mTotalBarWidth,
                                                   TodCurves::CURVE_EASE_IN);
        float aDiff = aBarWidth - mCurBarWidth;
        float aAcceleration = TodAnimateCurveFloatTime(0, 1, aLoadingPercent, 0.0001f, 0.00001f,
                                                       TodCurves::CURVE_LINEAR);
        if (mLoadingThreadComplete) {
        ...
        float aMinVelocity =
            TodAnimateCurveFloatTime(0, 1, aLoadingPercent, 0.2f, 0.01f, TodCurves::CURVE_LINEAR);
    ...
    float aTriggerPoint[] = {mTotalBarWidth * 0.11f, mTotalBarWidth * 0.32f, mTotalBarWidth * 0.54f,
                             mTotalBarWidth * 0.72f, mTotalBarWidth * 0.91f};
