...
LawnDialog::LawnDialog(LawnApp* theApp, int theId, bool isModal, const SexyString& theDialogHeader,
    ...
    if (theButtonMode == 1) {
        ...
        mLawnYesButton = MakeButton(ID_OK, this, theDialogFooter);
...
int LawnDialog::GetWidth() {
    return mWidth - mContentInsets.mLeft - mContentInsets.mRight - mBackgroundInsets.mLeft -
...
void LawnDialog::CalcSize(int theExtraX, int theExtraY) {
    int aWidth = mBackgroundInsets.mLeft + mBackgroundInsets.mRight + mContentInsets.mLeft +
                 mContentInsets.mRight + theExtraX;
    if (mDialogHeader.size() > 0)
        aWidth += mHeaderFont->StringWidth(mDialogHeader);
    int aTopMidWidth = Sexy::IMAGE_DIALOG_TOPMIDDLE->mWidth;
    int aImageWidth =
        Sexy::IMAGE_DIALOG_TOPLEFT->mWidth + Sexy::IMAGE_DIALOG_TOPRIGHT->mWidth + aTopMidWidth;
    ...
    int aHeight = mBackgroundInsets.mTop + mBackgroundInsets.mBottom + mContentInsets.mTop +
                  mContentInsets.mBottom + theExtraY + DIALOG_HEADER_OFFSET;
    if (mDialogHeader.size() > 0) {
    ...
    if (mDialogLines.size() > 0) {
        ...
        int aBasicWidth = aWidth - mBackgroundInsets.mLeft - mBackgroundInsets.mRight -
                          mContentInsets.mLeft - mContentInsets.mRight - 4;
        aHeight += GetWordWrappedHeight(&g, aBasicWidth, mDialogLines,
                                        mLinesFont->GetLineSpacing() + mLineSpacingOffset) +
                   30;
    }
    aHeight += mButtonHeight;
    int aBottomHeight =
        (mTallBottom ? Sexy::IMAGE_DIALOG_BIGBOTTOMLEFT : Sexy::IMAGE_DIALOG_BOTTOMLEFT)->mHeight;
    ...
    if (aHeight < aImageHeight) {
        ...
        if (anExtraHeight)
            aHeight += aCenterHeight - anExtraHeight;
    }
...
void LawnDialog::Resize(int theX, int theY, int theWidth, int theHeight) {
    ...
    int aButtonAreaY = mHeight - mContentInsets.mBottom - mBackgroundInsets.mBottom -
                       IMAGE_BUTTON_LEFT->mHeight + 2;
    int aButtonAreaWidth = mWidth - mContentInsets.mRight - mBackgroundInsets.mRight -
                           mBackgroundInsets.mLeft - mContentInsets.mLeft + 8;
    ...
    if (mLawnYesButton && mLawnNoButton) {
        mLawnYesButton->Resize(aButtonAreaX, aButtonAreaY, aButtonWidth,
                               IMAGE_BUTTON_LEFT->mHeight);
        mLawnNoButton->Resize(aButtonAreaWidth - aButtonWidth + aButtonAreaX, aButtonAreaY,
                              aButtonWidth, IMAGE_BUTTON_LEFT->mHeight);
        ...
        mLawnYesButton->Resize(aButtonAreaX + (aButtonAreaWidth - aButtonWidth) / 2, aButtonAreaY,
                               aButtonWidth, IMAGE_BUTTON_LEFT->mHeight);
    }

    if (mReanimation->mReanim) {
        mReanimation->Resize(mReanimation->mPosX, mReanimation->mPosY + DIALOG_HEADER_OFFSET,
                             mReanimation->mWidth, mReanimation->mHeight);
    }
}
...
void LawnDialog::Draw(Graphics* g) {
    ...
    int aRepeatY = (mHeight - IMAGE_DIALOG_TOPLEFT->mHeight - aBottomLeftImage->mHeight -
    ...
    int aLinesAreaWidth = mWidth - mContentInsets.mLeft - mContentInsets.mRight -
                          mBackgroundInsets.mLeft - mBackgroundInsets.mRight - 4;
    Rect aRect(mBackgroundInsets.mLeft + mContentInsets.mLeft + 2, aFontY, aLinesAreaWidth, 0);
    if (mVerticalCenterText) {
        int aLinesHeight = GetWordWrappedHeight(g, aLinesAreaWidth, mDialogLines,
                                                mLinesFont->GetLineSpacing() + mLineSpacingOffset);
        int aLinesAreaHeight = mHeight - mContentInsets.mBottom - mBackgroundInsets.mBottom -
                               mButtonHeight - aFontY - 55;
        if (mTallBottom) {
            aLinesAreaHeight -= 36;
        }
        aRect.mY += (aLinesAreaHeight - aLinesHeight) / 2;
    }
    WriteWordWrapped(g, aRect, mDialogLines, mLinesFont->GetLineSpacing() + mLineSpacingOffset,
                     mTextAlign);
}
...
void ReanimationWidget::AddReanimation(float x, float y, ReanimationType theReanimationType) {
    ...
    mPosY = y;
    mReanim =
        mApp->mEffectSystem->mReanimationHolder->AllocReanimation(x, y, 0, theReanimationType);
...
GameOverDialog::GameOverDialog(const SexyString& theMessage, bool theShowChallengeName)
    : LawnDialog(gLawnApp, Dialogs::DIALOG_GAME_OVER, true, "[GAME_OVER]", theMessage, "",
