...
AwardScreen::AwardScreen(LawnApp* theApp, AwardType theAwardType) {
    ...
    if (theAwardType == AwardType::AWARD_HELP_ZOMBIENOTE ||
        theAwardType == AwardType::AWARD_CREDITS_ZOMBIENOTE)
    ...
    if (mHasAchievementsToShow) {
        ...
        for (int i = 0; i < NUM_ACHIEVEMENT_TYPES; i++) {
            if (mApp->mPlayerInfo->mEarnedAchievements[i] &&
                !mApp->mPlayerInfo->mShownAchievements[i]) {
...
bool AwardScreen::IsPaperNote() {
    ...
    int aLevel = mApp->mPlayerInfo->GetLevel();
    return mApp->IsAdventureMode() &&
           (aLevel == 10 || aLevel == 20 || aLevel == 30 || aLevel == 40 || aLevel == 50);
}
    ...
    if (mHasAchievementsToShow) {
        ...
        for (size_t i = 0; i < mAchievementItems.size(); i++) {
            AchievementDefinition aDefinition =
                Achievements::GetAchievementDefinition(mAchievementItems[i].mAchievement);

            SexyString aAchievementText = TodReplaceString("[ACHIEVEMENT_EARNED]", "{ACHIEVEMENT}",
                                                           TodStringTranslate(aDefinition.mName));

            Rect aSrcRect = Rect(70 * (mAchievementItems[i].mAchievement % 7),
                                 70 * (mAchievementItems[i].mAchievement / 7), 70, 70);
        ...
        if (mAwardType == AWARD_CREDITS_ZOMBIENOTE) {
            ...
            if (!mApp->IsAdventureMode()) {
                if (mApp->EarnedGoldTrophy()) {
                    DrawBottom(g, "[BEAT_GAME_MESSAGE1]", "[GOLD_SUNFLOWER_TROPHY]",
                               "[BEAT_GAME_MESSAGE2]");
                    TodDrawImageCelCenterScaledF(g, Sexy::IMAGE_SUNFLOWER_TROPHY, 325, 65, 1, 0,
                                                 0.7f, 0.7f);
                } else {
                    const char* aMsgChar;
                    if (mApp->IsSurvivalMode()) {
                        ...
                        aMsgChar = mApp->GetNumTrophies(CHALLENGE_PAGE_CHALLENGE) <= 17
                                       ? "[CHALLENGE_UNLOCKED]"
                                       : "[GET_MORE_TROPHIES]";

                    DrawBottom(g, "[GOT_TROPHY]", "[TROPHY]", aMsgChar);
                    g->DrawImage(Sexy::IMAGE_TROPHY_HI_RES,
                                 BOARD_WIDTH / 2 - Sexy::IMAGE_TROPHY_HI_RES->mWidth / 2, 137);
                }
            } else if (aLevel == 5) {
                DrawBottom(g, "[GOT_SHOVEL]", "[SHOVEL]", "[SHOVEL_DESCRIPTION]");
                g->DrawImage(Sexy::IMAGE_SHOVEL_HI_RES,
                             BOARD_WIDTH / 2 - Sexy::IMAGE_SHOVEL_HI_RES->mWidth / 2, 137);
            } else if (aLevel == 10) {
                g->DrawImage(Sexy::IMAGE_BACKGROUND1, -700, -300, 2800, 1200);
                g->DrawImage(Sexy::IMAGE_ZOMBIE_NOTE, 80, 80);
                ...
                g->DrawImage(Sexy::IMAGE_ALMANAC, BOARD_WIDTH / 2 - Sexy::IMAGE_ALMANAC->mWidth / 2,
                             160);
            } else if (aLevel == 20) {
                ...
                g->DrawImage(Sexy::IMAGE_WATERINGCAN,
                             BOARD_WIDTH / 2 - Sexy::IMAGE_WATERINGCAN->mWidth / 2, 160);
            } else if (aLevel == 50) {
                g->DrawImage(Sexy::IMAGE_BACKGROUND1, -700, -300, 2800, 1200);
                g->DrawImage(Sexy::IMAGE_ZOMBIE_NOTE, 80, 80);
        ...
        for (int i = 0; i < mAchievementItems.size(); i++) {
            if (mAchievementTime >= mAchievementItems[i].mStartAnimTime) {
                mAchievementItems[i].mY = TodAnimateCurve(
                    mAchievementItems[i].mStartAnimTime, mAchievementItems[i].mEndAnimTime,
                    mAchievementTime, mAchievementItems[i].mStartY, mAchievementItems[i].mDestY,
                    CURVE_EASE_IN_OUT);
            }

            if (mAchievementItems[mAchievementItems.size() - 1].mY ==
                mAchievementItems[mAchievementItems.size() - 1].mDestY) {
                mAchievementButton->mBtnNoDraw = false;
                mAchievementButton->mDisabled = false;
            }
        }
    }
    mApp->SetCursor(mStartButton->IsMouseOver() || mMenuButton->IsMouseOver() ||
                            mAchievementButton->IsMouseOver()
                        ? CURSOR_HAND
                        : CURSOR_POINTER);
    MarkDirty();
    if (mFadeInCounter > 0)
