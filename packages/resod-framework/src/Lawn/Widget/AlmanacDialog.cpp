...
void AlmanacDialog::SetPage(AlmanacPage thePage) {
    ...
    if (mOpenPage == AlmanacPage::ALMANAC_PAGE_INDEX) {
        ...
        mZombie = new Zombie();
        mZombie->mBoard = nullptr;
        mZombie->ZombieInitialize(0, ZombieType::ZOMBIE_NORMAL, false, nullptr,
                                  Zombie::ZOMBIE_WAVE_UI);
...
void AlmanacDialog::Update() {
    ...
    if (SeedHitTest(aMouseX, aMouseY) != SeedType::SEED_NONE ||
        (aZombieType != ZombieType::ZOMBIE_INVALID && ZombieIsShown(aZombieType)) ||
        mCloseButton->IsMouseOver() || mIndexButton->IsMouseOver() || mPlantButton->IsMouseOver() ||
        mZombieButton->IsMouseOver()) {
...
void AlmanacDialog::DrawIndex(Graphics* g) {
    ...
    TodDrawString(g, "[SUBURBAN_ALMANAC_INDEX]", BOARD_WIDTH / 2, 60, Sexy::FONT_HOUSEOFTERROR28,
                  Color(220, 220, 220), DrawStringJustification::DS_ALIGN_CENTER);
...
void AlmanacDialog::DrawPlants(Graphics* g) {
    ...
    TodDrawString(g, "[SUBURBAN_ALMANAC_PLANTS]", BOARD_WIDTH / 2, 48, Sexy::FONT_HOUSEOFTERROR20,
                  Color(213, 159, 43), DrawStringJustification::DS_ALIGN_CENTER);

    SeedType aSeedMouseOn =
        SeedHitTest(mApp->mWidgetManager->mLastMouseX, mApp->mWidgetManager->mLastMouseY);
    for (SeedType aSeedType = SeedType::SEED_PEASHOOTER; aSeedType < NUM_ALMANAC_SEEDS;
        ...
        if (mApp->SeedTypeAvailable(aSeedType)) {
            if (aSeedType == SeedType::SEED_IMITATER) {
                ...
                DrawSeedPacket(g, aPosX, aPosY, aSeedType, SeedType::SEED_NONE, 0, 255, true,
                               false);
                if (aSeedType == aSeedMouseOn)
    ...
    if (mSelectedSeed == SeedType::SEED_LILYPAD || mSelectedSeed == SeedType::SEED_TANGLEKELP ||
        ...
        g->DrawImage(aNight ? Sexy::IMAGE_ALMANAC_GROUNDNIGHTPOOL : Sexy::IMAGE_ALMANAC_GROUNDPOOL,
                     521, 107);
        ...
        g->DrawImage(Plant::IsNocturnal(mSelectedSeed) ||
                             mSelectedSeed == SeedType::SEED_GRAVEBUSTER ||
    ...
    TodDrawString(g, aName, 617, 288, Sexy::FONT_DWARVENTODCRAFT18YELLOW, Color::White,
                  DS_ALIGN_CENTER);
    int aDistanceHeader = TodDrawStringWrappedHelper(
        g, TodStringTranslate(aHeaderName), Rect(485, 309, 258, ALMANAC_PLANT_MAX_SPACE),
        Sexy::FONT_BRIANNETOD12, Color(40, 50, 90), DS_ALIGN_LEFT, true);
    int aDescriptionHeight = TodDrawStringWrappedHelper(
        g, TodStringTranslate(aDescriptionName),
        Rect(485, 309 + aDistanceHeader, 258, ALMANAC_PLANT_MAX_SPACE), Sexy::FONT_BRIANNETOD12,
        Color(40, 50, 90), DS_ALIGN_LEFT, false);
    int aVisibleHeight = ALMANAC_PLANT_MAX_SPACE - aDistanceHeader;

    // FIXME: Figure out a formula to properly move the text so the last line is at the buttom of
    // the space.
    ...
    if (!mDescriptionScrollbar->mDisabled) {
        ...
        g->SetClipRect(484, 309 + aDistanceHeader, 258,
                       ALMANAC_PLANT_MAX_SPACE - aDistanceHeader - 28); // Cost and Time offset
    }
    TodDrawStringWrapped(g, aDescriptionName,
                         Rect(485, 309 + aDistanceHeader - anOffsetSlider, 258 - aMaxWidthOffset,
                              ALMANAC_PLANT_MAX_SPACE),
    ...
    if (mSelectedSeed != SeedType::SEED_IMITATER) {
        ...
        TodDrawStringWrapped(g, aCostStr, Rect(485, 520, 134, 50), Sexy::FONT_BRIANNETOD12,
                             Color::White, DS_ALIGN_LEFT);

        SexyString aRechargeStr =
            TodReplaceString("{KEYWORD}{WAIT_TIME}:{STAT} {WAIT_TIME_LENGTH}", "{WAIT_TIME_LENGTH}",
        ...
        TodDrawStringWrapped(g, aRechargeStr, Rect(600, 520, 139, 50), Sexy::FONT_BRIANNETOD12,
                             Color(40, 50, 90), DS_ALIGN_RIGHT);
    }
}

void AlmanacDialog::DrawZombies(Graphics* g) {
    g->DrawImage(Sexy::IMAGE_ALMANAC_ZOMBIEBACK, 0, 0);
    TodDrawString(g, "[SUBURBAN_ALMANAC_ZOMBIES]", BOARD_WIDTH / 2, 54,
                  Sexy::FONT_DWARVENTODCRAFT24, Color(0, 196, 0), DS_ALIGN_CENTER);

    ZombieType aZombieMouseOn =
        ZombieHitTest(mApp->mWidgetManager->mLastMouseX, mApp->mWidgetManager->mLastMouseY);
    ...
    if (mZombie && !ZombieHasSilhouette(mZombie->mZombieType)) {
        ...
        if (mZombie->mZombieType != ZombieType::ZOMBIE_BUNGEE &&
            mZombie->mZombieType != ZombieType::ZOMBIE_BOSS &&
            mZombie->mZombieType != ZombieType::ZOMBIE_ZAMBONI &&
            mZombie->mZombieType != ZombieType::ZOMBIE_CATAPULT)
    ...
    SexyString aName =
        ZombieHasSilhouette(mSelectedZombie) ? "???" : StrFormat("[%s]", aZombieDef.mZombieName);
    TodDrawString(g, aName, 613, 362, Sexy::FONT_DWARVENTODCRAFT18GREENINSET,
                  Color(190, 255, 235, 255), DS_ALIGN_CENTER);
    ...
    if (ZombieHasDescription(mSelectedZombie)) {
        ...
        aHeaderName =
            TodStringTranslate(StrFormat("[%s_DESCRIPTION_HEADER]", aZombieDef.mZombieName));
    ...
    for (TodStringListFormat& aFormat : gLawnStringFormats) {
        if (TestBit(aFormat.mFormatFlags,
                    TodStringFormatFlag::TOD_FORMAT_HIDE_UNTIL_MAGNETSHROOM)) {
    ...
    int aDistanceHeader =
        TodDrawStringWrappedHelper(g, aHeaderName, Rect(484, 377, 258, ALMANAC_ZOMBIE_MAX_SPACE),
                                   Sexy::FONT_BRIANNETOD12, Color(40, 50, 90), DS_ALIGN_LEFT, true);
    int aDescriptionHeight = TodDrawStringWrappedHelper(
        g, aDescription, Rect(484, 377 + aDistanceHeader, 258, ALMANAC_ZOMBIE_MAX_SPACE),
        Sexy::FONT_BRIANNETOD12, Color(40, 50, 90), aAlign, false);
    ...
    TodDrawStringWrapped(g, aDescription,
                         Rect(484, 377 + aDistanceHeader - anOffsetSlider, 258 - aMaxWidthOffset,
                              ALMANAC_ZOMBIE_MAX_SPACE),
...
bool AlmanacDialog::ZombieHasSilhouette(ZombieType theZombieType) {
    ...
    return mApp->HasFinishedAdventure() ||
           mApp->mPlayerInfo->GetLevel() >
               GetZombieDefinition(ZombieType::ZOMBIE_YETI).mStartingLevel;
}
...
bool AlmanacDialog::ZombieIsShown(ZombieType theZombieType) {
    ...
    if (theZombieType <= ZombieType::ZOMBIE_BOSS) {
        ...
        return aStart <= aLevel &&
               (aStart != aLevel || !Board::IsZombieTypeSpawnedOnly(theZombieType) ||
                gZombieDefeated[theZombieType]);
    }
...
void AlmanacDialog::MouseUp(int x, int y, int theClickCount) {
    if (mPlantButton->IsMouseOver())
        ...
        SetPage(ALMANAC_PAGE_INDEX);
    mDescriptionScrollbar->MouseUp(x - mDescriptionScrollbar->mX, y - mDescriptionScrollbar->mY,
                                   theClickCount);
}
...
void AlmanacDialog::MouseDown(int x, int y, int theClickCount) {
    ...
    mDescriptionScrollbar->MouseDown(x - mDescriptionScrollbar->mX, y - mDescriptionScrollbar->mY,
                                     theClickCount);
