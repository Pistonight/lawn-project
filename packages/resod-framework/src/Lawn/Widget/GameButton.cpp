...
void DrawStoneButton(Graphics* g, int x, int y, int theWidth, int theHeight, bool isDown,
                     bool isHighLighted, const SexyString& theLabel) {
...
void GameButton::Draw(Graphics* g) {
    ...
    if (!isDown) {
        ...
        g->SetColor(
            mColors[isHighLighted ? GameButton::COLOR_LABEL_HILITE : GameButton::COLOR_LABEL]);
...
void GameButton::Update() {
    ...
    if ((aManager->mFocusWidget && aManager->mFocusWidget == mParentWidget) ||
        mApp->GetDialogCount() <= 0) {
...
void NewLawnButton::Draw(Graphics* g) {
    ...
    if (!isDown) {
        ...
        g->SetColor(mColors[ButtonWidget::COLOR_BKG]);
        if (HaveButtonImage(mDownImage, mDownRect))
            DrawButtonImage(g, mDownImage, mDownRect, mButtonOffsetX + mTranslateX,
                            mButtonOffsetY + mTranslateY);
        else if (HaveButtonImage(mOverImage, mOverRect))
            DrawButtonImage(g, mOverImage, mOverRect, mButtonOffsetX + mTranslateX,
                            mButtonOffsetY + mTranslateY);
        else if (mButtonImage != nullptr)
            DrawButtonImage(g, mButtonImage, mNormalRect, mButtonOffsetX + mTranslateX,
                            mButtonOffsetY + mTranslateY);
...
NewLawnButton* MakeNewButton(int theId, ButtonListener* theListener, const SexyString& theText,
                             Font* theFont, Image* theImageNormal, Image* theImageOver,
                             Image* theImageDown) {
