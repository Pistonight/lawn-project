...
ImitaterDialog::ImitaterDialog()
    : LawnDialog(gLawnApp, Dialogs::DIALOG_IMITATER, true, "[CHOOSE_SEED_TO_COPY]", "",
                 "[DIALOG_BUTTON_OK]", Dialog::BUTTONS_YES_NO) {
...
void ImitaterDialog::UpdateCursor() {
    ...
    if (aSeedType != SeedType::SEED_NONE &&
        !mApp->mSeedChooserScreen->SeedNotAllowedToPick(aSeedType)) {
...
void ImitaterDialog::Draw(Graphics* g) {
    ...
    for (SeedType aSeedType = (SeedType)0; aSeedType < SeedType::SEED_GATLINGPEA;
        ...
        if (mApp->SeedTypeAvailable(aSeedType)) {
            ...
            DrawSeedPacket(g, aSeedX, aSeedY, SeedType::SEED_IMITATER, aSeedType, 0,
