...
CreditScreen::CreditScreen(LawnApp* theApp) {
    ...
    mMainMenuButton =
        MakeButton(CreditScreen::Credits_Button_MainMenu, this, "[CREDITS_MAIN_MENU_BUTTON]");
    mMainMenuButton->Resize(298, 554, 209, 46);
    mMainMenuButton->SetVisible(false);

    mReplayButton =
        MakeNewButton(CreditScreen::Credits_Button_Replay, this, "[CREDITS_REPLAY_BUTTON]",
...
void CreditScreen::PreLoadCredits() {
    ...
    ReanimatorDefinition* aMain2Def =
        &gReanimatorDefArray[(int)ReanimationType::REANIM_CREDITS_MAIN2];
    for (int aTrackIndex = 0; aTrackIndex < aMain2Def->mTrackCount; aTrackIndex++) {
        ...
        for (int aTransIndex = 0; aTransIndex < aTrack->mTransformCount; aTransIndex++) {
            ...
            if (aTransIndex < 124 &&
                (strcmp(aTrack->mName, "Words") == 0 || strcmp(aTrack->mName, "Words2") == 0)) {
                aTrans.mFont = FONT_BRIANNETOD32BLACK;
            }
        }
    }
    ReanimatorDefinition* aMain3Def =
        &gReanimatorDefArray[(int)ReanimationType::REANIM_CREDITS_MAIN3];
    ...
    ReanimatorDefinition* aUndeadDef =
        &gReanimatorDefArray[(int)ReanimationType::REANIM_CREDITS_WEARETHEUNDEAD];
...
void CreditScreen::GetTiming(CreditsTiming** theBeforeTiming, CreditsTiming** theAfterTiming,
                             float* theFraction) {
    Reanimation* aCreditsReanim = mApp->ReanimationGet(mCreditsReanimID);
    float aFrameCount =
        aCreditsReanim->mDefinition->mTracks->mTransformCount * aCreditsReanim->mAnimTime - 1.0f;
        ...
        *theFraction = aFrameCount / gCreditsTiming[0].mFrame;
        ...
        for (int i = 0; i < gCreditsTimingCount - 1; i++) {
            ...
            if (aTiming2->mFrame > aFrameCount) {
                ...
                *theFraction =
                    (aFrameCount - aTiming1->mFrame) / (aTiming2->mFrame - aTiming1->mFrame);
                return;
            }
        }
...
void DrawDisco(Graphics* g, float aCenterX, float aCenterY, float theTime) {
    ...
    g->mDestImage->BltTrianglesTex(IMAGE_REANIM_CREDITS_DISCOLIGHTS, aVerts, 2,
                                   Rect(0, 0, BOARD_WIDTH, BOARD_HEIGHT), Color::White,
...
void CreditScreen::DrawFogEffect(Graphics* g, float theTime) {
    ...
    for (int x = 0; x < 14; x++) {
        for (int y = 0; y < 7; y++) {
            ...
            float aAnimTime = aCreditsReanim->mDefinition->mTracks->mTransformCount *
...
void CreditScreen::DrawOverlay(Graphics* g) {
    if (mCreditsPhase == CreditsPhase::CREDITS_END) {
        int aFadeAlpha =
            TodAnimateCurve(50, 100, mCreditsPhaseCounter, 255, 0, TodCurves::CURVE_LINEAR);
        if (aFadeAlpha > 0) {
...
void CreditScreen::DrawFinalCredits(Graphics* g) {
    ...
    SexyString aCurrentRoleStr =
        TodStringTranslate(StrFormat(aRawRoleStr.c_str(), aCurrentRoleIndex++));
    int aScrollY = BOARD_HEIGHT - (int)(mFinalScrollProgress * 5.0f);
    while (aCurrentRoleStr.find("<Missing") == std::string::npos) {
        SexyString aNameStr =
            TodStringTranslate(StrFormat("[CREDITS_NAMES%d]", aCurrentRoleIndex - 1));

        if (aCurrentRoleStr[0] == '^') {
            SexyString aHeaderText = aCurrentRoleStr.substr(1); // remove the ^ lol
            aScrollY -= 4;
            aScrollY += TodDrawStringWrappedHelper(g, aHeaderText, Rect(60, aScrollY, 680, 250),
                                                   FONT_HOUSEOFTERROR16, Color::White,
                                                   DrawStringJustification::DS_ALIGN_CENTER, true);
            aScrollY -= 4;
        } else {
            TodDrawStringWrapped(g, aCurrentRoleStr, Rect(60, aScrollY, 300, 250),
                                 FONT_HOUSEOFTERROR16, Color::White,
                                 DrawStringJustification::DS_ALIGN_RIGHT);
            TodDrawStringWrapped(g, aNameStr, Rect(390, aScrollY, 450, 250), FONT_HOUSEOFTERROR16,
                                 Color::White, DrawStringJustification::DS_ALIGN_LEFT);
            int aRoleHeight = TodDrawStringWrappedHelper(
                g, aCurrentRoleStr, Rect(30, aScrollY, 300, 250), FONT_HOUSEOFTERROR16,
                Color::White, DrawStringJustification::DS_ALIGN_RIGHT, false);
            int aNameHeight = TodDrawStringWrappedHelper(
                g, aNameStr, Rect(390, aScrollY, 450, 250), FONT_HOUSEOFTERROR16, Color::White,
                DrawStringJustification::DS_ALIGN_LEFT, false);
...
void CreditScreen::Draw(Graphics* g) {
    ...
    if (mCreditsPhase == CreditsPhase::CREDITS_MAIN2 &&
        aFrameFactor * 125.0f > aCreditsReanim->mAnimTime) {
    ...
    bool aDrawFog = false;
    if (mCreditsPhase == CreditsPhase::CREDITS_MAIN1) {
        if (aCreditsReanim->mAnimTime > aFrameFactor * 203.0f &&
            aCreditsReanim->mAnimTime < aFrameFactor * 268.0f) {
            aDrawPool = true;
        }
        if (aCreditsReanim->mAnimTime > aFrameFactor * 305.0f &&
            aCreditsReanim->mAnimTime < aFrameFactor * 339.0f) {
            aDrawPool = true;
            aDrawDoorBottom = true;
        }
    }
    if (mCreditsPhase == CreditsPhase::CREDITS_MAIN2) {
        if (aCreditsReanim->mAnimTime > aFrameFactor * 187.0f &&
            aCreditsReanim->mAnimTime < aFrameFactor * 249.0f) {
            aDrawPool = true;
            aDrawNightPool = true;
        ...
        if (aCreditsReanim->mAnimTime > aFrameFactor * 123.0f &&
            aCreditsReanim->mAnimTime < aFrameFactor * 189.0f) {
            if (aTransformBackground1.mFrame != -1.0f) {
                aDrawDiscoLights = true;
            }
        }
        if (aCreditsReanim->mAnimTime > aFrameFactor * 189.0f &&
            aCreditsReanim->mAnimTime < aFrameFactor * 249.0f) {
            if (aTransformBackground2.mFrame != -1.0f) {
                aDrawDiscoLights = true;
                aDrawFog = true;
    ...
    if (mCreditsPhase == CreditsPhase::CREDITS_MAIN3) {
        if (aCreditsReanim->mAnimTime > aFrameFactor * 123.0f &&
            aCreditsReanim->mAnimTime < aFrameFactor * 218.0f) {
            aDrawChimney = true;
        }
    ...
    if (aDrawDiscoLights) {
        float aDiscoTime = aCreditsReanim->mDefinition->mTracks->mTransformCount *
    ...
    if (aDrawFog) {
        float aPercent = TodAnimateCurveFloatTime(aFrameFactor * 189.0f, aFrameFactor * 249.0f,
                                                  aCreditsReanim->mAnimTime, 0.0f, 1.0f,
...
void CreditScreen::UpdateBlink() {
    ...
    mBlinkCountdown = 700;
    Reanimation* aCreditsReanim = mApp->ReanimationGet(mCreditsReanimID);
    Reanimation* aSunFlowerReanim =
        FindSubReanim(aCreditsReanim, ReanimationType::REANIM_SUNFLOWER);
    if (aSunFlowerReanim) {
        if (mCreditsPhase == CreditsPhase::CREDITS_MAIN3) {
            int aFrameCount = aCreditsReanim->mDefinition->mTracks->mTransformCount - 1;
            float aFrameFactor = 1.0f / aFrameCount;
        ...
        Reanimation* aBlinkReanim =
            mApp->AddReanimation(0.0f, 0.0f, 0, ReanimationType::REANIM_SUNFLOWER);
...
void CreditScreen::Update() {
    ...
    if (mUpdateCount == 1) {
        ...
        int aDurationReanimation = (aCreditsReanim->mDefinition->mTracks->mTransformCount *
                                    aCreditsReanim->mAnimTime / aCreditsReanim->mAnimRate) *
                                   1000.0f;
        if (mCreditsPhase == CreditsPhase::CREDITS_MAIN2) {
...
void CreditScreen::UpdateMovie() {
    ...
    if (mCreditsPhase == CreditsPhase::CREDITS_MAIN1) {
        if (aCreditsReanim->ShouldTriggerTimedEvent(aFrameFactor * 128.0f) ||
            ...
            mApp->AddTodParticle(BOARD_WIDTH / 2, BOARD_HEIGHT / 2,
    ...
    if (mCreditsPhase == CreditsPhase::CREDITS_MAIN2 ||
        ...
        Reanimation* aUndeadReanim =
            FindSubReanim(aCreditsReanim, ReanimationType::REANIM_CREDITS_WEARETHEUNDEAD);
        if (aUndeadReanim) {
            ReanimatorTrackInstance* aTrackWordsUndead1 =
                aUndeadReanim->GetTrackInstanceByName("bubbletext1");
            ReanimatorTrackInstance* aTrackWordsUndead2 =
                aUndeadReanim->GetTrackInstanceByName("bubbletext2");
            ReanimatorTrackInstance* aTrackWordsUndead3 =
                aUndeadReanim->GetTrackInstanceByName("bubbletext3");
            ...
            ReanimatorTrackInstance* aTrackWordsShake =
                aUndeadReanim->GetTrackInstanceByName("ShakyText");
    ...
    if (mCreditsPhase == CreditsPhase::CREDITS_MAIN2) {
        ...
        if (aCreditsReanim->ShouldTriggerTimedEvent(aFrameFactor * 188.0f)) {
            TodParticleSystem* aFogParticle =
                mApp->AddTodParticle(aFogPosX, 230.0f, (int)RenderLayer::RENDER_LAYER_TOP,
    ...
    if (aBeforeTiming && aAfterTiming) {
        if (aAfterTiming->mBrainType == CreditBrainType::BRAIN_FLY_ON) {
            mBrainPosX = TodAnimateCurveFloatTime(0.0f, 1.0f, aFraction, -50.0f,
                                                  aAfterTiming->mWordX - 15.0f,
                                                  TodCurves::CURVE_EASE_IN_OUT);
            mBrainPosY = TodAnimateCurveFloatTime(0.0f, 1.0f, aFraction, 505.0f, 485.0f,
                                                  TodCurves::CURVE_BOUNCE_FAST_MIDDLE);
            mDrawBrain = true;
        } else if (aAfterTiming->mBrainType == CreditBrainType::BRAIN_FAST_ON) {
            mBrainPosX = TodAnimateCurveFloatTime(
                0.0f, 1.0f, aFraction, aAfterTiming->mWordX - 50.0f, aAfterTiming->mWordX - 15.0f,
                TodCurves::CURVE_EASE_IN_OUT);
            mBrainPosY = TodAnimateCurveFloatTime(0.0f, 1.0f, aFraction, 485.0f, 505.0f,
                                                  TodCurves::CURVE_EASE_IN_OUT);
            mDrawBrain = true;
        } else if (aAfterTiming->mBrainType == CreditBrainType::BRAIN_FLY_OFF) {
            mBrainPosX =
                TodAnimateCurveFloatTime(0.0f, 1.0f, aFraction, aBeforeTiming->mWordX - 15.0f,
                                         850.0f, TodCurves::CURVE_EASE_IN_OUT);
            mBrainPosY = TodAnimateCurveFloatTime(0.0f, 1.0f, aFraction, 505.0f, 485.0f,
                                                  TodCurves::CURVE_BOUNCE_FAST_MIDDLE);
            mDrawBrain = true;
        } else if (aAfterTiming->mBrainType == CreditBrainType::BRAIN_FAST_OFF) {
            mBrainPosX = TodAnimateCurveFloatTime(
                0.0f, 1.0f, aFraction, aBeforeTiming->mWordX - 15.0f, aBeforeTiming->mWordX + 25.0f,
                TodCurves::CURVE_EASE_IN_OUT);
            mBrainPosY = TodAnimateCurveFloatTime(0.0f, 1.0f, aFraction, 505.0f, 485.0f,
                                                  TodCurves::CURVE_EASE_IN_OUT);
            mDrawBrain = true;
        ...
        if (aBeforeTiming->mWordType != CreditWordType::WORD_OFF &&
            aFramesForWord * aFraction < 0.2f) {
            aSunflowerFrame = 1;
    ...
    if (aSunFlowerReanim) {
        if (mCreditsPhase == CreditsPhase::CREDITS_MAIN3 &&
            aCreditsReanim->mAnimTime > aFrameFactor * 255.0f) {
            aSunFlowerReanim->SetImageOverride("anim_idle", IMAGE_REANIM_SUNFLOWER_HEAD_WINK);
            Reanimation* aStageReanim =
                FindSubReanim(aCreditsReanim, ReanimationType::REANIM_CREDITS_STAGE);
            aSunFlowerReanim->mAnimTime -=
                aSunFlowerReanim->mAnimRate * SECONDS_PER_UPDATE / aSunFlowerReanim->mFrameCount;
            aStageReanim->mAnimTime -=
                aStageReanim->mAnimRate * SECONDS_PER_UPDATE / aStageReanim->mFrameCount;
        } else {
...
void CreditScreen::TurnOffTongues(Reanimation* theReanim, int aParentTrack) {
    ...
    for (int aTrackIndex = 0; aTrackIndex < theReanim->mDefinition->mTrackCount; aTrackIndex++) {
        ...
        if (theReanim->mReanimationType == ReanimationType::REANIM_ZOMBIE_CREDITS_DANCE &&
            aParentTrack % 4 != 1 &&
            stricmp(theReanim->mDefinition->mTracks[aTrackIndex].mName, "anim_tongue") == 0) {
...
void CreditScreen::JumpToFrame(CreditsPhase thePhase, float theFrame) {
    ...
    mApp->mMusic->PlayFromOffset(MusicFile::MUSIC_FILE_CREDITS_ZOMBIES_ON_YOUR_LAWN,
                                 aMusicOffset - 900, 1.0f);
    if (thePhase == CreditsPhase::CREDITS_END) {
...
void CreditScreen::PauseCredits() {
    ...
    if (mApp->LawnMessageBox(Dialogs::DIALOG_MESSAGE, "[CREDITS_PAUSE_HEADER]",
                             "[CREDITS_PAUSE_BODY]", "[CREDITS_RESUME_BUTTON]",
                             "[MAIN_MENU_BUTTON]", Dialog::BUTTONS_YES_NO) == Dialog::ID_NO) {
