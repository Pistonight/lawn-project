...
void GridItem::DrawIZombieBrain(Graphics* g) {
    ...
    if (mBoard->mAdvice->mDuration > 0 &&
        mBoard->mHelpIndex == AdviceType::ADVICE_I_ZOMBIE_EAT_ALL_BRAINS) {
...
void GridItem::DrawGraveStone(Graphics* g) {
    ...
    int aHeightPosition =
        TodAnimateCurve(0, 100, mGridItemCounter, 1000, 0, TodCurves::CURVE_EASE_IN_OUT);
    ...
    int aVisibleHeight =
        TodAnimateCurve(0, 1000, aHeightPosition, aCelHeight, 0, TodCurves::CURVE_EASE_IN_OUT);
    int aExtraBottomClip =
        TodAnimateCurve(0, 50, aHeightPosition, 0, 14, TodCurves::CURVE_EASE_IN_OUT);
    int aVisibleHeightDirt =
        TodAnimateCurve(500, 1000, aHeightPosition, aCelHeight, 0, TodCurves::CURVE_EASE_IN_OUT);
    int aExtraTopClip = 0;
    Plant* aPlant =
        mBoard->GetTopPlantAt(mGridX, mGridY, PlantPriority::TOPPLANT_ONLY_NORMAL_POSITION);
    if (aPlant && aPlant->mState == PlantState::STATE_GRAVEBUSTER_EATING) {
        aExtraTopClip = TodAnimateCurveFloat(400, 0, aPlant->mStateCountdown, 10.0f, 40.0f,
...
void GridItem::DrawScaryPot(Graphics* g) {
    ...
    TodDrawImageCelCenterScaledF(g, IMAGE_PLANTSHADOW2, aXPos - 5.0f, aYPos + 72.0f, 0, 0, 1.3f,
                                 1.3f);
    ...
    if (mTransparentCounter > 0) {
        ...
        if (mScaryPotType == ScaryPotType::SCARYPOT_SEED) {
            ...
            DrawSeedPacket(&aInsideGraphics, aXPos + 23.0f, aYPos + 33.0f, mSeedType,
                           SeedType::SEED_NONE, 0.0f, 255, false, false);
            ...
            mApp->mReanimatorCache->DrawCachedZombie(&aInsideGraphics, aXPos + aOffsetX,
                                                     aYPos + aOffsetY, mZombieType);
...
void GridItem::DrawSquirrel(Graphics* g) {
    ...
    if (mGridItemState == GridItemState::GRIDITEM_STATE_SQUIRREL_PEEKING) {
        aYPos +=
            TodAnimateCurve(50, 0, mGridItemCounter, 0, -40, TodCurves::CURVE_BOUNCE_SLOW_MIDDLE);
...
void GridItem::ClosePortal() {
    Reanimation* aPortalReanim = mApp->ReanimationTryToGet(mGridItemReanimID);
    if (aPortalReanim) {
        aPortalReanim->PlayReanim("anim_dissapear", ReanimLoopType::REANIM_PLAY_ONCE_AND_HOLD, 0,
                                  12.0f);
    }
...
void GridItem::UpdatePortal() {
    ...
    } else if (aPortalReanim->mLoopType == ReanimLoopType::REANIM_PLAY_ONCE_AND_HOLD &&
               aPortalReanim->mLoopCount > 0) {
...
void GridItem::Update() {
    ...
    if (mGridItemType == GridItemType::GRIDITEM_PORTAL_CIRCLE ||
        mGridItemType == GridItemType::GRIDITEM_PORTAL_SQUARE) {
        UpdatePortal();
    }
...
Zombie* GridItem::RakeFindZombie() {
    ...
    while (mBoard->IterateZombies(aZombie)) {
        if (!aZombie->IsDeadOrDying() && !aZombie->IsBobsledTeamWithSled() &&
            aZombie->mRow - mGridY == 0 &&
