...
CursorObject::CursorObject() {
    ...
    if (mApp->IsWhackAZombieLevel()) {
        ...
        Reanimation* aHammerReanim =
            mApp->AddReanimation(-25.0f, 16.0f, 0, ReanimationType::REANIM_HAMMER);
        aHammerReanim->mIsAttachment = true;
        aHammerReanim->PlayReanim("anim_whack_zombie", ReanimLoopType::REANIM_PLAY_ONCE_AND_HOLD, 0,
                                  24.0f);
...
void CursorObject::Draw(Graphics* g) {
    ...
    case CursorType::CURSOR_TYPE_PLANT_FROM_DUPLICATOR: {
        ...
        if (mApp->UsingGamepad()) {
            break; // CursorPreview draws the ghost plant in gamepad mode; skip this to avoid a
                   // double draw
        }
#endif
        ...
        Plant::DrawSeedType(g, mType, mImitaterType, DrawVariation::VARIATION_NORMAL, aOffsetX,
                            aOffsetY);
        break;
    }
...
void CursorPreview::Draw(Graphics* g) {
    ...
    if (mBoard->mCursorObject->mCursorType == CursorType::CURSOR_TYPE_WHEEELBARROW ||
        ...
        aPottedPlant =
            &mApp->mPlayerInfo->mPottedPlant
                 [mBoard->mPlants.DataArrayGet((unsigned int)mBoard->mCursorObject->mGlovePlantID)
                      ->mPottedPlantIndex];
    }
    ...
    if (aPottedPlant) {
        ...
        Plant::DrawSeedType(g, mBoard->mCursorObject->mType, mBoard->mCursorObject->mImitaterType,
                            DrawVariation::VARIATION_NORMAL, aOffsetX, aOffsetY);
    }

    if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_COLUMN) {
        for (int y = 0; y < MAX_GRID_SIZE_Y; y++) {
            if (y != mGridY &&
                mBoard->CanPlantAt(mGridX, y, aSeedType) == PlantingReason::PLANTING_OK) {
                float aOffsetY = 85.0f * (y - mGridY) +
                                 PlantDrawHeightOffset(mBoard, nullptr, aSeedType, mGridX, y);
                Plant::DrawSeedType(g, mBoard->mCursorObject->mType,
