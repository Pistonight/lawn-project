...
void ToolTipWidget::GetLines(std::vector<SexyString>& theLines) {
    ...
    while (aIndexInLine != mLabel.size()) {
        ...
        if (aIndexInLine != mLabel.size() && aLineWidth < mGetsLinesWidth &&
            mLabel[aIndexInLine] != '\n') {
            aLineWidth += FONT_PICO129->CharWidth(mLabel[aIndexInLine]);
            aIndexInLine++;
        } else {
...
void ToolTipWidget::Draw(Graphics* g) {
    ...
    if (!mTitle.empty()) {
        ...
        g->DrawString(mTitle, aPosX + (mWidth - FONT_TINYBOLD->StringWidth(mTitle)) / 2,
                      aPosY + FONT_TINYBOLD->GetAscent());
    ...
    for (int i = 0; i < aLines.size(); i++) {
        ...
        g->DrawString(aLine, aPosX + (mWidth - FONT_PICO129->StringWidth(aLine)) / 2,
                      aPosY + FONT_PICO129->GetAscent());
        aPosY += FONT_PICO129->GetAscent() + 2;
    }
}
