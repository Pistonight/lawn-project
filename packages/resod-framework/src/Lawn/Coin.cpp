...
void Coin::CoinInitialize(int theX, int theY, CoinType theCoinType, CoinMotion theCoinMotion) {
    ...
    if (IsSun()) {
        ...
        float aPosX = 9.0f;
        float aPosY = 9.0f;
        Reanimation* aCoinReanim =
            mApp->AddReanimation(0.0f, 0.0f, 0, ReanimationType::REANIM_COIN_SILVER);
        ...
        float aPosY = 9.0f;
        Reanimation* aCoinReanim =
            mApp->AddReanimation(0.0f, 0.0f, 0, ReanimationType::REANIM_COIN_GOLD);
        aCoinReanim->SetPosition(mPosX + aPosX, mPosY + aPosY);
        aCoinReanim->mLoopType = ReanimLoopType::REANIM_LOOP;
        aCoinReanim->mAnimTime = RandRangeFloat(0.0f, 0.99f);
        aCoinReanim->mAnimRate *= RandRangeFloat(0.6f, 1.0f);
        ...
        float aPosY = 4.0f;
        Reanimation* aCoinReanim =
            mApp->AddReanimation(0.0f, 0.0f, 0, ReanimationType::REANIM_DIAMOND);
        aCoinReanim->SetPosition(mPosX + aPosX, mPosY + aPosY);
        aCoinReanim->mLoopType = ReanimLoopType::REANIM_LOOP;
        aCoinReanim->mAnimTime = RandRangeFloat(0.0f, 0.99f);
    ...
    if (mType == CoinType::COIN_FINAL_SEED_PACKET) {
        ...
        if (mApp->IsSurvivalEndless(mApp->mGameMode) || mApp->IsEndlessIZombie(mApp->mGameMode) ||
            ...
            SeedType aSeedList[] = {SeedType::SEED_PEASHOOTER, SeedType::SEED_SUNFLOWER,
            ...
            SeedType aSeedList[] = {SeedType::SEED_LILYPAD,     SeedType::SEED_SQUASH,
            ...
            SeedType aSeedList[] = {SeedType::SEED_SEASHROOM,    SeedType::SEED_PLANTERN,
    ...
    case CoinMotion::COIN_MOTION_COIN:
        ...
        if (mType == CoinType::COIN_AWARD_SILVER_SUNFLOWER ||
            mType == CoinType::COIN_AWARD_GOLD_SUNFLOWER) {
            mPosY -= 100.0f;
        ...
        if (mType == CoinType::COIN_FINAL_SEED_PACKET ||
            mType == CoinType::COIN_USABLE_SEED_PACKET || mType == CoinType::COIN_TROPHY ||
            mType == CoinType::COIN_SHOVEL || mType == CoinType::COIN_CARKEYS ||
            mType == CoinType::COIN_ALMANAC || mType == CoinType::COIN_VASE ||
            mType == CoinType::COIN_WATERING_CAN || mType == CoinType::COIN_TACO ||
...
bool Coin::IsMoney(CoinType theType) {
    return theType == CoinType::COIN_SILVER || theType == CoinType::COIN_GOLD ||
...
bool Coin::IsSun() {
    return mType == CoinType::COIN_SUN || mType == CoinType::COIN_SMALLSUN ||
           mType == CoinType::COIN_LARGESUN;
}

bool Coin::IsPresentWithAdvice() {
    return mType == CoinType::COIN_PRESENT_MINIGAMES ||
           mType == CoinType::COIN_PRESENT_PUZZLE_MODE ||
...
void Coin::UpdateFall() {
    ...
    if (mCoinMotion == CoinMotion::COIN_MOTION_FROM_PRESENT) {
        ...
        if (mCoinMotion == CoinMotion::COIN_MOTION_FROM_PLANT) {
            mVelY += 0.09f;
        } else if (mCoinMotion == CoinMotion::COIN_MOTION_COIN ||
                   mCoinMotion == CoinMotion::COIN_MOTION_FROM_BOSS) {
            mVelY += 0.15f;
        }
        ...
        if (mNeedsBouncyArrow && !mHasBouncyArrow) {
            ...
            TodParticleSystem* aParticle = mApp->AddTodParticle(
                mPosX + aParticleOffsetX, mPosY + aParticleOffsetY, 0, aEffect);
        ...
        if (mApp->mGameMode != GameMode::GAMEMODE_CHALLENGE_LAST_STAND || mBoard == nullptr ||
            mBoard->mChallenge->mChallengeState ==
                ChallengeState::STATECHALLENGE_LAST_STAND_ONSLAUGHT) {
...
void Coin::UpdateCollected() {
    ...
    if (IsLevelAward()) {
        mScale = TodAnimateCurveFloat(0, 400, mDisappearCounter, 1.01f, 2.0f,
                                      TodCurves::CURVE_EASE_IN_OUT);
        mPosX = TodAnimateCurveFloat(0, 350, mDisappearCounter, mCollectX, aDestX,
                                     TodCurves::CURVE_EASE_OUT);
        mPosY = TodAnimateCurveFloat(0, 350, mDisappearCounter, mCollectY, aDestY,
                                     TodCurves::CURVE_EASE_OUT);
        return;
    }
    ...
    if (IsPresentWithAdvice()) {
        if (mCollectionDistance < 15.0f) {
            ...
            } else if (mBoard->mHelpIndex != AdviceType::ADVICE_UNLOCKED_MODE ||
                       !mBoard->mAdvice->IsBeingDisplayed()) {
                Die();
            }
        }
    } else {
...
void Coin::Update() {
    ...
    if (mApp->mGameScene != GameScenes::SCENE_PLAYING &&
        mApp->mGameScene != GameScenes::SCENE_AWARD && mBoard &&
    ...
    if (mAttachmentID != AttachmentID::ATTACHMENTID_NULL) {
        ...
        if ((!mHitGround || mIsBeingCollected) &&
...
void Coin::Draw(Graphics* g) {
    ...
    if ((mType == CoinType::COIN_SILVER || mType == CoinType::COIN_GOLD) && mHitGround &&
        !mIsBeingCollected) {
        return;
    }
    ...
    if (mType == CoinType::COIN_SILVER || mType == CoinType::COIN_GOLD) {
        g->SetColorizeImages(true);
        TodDrawImageCenterScaledF(g, IMAGE_REANIM_COINGLOW, mPosX - 14.0f, mPosY - 12.0f, mScale,
                                  mScale);
        g->SetColorizeImages(false);
    }
    ...
    if (mType == CoinType::COIN_SILVER) {
        ...
        DrawSeedPacket(g, 0.5f * (mWidth - mScale * mWidth) + mPosX,
                       0.5f * (mHeight - mScale * mHeight) + mPosY, aSeedType, SeedType::SEED_NONE,
                       0.0f, 255, true, false);
        g->SetScale(1.0f, 1.0f, 0.0f, 0.0f);
        return;
    } else if (mType == CoinType::COIN_PRESENT_PLANT || mType == CoinType::COIN_AWARD_PRESENT) {
        if (mIsBeingCollected) {
            mApp->mZenGarden->DrawPottedPlantIcon(g, mPosX + 10.0f, mPosY - 20.0f,
                                                  &mPottedPlantSpec);
            return;
        }
        ...
        g->SetColorizeImages(true);
        DrawSeedPacket(g, (int)mPosX, (int)mPosY, mUsableSeedType, SeedType::SEED_NONE, 0.0f,
                       aGrayness, false, false);
        g->SetColorizeImages(false);
    ...
    g->SetColorizeImages(true);
    TodDrawImageCelCenterScaledF(g, aImage, mPosX + aOffsetX, mPosY + aOffsetY, aImageCelCol, 0,
                                 aDrawScale, aDrawScale);
    g->SetColorizeImages(false);
}
...
void Coin::FanOutCoins(CoinType theCoinType, int theNumCoins) {
    ...
    for (int i = 0; i < theNumCoins; i++) {
        ...
        float aPosY = mPosY;
        Coin* aCoin = mBoard->AddCoin((int)aPosX, (int)aPosY, theCoinType,
                                      CoinMotion::COIN_MOTION_FROM_PRESENT);
...
void Coin::TryAutoCollectAfterLevelAward() {
    ...
    if (mType == CoinType::COIN_PRESENT_PLANT || mType == CoinType::COIN_CHOCOLATE ||
        IsPresentWithAdvice()) {
        aCanBeAutoCollected = true;
    }
...
void Coin::Collect() {
    ...
    if (mType == CoinType::COIN_PRESENT_PLANT || mType == CoinType::COIN_AWARD_PRESENT) {
        ...
        if (mApp->mZenGarden->IsZenGardenFull(false)) {
            mBoard->DisplayAdvice("[DIALOG_ZEN_GARDEN_FULL]", MessageStyle::MESSAGE_STYLE_HINT_FAST,
                                  AdviceType::ADVICE_NONE);
        } else {
            mBoard->mPottedPlantsCollected++;
            mBoard->DisplayAdvice("[ADVICE_FOUND_PLANT]", MessageStyle::MESSAGE_STYLE_HINT_FAST,
                                  AdviceType::ADVICE_NONE);
        ...
        if (aIsEndlessAward) {
            AttachmentDetachCrossFadeParticleType(
                mAttachmentID, ParticleEffect::PARTICLE_AWARD_PICKUP_ARROW, nullptr);
            mBoard->FadeOutLevel();
        }
    ...
    if (mType == CoinType::COIN_PRESENT_MINIGAMES) {
        ...
        AttachmentDetachCrossFadeParticleType(mAttachmentID,
                                              ParticleEffect::PARTICLE_AWARD_PICKUP_ARROW, nullptr);
    ...
    if (mType == CoinType::COIN_CHOCOLATE || mType == CoinType::COIN_AWARD_CHOCOLATE) {
        ...
        if (mApp->mPlayerInfo->mPurchases[(int)StoreItem::STORE_ITEM_CHOCOLATE] <
            ...
            mBoard->DisplayAdvice("[ADVICE_FOUND_CHOCOLATE]",
    ...
    if (IsLevelAward()) {
        ...
        mApp->AddTodParticle(mPosX + 30.0f, mPosY + 30.0f, mRenderOrder + 1,
                             ParticleEffect::PARTICLE_STARBURST);

        TOD_ASSERT(mBoard);

        mBoard->FadeOutLevel();
        AttachmentDetachCrossFadeParticleType(mAttachmentID, ParticleEffect::PARTICLE_SEED_PACKET,
                                              nullptr);
        AttachmentDetachCrossFadeParticleType(mAttachmentID,
                                              ParticleEffect::PARTICLE_AWARD_PICKUP_ARROW, nullptr);
        AttachmentDetachCrossFadeParticleType(mAttachmentID,
                                              ParticleEffect::PARTICLE_COIN_PICKUP_ARROW, nullptr);
        ...
        if (mType == CoinType::COIN_NOTE) {
            ...
            TodParticleSystem* aParticle =
                mApp->AddTodParticle(mPosX + aParticleOffsetX, mPosY + aParticleOffsetY,
    ...
    if (IsSun() && mBoard && !mBoard->HasConveyorBeltSeedBank()) {
        for (int i = 0; i < mBoard->mSeedBank->mNumPackets; i++) {
            ...
            int aCost =
                mBoard->GetCurrentPlantCost(aSeedPacket->mPacketType, aSeedPacket->mImitaterType);
    ...
    AttachmentDetachCrossFadeParticleType(mAttachmentID, ParticleEffect::PARTICLE_COIN_PICKUP_ARROW,
                                          nullptr);
    if (mApp->IsFirstTimeAdventureMode() && mBoard && mBoard->mLevel == 11 &&
        (mType == CoinType::COIN_GOLD || mType == CoinType::COIN_SILVER)) {
        mBoard->DisplayAdvice("[ADVICE_CLICKED_ON_COIN]", MessageStyle::MESSAGE_STYLE_HINT_FAST,
...
void Coin::PlayCollectSound() {
    ...
    if (mType == CoinType::COIN_CHOCOLATE || mType == CoinType::COIN_PRESENT_PLANT ||
        IsPresentWithAdvice() || mType == CoinType::COIN_AWARD_PRESENT ||
...
void Coin::MouseDown(int x, int y, int theClickCount) {
    if (mBoard == nullptr || mBoard->mPaused || mApp->mGameScene != GameScenes::SCENE_PLAYING ||
        mDead) {
        return;
    }
...
bool Coin::IsLevelAward() {
    return mType == CoinType::COIN_FINAL_SEED_PACKET || mType == CoinType::COIN_TROPHY ||
           ...
           mType == CoinType::COIN_AWARD_GOLD_SUNFLOWER || mType == CoinType::COIN_SHOVEL ||
           mType == CoinType::COIN_CARKEYS || mType == CoinType::COIN_ALMANAC ||
           mType == CoinType::COIN_VASE || mType == CoinType::COIN_WATERING_CAN ||
           mType == CoinType::COIN_TACO || mType == CoinType::COIN_NOTE ||
           mType == CoinType::COIN_AWARD_MONEY_BAG || mType == CoinType::COIN_AWARD_BAG_DIAMOND ||
...
int Coin::GetDisappearTime() {
    ...
    if (mType == CoinType::COIN_DIAMOND || mType == CoinType::COIN_PRESENT_PLANT ||
        mType == CoinType::COIN_CHOCOLATE || mHasBouncyArrow) {
        aTime = 1500;
    }
