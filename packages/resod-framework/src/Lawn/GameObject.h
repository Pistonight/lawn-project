...
public:
    GameObject();
    /// @brief Begin the drawing a GameObject by translating the Graphics object by it's X and Y
    /// values
    /// @return True if it began to draw it, False if it was invisible
    bool BeginDraw(Graphics* g);
    /// @brief End the drawing a GameObject by translating the Graphics object by the negative of
    /// it's X and Y values
