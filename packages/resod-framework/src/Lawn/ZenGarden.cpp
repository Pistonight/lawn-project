...
static SpecialGridPlacement gGreenhouseGridPlacement[] = {
    {73, 73, 0, 0},   {155, 71, 1, 0},  {239, 68, 2, 0},  {321, 73, 3, 0},  {406, 71, 4, 0},
    {484, 67, 5, 0},  {566, 70, 6, 0},  {648, 72, 7, 0},  {67, 168, 0, 1},  {150, 165, 1, 1},
    {232, 170, 2, 1}, {314, 175, 3, 1}, {416, 173, 4, 1}, {497, 170, 5, 1}, {578, 164, 6, 1},
    {660, 168, 7, 1}, {41, 268, 0, 2},  {130, 266, 1, 2}, {219, 260, 2, 2}, {310, 266, 3, 2},
    {416, 267, 4, 2}, {504, 261, 5, 2}, {594, 265, 6, 2}, {684, 269, 7, 2}, {37, 371, 0, 3},
    {124, 369, 1, 3}, {211, 368, 2, 3}, {302, 369, 3, 3}, {425, 375, 4, 3}, {512, 368, 5, 3},
    {602, 365, 6, 3}, {691, 368, 7, 3}};

static SpecialGridPlacement gMushroomGridPlacement[] = {
    {110, 441, 0, 0}, {237, 360, 1, 0}, {298, 458, 2, 0}, {355, 296, 3, 0},
    {387, 203, 4, 0}, {460, 385, 5, 0}, {486, 478, 6, 0}, {552, 283, 7, 0}};

static SpecialGridPlacement gAquariumGridPlacement[] = {
    {113, 185, 0, 0}, {306, 120, 1, 0}, {356, 270, 2, 0}, {622, 120, 3, 0},
    {669, 270, 4, 0}, {122, 355, 5, 0}, {365, 458, 6, 0}, {504, 417, 7, 0}};
...
void ZenGarden::DrawPottedPlant(Graphics* g, float x, float y, PottedPlant* thePottedPlant,
                                float theScale, bool theDrawPot) {
    ...
    } else if ((aSeedType == SeedType::SEED_TANGLEKELP || aSeedType == SeedType::SEED_SEASHROOM) &&
               thePottedPlant->mWhichZenGarden == GardenType::GARDEN_AQUARIUM) {
    ...
    if (theDrawPot) {
        ...
        Plant::DrawSeedType(&aPottedPlantG, SeedType::SEED_FLOWERPOT, SeedType::SEED_NONE,
                            aPotVariation2, x, y + aPotOffsetY * theScale);
    }
    ...
    Plant::DrawSeedType(&aPottedPlantG, aSeedType, SeedType::SEED_NONE, aPlantVariation,
                        x + aOffsetX, y + aOffsetY);
}
...
Plant* ZenGarden::PlacePottedPlant(int thePottedPlantIndex) {
    ...
    if (needPot) {
        Plant* aPot = mBoard->NewPlant(aPottedPlant->mX, aPottedPlant->mY, SeedType::SEED_FLOWERPOT,
                                       SeedType::SEED_NONE);
    ...
    Plant* aPlant =
        mBoard->NewPlant(aPottedPlant->mX, aPottedPlant->mY, aSeedType, SeedType::SEED_NONE);
    aPlant->mPottedPlantIndex = thePottedPlantIndex;
    aPlant->mRenderOrder =
        Board::MakeRenderOrder(RenderLayer::RENDER_LAYER_PLANT, 0, aPlant->mY + 1);
        ...
        if (aSeedType == SeedType::SEED_SPROUT) {
            if (aPottedPlant->mSeedType != SeedType::SEED_MARIGOLD) {
                aPlantReanim->SetFramesForLayer("anim_idle_noflower");
                    ...
                    aSeedType == SeedType::SEED_SEASHROOM) &&
            ...
            mApp->mReanimatorCache->UpdateReanimationForVariation(aPlantReanim,
...
void ZenGarden::RemovePottedPlant(Plant* thePlant) {
    thePlant->Die();
    Plant* aPot = mBoard->GetTopPlantAt(thePlant->mPlantCol, thePlant->mRow,
...
PottedPlant* ZenGarden::PottedPlantFromIndex(int thePottedPlantIndex) {
    TOD_ASSERT(thePottedPlantIndex >= 0 &&
               thePottedPlantIndex < mApp->mPlayerInfo->mNumPottedPlants);
...
bool ZenGarden::PlantCanHaveChocolate(Plant* thePlant) {
    ...
    return aPottedPlant->mPlantAge == PottedPlantAge::PLANTAGE_FULL &&
           WasPlantNeedFulfilledToday(aPottedPlant) && !PlantHighOnChocolate(aPottedPlant);
}
...
bool ZenGarden::IsZenGardenFull(bool theIncludeDroppedPresents) {
    ...
    return aNumDroppedPresents + aNumPottedPlantsInGarden >=
           ZEN_MAX_GRIDSIZE_X * ZEN_MAX_GRIDSIZE_Y;
}
    ...
    for (int x = 0; x < ZEN_MAX_GRIDSIZE_X; x++) {
        for (int y = 0; y < ZEN_MAX_GRIDSIZE_Y; y++) {
            ...
            for (int i = 0; i < mApp->mPlayerInfo->mNumPottedPlants; i++) {
                PottedPlant* aPottedPlant = PottedPlantFromIndex(i);
                if (aPottedPlant->mWhichZenGarden == GardenType::GARDEN_MAIN &&
                    aPottedPlant->mX == x && aPottedPlant->mY == y) {
                    continue;
                }
            }
...
int ZenGarden::GetPlantSellPrice(Plant* thePlant) {
    ...
    if (aPottedPlant->mPlantAge == PottedPlantAge::PLANTAGE_FULL) {
        if (Plant::IsNocturnal(aPottedPlant->mSeedType) ||
            Plant::IsAquatic(aPottedPlant->mSeedType)) {
            return 1000;
        }
        return 800;
    }
...
void ZenGarden::MouseDownWithMoneySign(Plant* thePlant) {
    ...
    if (thePlant->mSeedType == SeedType::SEED_SPROUT &&
        aPottedPlant->mSeedType == SeedType::SEED_MARIGOLD) {
    ...
    aCrazyDaveReanim->PlayReanim("anim_blahblah", ReanimLoopType::REANIM_PLAY_ONCE_AND_HOLD, 20,
                                 12.0f);

    Dialog* aDialog =
        mApp->DoDialog(Dialogs::DIALOG_ZEN_SELL, true, aHeader, aLines, "", Dialog::BUTTONS_YES_NO);
    aDialog->mX += 120;
    aDialog->mY += 60;
    mBoard->ShowCoinBank();
    ...
    if (aResult == Dialog::ID_YES) {
        ...
        int aNumPlantsAfterThis =
            mApp->mPlayerInfo->mNumPottedPlants - thePlant->mPottedPlantIndex - 1;
...
void ZenGarden::PlantFertilized(Plant* thePlant) {
    ...
    if (aPottedPlant->mPlantAge == PottedPlantAge::PLANTAGE_SMALL) {
        mBoard->AddCoin(thePlant->mX + 40, thePlant->mY, CoinType::COIN_GOLD,
                        CoinMotion::COIN_MOTION_COIN);
    } else if (aPottedPlant->mPlantAge == PottedPlantAge::PLANTAGE_MEDIUM) {
        mBoard->AddCoin(thePlant->mX + 30, thePlant->mY, CoinType::COIN_GOLD,
                        CoinMotion::COIN_MOTION_COIN);
        mBoard->AddCoin(thePlant->mX + 50, thePlant->mY, CoinType::COIN_GOLD,
                        CoinMotion::COIN_MOTION_COIN);
    } else if (aPottedPlant->mPlantAge == PottedPlantAge::PLANTAGE_FULL) {
        if (aPottedPlant->mSeedType == SeedType::SEED_MARIGOLD) {
            mBoard->AddCoin(thePlant->mX + 40, thePlant->mY, CoinType::COIN_DIAMOND,
                            CoinMotion::COIN_MOTION_COIN);
        } else {
            mBoard->AddCoin(thePlant->mX + 10, thePlant->mY, CoinType::COIN_DIAMOND,
                            CoinMotion::COIN_MOTION_COIN);
            mBoard->AddCoin(thePlant->mX + 70, thePlant->mY, CoinType::COIN_DIAMOND,
                            CoinMotion::COIN_MOTION_COIN);
        }
    }
}
...
void ZenGarden::PlantFulfillNeed(Plant* thePlant) {
    ...
    if (Plant::IsNocturnal(thePlant->mSeedType) || Plant::IsAquatic(thePlant->mSeedType)) {
        mBoard->AddCoin(thePlant->mX + 10, thePlant->mY, CoinType::COIN_GOLD,
                        CoinMotion::COIN_MOTION_COIN);
        mBoard->AddCoin(thePlant->mX + 70, thePlant->mY, CoinType::COIN_GOLD,
                        CoinMotion::COIN_MOTION_COIN);
    }
}
...
int ZenGarden::CountPlantsNeedingFertilizer() {
    ...
    for (int i = 0; i < mApp->mPlayerInfo->mNumPottedPlants; i++) {
        PottedPlant* aPottedPlant = PottedPlantFromIndex(i);
        if (mApp->mZenGarden->GetPlantsNeed(aPottedPlant) ==
            PottedPlantNeed::PLANTNEED_FERTILIZER) {
            aCount++;
        }
    }
    return aCount;
...
void ZenGarden::PlantWatered(Plant* thePlant) {
    ...
    mApp->PlayFoley(FoleyType::FOLEY_SPAWN_SUN);
    mBoard->AddCoin(thePlant->mX + 40, thePlant->mY, CoinType::COIN_SILVER,
                    CoinMotion::COIN_MOTION_COIN);
    if (aPottedPlant->mPlantAge == PottedPlantAge::PLANTAGE_FULL &&
        aPottedPlant->mPlantNeed == PottedPlantNeed::PLANTNEED_NONE) {
        aPottedPlant->mPlantNeed = (PottedPlantNeed)RandRangeInt(
            (int)PottedPlantNeed::PLANTNEED_BUGSPRAY, (int)PottedPlantNeed::PLANTNEED_PHONOGRAPH);
    }
...
void ZenGarden::UpdatePlantEffectState(Plant* thePlant) {
    ...
    Plant* aFlowerPot = mBoard->GetTopPlantAt(thePlant->mPlantCol, thePlant->mRow,
                                              PlantPriority::TOPPLANT_ONLY_UNDER_PLANT);
...
void ZenGarden::AddHappyEffect(Plant* thePlant) {
    ...
    if (aFlowerPot == nullptr) {
        thePlant->AddAttachedParticle(thePlant->mX + 40, thePlant->mY + 60,
                                      thePlant->mRenderOrder - 1,
                                      ParticleEffect::PARTICLE_POTTED_ZEN_GLOW);
    } else if (Plant::IsAquatic(thePlant->mSeedType)) {
        aFlowerPot->AddAttachedParticle(aFlowerPot->mX + 40, aFlowerPot->mY + 61,
                                        aFlowerPot->mRenderOrder - 1,
                                        ParticleEffect::PARTICLE_POTTED_WATER_PLANT_GLOW);
    } else {
        aFlowerPot->AddAttachedParticle(aFlowerPot->mX + 40, aFlowerPot->mY + 63,
                                        aFlowerPot->mRenderOrder - 1,
                                        ParticleEffect::PARTICLE_POTTED_ZEN_GLOW);
    }
}
...
#endif
    return aNowTM.tm_year <= aLastNeedFulfilledTM.tm_year &&
           aNowTM.tm_yday <= aLastNeedFulfilledTM.tm_yday;
}
...
void ZenGarden::RefreshPlantNeeds(PottedPlant* thePottedPlant) {
    if (thePottedPlant->mPlantAge != PottedPlantAge::PLANTAGE_FULL ||
        !PlantShouldRefreshNeed(thePottedPlant)) {
        return;
    }

    if (Plant::IsAquatic(thePottedPlant->mSeedType)) {
        thePottedPlant->mLastWateredTime = std::time(nullptr);
        thePottedPlant->mPlantNeed = (PottedPlantNeed)RandRangeInt(
            (int)PottedPlantNeed::PLANTNEED_BUGSPRAY, (int)PottedPlantNeed::PLANTNEED_PHONOGRAPH);
    } else {
...
PottedPlantNeed ZenGarden::GetPlantsNeed(PottedPlant* thePottedPlant) {
    ...
    if (WasPlantFertilizedInLastHour(thePottedPlant) ||
        WasPlantNeedFulfilledToday(thePottedPlant)) {
        return PottedPlantNeed::PLANTNEED_NONE;
    }
    if (Plant::IsAquatic(thePottedPlant->mSeedType) &&
        thePottedPlant->mPlantAge != PottedPlantAge::PLANTAGE_SPROUT) {
        if (thePottedPlant->mPlantAge == PottedPlantAge::PLANTAGE_FULL) {
...
void ZenGarden::MouseDownWithFeedingTool(int x, int y, CursorType theCursorType) {
    ...
    if (theCursorType == CursorType::CURSOR_TYPE_CHOCOLATE) {
        TOD_ASSERT(mApp->mPlayerInfo->mPurchases[(int)StoreItem::STORE_ITEM_CHOCOLATE] >
                   PURCHASE_COUNT_OFFSET);

        GridItem* aStinky = GetStinky();
        if (aStinky && aStinky->mHighlighted) {
            WakeStinky();
            mApp->AddTodParticle(aStinky->mPosX + 40.0f, aStinky->mPosY + 40.0f,
    ...
    if (aPlantToFeed) {
        ...
        if (theCursorType == CursorType::CURSOR_TYPE_WATERING_CAN) {
            if (mApp->mPlayerInfo->mPurchases[(int)StoreItem::STORE_ITEM_GOLD_WATERINGCAN]) {
                ...
                aWateringCanReanim->PlayReanim("anim_water_area",
                                               ReanimLoopType::REANIM_PLAY_ONCE_AND_HOLD, 0, 8.0f);
                ...
                Reanimation* aWateringCanReanim =
                    mApp->AddReanimation(aPlantToFeed->mX + 32, aPlantToFeed->mY, 0,
                                         ReanimationType::REANIM_ZENGARDEN_WATERINGCAN);
                aWateringCanReanim->PlayReanim("anim_water",
                                               ReanimLoopType::REANIM_PLAY_ONCE_AND_HOLD, 0, 0.0f);
                aZenTool->mGridItemReanimID = mApp->ReanimationGetID(aWateringCanReanim);
                aZenTool->mGridItemState = GridItemState::GRIDITEM_STATE_ZEN_TOOL_GOLD_WATERING_CAN;
                mApp->PlayFoley(FoleyType::FOLEY_WATERING);
            }
            ...
            Reanimation* aFertilizerReanim =
                mApp->AddReanimation(aPlantToFeed->mX, aPlantToFeed->mY, 0,
                                     ReanimationType::REANIM_ZENGARDEN_FERTILIZER);
            ...
            TOD_ASSERT(mApp->mPlayerInfo->mPurchases[(int)StoreItem::STORE_ITEM_FERTILIZER] >
                       PURCHASE_COUNT_OFFSET);
            mApp->mPlayerInfo->mPurchases[(int)StoreItem::STORE_ITEM_FERTILIZER]--;
        } else if (theCursorType == CursorType::CURSOR_TYPE_BUG_SPRAY) {
            Reanimation* aBugSprayReanim =
                mApp->AddReanimation(aPlantToFeed->mX + 54, aPlantToFeed->mY, 0,
                                     ReanimationType::REANIM_ZENGARDEN_BUGSPRAY);
            ...
            TOD_ASSERT(mApp->mPlayerInfo->mPurchases[(int)StoreItem::STORE_ITEM_BUG_SPRAY] >
                       PURCHASE_COUNT_OFFSET);
            mApp->mPlayerInfo->mPurchases[(int)StoreItem::STORE_ITEM_BUG_SPRAY]--;
        } else if (theCursorType == CursorType::CURSOR_TYPE_PHONOGRAPH) {
            Reanimation* aPhonographReanim =
                mApp->AddReanimation(aPlantToFeed->mX + 20, aPlantToFeed->mY + 34, 0,
                                     ReanimationType::REANIM_ZENGARDEN_PHONOGRAPH);
...
void ZenGarden::FeedChocolateToPlant(Plant* thePlant) {
    ...
    mApp->AddTodParticle(thePlant->mX + 40.0f, thePlant->mY + 40.0f, thePlant->mRenderOrder + 1,
                         ParticleEffect::PARTICLE_PRESENT_PICKUP);
}
    ...
    if (aPlant) {
        ...
        if (aNeed == PottedPlantNeed::PLANTNEED_WATER &&
            ...
            PlantFertilized(aPlant);
                   ...
                   theToolType == GridItemState::GRIDITEM_STATE_ZEN_TOOL_BUG_SPRAY) {
            PlantFulfillNeed(aPlant);
        } else if (aNeed == PottedPlantNeed::PLANTNEED_PHONOGRAPH &&
                   theToolType == GridItemState::GRIDITEM_STATE_ZEN_TOOL_PHONOGRAPH) {
            PlantFulfillNeed(aPlant);
        }
        ...
        if (mBoard->mTutorialState == TutorialState::TUTORIAL_ZEN_GARDEN_FERTILIZE_PLANTS &&
            ...
            } else if (mApp->mPlayerInfo->mPurchases[(int)StoreItem::STORE_ITEM_FERTILIZER] ==
                       PURCHASE_COUNT_OFFSET) {
                mApp->mPlayerInfo->mPurchases[(int)StoreItem::STORE_ITEM_FERTILIZER] =
                    PURCHASE_COUNT_OFFSET + 5;
...
void ZenGarden::MovePlant(Plant* thePlant, int theGridX, int theGridY) {
    ...
    thePlant->SetSleeping(false);
    Plant* aTopPlantAtGrid = mBoard->GetTopPlantAt(thePlant->mPlantCol, thePlant->mRow,
                                                   PlantPriority::TOPPLANT_ONLY_UNDER_PLANT);
    if (aTopPlantAtGrid) {
        ...
        aTopPlantAtGrid->mRenderOrder =
            Board::MakeRenderOrder(RenderLayer::RENDER_LAYER_PLANT, 0, aPosY);
    }
    ...
    if (aParticle && aParticle->mEmitterList.mSize) {
        ...
        aParticle->SystemMove(aEmitter->mSystemCenter.x + aDeltaX,
                              aEmitter->mSystemCenter.y + aDeltaY);
    }
...
float ZenGarden::PlantPottedDrawHeightOffset(SeedType theSeedType, float theScale) {
    ...
    if (theSeedType == SeedType::SEED_GRAVEBUSTER) {
        ...
        aScaleOffsetFix += 0.0f;
               ...
               theSeedType == SeedType::SEED_MARIGOLD || theSeedType == SeedType::SEED_PEASHOOTER ||
               theSeedType == SeedType::SEED_REPEATER || theSeedType == SeedType::SEED_LEFTPEATER ||
               theSeedType == SeedType::SEED_SNOWPEA || theSeedType == SeedType::SEED_THREEPEATER ||
               theSeedType == SeedType::SEED_SUNFLOWER || theSeedType == SeedType::SEED_MARIGOLD) {
...
void ZenGarden::AddStinky() {
    ...
    aStinky->mRenderOrder =
        Board::MakeRenderOrder(RenderLayer::RENDER_LAYER_PLANT, 0, aStinky->mPosY - 30.0f);
    aReanimStinky->SetPosition(aStinky->mPosX, aStinky->mPosY);
}

void ZenGarden::StinkyPickGoal(GridItem* theStinky) {
    float aCurDistToGoal =
        Distance2D(theStinky->mGoalX, theStinky->mGoalY, theStinky->mPosX, theStinky->mPosY);
    ...
    float aCurWeight = 0.0f;
    {
        ...
        while (mBoard->IterateCoins(aCoin)) {
            if (!aCoin->mIsBeingCollected && aCoin->mPosY == aCoin->mGroundY) {
                float aWeight = Distance2D(aCoin->mPosX, aCoin->mPosY + 30.0f, theStinky->mPosX,
                                           theStinky->mPosY);
                ...
                float aDistFromLastGoal = Distance2D(aCoin->mPosX, aCoin->mPosY + 30.0f,
                                                     theStinky->mGoalX, theStinky->mGoalY);
                if (aDistFromLastGoal < 5.0f) {
                    aWeight -= 20.0f;
                    aWeight += TodAnimateCurve(3000, 6000, aCoin->mDisappearCounter, 0, -40,
    ...
    if (aBestCoin) {
        ...
        for (int i = 0; i < aCount; i++) {
            ...
            Plant* aPlant =
                mBoard->GetTopPlantAt(aGrid.mGridX, aGrid.mGridY, PlantPriority::TOPPLANT_ANY);
    ...
    } else if (theStinky->mGoalX > theStinky->mPosX &&
...
void ZenGarden::StinkyFinishFallingAsleep(GridItem* theStinky, int theBlendTime) {
    Reanimation* aStinkyReanim = mApp->ReanimationGet(theStinky->mGridItemReanimID);
    aStinkyReanim->PlayReanim("anim_out", ReanimLoopType::REANIM_PLAY_ONCE_AND_HOLD, theBlendTime,
                              0.0f);
    aStinkyReanim->mAnimRate = 0.0f;

    Reanimation* aSleepingReanim =
        mApp->AddReanimation(0.0f, 0.0f, 0, ReanimationType::REANIM_SLEEPING);
    aSleepingReanim->mAnimRate = 3.0f;
    aSleepingReanim->mLoopType = ReanimLoopType::REANIM_LOOP;
    AttachReanim(aStinkyReanim->GetTrackInstanceByName("shell")->mAttachmentID, aSleepingReanim,
                 34.0f, 39.0f);

    theStinky->mGridItemState = GridItemState::GRIDITEM_STINKY_SLEEPING;
    if (!gLawnApp->mPlayerInfo->mHasWokenStinky) {
        mApp->mBoard->DisplayAdvice("[ADVICE_STINKY_SLEEPING]",
                                    MessageStyle::MESSAGE_STYLE_HINT_LONG,
                                    AdviceType::ADVICE_STINKY_SLEEPING);
    }
}
...
void ZenGarden::UpdateStinkyMotionTrail(GridItem* theStinky, bool theStinkyHighOnChocolate) {
    ...
    if (theStinky->mMotionTrailCount > 0) {
        memcpy(theStinky->mMotionTrailFrames + 1, theStinky->mMotionTrailFrames,
...
void ZenGarden::StinkyUpdate(GridItem* theStinky) {
    ...
    Coin* aCoin = nullptr;
    while (mBoard->IterateCoins(aCoin)) {
        if (!aCoin->mIsBeingCollected && Distance2D(aCoin->mPosX, aCoin->mPosY + 30.0f,
                                                    theStinky->mPosX, theStinky->mPosY) < 20.0f) {
    ...
    aSpeedY *=
        TodAnimateCurveFloatTime(20.0f, 5.0f, fabs(aDeltaY), 1.0f, 0.2f, TodCurves::CURVE_LINEAR);
    if (theStinky->mGridItemState == GridItemState::GRIDITEM_STINKY_WALKING_LEFT) {
    ...
    theStinky->mRenderOrder =
        Board::MakeRenderOrder(RenderLayer::RENDER_LAYER_PLANT, 0, theStinky->mPosY - 30.0f);
}
...
void ZenGarden::ZenGardenUpdate() {
    ...
    if (mBoard->mTutorialState == TutorialState::TUTORIAL_ZEN_GARDEN_KEEP_WATERING &&
        ...
        mBoard->DisplayAdvice("[ADVICE_ZEN_GARDEN_VISIT_STORE]",
                              MessageStyle::MESSAGE_STYLE_HINT_TALL_LONG, AdviceType::ADVICE_NONE);
...
void ZenGarden::MouseDownWithFullWheelBarrow(int x, int y) {
    ...
    if (mApp->mZenGarden->mGardenType == GardenType::GARDEN_AQUARIUM &&
        !Plant::IsAquatic(aPottedPlant->mSeedType)) {
        mBoard->DisplayAdvice("[ZEN_ONLY_AQUATIC_PLANTS]",
                              MessageStyle::MESSAGE_STYLE_HINT_TALL_FAST, AdviceType::ADVICE_NONE);
        return;
    }
    ...
    if (aGridX == -1 || aGridY == -1 ||
        mBoard->CanPlantAt(aGridX, aGridY, aPottedPlant->mSeedType) !=
    ...
    int aPottedPlantIndex =
        (int)((uintptr_t)aPottedPlant - (uintptr_t)mApp->mPlayerInfo->mPottedPlant) /
        (int)sizeof(PottedPlant);
    Plant* aPlant = PlacePottedPlant(aPottedPlantIndex);
    mBoard->DoPlantingEffects(aPottedPlant->mX, aPottedPlant->mY, aPlant);
}
...
void ZenGarden::DrawBackdrop(Graphics* g) {
    ...
    if (mBoard->mCursorObject->mCursorType == CursorType::CURSOR_TYPE_PLANT_FROM_WHEEL_BARROW ||
        ...
        for (int i = 0; i < aCount; i++) {
            SpecialGridPlacement& aGrid = aSpecialGrids[i];
            if (mBoard->GetTopPlantAt(aGrid.mGridX, aGrid.mGridY,
                                      PlantPriority::TOPPLANT_ZEN_TOOL_ORDER) == nullptr) {
                TodDrawImageCelScaled(g, IMAGE_PLANTSHADOW, aGrid.mPixelX - 35,
                                      (aGrid.mPixelY + 33), 0, 0, 1.7f, 1.7f);
            }
        }
    }
}
...
void ZenGarden::ShowTutorialArrowOnWateringCan() {
    ...
    mBoard->DisplayAdvice("[ADVICE_ZEN_GARDEN_PICK_UP_WATER]",
                          MessageStyle::MESSAGE_STYLE_ZEN_GARDEN_LONG, AdviceType::ADVICE_NONE);
...
bool ZenGarden::MouseDownZenGarden(int x, int y, int theClickCount, HitResult* theHitResult) {
    ...
    if (theHitResult->mObjectType == GameObjectType::OBJECT_TYPE_STINKY &&
        ...
        if (mBoard->CanUseGameObject(GameObjectType::OBJECT_TYPE_WHEELBARROW)) {
            ...
            if (aButtonRect.Contains(x, y) && aPottedPlant) {
                ...
                mBoard->mCursorObject->mCursorType =
    ...
    } else if (theHitResult->mObjectType == GameObjectType::OBJECT_TYPE_NONE &&
               mBoard->mCursorObject->mCursorType == CursorType::CURSOR_TYPE_NORMAL &&
...
void ZenGarden::SetPlantAnimSpeed(Plant* thePlant) {
    ...
    float aTargetRate;
    if (thePlant->mSeedType == SeedType::SEED_PEASHOOTER ||
        thePlant->mSeedType == SeedType::SEED_SNOWPEA ||
        thePlant->mSeedType == SeedType::SEED_REPEATER ||
        thePlant->mSeedType == SeedType::SEED_LEFTPEATER ||
        thePlant->mSeedType == SeedType::SEED_GATLINGPEA ||
        thePlant->mSeedType == SeedType::SEED_SPLITPEA ||
        thePlant->mSeedType == SeedType::SEED_THREEPEATER ||
        thePlant->mSeedType == SeedType::SEED_MARIGOLD) {
...
void ZenGarden::PlantUpdateProduction(Plant* thePlant) {
    ...
    if (thePlant->mLaunchCounter <= 0) {
        ...
        aCoinHit += TodAnimateCurve(5, 30, PlantGetMinutesSinceHappy(thePlant), 0, 80,
                                    TodCurves::CURVE_LINEAR);
...
void ZenGarden::PottedPlantUpdate(Plant* thePlant) {
    ...
    if (aPottedPlant->mPlantAge == PottedPlantAge::PLANTAGE_FULL &&
        WasPlantNeedFulfilledToday(aPottedPlant)) {
...
bool ZenGarden::ShouldStinkyBeAwake() {
    ...
    return time(nullptr) -
               mApp->mPlayerInfo->mPurchases[(int)StoreItem::STORE_ITEM_STINKY_THE_SNAIL] <
           180;
}
...
SeedType ZenGarden::PickRandomSeedType() {
    ...
    return (SeedType)TodPickFromArray(aSeedList, aSeedCount);
}
