...
static const CoinType kCTypes[] = {COIN_SILVER, COIN_GOLD,     COIN_DIAMOND,
                                   COIN_SUN,    COIN_SMALLSUN, COIN_LARGESUN};

static const CoinMotion kCMotionTypes[] = {
    COIN_MOTION_FROM_SKY,       COIN_MOTION_FROM_SKY_SLOW, COIN_MOTION_FROM_PLANT, COIN_MOTION_COIN,
    COIN_MOTION_LAWNMOWER_COIN, COIN_MOTION_FROM_PRESENT,  COIN_MOTION_FROM_BOSS};
...
void DebuggerWindow::DrawBoardCheats() {
    ...
    ImGui::SetNextItemWidth(130);
    {
        const char* motionPreview = CoinMotionName(
            kCMotionTypes[mSpawnCoinMotion < NUM_COIN_MOTIONS ? mSpawnCoinMotion : 0]);
    ...
    if (ImGui::Button("Spawn Coin")) {
        b->AddCoin(BOARD_WIDTH / 2, BOARD_HEIGHT / 2, kCTypes[mSpawnCoinType],
                   kCMotionTypes[mSpawnCoinMotion]);
    }
...
void DebuggerWindow::DrawBoardPlants() {
    ...
    if (mApp->mBoard->mDebugSelectedPlant) {
        ...
        ImGui::Text("Selected: %s  Col=%d Row=%d", SeedTypeName(p->mSeedType), p->mPlantCol,
                    p->mRow);
...
void DebuggerWindow::DrawBoardZombies() {
    ...
    ImGui::SeparatorText("Debug Spawn");
    {
        ...
        if (ImGui::Button("Spawn##z"))
            b->AddZombieInRow((ZombieType)mSpawnZombieTypeIdx, mSpawnZombieRow,
...
void DebuggerWindow::DrawBoardWaves() {
    Board* b = mApp->mBoard;
    ImGui::Text("Waves: %d  Current: %d  Spawned: %d", b->mNumWaves, b->mCurrentWave,
                b->mTotalSpawnedWaves);
            ...
            for (int s = 0; s < MAX_ZOMBIES_IN_WAVE; s++) {
                ...
                ImGui::TableSetColumnIndex(1);
                {
                    ...
                    if (ImGui::SmallButton("Spawn Now"))
                        b->AddZombieInRow(
                            (ZombieType)b->mZombiesInWave[wi][s],
    ...
    if (ImGui::BeginTabBar("SaveTabs")) {
        if (ImGui::BeginTabItem("Info")) {
            ...
            if (ImGui::Button("Full Unlock")) {
                ...
                pi->mHasUnlockedMinigames = pi->mHasUnlockedPuzzleMode =
                    pi->mHasUnlockedSurvivalMode = true;
                pi->mHasNewMiniGame = pi->mHasNewScaryPotter = pi->mHasNewIZombie =
                    pi->mHasNewSurvival = true;
                pi->mCoins += 100000;
...
void DebuggerWindow::DrawAppTab() {
    ...
    {
        static const GameMode kModes[] = {GAMEMODE_ADVENTURE,
        ...
        static const char* kModeNames[] = {
            "Adventure",    "Surv Normal", "Surv Hard",  "Surv Endless", "War&Peas",
            "Wallnut Bowl", "Last Stand",  "Final Boss", "Zen Garden",   "Tree Wisdom"};
    ...
    if (mApp->mPlayerInfo) {
        ...
        ImGui::Text("Name: %s  Coins: %d  Level: %d", mApp->mPlayerInfo->mName.c_str(),
                    mApp->mPlayerInfo->mCoins, mApp->mPlayerInfo->mLevel);
    }
}
