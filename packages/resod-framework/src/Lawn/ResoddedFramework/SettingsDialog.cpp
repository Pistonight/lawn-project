...
SettingsDialog::SettingsDialog(LawnApp* theApp)
    ...
    mHighQualityCheckbox =
        MakeNewCheckbox(SettingsDialog::SETTINGS_HIGHQUALITY, this, theApp->mIs3D);

    mHighResolutionCheckbox =
        MakeNewCheckbox(SettingsDialog::SETTINGS_HIGHRESOLUTION, this, theApp->mHighResolution);
    ...
    for (int i = OutputFilteringMode::MODE_NEAREST; i < OutputFilteringMode::NUM_MODES; i++) {
        mFilterList->AddLine(StrFormat("%s", TodStringTranslate(gTranslationFilterKeys[i]).c_str()),
                             false);
    }
    ...
    for (const auto& aSize : mValidSizes) {
        ...
        if (mApp->mRenderer->mPresentationRect.mWidth == aSize.mWidth &&
            mApp->mRenderer->mPresentationRect.mHeight == aSize.mHeight)
...
void SettingsDialog::Draw(Graphics* g) {
    ...
    TodDrawString(g, "[SETTINGS_VIDEO]", 20, 20 + aY, Sexy::FONT_BRIANNETOD12, Color::White,
    ...
    mVSyncCheckbox->mDisabled =
        (mVSyncCheckbox->mY + mY + mVSyncCheckbox->mHeight) <
    ...
    TodDrawString(g, "[SETTINGS_VSYNC]", mVSyncCheckbox->mX + 20, aY + 30, Sexy::FONT_BRIANNETOD12,
    ...
    mFullscreenCheckbox->mDisabled =
        (mFullscreenCheckbox->mY + mY + mFullscreenCheckbox->mHeight) <
            mOptionsSlider->mAllowedMouseZone.mY ||
        (mFullscreenCheckbox->mY + mY) >
            (mOptionsSlider->mAllowedMouseZone.mY + mOptionsSlider->mAllowedMouseZone.mHeight);

    TodDrawString(g, "[SETTINGS_FULLSCREEN]", mFullscreenCheckbox->mX + 20, aY + 30,
                  Sexy::FONT_BRIANNETOD12, Color::White, DrawStringJustification::DS_ALIGN_LEFT);
    ...
    mHighQualityCheckbox->mDisabled =
        (mHighQualityCheckbox->mY + mY + mHighQualityCheckbox->mHeight) <
            mOptionsSlider->mAllowedMouseZone.mY ||
        (mHighQualityCheckbox->mY + mY) >
            (mOptionsSlider->mAllowedMouseZone.mY + mOptionsSlider->mAllowedMouseZone.mHeight);

    TodDrawString(g, "[SETTINGS_HIGHQUALITY]", mHighQualityCheckbox->mX + 20, aY + 30,
                  Sexy::FONT_BRIANNETOD12, Color::White, DrawStringJustification::DS_ALIGN_LEFT);
    ...
    mHighResolutionCheckbox->mDisabled =
        (mHighResolutionCheckbox->mY + mY + mHighResolutionCheckbox->mHeight) <
            mOptionsSlider->mAllowedMouseZone.mY ||
        (mHighResolutionCheckbox->mY + mY) >
            (mOptionsSlider->mAllowedMouseZone.mY + mOptionsSlider->mAllowedMouseZone.mHeight);

    TodDrawString(g, "[SETTINGS_RESOLUTION]", mHighResolutionCheckbox->mX + 20, aY + 30,
                  Sexy::FONT_BRIANNETOD12, Color::White, DrawStringJustification::DS_ALIGN_LEFT);
    ...
    if (mApp->mRenderer->mCurrentBackend != mApp->mDesiredBackend) {
        TodDrawString(g, "[SETTINGS_RENDERER_RESTART_NOTIF]", 20, aY, Sexy::FONT_BRIANNETOD12,
                      Color::White, DrawStringJustification::DS_ALIGN_LEFT);
    ...
    TodDrawString(g, aVersionString,
                  mOptionsSlider->mAllowedMouseZone.mWidth -
                      Sexy::FONT_BRIANNETOD12->StringWidth(aVersionString) - 27,
                  aMaxContentHeight - Sexy::FONT_BRIANNETOD12->GetHeight(), Sexy::FONT_BRIANNETOD12,
                  Color::White, DrawStringJustification::DS_ALIGN_LEFT);
    ...
    TodDrawString(g, aGitString,
                  mOptionsSlider->mAllowedMouseZone.mWidth -
                      Sexy::FONT_BRIANNETOD12->StringWidth(aGitString) - 27,
...
void SettingsDialog::ListClicked(int theId, int theIdx, int theClickCount) {
    if (theId == SETTINGS_RENDER_LIST) {
        ...
        if (theIdx < mValidSizes.size()) {
            ...
            SDL_SetWindowSize(mApp->mWindow->mInternalWindow, mValidSizes[theIdx].mWidth,
                              mValidSizes[theIdx].mHeight);
            int aCurrentX;
            int aCurrentY;
            SDL_GetWindowPosition(mApp->mWindow->mInternalWindow, &aCurrentX, &aCurrentY);
            int aNewX =
                aCurrentX -
                ((mValidSizes[theIdx].mWidth - mApp->mRenderer->mPresentationRect.mWidth) / 2);
            int aNewY =
                aCurrentY -
                ((mValidSizes[theIdx].mHeight - mApp->mRenderer->mPresentationRect.mHeight) / 2);
            SDL_Rect aUsableBounds{};
            SDL_GetDisplayUsableBounds(SDL_GetDisplayForWindow(mApp->mWindow->mInternalWindow),
                                       &aUsableBounds);
            if (aNewX < 0)
...
void SettingsDialog::CheckboxChecked(int theId, bool checked) {
    ...
    case SettingsDialog::SETTINGS_HIGHRESOLUTION:
        ...
        if (mHighResolutionCheckbox->IsChecked()) {
            ...
            aResult = mApp->LawnMessageBox(
                Dialogs::DIALOG_MESSAGE, "[HIGH_RESOLUTION_WARNING_HEADER]",
                "[HIGH_RESOLUTION_WARNING]", "[BUTTON_YES]", "[BUTTON_NO]", Dialog::BUTTONS_YES_NO);
        }
