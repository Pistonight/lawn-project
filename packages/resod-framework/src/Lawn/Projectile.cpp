...
void Projectile::ProjectileInitialize(int theX, int theY, int theRenderOrder, int theRow,
    ...
    mOnHighGround =
        mBoard->mGridSquareType[aGridX][theRow] == GridSquareType::GRIDSQUARE_HIGH_GROUND;
    ...
    if (mProjectileType == ProjectileType::PROJECTILE_CABBAGE ||
        ...
        TodParticleSystem* aParticle = mApp->AddTodParticle(mPosX + 8.0f, mPosY + 13.0f, 400000,
        ...
        TodParticleSystem* aParticle = mApp->AddTodParticle(
            mPosX + 13.0f, mPosY + 13.0f, 400000, ParticleEffect::PARTICLE_PUFFSHROOM_TRAIL);
...
Plant* Projectile::FindCollisionTargetPlant() {
    ...
    while (mBoard->IteratePlants(aPlant)) {
        ...
        if (GetRectOverlap(aProjectileRect, aPlantRect) > 8) {
            if (mProjectileType == ProjectileType::PROJECTILE_ZOMBIE_PEA) {
                return mBoard->GetTopPlantAt(aPlant->mPlantCol, aPlant->mRow,
...
bool Projectile::PeaAboutToHitTorchwood() {
    ...
    Plant* aPlant = nullptr;
    while (mBoard->IteratePlants(aPlant)) {
        if (aPlant->mSeedType == SeedType::SEED_TORCHWOOD && aPlant->mRow == mRow &&
            !aPlant->NotOnGround() && mHitTorchwoodGridX != aPlant->mPlantCol) {
...
Zombie* Projectile::FindCollisionTarget() {
    ...
    while (mBoard->IterateZombies(aZombie)) {
        ...
        if ((aZombie->mZombieType == ZombieType::ZOMBIE_BOSS || aZombie->mRow == mRow) &&
            ...
            if (mProjectileType == ProjectileType::PROJECTILE_STAR && mProjectileAge < 25 &&
                mVelX >= 0.0f && aZombie->mZombieType == ZombieType::ZOMBIE_DIGGER) {
                continue;
            }
...
void Projectile::CheckForCollision() {
    ...
    if (mProjectileType == ProjectileType::PROJECTILE_ZOMBIE_PEA) {
        ...
        if (aPlant) {
            ...
            mApp->AddTodParticle(mPosX - 3.0f, mPosY + 17.0f, mRenderOrder + 1,
...
bool Projectile::CantHitHighGround() {
    ...
    return (mProjectileType == ProjectileType::PROJECTILE_PEA ||
            mProjectileType == ProjectileType::PROJECTILE_SNOWPEA ||
            mProjectileType == ProjectileType::PROJECTILE_STAR ||
...
bool Projectile::IsSplashDamage(Zombie* theZombie) {
    ...
    return mProjectileType == ProjectileType::PROJECTILE_MELON ||
           mProjectileType == ProjectileType::PROJECTILE_WINTERMELON ||
           mProjectileType == ProjectileType::PROJECTILE_FIREBALL;
}
...
void Projectile::UpdateLobMotion() {
    ...
    if (aPlant) {
        ...
        mBoard->KillAllZombiesInRadius(mRow, mPosX + 80, mPosY + 40, 115, 1, true,
                                       mDamageRangeFlags);
...
void Projectile::UpdateNormalMotion() {
    if (mMotionType == ProjectileMotion::MOTION_BACKWARDS) {
        ...
        if (aZombie && aZombie->EffectedByDamage((unsigned int)mDamageRangeFlags)) {
            Rect aZombieRect = aZombie->GetZombieRect();
            SexyVector2 aTargetCenter(aZombie->ZombieTargetLeadX(0.0f),
...
void Projectile::PlayImpactSound(Zombie* theZombie) {
    ...
    if (aPlayHelmSound && theZombie) {
        if (theZombie->mHelmType == HELMTYPE_PAIL) {
            ...
            aPlaySplatSound = false;
        } else if (theZombie->mHelmType == HELMTYPE_TRAFFIC_CONE ||
                   theZombie->mHelmType == HELMTYPE_DIGGER ||
                   theZombie->mHelmType == HELMTYPE_FOOTBALL) {
...
void Projectile::DoImpact(Zombie* theZombie) {
    ...
    if (mProjectileType == ProjectileType::PROJECTILE_MELON) {
        ...
        int aRenderOrder =
            Board::MakeRenderOrder(RenderLayer::RENDER_LAYER_GROUND, mCobTargetRow, 2);
        mApp->AddTodParticle(mPosX + 80.0f, mPosY + 40.0f, aRenderOrder,
                             ParticleEffect::PARTICLE_BLASTMARK);
        mApp->AddTodParticle(mPosX + 80.0f, mPosY + 40.0f, mRenderOrder + 1,
        ...
        if (IsSplashDamage(theZombie)) {
            Reanimation* aFireReanim =
                mApp->AddReanimation(mPosX + 38.0f, mPosY - 20.0f, mRenderOrder + 1,
...
void Projectile::Draw(Graphics* g) {
    ...
    if (aImage) {
        ...
        Rect aSrcRect(aCelWidth * mFrame, aCelHeight * aProjectileDef.mImageRow, aCelWidth,
                      aCelHeight);
        if (FloatApproxEqual(mRotation, 0.0f) && FloatApproxEqual(aScale, 1.0f)) {
            ...
            TodScaleRotateTransformMatrix(aTransform, aOffsetX + mBoard->mX, aOffsetY + mBoard->mY,
                                          mRotation, aScale, aScale);
...
void Projectile::DrawShadow(Graphics* g) {
    ...
    TodDrawImageCelScaledF(g, IMAGE_PEA_SHADOWS, aOffsetX, (mShadowY - mPosY + aOffsetY), aCelCol,
                           0, aScale * aStretch, aScale);
}
...
void Projectile::ConvertToFireball(int theGridX) {
    ...
    Reanimation* aFirePeaReanim =
        mApp->AddReanimation(0.0f, 0.0f, 0, ReanimationType::REANIM_FIRE_PEA);
    if (mMotionType == ProjectileMotion::MOTION_BACKWARDS) {
