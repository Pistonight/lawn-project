...
public:
    ...
    void ZombieInitialize(int theRow, ZombieType theType, bool theVariant, Zombie* theParentZombie,
                          int theFromWave);
    ...
    /// @param theDrawPos Offsets for the limbs
    void DrawZombiePart(Graphics* g, Image* theImage, int theFrame, int theRow,
    ...
    /// @brief Converts this Zombie into a standard normal Zombie, removing any special type
    /// behavior
    ...
    /// @brief Shows all Reanimation tracks that match the given prefix on the specified render
    /// group
    ...
    void PlayZombieReanim(const char* theTrackName, ReanimLoopType theLoopType, int theBlendTime,
                          float theAnimRate);
    ...
    /// @brief Draws the bobsled Reanimation relative to the Zombie, either before or after the
    /// Zombie sprite
    /// @param g Graphics context to draw to
    /// @param theDrawPos The position data used to place the bobsled on screen
    /// @param theBeforeZombie If true, draws the bobsled behind the Zombie; otherwise draws it in
    /// front
    ...
    /// @brief Runs over and kills plants/zombies in the Zombie's path (used by lawnmower-type
    /// zombies)
    void MowDown();
    ...
    /// @brief Checks whether the Zombie is a valid squash target, optionally ignoring a specific
    /// plant
