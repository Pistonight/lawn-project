...
void LawnMower::UpdatePool() {
    ...
    if (isPoolRange && mMowerHeight == MowerHeight::MOWER_HEIGHT_LAND) {
        Reanimation* aSplashReanim = mApp->AddReanimation(
            mPosX + 0.0f, mPosY + 25.0f, mRenderOrder + 1, ReanimationType::REANIM_SPLASH);
    ...
    if (mMowerHeight == MowerHeight::MOWER_HEIGHT_IN_POOL &&
        aMowerReanim->mLoopType == ReanimLoopType::REANIM_PLAY_ONCE_AND_HOLD &&
        aMowerReanim->mLoopCount > 0) {
...
void LawnMower::MowZombie(Zombie* theZombie) {
    ...
    if (mMowerType == LawnMowerType::LAWNMOWER_POOL) {
        ...
        if (mMowerHeight == MowerHeight::MOWER_HEIGHT_IN_POOL) {
            Reanimation* aMowerReanim = mApp->ReanimationGet(mReanimID);
            aMowerReanim->PlayReanim("anim_suck", ReanimLoopType::REANIM_PLAY_ONCE_AND_HOLD, 10,
                                     35.0f);
        } else {
            Reanimation* aMowerReanim = mApp->ReanimationGet(mReanimID);
            aMowerReanim->PlayReanim("anim_landsuck", ReanimLoopType::REANIM_PLAY_ONCE_AND_HOLD, 10,
                                     35.0f);
        }
...
void LawnMower::Update() {
    ...
    if (mMowerState == LawnMowerState::MOWER_ROLLING_IN) {
        mRollingInCounter++;
        mPosX = TodAnimateCurveFloat(0, 100, mRollingInCounter, -160.0f, -21.0f,
    ...
    if (mChompCounter > 0) {
        mChompCounter--;
        aSpeed = TodAnimateCurveFloat(50, 0, mChompCounter, aSpeed, 1.0f,
    ...
    if (mMowerType == LawnMowerType::LAWNMOWER_LAWN &&
        mBoard->mPlantRow[mRow] == PlantRowType::PLANTROW_POOL && mPosX > 50.0f) {
        Reanimation* aSplashReanim = mApp->AddReanimation(mPosX, mPosY + 25.0f, mRenderOrder + 1,
                                                          ReanimationType::REANIM_SPLASH);
        aSplashReanim->OverrideScale(1.2f, 0.8f);
        mApp->AddTodParticle(mPosX + 50.0f, mPosY + 67.0f, mRenderOrder + 1,
                             ParticleEffect::PARTICLE_PLANTING_POOL);
...
void LawnMower::Draw(Graphics* g) {
    ...
    if (mMowerHeight != MowerHeight::MOWER_HEIGHT_UP_TO_LAND &&
        ...
        if (aShadowType == 0) {
            TodDrawImageCelCenterScaledF(g, IMAGE_PLANTSHADOW, aShadowX, aShadowY, 0, 0, aScaleX,
                                         aScaleY);
        } else {
            TodDrawImageCelCenterScaledF(g, IMAGE_PLANTSHADOW2, aShadowX, aShadowY, 0, 0, aScaleX,
                                         aScaleY);
        }
    }
