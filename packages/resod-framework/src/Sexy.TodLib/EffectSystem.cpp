...
static inline void Tod_Tod_lClip(TriVertex& dst, const TriVertex& on, const TriVertex& off,
                                 const float edge) {
    ...
    dst.color =
        ((int)((off.color >> 24 & 0xff) +
               ((on.color >> 24 & 0xff) - (off.color >> 24 & 0xff)) * delta)
         << 24) |
        ((int)((off.color >> 16 & 0xff) +
               ((on.color >> 16 & 0xff) - (off.color >> 16 & 0xff)) * delta)
         << 16) |
        ((int)((off.color >> 8 & 0xff) + ((on.color >> 8 & 0xff) - (off.color >> 8 & 0xff)) * delta)
         << 8) |
        ((int)((off.color & 0xff) + ((on.color & 0xff) - (off.color & 0xff)) * delta));
}
static inline void rClip(TriVertex& dst, const TriVertex& on, const TriVertex& off,
                         const float edge) {
    float delta = (edge - off.x) / (on.x - off.x);
    dst.x = off.x + (on.x - off.x) * delta;
    dst.y = off.y + (on.y - off.y) * delta;
...
static inline void Tod_tClip(TriVertex& dst, const TriVertex& on, const TriVertex& off,
                             const float edge) {
...
static inline void Tod_bClip(TriVertex& dst, const TriVertex& on, const TriVertex& off,
                             const float edge) {
    float delta = (edge - off.y) / (on.y - off.y);
    dst.x = off.x + (on.x - off.x) * delta;
    dst.y = off.y + (on.y - off.y) * delta;
...
static inline int Tod_clipShape(TriVertex** dst, TriVertex* src, const float left,
...
#include <Sexy.TodLib/TodDrawTriangleInc.cpp>
...
void TodTriangleGroup::DrawGroup(Graphics* g) {
    if (mImage && mTriangleCount) {
        ...
        if (GPUImage::Check3D(g->mDestImage)) {
            GPUImage* anImage = (GPUImage*)g->mDestImage;
            mImage->mDrawn = true;
            anImage->mRenderer->DrawTrianglesTex(mVertArray, mTriangleCount, Color::White,
                                                 mDrawMode, mImage, 0.0f, 0.0f, g->mLinearBlend);
        } else {
            g->mDestImage->BltTrianglesTex(mImage, mVertArray, mTriangleCount,
                                           Rect(0, 0, BOARD_WIDTH, BOARD_HEIGHT), Color::White,
                                           mDrawMode, 0.0f, 0.0f, g->mLinearBlend);
        }
...
void TodTriangleGroup::AddTriangle(Graphics* g, Image* theImage, const SexyMatrix3& theMatrix,
    ...
    TOD_ASSERT(theSrcRect.mX >= 0 && theSrcRect.mWidth >= 0 &&
               theSrcRect.mX + theSrcRect.mWidth <= mImage->mWidth);
    TOD_ASSERT(theSrcRect.mY >= 0 && theSrcRect.mHeight >= 0 &&
               theSrcRect.mY + theSrcRect.mHeight <= mImage->mHeight);
