...
#include <Sexy.TodLib/CompiledDefinition.h>
...
/*
        ...
        A class used to store definition data for other classes is called a definition data class,
   denoted by _DefClass. Correspondingly, the class defined by _DefClass is denoted by _Class.

        For example,
        ReanimatorDefinition serves as the definition data class for the Reanimation class (an
   animation class) TodParticleDefinition serves as the definition data class for the
   TodParticleSystem class (a particle system class), etc.
*/

// ====================================================================================================
// Define the mark
// ----------------------------------------------------------------------------------------------------
// For data of an enumeration type determined by a flag bit, a DefSymbol records the value of one of
// its flag bits (of an enumeration item).
// ====================================================================================================
class DefSymbol {
public:
    int mSymbolValue; // The value of the flag bit or the numerical value corresponding to the
                      // enumeration item; if it is -1, it means that the item does not exist.
    const char* mSymbolName; // The flag or the name of the enumeration item, when a null pointer
                             // indicates that the item does not exist, is used as a marker to
                             // indicate the end of the read.
};
...
// Structure Fields
// ----------------------------------------------------------------------------------------------------
// The structure field records the data of a member variable (_MemVar) in a class and its structure
// within the class.
// ====================================================================================================
class DefField {
public:
    const char* mFieldName; // The name of _MemVar. Pointing to an empty character array indicates
                            // that this variable does not exist, and is therefore used as a marker
                            // to indicate the end of reading.
...
// ----------------------------------------------------------------------------------------------------
// The definition structure diagram describes the storage format and retrieval method of defined
// data in a defined data class (_DefClass), similar to "_DefDefClass".
// ====================================================================================================
class DefMap {
public:
    DefField* mMapFields; // An array of structure fields, recording the structure of each member
                          // variable in the _DefClass class within _DefClass (each record
                          // represents one structure).
    int mDefSize; // The memory size occupied by a _DefClass instance, which is also the length of
                  // the initial read, is typically sizeof(_DefClass).
    void* (*mConstructorFunc)(
        void*); // A pointer to the constructor of an instance of type _DefClass.
};
...
public:
    void* mArrayData; // An array consisting of instances of a specific defined data type, such as
...
// Define path
// ----------------------------------------------------------------------------------------------------
// Define a path to establish an association between a texture prefix and the folder path where the
// textures with that prefix are stored.
// ====================================================================================================
class DefLoadResPath {
public:
    const char* mPrefix;    // Image prefixes, such as "IMAGE_"
    const char* mDirectory; // The folder containing the textures corresponding to the prefix, such
                            // as "images\".
};

SexyString /*__cdecl*/
DefinitionGetCompiledFilePathFromXMLFilePath(const SexyString& theXMLFilePath);
...
uint32_t /*__cdecl*/ DefinitionCalcHashDefMap(int aSchemaHash, DefMap* theDefMap,
...
void DefWriteToCacheArray(DefinitionCompiler* theReadPtr, DefinitionArrayDef* theArray,
                          DefMap* theDefMap);
