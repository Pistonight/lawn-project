...
bool PakInterface::AddPakFile(const std::string& theFileName) {
    ...
    if (!aFile.read(reinterpret_cast<char*>(buffer.data()),
                    static_cast<std::streamsize>(aFileSize)))
        return false;
    ...
    PakRecordMap::iterator aRecordItr =
        mPakRecordMap.insert(PakRecordMap::value_type(StringToUpper(theFileName), PakRecord()))
            .first;
    ...
    if (aMagic != 0xBAC04AC0) {
        ...
        aFileOffset -= sizeof(uint32_t);
        memcpy(&aMagic, aPakCollection->mData.data() + aFileOffset,
               sizeof(uint32_t)); // re-read the magic number
        aFileOffset += sizeof(uint32_t);
...
PFILE* PakInterface::FOpen(const char* theFileName, const char* anAccess) {
    if (std::filesystem::exists(theFileName)) {
        ...
        aPFP->mFP = aFP;
        return aPFP;
    } else if ((stricmp(anAccess, "r") == 0) || (stricmp(anAccess, "rb") == 0) ||
               (stricmp(anAccess, "rt") == 0)) {
        char anUpperName[256];
...
int PakInterface::FSeek(PFILE* theFile, long theOffset, int theOrigin) {
    ...
    if (theFile->mRecord != NULL) {
        ...
        // The current pointer position cannot exceed the entire file size, and cannot be less than
        // 0.
...
size_t PakInterface::FRead(void* thePtr, int theElemSize, int theCount, PFILE* theFile) {
    if (theFile->mFP != nullptr)
        ...
        int aSizeBytes =
            std::min(1l * theElemSize * theCount, theFile->mRecord->mSize - theFile->mPos);

        // obtain pointer to start reading relative to the whole pak
        uint8_t* src = theFile->mRecord->mCollection->mData.data() + theFile->mRecord->mStartPos +
                       theFile->mPos;
    ...
    if (theFile->mFP != nullptr)
        ...
        for (;;) {
            if (theFile->mPos >= theFile->mRecord->mSize)
                return EOF;
            char aChar = *((char*)theFile->mRecord->mCollection->mData.data() +
