...
bool ImageLib::WriteImage(const std::string& theFileName, Image* theImage,
                          const std::string& theExtension, bool add_extension) {
...
Image* ImageLib::GetImageBackend(const std::string& theFileName, const std::string& theExtension) {
    ...
    if (theExtension != ".gif") {
        ...
        stb_image = stbi_load_gif_from_memory(data.data(), fileSize, &delays, &width, &height,
                                              &frame_count, &num_channels, 4);
